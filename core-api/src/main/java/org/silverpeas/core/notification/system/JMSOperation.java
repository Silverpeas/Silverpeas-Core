 ...
 * In fact, {@link jakarta.jms.JMSContext} can be used as <code>application-managed</code> way or it
 * can be used as <code>container-managed</code>.
 * <p>
 * In the first way and according to the documentation, the {@link jakarta.jms.JMSContext} must be
 * get by using {@link jakarta.jms.ConnectionFactory} and it has to be explicitly closed when it is
 ...
 * <p>
 * In the second way, the {@link jakarta.jms.JMSContext} must be injected and used directly. For
 * example:
 * <pre>
 *   {@literal}@Inject
 ...
 * </pre>
 * <p>
 * There is no manual management here concerning the {@link jakarta.jms.JMSContext} because that is
 * handled by the container.<br>
 * But to be managed efficiently, a JTA transaction must exist, and also, the following rule must
 * be verified (from documentation of {@link jakarta.jms.JMSContext}): Applications running in the
