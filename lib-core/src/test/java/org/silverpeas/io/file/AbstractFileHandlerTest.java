...
public class AbstractFileHandlerTest {
  ...
  public void testGetRootPath() {
    File test = fileHandler.getRootPath(BASE_PATH_TEST, componentInstanceId);
    File expected = sessionPath;
    assertFileNames(test, expected);

    test = fileHandler.getRootPath(BASE_PATH_TEST, componentInstanceId, "a", "b");
    expected = getFile(sessionPath, "a", "b");
    assertFileNames(test, expected);
  }

  @Test
  public void testGetRealRootPath() {
    File test = fileHandler.getRealRootPath(BASE_PATH_TEST, componentInstanceId);
    File expected = getFile(realPath);
    assertFileNames(test, expected);

    test = fileHandler.getRealRootPath(BASE_PATH_TEST, componentInstanceId, "a", "b");
    expected = getFile(realPath, "a", "b");
    assertFileNames(test, expected);
  }
  ...
  private IOSession createSessionTest() {
    return SimpleIOSession.create();
  }

  private class FileHandlerTest extends AbstractFileHandler {
    protected FileHandlerTest(final IOSession session) {
      super(session);
    }
    ...
    public File getRootPathForTest() {
      return fileHandler.getRootPath(BASE_PATH_TEST, componentInstanceId);
    }
  }
}
