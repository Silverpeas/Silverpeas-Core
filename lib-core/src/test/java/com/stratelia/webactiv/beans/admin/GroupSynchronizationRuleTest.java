...
import com.stratelia.webactiv.organization.UserTable;
...
import static com.stratelia.webactiv.beans.admin.GroupSynchronizationRule.from;
...
import static org.silverpeas.admin.user.constant.UserAccessLevel.*;
...
public class GroupSynchronizationRuleTest {
  ...
  public void setup() throws Exception {
    ...
    UserManager userManager =
        reflectionRule.mockField(UserManager.class, UserManager.class, "instance");

    GroupManager groupManager =
        reflectionRule.mockField(GroupManager.class, GroupManager.class, "instance");
    ...
    reflectionRule.setField(DomainDriverManagerFactory.class, domainDriverManagerRef,
        "instance.domainDriverManagerRef");

    when(userManager.getAllUsersIds(any(DomainDriverManager.class)))
        .then(new Answer<String[]>() {
          @Override
          public String[] answer(final InvocationOnMock invocation) throws Throwable {
            return extractUserIds(ALL_USERS);
          }
        });
    ...
    when(userManager.getUserIdsOfDomain(any(DomainDriverManager.class), anyString()))
        .then(new Answer<String[]>() {
          ...
          public String[] answer(final InvocationOnMock invocation) throws Throwable {
            ...
            List<UserDetail> users = new ArrayList<UserDetail>(ALL_USERS);
          ...
          public List<String> answer(final InvocationOnMock invocation) throws Throwable {
            ...
            for (Group group : ALL_GROUPS) {
              if (groupIds.contains(group.getId())) {
                Collections.addAll(userIds, group.getUserIds());
              }
            }
    ...
    Admin admin = reflectionRule.mockField(AdminReference.class, Admin.class, "instance.admin");
    ...
    when(domainDriverManager.getDomainDriver(anyInt())).then(new Answer<DomainDriver>() {
      @Override
      public DomainDriver answer(final InvocationOnMock invocation) throws Throwable {
        Integer domainId = (Integer) invocation.getArguments()[0];
    ...
    when(domainDriverA.getUsersBySpecificProperty(anyString(), anyString()))
        .then(new Answer<UserDetail[]>() {
          ...
          public UserDetail[] answer(final InvocationOnMock invocation) throws Throwable {
            ...
            List<UserDetail> users = new ArrayList<UserDetail>(DOMAIN_A_USERS);
            Iterator<UserDetail> userIt = users.iterator();
            while (userIt.hasNext()) {
              UserDetail user = userIt.next();
    ...
    when(domainDriverB.getUsersBySpecificProperty(anyString(), anyString()))
        .then(new Answer<UserDetail[]>() {
          ...
          public UserDetail[] answer(final InvocationOnMock invocation) throws Throwable {
            ...
            String propertyValue = (String) invocation.getArguments()[1];
            String specificSuffix = "_" + propertyName + "=" + propertyValue;
            List<UserDetail> users = new ArrayList<UserDetail>(DOMAIN_B_USERS);
            Iterator<UserDetail> userIt = users.iterator();
            while (userIt.hasNext()) {
              UserDetail user = userIt.next();
              if (!user.getSpecificId().endsWith(specificSuffix)) {
    ...
    when(userTable.getUsersBySpecificIds(anyInt(), anyListOf(String.class)))
        .then(new Answer<UserRow[]>() {
          @SuppressWarnings("unchecked")
          @Override
          public UserRow[] answer(final InvocationOnMock invocation) throws Throwable {
            Integer domainId = (Integer) invocation.getArguments()[0];
            List<String> specificIds = (List<String>) invocation.getArguments()[1];
            List<UserDetail> users = new ArrayList<UserDetail>(
                String.valueOf(domainId).equals(DOMAIN_A.getId()) ? DOMAIN_A_USERS :
                    DOMAIN_B_USERS);
            List<UserRow> userRows = new ArrayList<UserRow>();
            for(UserDetail user : users) {
              if (specificIds.contains(user.getSpecificId())) {
  ...
  public void getUserIdsFromUserAccessLevelRuleShouldReturnSimpleUsers() throws Exception {
    ...
    assertThat(userIds, Matchers.<String>empty());
  }
