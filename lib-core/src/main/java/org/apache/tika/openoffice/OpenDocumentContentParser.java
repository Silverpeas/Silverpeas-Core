...
public class OpenDocumentContentParser implements Parser {
  ...
  static {
    ...
    tableCellAttsMapping.put(
        ...
        new QName("rowspan"));
    /*
     * TODO: The following is not correct, the cell should be repeated not spanned! Code generates a
     * HTML cell, spanning all repeated columns, to make the cell look correct. Problems may occur
     * when both spanning and repeating is given, which is not allowed by spec. Cell spanning
     * instead of repeating is not a problem, because OpenOffice uses it only for empty cells.
     */
    tableCellAttsMapping.put(
