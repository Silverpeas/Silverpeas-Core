...
public class ExcelExtractor extends AbstractPOIFSExtractor {
   ...
   * <code>false</code> (the default) if the listener should be configured to only receive specified
   * records.
   */
   ...
   * @param listenForAllRecords <code>true</code> if the HSSFListener should be registered to listen
   * for all records or <code>false</code> if the listener should be configured to only receive
   * specified records.
   */
   ...
   * @throws IOException if an error occurs processing the workbook or writing the extracted content
   */
  ...
  private static class TikaHSSFListener implements HSSFListener {
    ...
    /**
     * Potential exception thrown by the content handler. When set to non-<code>null</code>, causes
     * all subsequent HSSF records to be ignored and the stored exception to be thrown when
    ...
    /**
     * Index of the current worksheet within the workbook. Used to find the worksheet name in the
     * {@link #sheetNames} list.
     */
    private short currentSheetIndex;

    /**
     * Content of the current worksheet, or <code>null</code> if no worksheet is currently active.
     */
    ...
    /**
     * These aren't complete when we first see them, as the depend on continue records that aren't
     * always contiguous. Collect them for later processing.
     */
     ...
     * @param filesystem POI file system.
     * @param listenForAllRecords sets whether the listener is configured to listen for all records
     * types or not.
    ...
    private void internalProcessRecord(Record record) throws SAXException, TikaException,
        IOException {
    ...
    private void findPictures(List<EscherRecord> records) throws IOException, SAXException,
        TikaException {
