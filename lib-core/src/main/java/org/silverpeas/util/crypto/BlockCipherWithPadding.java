 ...
 * Block ciphers are important elementary components in the design of many cryptographic protocols,
 ...
 * have been designed and specified in national recommendations such as NIST 800-38A and
 * BSI TR-02102 and international standards such as ISO/IEC 10116. The general concept is to use
 ...
 * ciphertext block is then used as the new initialization vector for the next plaintext block.
 ...
 * feedback (OFB) mode repeatedly encrypts the initialization vector to create a key stream for
 * the emulation of a synchronous stream cipher. The newer counter (CTR) mode similarly creates a
 * key stream, but has the advantage of only needing unique and not (pseudo-)random values as
 ...
 * scheme is therefore needed to extend the last plaintext block to the cipher's block size.
 * While many popular schemes described in standards and in the literature have been shown to be
 ...
 * to encrypt when it is not divisible into blocks of expected size. All the subclasses will use
 * the CBC operation mode with the PKCS#5 padding scheme.
 *
...
public abstract class BlockCipherWithPadding implements Cipher {
   ...
   * encrypted data that was produced by the specified block cipher instance. This method
   * consists in extracting the necessary information to other implementations of AES encryption
   * can decrypt the ciphertext (only if they use the same mode of cryptographic operation).
  ...
  public byte[] encrypt(final String data, final CipherKey keyCode) throws CryptoException {
    try {
      ...
      javax.crypto.Cipher cipher = javax.crypto.Cipher.getInstance(getTransformation());
