...
/**
 * A FullIndexEntry is an IndexEntry completed
 * with data usefull uniquely at the index creation time
 * (mainly all the data contents which must be indexed
 * but which is useless at retrieve time).
 * This extra-content is indexed but not stored in the index.
 */
public class FullIndexEntry extends IndexEntry
                            implements Serializable
{
   ...
   *
   * All this text fragments will be indexed but not stored
   * in the index. They may be added in any order.
   */
   ...
   * </UL>
   *
   * All this files will be parsed and then indexed but not stored
   * in the index. They may be added in any order.
   */
  ...
  {
	  getFields().add(new FieldDescription(fieldName, DateFormatter.date2IndexFormat(value), language));
  }

  /**
   * Return the List of all the added texts.
   * The returned List is a list of String.
   */
   ...
   * The returned List is a list of FileDescription.
   */
  ...
  /**
   * All the added texts and files are collected
   * in two lists to be later retrieved by the index engine.
   *
   * textList is a list of String.
   * fileList is a list of FileDescription.
   */
