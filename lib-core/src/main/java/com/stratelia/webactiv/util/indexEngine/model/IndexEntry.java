...
/**
 * IndexEntry is the base class for all the entries
 * which are indexed in the web'activ index.
 *
 * A IndexEntry is create by a web'activ's component
 * when it creates a new element or document.
 * This IndexEntry will be return later when the document matchs a query.
 */

public class IndexEntry implements Serializable
{
  /**
   * This constructor set the key part of the IndexEntry
   * but leave empty the object type.
   *
   * This constructor can be used by any component which indexes
   * only one kind of entities and then doesn't need to tag them
   * with a type.
   */
  ...
  /**
   * This constructor set the key part of the IndexEntry from
   * WAPrimaryKey completed with a type given as a String.
   *
   * When the indexed document will be retrieved, this type will be used
   * to distinguish the real type of the document
   * between all the indexed document kind.
   */
  ...
  /**
   * Returns as a string the key part of the indexEntry.
   * the key part of the IndexEntry
   */
  ...
  /**
   * Set the title of the index entry as it will be displayed
   * to the user if the indexed document match his query.
   */
  ...
  {	  
	  /*if (lang == null || I18NHelper.isDefaultLanguage(lang))
		  this.title = title;
	  else
	  {*/
	  if (title != null)
  ...
  /**
   * Return the title of the index entry as it will be displayed
   * to the user if the indexed document match his query.
   */
  ...
  /**
   * Set a pre-view text for the index entry
   * as it will be displayed at the user when the document
   * will be retrieved.
   */
  ...
  /**
   * Return the pre-view text for the index entry
   * as it will be displayed at the user when the document
   * will be retrieved.
   */
  ...
  public String getCreationDate()
  {
    ...
    else return new SimpleDateFormat("yyyy/MM/dd").format(new Date());
  }
  ...
  {
    if (creationUser != null) return creationUser;
    else return "";
  }

  /**
   * Set the start date from which
  ...
  /**
   * The start date defaults to 0000/00/00
   * so the document is visible as soon as published.
   */
  static public final String STARTDATE_DEFAULT = "0000/00/00";

  /**
   * Get the start date from which
   * the document will be displayed.
   *
  ...
  public String getStartDate()
  {
    ...
    else return STARTDATE_DEFAULT;
  }

  /**
   * The end date defaults to 9999/99/99
   * so the document will be visible for ever.
   */
   ...
   * so IndexEntry objects can be put in a Set or used as Map key.
   */
   ...
   * so IndexEntry objects can be put in a Set or used as Map key.
   *
   * Only the primary key is used to compute the hash code,
   ...
   *
   * The title should been set in order to display the
   * entry to the user when the document match his query.
   *
   * The index engine may set with a default value
   * any of this attributes if null.
   */
