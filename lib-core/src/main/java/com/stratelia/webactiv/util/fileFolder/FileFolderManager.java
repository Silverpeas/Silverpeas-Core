...
public class FileFolderManager  {
   ...
   getAllSubFolder : retourne une Collection d'objets File qui representent 
                     les repertoires (et seulement les repertoires, pas les fichiers) 
                     contenus dans le repertoire passe en parametre
   ...
   */  
   ...
   /**
   getAllFile : retourne une Collection d'objets File qui representent 
                     les fichiers (et seulement les fichiers, pas les repertoires) 
                     contenus dans le repertoire passe en parametre
    Param = chemin du repertoire
   */  
   ...
   /**
   getAllImages : retourne une Collection d'objets File qui representent 
                     les fichiers images (type GIF ou JPEG) 
                     contenus dans le repertoire passe en parametre
    Param = chemin du repertoire
   */  
           ...
           while (list != null && i < list.length) {
                if (list[i].isFile()) {
                    ...
                    if (type.equals("gif") || type.equals("GIF") || 
                        type.equals("jpg") || type.equals("JPG") ||
                        type.equals("png") || type.equals("PNG") ||
                        type.equals("bmp") || type.equals("BMP") ||
                        type.equals("pcd") || type.equals("PCD") ||
                        type.equals("tga") || type.equals("TGA") ||
   ...
   /**
   getAllWebPages : retourne une Collection d'objets File qui representent 
                     les fichiers du site web 
                     contenus dans le repertoire passe en parametre et ses sous repertoires 
    Param = chemin du repertoire du site
   */  
       ...
       if (directory.isDirectory()) {
           ...
           while (list != null && i < list.length) {
                if (list[i].isFile()) {
                    ...
                    int indexPoint = fichier.lastIndexOf(".");
                    String type = fichier.substring(indexPoint + 1);
                    if (type.equals("htm") || type.equals("HTM") || type.equals("html") || type.equals("HTML"))
                    NEWF CBO
                    */
                	resultat.add(list[i]);
                }
   ...
   getAllWebPages2 : retourne une Collection d'objets File qui representent 
                     les fichiers web (type HTML) 
                     contenus dans le repertoire passe en parametre et seulement dans ce repertoire
    Param = chemin du repertoire du site
   */  
   ...
   /**
   createFolder : creation d'un repertoire
   Param = chemin du repertoire
   */  
   public static void createFolder(String chemin) throws UtilException {
	...
	public static void createFolder(File directory) throws UtilException {
		if (! directory.mkdirs()) {
			SilverTrace.error("util", "FileFolderManager.createFolder", "util.EX_REPOSITORY_CREATION", directory.getPath());
			throw new UtilException("FileFolderManager.createFolder", "util.EX_REPOSITORY_CREATION", directory.getPath());
		}
	}
  
  
   /**
   renameFolder : modification du nom d'un repertoire
   Param = chemin du repertoire
   */  
   public static void renameFolder(String cheminRep, String newCheminRep) throws UtilException {
      ...
      if (directory.isDirectory()) {
        ...
        if (! directory.renameTo(newDirectory)) {
            SilverTrace.error("util", "FileFolderManager.renameFolder", "util.EX_REPOSITORY_RENAME", cheminRep.toString() +" en "+ newCheminRep.toString());
            throw new UtilException("FileFolderManager.renameFolder", "util.EX_REPOSITORY_RENAME", cheminRep.toString() +" en "+ newCheminRep.toString());
        }
      }
   ...
   /**
   deleteFolder : destruction d'un repertoire et de tout ce qu'il contient (fichiers et ss-repertoires)
   Param = chemin du repertoire
   */  
  ...
  private static void delDir(File dir) { 
   ...
   Param = cheminFichier = c:\\j2sdk\\public_html\\WAUploads\\WA0webSite10\\nomSite\\rep1\\rep2 
   ...
   */  
   ...
   /**
   renameFile : modification du nom d'un fichier
   ...
   */  
   ...
   /**
   deleteFile : destruction d'un fichier
   Param = chemin du fichier
   */  
   ...
   /**
   getCode : Récupération du contenu d'un fichier
   Param = cheminFichier = c:\\j2sdk\\public_html\\WAUploads\\WA0webSite10\\nomSite\\rep1\\rep2 
           nomFichier = index.html 
   */  
        ...
        if (directory.isDirectory()) {
            try {
              ...
              while ((ligne = flux_in.readLine()) != null) {
                  res = res + ligne;
                  //Debug.debug(700, "util fileFolderManager", "getCode : res = "+res, null, null);      
              }
              flux_in.close();
              //file_read.close();
            } catch (IOException e) {
