...
/**
 * The ParserManager class manages all the parsers
 * which will be used to parse the indexed files.
 */
public final class ParserManager
{
  ...
  static public Parser getParser(String format)
  {
    ...
    if (parser == null)
    {
		SilverTrace.debug("indexEngine", "ParserManager", "indexEngine.MSG_UNKNOWN_FILE_FORMAT", format);
    }
  ...
  static private void init()
  {
    ...
    try
    {
      ResourceLocator MyResource = new ResourceLocator("com.stratelia.webactiv.util.indexEngine.Parser", "");
      ...
      while (FormatNames.hasMoreElements())
      {
        ...
        catch (ClassNotFoundException e)
        {
		  SilverTrace.error("indexEngine", "ParserManager",
		                    "indexEngine.MSG_UNKNOWN_PARSER_CLASS",
                            name +", "+className);
        }
        catch (Exception e)
        {
		  SilverTrace.fatal("indexEngine", "ParserManager",
		                    "indexEngine.MSG_PARSER_INITIALIZATION_FAILED",
                            name);
        }
      }
    }
  ...
  static private String getClassName(String newCall)
  {
    ...
    if (par == -1) return newCall.trim();
    else return newCall.substring(0, par).trim();
  }
  ...
  static private Object[] getParameters(String newCall)
  {
    ...
    if (lPar == -1 || rPar == -1 || lPar+1>=rPar) return new Object[0];
  ...
  static private Class[] getParametersClass(String newCall)
  {
    int lPar = newCall.indexOf("(");
    int rPar = newCall.indexOf(")");
    if (lPar == -1 || rPar == -1 || lPar+1>=rPar) return new Class[0];
  ...
  /**
   * At class initialization time, the parser's map is built
   * and initialized from the Parsers.properties file.
   */
