...
public class Test
{
  static public void main(String[] args)
  {
    ...
    catch (Exception e)
    {
	 ...
	 public static void indexDirectory(IndexManager indexEngine, File file, int cp) throws Exception
	 {
		String format;
		...
		else
		{
			...
			indexEntry.setPreView("...ceci est le test ou le meilleur est  ce test...");
			indexEntry.setKeyWords("... aller la exemple le simple de marche java les publication test...");
     ...
     public static String getFormat(File file) throws Exception
	 {
		...
		if((Temp.endsWith("pdf")) || (Temp.endsWith("PDF"))) return "application/pdf";
		if((Temp.endsWith("html")) || (Temp.endsWith("HTML"))) return "text/html";
		if((Temp.endsWith("doc")) || (Temp.endsWith("DOC"))) return "application/msword";
		if((Temp.endsWith("txt")) || (Temp.endsWith("TXT"))) return "text/plain";
		if((Temp.endsWith("xls")) || (Temp.endsWith("XLS"))) return "application/msexcel";
		if((Temp.endsWith("ppt")) || (Temp.endsWith("PPT"))) return "application/mspowerpoint";
		return Temp;
	 }
