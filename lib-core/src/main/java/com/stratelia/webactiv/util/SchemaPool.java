...
/**
 * The SchemaPool class manages a pool of Schema shared
...
public abstract class SchemaPool
{
   ...
   * Returns an Shema.
   *
   * The returned schema is removed from the pool :
   * so, if our client forgets to release this schema
   * we don't keep a useless entry.
   */
  protected Schema getInstance() throws UtilException
  {
      ...
      while (instance==null)
      {
          ...
          else
          {
              ...
              if (!instance.isOk()) instance = null;
          }
      }
      return instance;
  }
