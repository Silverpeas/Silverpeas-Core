...
public class RepositoryIndexer
{
	...
	public RepositoryIndexer(String spaceId, String componentId)
	{
		...
		resource = new ResourceLocator("com.stratelia.webactiv.util.attachment.mime_types", "fr");
	...
	public void pathIndexer(String path, String creationDate, String creatorId, String action)
	{
		...
		SilverTrace.debug("indexEngine", "RepositoryIndexer.pathIndexer()", "root.MSG_GEN_PARAM_VALUE", "separator = "+separator);
		...
		SilverTrace.info("indexEngine", "RepositoryIndexer.pathIndexer()", "root.MSG_GEN_PARAM_VALUE", "Fichiers à indexer="+new Integer(count).toString());
	}
    ...
    {
		//SilverTrace.debug("indexEngine", "RepositoryIndexer.processFileList()", "root.MSG_GEN_ENTER_METHOD", "path="+path);
		
		if (count%10000 == 0)
			SilverTrace.info("indexEngine", "RepositoryIndexer.processFileList()", "root.MSG_GEN_PARAM_VALUE", "# of indexed documents ="+count);
		...
		{
			//SilverTrace.debug("indexEngine", "RepositoryIndexer.processFileList()", "root.MSG_GEN_PARAM_VALUE", "fileList["+i+"]="+fileList[i]);
			//fileName = fileList[i];
			//currentFile = new File(path + separator + fileName);
			currentFile = fileList[i];
			//SilverTrace.debug("indexEngine", "RepositoryIndexer.processFileList()", "root.MSG_GEN_PARAM_VALUE", "currentFile="+currentFile.getPath());

			if (!currentFile.isDirectory())
			{
				//SilverTrace.debug("indexEngine", "RepositoryIndexer.processFileList()", "root.MSG_GEN_PARAM_VALUE", "(IsDirectory false) path+fileName="+path+fileName+", action="+action+", currentFile="+currentFile.getPath());
				//It's a document, index or remove it !
				indexFile(action, creationDate, creatorId, currentFile, false);
			}
			else
			{
				...
				// recursive call to get the current object
				//SilverTrace.debug("indexEngine", "RepositoryIndexer.processFileList()", "root.MSG_GEN_PARAM_VALUE", "RecursiveCall: (IsDirectory true): path+fileName"+path+fileName);
	...
	private void indexDirectory(String action, String creationDate, String creatorId, File directory)
	{
		if (action.equals("add"))
		{
			...
			FullIndexEntry fullIndexEntry = new FullIndexEntry(getComponentId(), "LinkedDir", directory.getPath());
			...
			//SilverTrace.debug("indexEngine", "RepositoryIndexer.indexDirectory()", "root.MSG_GEN_PARAM_VALUE", "Add index Répository : name = "+directory.getName());
			count++;
		}
	...
	public void indexFile(String action, String creationDate, String creatorId, File file, boolean closeIndex)
	{
