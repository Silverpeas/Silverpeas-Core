...
public class IndexEngineInitialize implements IInitialize
{
	 ...
	 */
	/**
	 * Since version 1.3 of Lucene, lock files are stored in the java.io.tmpdir system's property
	 * By default on Windows, it's C:\Documents and Settings\neysseri\Local Settings\TEMP and /tmp on Unix
	 */
    public boolean Initialize()
    {
        // Remove all remaining *.lock files in index path
        ResourceLocator resource = new ResourceLocator("com.stratelia.webactiv.general","");
     ...
     * Since version 2.3 of Lucene, lock files are in index dirs and named "write.lock" 
	 * @param fileName - the file to test
	 * @return true if the file is a lucene's lock file, false otherwise.
	 */
	protected boolean isLockFile(String fileName)
    {
        //return (("commit.lock".equalsIgnoreCase(fileName)) || ("write.lock".equalsIgnoreCase(fileName)));
