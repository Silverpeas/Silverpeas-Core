...
/**
 * A QueryDescription packs a query with the
 * different spaces and components to be searched.
 */
public class QueryDescription implements Serializable
{
   ...
   *
   * The setQuery and addSpaceComponentPair()
   * methods should be called to initialize the query.
   *
   * Other criterium (language, creation date ...)
   ...
   *
   * The addSpaceComponentPair()
   * method should be called to set the components instances
   * whose documents will be searched.
   *
   * Other criterium (language, creation date ...)
   * can be set before the request is sent to the searchEngine.
   */
  ...
  /**
   * Add the given instance of the component
   * to the searched instances set.
   */
  ...
  /**
   * Return the set of all the component's instances
   * whose the documents must be searched.
   *
   ...
   */
	...
	*/
	 ...
	 */
	 ...
	 {
		 return !StringUtil.isDefined(query) && getMultiFieldQuery() == null && getXmlQuery() == null && !StringUtil.isDefined(xmlTitle);
	 }

  /**
   * Find and replace used for the AND OR NOT lucene's
   * keyword in the search engine query.
   * 
   * 26/01/2004
   */
	...
	/**
	 * Function to get the substitute char for the current language.
	 * (Same function used for the Indexer)
	 * 
	 * PHiL 29/01/2004
	 */
    ...
    private CharReplacer getCharReplacer()
    {
        ...
        try
        {
            ResourceLocator resource = new ResourceLocator("com.stratelia.webactiv.util.indexEngine.SpecialChars", "");
        ...
        {
            SilverTrace.warn("indexEngine", "getCharReplacer", "indexEngine.MSG_MISSING_SPECIALCHARS_DEFINITION");
        }
  ...
  /**
   * The searched components' instance is built empty.
   * To be searched any space or component must be explicitly
   * added with the addSpaceComponent() method.
   ...
   *
   * This is an error to set the query to null :
   * a query is needed to perform the search.
   */
   ...
   *
   * This is an error to set the user to null :
   * a search is done for a given user.
   */
   ...
   *
   * When the requested Date range is null, all documents are returned
   * even if the creation dates are unknown.
   */
