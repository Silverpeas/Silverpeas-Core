...
/**
 * The WAIndexSearcher class implements search over all the
 * WebActiv's index.
 * 
...
{
	/**
	 * The primary and secondary factor are used
	 * to give a better score to entries whose title or abstract
	 * match the query.
	 * 
	 * @see #merge
	 */
	...
	/**
	 * The no parameters constructor retrieves all the needed data
	 * from the IndexEngine.properties file.
	 */
	...
	public WAIndexSearcher()
	{
		...
		try
		{
			ResourceLocator resource =	new ResourceLocator("com.silverpeas.searchEngine.searchEngineSettings", "");
											  ...
											  int defaultValue)
	{
		...
		try
		{
			ResourceLocator resource = new ResourceLocator("com.stratelia.webactiv.util.indexEngine.IndexEngine", "");
	 ...
	 * 
	 * All entries found whose startDate is not reached
	 * or whose endDate is passed are pruned from the results set.
	 */
	  ...
	  throws com.stratelia.webactiv.searchEngine.model.ParseException
	{
		...
		try
		{
			...
			else
			{
				...
				{
					List contentMatchingResults = makeList(getHits(query, IndexManager.CONTENT, searcher), query);
	
					if (contentMatchingResults.size() > 0)
					{
						List headerMatchingResults = makeList(getHits(query, IndexManager.HEADER, searcher), query);
		...
		finally
		{
			...
			{
				SilverTrace.fatal("searchEngine", "WAIndexSearcher.search()", "searchEngine.MSG_CANNOT_CLOSE_SEARCHER", ioe);
			}
		}

		return (MatchingIndexEntry[]) results.toArray(new MatchingIndexEntry[results.size()]);
	}
	 ...
	 */
	private Hits getHits(QueryDescription query, String searchField, Searcher searcher) 
	  throws com.stratelia.webactiv.searchEngine.model.ParseException
	{
		Hits hits = null;
		...
		try
		{
			...
			else
			{
				...
				SilverTrace.info("searchEngine", "WAIndexSearcher.getHits", "root.MSG_GEN_PARAM_VALUE", "defaultOperand = " + defaultOperand);
				parsedQuery = queryParser.parse(query.getQuery());
				SilverTrace.info("searchEngine", "WAIndexSearcher.getHits", "root.MSG_GEN_PARAM_VALUE", "getOperator() = " + queryParser.getDefaultOperator());
			}
			
			SilverTrace.info("searchEngine", "WAIndexSearcher.getHits", "root.MSG_GEN_PARAM_VALUE", "parsedQuery = " + parsedQuery.toString());
		...
		{
			SilverTrace.fatal("searchEngine", "WAIndexSearcher", "searchEngine.MSG_CORRUPTED_INDEX_FILE", e);
			hits = null;
		}
	  ...
	  throws com.stratelia.webactiv.searchEngine.model.ParseException, IOException
	{
		...
		try
		{
			...
			String[] fields 	= (String[]) xmlQuery.keySet().toArray(new String[nbFields]);
			String[] queries 	= (String[]) xmlQuery.values().toArray(new String[nbFields]);
		...
		{
			SilverTrace.fatal("searchEngine", "WAIndexSearcher.getXMLHits", "searchEngine.MSG_CORRUPTED_INDEX_FILE", e);
			hits = null;
		}
	...
	private Hits getMultiFieldHits(QueryDescription query, Searcher searcher) throws com.stratelia.webactiv.searchEngine.model.ParseException, IOException
	{
		Hits hits = null;
		...
		{
			SilverTrace.fatal("searchEngine", "WAIndexSearcher.getMultiFieldHits", "searchEngine.MSG_CORRUPTED_INDEX_FILE", e);
			hits = null;
		}
	 ...
	 * 
	 * All entries found whose startDate is not reached
	 * or whose endDate is passed are pruned from the results list.
	 */
private List makeList(Hits hits, QueryDescription query) throws IOException
	{
		...
		SilverTrace.info("searchEngine", "WAIndexSearcher.makeList", "root.MSG_GEN_PARAM_VALUE", "beforeDate = " + beforeDate);
		SilverTrace.info("searchEngine", "WAIndexSearcher.makeList", "root.MSG_GEN_PARAM_VALUE", "afterDate = " + afterDate);
		...
		if (hits != null)
		{
			for (int i = 0; i < hits.length(); i++)
			{
				...
				//{
					...
					SilverTrace.info("searchEngine", "WAIndexSearcher.makeList", "root.MSG_GEN_PARAM_VALUE", "startDate = " + startDate);
					SilverTrace.info("searchEngine", "WAIndexSearcher.makeList", "root.MSG_GEN_PARAM_VALUE", "endDate = " + endDate);
					SilverTrace.info("searchEngine", "WAIndexSearcher.makeList", "root.MSG_GEN_PARAM_VALUE", "creationDate = " + creationDate);
                    ...
                    if ((beforeDate!=null) && (afterDate!=null)) {
						...
						SilverTrace.info("searchEngine", "WAIndexSearcher.makeList", "root.MSG_GEN_PARAM_VALUE", "testDate = " + testDate);
					...
					if ((startDate.compareTo(today) <= 0 && today.compareTo(endDate) <= 0) 
					  || user.equals(author))
					{
						if (testDate &&  (requestedAuthor == null || requestedAuthor.equals(author)))
						{
	 ...
	 * </PRE>
	 * 
	 * If an entry is in the secondary list but not in the primary,
	 * his score is left unchanged.
	 * 
	 * If any, all entries in the primary list but not the secondary
	 * are ignored. In practice this case should not occurs
	 * as the secondary is extracted from the CONTENT index which
	 * contains all the HEADER contents from which is extracted the primary list.
	 */
	private List merge(List primaryList, int primaryFactor, 
					   List secondaryList, int secondaryFactor)
	{
		...
		while (j.hasNext())
		{
			...
			primaryEntry = 
				(MatchingIndexEntry) primaryMap.get(secondaryEntry.getPK());
	...
	/**
	 * Return a multi-searcher built on the searchers list
	 * matching the (space, component) pair set.
	 */
	private Searcher getSearcher(Set spaceComponentPairSet)
	{
		...
		try
		{
			return new MultiSearcher((Searcher[]) searcherList.toArray(new Searcher[searcherList.size()]));
		}
		catch (IOException e)
		{
	...
	/**
	 * Build the set of all the path to the directories index
	 * corresponding the given (space, component) pairs.
	 */
	private Set getIndexPathSet(Set spaceComponentPairSet)
	{
		...
		while (i.hasNext())
		{
			...
			//Both cases.
			//1 - space == null && component != null : search in an component's instance
			//2 - space != null && component != null : search in pdc or user's components (todo, agenda...) 
	 ...
	 * 
	 * The index readers are cached in a Map (path -> (timestamp, reader)).
	 * If a reader is found in the cache but appear to be too old
	 * (according to the timestamp) then it is re-open.
	 * 
	 * If the index files are not found, null is returned
	 * without any error (as this case comes each time a request
	 * is made on a space/component without any indexed documents).
	 */
	private Searcher getSearcher(String path)
	{
		...
		{
			if (cached == null 
			  || cached.timestamp != IndexReader.lastModified(path))
			{
				if (cached != null)
				{
					cached.reader.close();
			        SilverTrace.info("searchEngine", "WAIndexSearcher",
							  "searchEngine.INFO_REOPEN_INDEX_FILE", path);
				}
				else
				{
			        SilverTrace.info("searchEngine", "WAIndexSearcher",
		...
		catch (IOException e)
		{
			...
			SilverTrace.error("searchEngine", "WAIndexSearcher",
							  "searchEngine.MSG_CANT_READ_INDEX_FILE", e);
			return null;
		}
		...
		{
			SilverTrace.debug("searchEngine", "WAIndexSearcher.getSearcher()", "searchEngine.MSG_CANT_READ_INDEX_FILE", ioe);
		}
