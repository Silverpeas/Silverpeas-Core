...
* @param userId
  ...
  public UserDetail[] searchUsers(final UserDetailsSearchCriteria searchCriteria) throws AdminException {
    List<String> userIds = null;
    if (searchCriteria.isCriterionOnComponentInstanceIdSet()) {
      ...
      if (searchCriteria.isCriterionOnRoleIdsSet()) {
        listOfRoleNames = Arrays.asList(searchCriteria.getCriterionOnRoleIds());
      }
      ComponentInst instance = getComponentInst(searchCriteria.getCriterionOnComponentInstanceId());
      if (((listOfRoleNames != null && !listOfRoleNames.isEmpty()) && instance.isPublic()) || !instance.isPublic()) {
        List<ProfileInst> profiles = instance.getAllProfilesInst();
        userIds = new ArrayList<String>();
        for (ProfileInst aProfile : profiles) {
          if (listOfRoleNames == null || listOfRoleNames.isEmpty() || listOfRoleNames.contains(aProfile.getName())) {
  ...
  public Group[] searchGroups(final GroupsSearchCriteria searchCriteria) throws AdminException {
    ...
    if (searchCriteria.isCriterionOnComponentInstanceIdSet()) {
      ...
      criteria.onRoleIds(roleIds.toArray(new String[roleIds.size()]));
    }
