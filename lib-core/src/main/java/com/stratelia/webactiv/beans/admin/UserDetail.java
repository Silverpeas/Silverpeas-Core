...
public class UserDetail extends Object implements Serializable, Comparable
{
    ...
    public boolean isDomainAdminRestricted()
    {
        return ((GeneralPropertiesManager.getDomainVisibility() != GeneralPropertiesManager.DVIS_ALL) && 
            	(!isAccessAdmin()) &&
    			((GeneralPropertiesManager.getDomainVisibility() != GeneralPropertiesManager.DVIS_ONE) || (!getDomainId().equals("0"))));
    }
    ...
    {
        SilverTrace.info("admin","UserDetail.traceUser","admin.MSG_DUMP_USER","Id : " + m_sId);
        SilverTrace.info("admin","UserDetail.traceUser","admin.MSG_DUMP_USER","SpecificId : " + m_sSpecificId);
        SilverTrace.info("admin","UserDetail.traceUser","admin.MSG_DUMP_USER","DomainId : " + m_sDomainId);
        SilverTrace.info("admin","UserDetail.traceUser","admin.MSG_DUMP_USER","Login : " + m_sLogin);
        SilverTrace.info("admin","UserDetail.traceUser","admin.MSG_DUMP_USER","FirstName : " + m_sFirstName);
        SilverTrace.info("admin","UserDetail.traceUser","admin.MSG_DUMP_USER","LastName : " + m_sLastName);
        SilverTrace.info("admin","UserDetail.traceUser","admin.MSG_DUMP_USER","eMail : " + m_seMail);
        SilverTrace.info("admin","UserDetail.traceUser","admin.MSG_DUMP_USER","AccessLevel : " + m_sAccessLevel);
    }
