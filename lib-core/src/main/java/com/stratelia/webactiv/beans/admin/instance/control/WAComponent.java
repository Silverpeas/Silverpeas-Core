...
public class WAComponent
{
    ...
    private static ResourceLocator settings =
        new ResourceLocator("com.stratelia.webactiv.beans.admin.instance.control.castorSettings", "fr");
	...
	public WAComponent(String filename)
	{
		...
		try
		{
            SilverTrace.info("admin", "WAComponent.WAComponent", "root.MSG_GEN_PARAM_VALUE", "Reading File : " + filename);
			...
			this.instanceClassName	= ninstanceClassName.getFirstChild().getNodeValue();
            ...
            if ( (nprofilList!=null) && (nprofilList.hasChildNodes()) )
            {
                 ...
                 for (int i = 1; i < size; i=i+2)
                      if (list.item(i).hasChildNodes() )
					  {
						  ...
						  // Get name of component used in Silverpeas (if none, get label instead)
						  if(list.item(i).getAttributes() != null && list.item(i).getAttributes().getNamedItem("name")!=null)
                            strProfile = list.item(i).getAttributes().getNamedItem("name").getNodeValue();
						  else
           ...
           SilverTrace.error("admin", "WAComponent.WAComponent", "admin.EX_ERR_WACOMPONENT", "Reading File : " + filename, e);
  	 } 
   }

   public WAComponent(String    strName,
                      String    strLabel,
                      String    strDescription,
                      String    strSuite,
                      boolean   fIsVisible,
                      boolean   fIsPortlet,
                      String    strInstanceClass,
                      String[]  astrTableList,
                      List      listProfiles) {
      setName(strName);
      setLabel(strLabel);
  ...
  public String toString() {
     String   s = name+"|"+description+"|"+suite+"|"+ ((visible)?" Visible ":" NOT Visible ") +"|"+ instanceClassName+"|";
     Iterator iterProfile = profiles.iterator();
      int i=0;
      if (tableList !=null) {
      while( (i < tableList.length) && (tableList[i]!=null) )  s=s+","+tableList[i++];
     }
      s=s+"|";
      i=0;
...
//      String          strSchemaFileName  = settings.getString("ComponentDescriptorSchemaFileURL");
      ...
      try
      {
          ...
          mar = new Marshaller( new OutputStreamWriter( new FileOutputStream( strDescriptorFileName ),
                                                        strDescriptorFileEncoding ) ); 
...
//          mar.setNoNamespaceSchemaLocation( strSchemaFileName ); 
      ...
      catch ( MappingException me )
      {
          throw new InstanciationException(
              ...
              "admin.EX_ERR_CASTOR_LOAD_XML_MAPPING",
              "Mapping file name : " + ( strMappingFileName == null 
                                         ? "<null>" 
                                         : strMappingFileName ),
              me);
      }
      catch ( MarshalException me )
      {
          throw new InstanciationException(
              ...
              "Component descriptor file name : " + ( strDescriptorFileName == null 
                                         ? "<null>" 
                                         : strDescriptorFileName ),
              me);
      }
