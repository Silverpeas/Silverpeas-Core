...
/**
 * The <code> SQLRequest </code> class allows a component to get a SQL request in order by the server type (SQLServer, Oracle, PostgreSQL,...).
 * The role of this class is to read a specific property file (create, delete, update, insert), to extract the specific SQL
 * code with specific tag. And to return the complet rigth SQL code.
...
public class SQLRequest{
	...
	*/
	...
	/**
	* Contains all queries to create multiple tables for one SilverPeas component or to delete rows or to insert data ...
	*/
	private String allQueries;


	/**
	* Silverpeas Object to read properties files.
	* These files contain the SQL command to create, delete, insert and update
	*/
	...
	*/
	protected void setCreateQueries(){
		this.resourceLocation.setPropertyLocation(fullPathName+"."+dbServerName+"_create","fr");
	...
	*/
	protected void setDeleteQueries(){
		this.resourceLocation.setPropertyLocation(fullPathName+"."+dbServerName+"_delete","fr");
		this.allQueries = this.resourceLocation.getString("cle");
	}
	...
	*/
	protected void setInsertQueries(){
		this.resourceLocation.setPropertyLocation(fullPathName+"."+dbServerName+"_insert","fr");
		this.allQueries = this.resourceLocation.getString("cle");
	}
	...
	*/
	protected void setUpdateQueries(){
		this.resourceLocation.setPropertyLocation(fullPathName+"."+dbServerName+"_update","fr");
		this.allQueries = this.resourceLocation.getString("cle");
	}
	...
	* @param tableType (String) the type of the table which must be created (forum, rights, message, event, ...)
	...
	* @param instanceId (String) the id of the component where their data must be updated into tables.
	* @param tableType (String) the type of the table which must be created (forum, rights, message, event, ...)
	...
	* @param instanceId (String) the id of the component where their data must be inserted into tables.
	* @param tableType (String) the type of the table which must be created (forum, rights, message, event, ...)
	...
	*/
	private void init(){
		...
		internalTag.put("__TextFieldLength__",String.valueOf(DBUtil.TextFieldLength));
		internalTag.put("__TextAreaLength__",String.valueOf(DBUtil.TextAreaLength));
		internalTag.put("__DateFieldLength__",String.valueOf(DBUtil.DateFieldLength));
		internalTag.put("__TextMaxiLength__",String.valueOf(DBUtil.TextMaxiLength));
	}

	/**
	* Substitute all intern tag of the file.
	* These tags ( __tableName__, ... ) will be replaced by values received or read into the DBUtil.
	...
	private String substituteInternalTag(String localQuery){
		...
		for (Enumeration e = internalTag.keys();e.hasMoreElements(); ){
			...
			while( posTag != -1 ){
				...
				localQuery = queryBeforeTag+(String)internalTag.get(currentTag)+queryAfterTag;
	...
	*/
	private String getSubQuery(String tableType){
		...
		int beginTableType = allQueries.indexOf("<"+tableType.toUpperCase()+">")+tableType.length()+2;
