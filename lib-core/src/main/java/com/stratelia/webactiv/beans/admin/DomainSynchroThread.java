...
public class DomainSynchroThread extends Thread
{
    ...
    public void run()
    {
        ...
        SilverTrace.info("admin", "DomainSynchroThread.run", "root.MSG_GEN_PARAM_VALUE", "------------DEBUT DU THREAD DE SYNCHRO-----------");
        while (!m_mustStop && (m_Domains.size() > 0))
        {
            for (i = 0; (i < m_Domains.size()) && (!m_mustStop); i++)
            {
                SilverTrace.info("admin", "DomainSynchroThread.run", "root.MSG_GEN_PARAM_VALUE", "------------DEBUT SYNCHRO DOMAINE #" + (String)m_Domains.get(i) + "-----------");
                try
                {
                ...
                {
                    m_ErrorOccured = e;
                    SilverTrace.error("admin", "DomainSynchroThread.run", "root.MSG_GEN_PARAM_VALUE", "------------ERREUR DANS LE THREAD DE SYNCHRO-----------",e);
                }
                SilverTrace.info("admin", "DomainSynchroThread.run", "root.MSG_GEN_PARAM_VALUE", "------------FIN SYNCHRO DOMAINE #" + (String)m_Domains.get(i) + "-----------");
            }
            if (!m_mustStop && (m_Domains.size() > 0))
            {
                ...
                {
                    SilverTrace.info("admin", "DomainSynchroThread.run", "root.MSG_GEN_PARAM_VALUE", "------------ERREUR DANS LE SLEEP DE SYNCHRO-----------",e);
                }
            }
        }
        m_isEncours = false;
        SilverTrace.info("admin", "DomainSynchroThread.run", "root.MSG_GEN_PARAM_VALUE", "------------FIN DU THREAD DE SYNCHRO-----------");
    }
