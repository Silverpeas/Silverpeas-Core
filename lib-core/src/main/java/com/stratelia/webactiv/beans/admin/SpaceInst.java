 ...
 */
public class SpaceInst extends AbstractI18NBean implements Serializable, Comparable
{
	// First page possible types
    final public static int FP_TYPE_STANDARD        = 0; // Page d'acueil standard
    final public static int FP_TYPE_COMPONENT_INST  = 1; // Composant (dans ce cas : firstPageExtraParam = composant instance ID)
    ...
    public void deleteComponentInst(ComponentInst componentInst)
    {
        for(int nI=0; nI < m_alComponentInst.size(); nI++)
            if(((ComponentInst)m_alComponentInst.get(nI)).getName().equals(componentInst.getName()))
            {
    ...
    public ComponentInst getComponentInst(String sComponentName)
    {
        if(m_alComponentInst.size() != 0)
            for(int nI=0; nI < m_alComponentInst.size(); nI++)
                if(((ComponentInst)m_alComponentInst.get(nI)).getName().equals(sComponentName))
    ...
    public void deleteSpaceProfileInst(SpaceProfileInst spaceProfileInst)
    {
        ...
        {
        	SpaceProfileInst profile = (SpaceProfileInst)m_alSpaceProfileInst.get(nI);
    ...
    private SpaceProfileInst getSpaceProfileInst(String sSpaceProfileName, boolean inherited)
    {
        for(int nI=0; nI < m_alSpaceProfileInst.size(); nI++)
        {
        	SpaceProfileInst profile = (SpaceProfileInst)m_alSpaceProfileInst.get(nI);
        	if(profile.isInherited() == inherited && profile.getName().equals(sSpaceProfileName))
        		return profile;
        }
        return null;
    }
