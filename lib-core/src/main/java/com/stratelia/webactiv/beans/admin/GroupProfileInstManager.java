...
public class GroupProfileInstManager 
{
    ...
    public String createGroupProfileInst(GroupProfileInst groupProfileInst, DomainDriverManager DDManager, String sgroupId) throws AdminException
    {
        try
        {
            ...
            for(int nI = 0; nI < groupProfileInst.getNumGroup(); nI++)
				DDManager.organization.groupUserRole.addGroupInGroupUserRole(idAsInt(groupProfileInst.getGroup(nI)), idAsInt(sProfileId) );

            for(int nI = 0; nI < groupProfileInst.getNumUser(); nI++)
				DDManager.organization.groupUserRole.addUserInGroupUserRole(idAsInt(groupProfileInst.getUser(nI)), idAsInt(sProfileId) );

            return sProfileId;
        }
        catch(Exception  e)
        {
			throw new AdminException("GroupProfileInstManager.addGroupProfileInst", SilverpeasException.ERROR, "admin.EX_ERR_ADD_SPACE_PROFILE", "space profile name: '" + groupProfileInst.getName() + "'", e);
        }
    }

    /**
	 * Get Space profile information with given id
	 * and creates a new GroupProfileInst 
	 */
    public GroupProfileInst getGroupProfileInst(DomainDriverManager ddManager,String sProfileId, String sGroupId) throws AdminException
    {
        if(sGroupId == null)
        {
            try
            {
                ...
                GroupRow group = ddManager.organization.group.getGroupOfGroupUserRole(idAsInt(sProfileId));
                if (group == null) group = new GroupRow();
                sGroupId = idAsString(group.id);
            }
            catch(Exception e)
            {
       			throw new AdminException("GroupProfileInstManager.getGroupProfileInst", SilverpeasException.ERROR, "admin.EX_ERR_GET_SPACE_PROFILE", "space profile Id: '" + sProfileId + "', groupId: '" + sGroupId + "'", e);
            }
			finally
			{
	 ...
	 * and store it in the given GroupProfileInst object
	 */
    public void setGroupProfileInst(GroupProfileInst groupProfileInst, DomainDriverManager ddManager, String sProfileId, String sGroupId) throws AdminException
    {
        try
        {
			...
			GroupUserRoleRow groupUserRole = ddManager.organization.groupUserRole.getGroupUserRoleByGroupId(idAsInt(sGroupId));
			...
			if (groupUserRole != null)
			{
				...
				// Get the groups
				String[] asGroupIds = ddManager.organization.group.getDirectGroupIdsInGroupUserRole(idAsInt(sProfileId));
				...
				// Get the Users
				String[] asUsersIds = ddManager.organization.user.getDirectUserIdsOfGroupUserRole(idAsInt(sProfileId));
        ...
        {
    		throw new AdminException("GroupProfileInstManager.setGroupProfileInst", SilverpeasException.ERROR, "admin.EX_ERR_SET_SPACE_PROFILE", "space profile Id: '" + sProfileId + "', groupId = '" + sGroupId + "'", e);
        }
		finally
		{
			ddManager.releaseOrganizationSchema();
	...
	public void deleteGroupProfileInst(GroupProfileInst groupProfileInst, DomainDriverManager ddManager) throws AdminException
    {
        ...
        {
            for(int nI = 0; nI < groupProfileInst.getNumGroup(); nI++)
				ddManager.organization.groupUserRole.removeGroupFromGroupUserRole(idAsInt(groupProfileInst.getGroup(nI)), idAsInt(groupProfileInst.getId()));				

            for(int nI = 0; nI < groupProfileInst.getNumUser(); nI++)
				ddManager.organization.groupUserRole.removeUserFromGroupUserRole(idAsInt(groupProfileInst.getUser(nI)), idAsInt(groupProfileInst.getId()));

            // delete the groupProfile node
			ddManager.organization.groupUserRole.removeGroupUserRole(idAsInt(groupProfileInst.getId()));
        }
        catch(Exception  e)
        {
    		throw new AdminException("GroupProfileInstManager.deleteGroupProfileInst", SilverpeasException.ERROR, "admin.EX_ERR_DELETE_SPACEPROFILE", "space profile Id: '" + groupProfileInst.getId() + "'", e);
        }
    }
    ...
    public String updateGroupProfileInst(GroupProfileInst groupProfileInst, DomainDriverManager ddManager, GroupProfileInst groupProfileInstNew) throws AdminException
    {
        ...
        try
        {
            ...
            for(int nI = 0; nI < alAddGroup.size(); nI++)
            {
				...
				ddManager.organization.groupUserRole.addGroupInGroupUserRole(idAsInt((String)alAddGroup.get(nI)), idAsInt(groupProfileInst.getId()));
            }
            ...
            for(int nI = 0; nI < alRemGroup.size(); nI++)
            {
				...
				ddManager.organization.groupUserRole.removeGroupFromGroupUserRole(idAsInt((String)alRemGroup.get(nI)), idAsInt(groupProfileInst.getId()));
            }
            ...
            for(int nI = 0; nI < alAddUser.size(); nI++)
            {
				...
				ddManager.organization.groupUserRole.addUserInGroupUserRole(idAsInt((String)alAddUser.get(nI)), idAsInt(groupProfileInst.getId()));
            }
            ...
            for(int nI = 0; nI < alRemUser.size(); nI++)
            {
				...
				ddManager.organization.groupUserRole.removeUserFromGroupUserRole(idAsInt((String)alRemUser.get(nI)), idAsInt(groupProfileInst.getId()));
            }
        ...
        {
    		throw new AdminException("GroupProfileInstManager.updateGroupProfileInst", SilverpeasException.ERROR, "admin.EX_ERR_UPDATE_SPACEPROFILE", "space profile Id: '" + groupProfileInst.getId() + "'", e);
        }
    }
	 ...
	 */
	public String[] getGroupProfileIdsOfUser(DomainDriverManager ddManager, String sUserId) throws AdminException
	{
		try
		{
			ddManager.getOrganizationSchema();
			return ddManager.organization.groupUserRole.getAllGroupUserRoleIdsOfUser(idAsInt(sUserId));
        }
        catch(Exception  e)
        {
    		throw new AdminException("GroupProfileInstManager.getSpaceProfileIdsOfUser", SilverpeasException.ERROR, "admin.EX_ERR_GET_USER_SPACEPROFILES", "user Id: '" + sUserId + "'", e);
        }
		finally
		{
			ddManager.releaseOrganizationSchema();
