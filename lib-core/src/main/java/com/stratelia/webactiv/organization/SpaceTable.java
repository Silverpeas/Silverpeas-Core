...
public class SpaceTable extends Table
{
       ...
       throws SQLException
   {
       ...
       if (rs.wasNull()) s.updatedBy = -1;
       
       s.removedBy = rs.getInt(14);
       if (rs.wasNull()) s.removedBy = -1;
   ...
   public SpaceRow[] getAllSpaces()
       throws AdminPersistenceException
   {
       return (SpaceRow[]) getRows(SELECT_ALL_SPACES)
           .toArray(new SpaceRow[0]);
   }

   static final private String SELECT_ALL_SPACES
      = "select "+SPACE_COLUMNS+" from ST_Space"
      + " order by orderNum";
   ...
   public String[] getAllSpaceIds()
       throws AdminPersistenceException
   {
       return (String[]) getIds(SELECT_ALL_SPACE_IDS)
           .toArray(new String[0]);
   }

   static final private String SELECT_ALL_SPACE_IDS
      = "select id from ST_Space"
      + " order by orderNum";
   ...
   public String[] getAllRootSpaceIds()
       throws AdminPersistenceException
   {
       return (String[]) getIds(SELECT_ALL_ROOT_SPACE_IDS)
           .toArray(new String[0]);
   }

   static final private String SELECT_ALL_ROOT_SPACE_IDS
      = "select id from ST_Space"
    ...
    */
   public SpaceRow[] getAllRootSpaces()
       throws AdminPersistenceException
   {
   ...
   static final private String SELECT_ALL_ROOT_SPACES
      = "select "+SPACE_COLUMNS+" from ST_Space"
       +" where domainFatherId is null"
       +" AND spaceStatus is null"
       +" order by orderNum";
   ...
   public SpaceRow[] getRemovedSpaces()
       throws AdminPersistenceException
   {
       return (SpaceRow[]) getRows(SELECT_REMOVED_SPACES)
           .toArray(new SpaceRow[0]);
   }

   static final private String SELECT_REMOVED_SPACES
      = "select "+SPACE_COLUMNS+" from ST_Space"
       +" where spaceStatus = '"+SpaceInst.STATUS_REMOVED+"'"
   ...
   static final private String SELECT_INSTANCE_SPACE
      = "select "+aliasColumns("s", SPACE_COLUMNS)
	  + " from ST_Space s, ST_ComponentInstance i"
	  + " where s.id = i.spaceId"
	  + " and   i.id = ?";
   ...
   public SpaceRow[] getAllRootSpacesOfUser(int userId)
       throws AdminPersistenceException
   {
       return (SpaceRow[]) getRows(SELECT_ALL_USER_ROOT_SPACES, userId)
           .toArray(new SpaceRow[0]);
   }

   static final private String SELECT_ALL_USER_ROOT_SPACES
   		= "select "+aliasColumns("ST_Space", SPACE_COLUMNS)
   ...
   public SpaceRow[] getAllSpacesOfUser(int userId)
       throws AdminPersistenceException
   {
       return (SpaceRow[]) getRows(SELECT_ALL_USER_SPACES, userId)
           .toArray(new SpaceRow[0]);
   }
       ...
       throws AdminPersistenceException
   {
	   ...
	   ids[2] = spaceId;
	   ids[3] = spaceId;
       return (SpaceRow[]) getRows(SELECT_USER_SUBSPACES, ids).toArray(new SpaceRow[0]);
   }

   static final private String SELECT_USER_SUBSPACES
      = "select "+aliasColumns("ST_Space", SPACE_COLUMNS)
   ...
   public String[] getDirectSubSpaceIds(int superSpaceId)
       throws AdminPersistenceException
   {
       return (String[]) getIds(SELECT_SUBSPACE_IDS, superSpaceId)
           .toArray(new String[0]);
   }
       ...
       throws AdminPersistenceException
   {
      ...
      if (space.domainFatherId != -1)
      {
          ...
          if (superSpace == null)
          {
			  throw new AdminPersistenceException("SpaceTable.createSpace", SilverpeasException.ERROR, "admin.EX_ERR_SPACE_NOT_FOUND", "father space id : '" + space.domainFatherId + "'");
          }
      }
      ...
      if (superSpace != null)
      {
          organization.userSet.addUserSetInUserSet("S", space.id,
                                                   "S", superSpace.id);
      }

      CallBackManager.invoke(CallBackManager.ACTION_AFTER_CREATE_SPACE, space.id, null, null);
   }

   static final private String INSERT_SPACE
      = "insert into"
      + " ST_Space("+SPACE_COLUMNS+")"
      + " values  (? ,? ,? ,? ,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
       ...
       throws SQLException
   {
       ...
       if (s.domainFatherId == -1) insert.setNull(2, Types.INTEGER);
       else insert.setInt(2, s.domainFatherId);
       ...
       // space creator
       if (s.createdBy == -1) insert.setNull(5, Types.INTEGER);
       else                   insert.setInt(5, s.createdBy);
   ...
   public void updateSpaceInheritance(int spaceId, boolean inheritanceBlocked) throws AdminPersistenceException
   {
	   int iInheritance = 0;
       ...
       throws SQLException
   {
	   ...
	   if (s.domainFatherId==0) update.setNull(1, Types.INTEGER);
	   else						update.setInt(1, s.domainFatherId);
       update.setString(2, truncate(s.name,100));
       update.setString(3, truncate(s.description,500));
       if (s.createdBy == -1) update.setNull(4, Types.INTEGER);
       else                   update.setInt(4, s.createdBy);
       ...
       if (s.updatedBy == -1) update.setNull(9, Types.INTEGER);
       else                   update.setInt(9, s.updatedBy);
       ...
       throws AdminPersistenceException
   {
       ...
       ComponentInstanceRow[] instances
           = organization.instance.getAllComponentInstancesInSpace(id);
   ...
   static final private String DELETE_SPACE
      = "delete from ST_Space where id = ?";
       ...
       throws AdminPersistenceException
   {
       ...
       {
		  throw new AdminPersistenceException("SpaceTable.sendSpaceToBasket", SilverpeasException.ERROR, "admin.EX_ERR_UPDATE", e);
       }
       finally
       {
       ...
       throws AdminPersistenceException
   {
       ...
       {
		  throw new AdminPersistenceException("SpaceTable.removeSpaceFromBasket", SilverpeasException.ERROR, "admin.EX_ERR_UPDATE", e);
       }
       finally
       {
    	   organization.releaseStatement(statement);
   ...
   public SpaceRow getSpaceOfSpaceUserRole(int spaceUserRoleId)
       throws AdminPersistenceException
   {
       return (SpaceRow) getUniqueRow(
           SELECT_SPACEUSERROLE_SPACE, spaceUserRoleId);
   }

   static final private String SELECT_SPACEUSERROLE_SPACE
      = "select "+aliasColumns("i", SPACE_COLUMNS)
	  + " from ST_Space i, ST_SpaceUserRole us"
	  + " where i.id = us.spaceId"
	  + " and   us.id = ?";
