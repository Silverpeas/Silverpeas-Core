...
public class DomainTable extends Table
{
   ...
   static final private String SELECT_DOMAIN_BY_ID
      = "select "+DOMAIN_COLUMNS+" from ST_Domain where id = ?";
   ...
   public DomainRow[] getAllDomains()
       throws AdminPersistenceException
   {
       return (DomainRow[]) getRows(SELECT_ALL_DOMAINS)
           .toArray(new DomainRow[0]);
   }

   static final private String SELECT_ALL_DOMAINS
      = "select "+DOMAIN_COLUMNS+" from ST_Domain where not id=-1 order by name asc";
   ...
   static final private String UPDATE_DOMAIN
      = "update ST_Domain set"
      + " name = ?,"
      + " description = ?,"
      + " propFileName = ?,"
      + " className = ?,"
   ...
   static final private String DELETE_DOMAIN
      = "delete from ST_Domain where id = ?";
