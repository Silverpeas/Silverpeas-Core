...
public class LDAPUtility extends Object
{
    ...
    static 
    {
        ResourceLocator rs = new ResourceLocator("com.stratelia.silverpeas.domains.allLDAP", "");
        String          res;
        ...
        {
            stringReplace[i][STRING_REPLACE_FROM] = rs.getString("stringReplace_" + Integer.toString(i+1) + ".From","");
            stringReplace[i][STRING_REPLACE_TO] = rs.getString("stringReplace_" + Integer.toString(i+1) + ".To","");
        }
    }
     ...
     * @see
     */
    static public LDAPConnection getConnection(String connectionId) throws AdminException
    {
     ...
     * @see
     */
    static public boolean recoverConnection(String connectionId, LDAPException ex) throws AdminException
    {
        int     nbRetry = 0;
    ...
    static private void InternalOpenConnection(String connectionId) throws AdminException
    {
        ...
        catch (LDAPException e)
        {
            ...
            {
                SilverTrace.error("admin", "LDAPUtility.openConnection", "admin.EX_ERR_LDAP_GENERAL", "ERROR CLOSING CONNECTION : ConnectionId=" + connectionId + " Error LDAP #" + Integer.toString(e.getResultCode()) + " " + e.getLDAPErrorMessage(), ee);
            }
            throw new AdminException("LDAPUtility.openConnection", SilverpeasException.ERROR, "admin.EX_ERR_LDAP_GENERAL", "Host : " + driverSettings.getLDAPHost() + " Port : " + Integer.toString(driverSettings.getLDAPPort()) + " LDAPLogin : " + driverSettings.getLDAPAccessLoginDN() + " ProtocolVer : " + Integer.toString(driverSettings.getLDAPProtocolVer()), e);
        }
    }
     ...
     * @see
     */
    static private void InternalCloseConnection(String connectionId) throws AdminException
    {
        ...
        if ((toClose != null) && (toClose.isConnected()))
        {
            ...
            {
                throw new AdminException("LDAPUtility.closeConnection", SilverpeasException.ERROR, "admin.EX_ERR_LDAP_GENERAL", "ConnectionId=" + connectionId + " Error LDAP #" + Integer.toString(e.getResultCode()) + " " + e.getLDAPErrorMessage(), e);
            }
        }
    }
     ...
     * If this attribute have multiple values, only the first is returned
     * 
     ...
     * @param scope   the scope (LDAPConnection.SCOPE_BASE, LDAPConnection.SCOPE_ONE or LDAPConnection.SCOPE_SUB)
...
//			SynchroReport.debug("LDAPUtility.getFirstEntryFromSearch()", "Requête LDAP : BaseDN="+baseDN+" scope="+Integer.toString(scope)+" Filter="+sureFilter,null);
			// Modif LBE : as more than on baseDN can be set, iterate on all baseDNs and stop when first entry is found
        ...
        {
            throw new AdminException("LDAPUtility.getFirstEntryFromSearch", SilverpeasException.ERROR, "admin.EX_ERR_LDAP_REFERRAL", "#" + Integer.toString(re.getResultCode()) + " " + re.getLDAPErrorMessage(), re);
        }
        catch (LDAPException e)
        {
            ...
            {
                throw new AdminException("LDAPUtility.getFirstEntryFromSearch", SilverpeasException.ERROR, "admin.EX_ERR_LDAP_GENERAL", "#" + Integer.toString(e.getResultCode()) + " " + e.getLDAPErrorMessage(), e);
            }
        }
        return theEntry;
    }
    ...
    static public String normalizeFilterValue(String theFilter)
    {
        ...
        for (int i = 0;i < stringReplaceSize; i++)
        {
            valret = replaceMultiple(valret, stringReplace[i][STRING_REPLACE_FROM], stringReplace[i][STRING_REPLACE_TO]);
        }
        return valret;
    }

    static protected String replaceMultiple(String theStringSrc, String fromString, String toString)
    {
        int          fromIndex = 0;
        int          bsPos;
        ...
        if (bsPos != -1)
        {
            ...
            {
                sb.append(theStringSrc.substring(fromIndex,theStringSrc.length())); // Copy the end of the String
            }
            return sb.toString();
        }
        else
     ...
     * @see
     */
    static public LDAPEntry[] search1000Plus(String lds, String baseDN, int scope, String filter, String varToSort, String[] args) throws AdminException
    {
        ...
        try
        {
			...
			for (int j=0; j<baseDNs.length; j++)
			{
	            ...
	            {
	                SilverTrace.debug("admin", "LDAPUtility.search1000Plus()", "LDAP query", "BaseDN="+baseDNs[j]+" scope="+Integer.toString(scope)+" Filter="+theFullFilter);
					SynchroReport.debug("LDAPUtility.search1000Plus()", "Requête sur le domaine LDAP distant (protocole v" + ld.getProtocolVersion() + "), BaseDN="+baseDNs[j]+" scope="+Integer.toString(scope)+" Filter="+theFullFilter,null);
	
					try
					{
						res = ld.search(baseDNs[j], scope, theFullFilter, args, false, cons);
						while (res.hasMore())
						{
					...
					catch (LDAPException le)
					{
						...
						{
							//throw new AdminException("LDAPUtility.search1000Plus", SilverpeasException.ERROR, "admin.EX_ERR_LDAP_REFERRAL", "#" + Integer.toString(le.getResultCode()) + " " + le.getLDAPErrorMessage(), le);
	                ...
	                {
	                    notTheFirst = true;
	                    theFullFilter = "(&" + filter + "(" + varToSort + ">=" + LDAPUtility.getFirstAttributeValue((LDAPEntry) entriesVector.get(nbReaded - 1), varToSort) + "))";
	                    SilverTrace.info("admin", "LDAPUtility.search1000Plus()", "root.MSG_GEN_PARAM_VALUE", "TOO MANY SEARCHES : " + theFullFilter);
	                }*/
					...
					else if (timeLimitReached && (nbRetryTimeLimit>MAX_NB_RETRY_TIMELIMIT))
					{
						throw lastException;
					}
	                else
        ...
        catch (LDAPReferralException re)
        {
            ...
            throw new AdminException("LDAPUtility.search1000Plus", SilverpeasException.ERROR, "admin.EX_ERR_LDAP_REFERRAL", "#" + Integer.toString(re.getResultCode()) + " " + re.getLDAPErrorMessage(), re);
        }
        catch (LDAPException e)
        {
            ...
            {
                throw new AdminException("LDAPUtility.search1000Plus", SilverpeasException.ERROR, "admin.EX_ERR_LDAP_GENERAL", "#" + Integer.toString(e.getResultCode()) + " " + e.getLDAPErrorMessage(), e);
            }
        }
