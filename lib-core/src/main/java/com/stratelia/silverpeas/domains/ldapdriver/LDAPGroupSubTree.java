 ...
 * The group object are root to their childs
 * 
 * Method used to retreive users and groups that are direct childs of a given group : GroupG (i.e. childs that have no groups between group node to their node) :
 * 1) Search all child groups of GroupG
 * 2) Sort the result by DN
 * 3) Remove from the search all the groups that are not directly under GroupG (i.e. there is another group between)
 * => Those groups are the direct childs of GroupG (result of "getChildGroupsEntryByLDAPEntry" call)
 * 4) Performs a search of the users that have for baseDN GroupG's DN and that have NOT : (DN substring of any direct child groups' DN)
 * 
 * @author tleroi
 */
...
public class LDAPGroupSubTree extends AbstractLDAPGroup
{
    protected String[] getMemberGroupIds(String lds, String memberId, boolean isGroup) throws AdminException
    {
        ...
        if (memberEntry == null)
        {
			throw new AdminException("LDAPGroupSubTree.getMemberGroupIds", SilverpeasException.ERROR, "admin.EX_ERR_LDAP_USER_ENTRY_ISNULL", "Id="+memberId+" IsGroup="+isGroup);
        }
    ...
    private LDAPEntry[] getChildGroupsEntryByLDAPEntry(String lds, LDAPEntry parentEntry) throws AdminException
    {
        ...
        for (i = 0; i < sortResult.length; i++)
        {
            ...
            if ((sortResult[i].getDN().equalsIgnoreCase(searchDN) == false) && ((sortResult[i].getDN().endsWith(previousDN) == false) || (previousDN.length() <= 0)))
            {
                includeGroup = true;
