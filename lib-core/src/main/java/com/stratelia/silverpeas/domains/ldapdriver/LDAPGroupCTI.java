 ...
 * 
 * For CTI : Add new functionality : - Build Name from path (dn)
 *                                   - Use dn as key
 * 
 * @author tleroi
 */
...
public class LDAPGroupCTI extends AbstractLDAPGroup
{
        ...
        while (groupsSet.size() > 0)
        {
            ...
            while (it.hasNext())
            {
                ...
                if (curGroup != null)
                {
                    ...
                    {
                        SilverTrace.error("admin", "LDAPGroupAllRoot.getUserIds()", "admin.MSG_ERR_LDAP_GENERAL", "GROUP NOT FOUND : " + grId, e);
                    }
                }
            }
            groupsSet = groupsCur;
    ...
    protected Vector getTRUEUserIds(String lds, LDAPEntry groupEntry) throws AdminException
    {
        ...
        SilverTrace.info("admin", "LDAPGroupAllRoot.getTRUEUserIds()", "root.MSG_GEN_ENTER_METHOD");
        ...
        else
        {
            ...
            catch (AdminException e)
            {
                if (synchroInProcess)
                {
					...
					SynchroReport.error("LDAPGroupAllRoot.getChildGroupsEntry()", "Erreur lors de la récupération des groupes racine",e);
                }
                else
                {
                    throw e;
    ...
    protected Vector getTRUEChildGroupsEntry(String lds, String parentId, LDAPEntry theEntry)
    {
        ...
        if ((parentId != null) && (parentId.length() > 0))
        {
			...
			// Checks cache
            if ( (TRUEChildGroupsEntrys!=null) && ( TRUEChildGroupsEntrys.containsKey(parentId) ) )
    ...
    // Surcharge de la fonction de la classe abstraite pour pouvoir construire le nom
    protected Group translateGroup(String lds,LDAPEntry groupEntry) throws AdminException
    {
        ...
        if (driverSettings.getGroupsNamingDepth() > 0)
        {
            ...
            while (theDN.hasMoreTokens() && (i < driverSettings.getGroupsNamingDepth())) 
            {
                theDN.nextToken();
                if (i > 0)
                {
        ...
        {
            theName = LDAPUtility.getFirstAttributeValue(groupEntry,driverSettings.getGroupsNameField());
        }
    ...
    * overidded to manage local cache
    */
