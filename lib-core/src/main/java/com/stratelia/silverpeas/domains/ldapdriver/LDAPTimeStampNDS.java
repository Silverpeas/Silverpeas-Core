...
public class LDAPTimeStampNDS  extends AbstractLDAPTimeStamp
{
    ...
    public void initFromServer(String lds, String baseDN, String filter, String fallbackSortBy) throws AdminException
    {
        ...
        LDAPEntry[]    theEntries = LDAPUtility.search1000Plus(lds, baseDN, driverSettings.getScope(), "(&(" + driverSettings.getTimeStampVar() + ">=" + timeStamp + ")" + filter + ")", fallbackSortBy, ttv);
        SilverTrace.info("admin", "LDAPTimeStampNDS.initFromServer()", "root.MSG_GEN_PARAM_VALUE", "# entries = "+theEntries.length);
        
    	String ttCurrent = "";
    	for (int i = 0; i < theEntries.length; i++)
    	{
        	ttCurrent = LDAPUtility.getFirstAttributeValue(theEntries[i],driverSettings.getTimeStampVar());
