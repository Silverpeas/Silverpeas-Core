...
public class LDAPGroupUniqueDescriptor extends AbstractLDAPGroup
{
    protected String[] getMemberGroupIds(String lds, String memberId, boolean isGroup) throws AdminException
    {
        ...
        if (memberEntry == null)
        {
			throw new AdminException("LDAPGroupUniqueDescriptor.getMemberGroupIds", SilverpeasException.ERROR, "admin.EX_ERR_LDAP_USER_ENTRY_ISNULL",  "Id="+memberId+" IsGroup="+isGroup);
        }
    ...
    protected String[] getUserIds(String lds, LDAPEntry groupEntry) throws AdminException
    {
        ...
        for (i = 0; i < stringVals.length; i++)
        {
            try
            {
//                userEntry = LDAPUtility.getFirstEntryFromSearch(lds, driverSettings.getLDAPUserBaseDN(), driverSettings.getScope(), "(&" + driverSettings.getUsersFullFilter() + "(distinguishedName=" + LDAPUtility.dblBackSlashesForDNInFilters(stringVals[i]) + "))");
            ...
            if ((parentId != null) && (parentId.length() > 0))
            {
                ...
                for (i = 0; i < stringVals.length; i++)
                {
                    try
                    {
                        ...
                        {
//                            theFilter = "(&" + extraFilter + "(&" + driverSettings.getGroupsFullFilter() + "(distinguishedName=" + LDAPUtility.dblBackSlashesForDNInFilters(stringVals[i]) + "))" + ")";
                            theFilter = "(&" + extraFilter + driverSettings.getGroupsFullFilter() + ")";
                        }
                        else
                        {
//                            theFilter = "(&" + driverSettings.getGroupsFullFilter() + "(distinguishedName=" + LDAPUtility.dblBackSlashesForDNInFilters(stringVals[i]) + "))";
                            theFilter = driverSettings.getGroupsFullFilter();
                        }
//                        childGroupEntry = LDAPUtility.getFirstEntryFromSearch(lds, driverSettings.getGroupsSpecificGroupsBaseDN(), driverSettings.getScope(), theFilter);
                        childGroupEntry = LDAPUtility.getFirstEntryFromSearch(lds, stringVals[i], driverSettings.getScope(), theFilter);
            ...
            else  // Retreives the ROOT groups : the groups that are under the base DN but that are not member of another group...
            {
                if ((extraFilter != null) && (extraFilter.length() > 0))
                {
                    theFilter = "(&" + extraFilter + driverSettings.getGroupsFullFilter() + ")";
                ...
                for (i = 0; i < theEntries.length; i++)
                {
                    // Search for groups that have at least one member attribute that point to the group
                    try
                    {
                        parentGroupEntry = LDAPUtility.getFirstEntryFromSearch(lds, driverSettings.getGroupsSpecificGroupsBaseDN(), driverSettings.getScope(), "(&" + driverSettings.getGroupsFullFilter() + "(" + driverSettings.getGroupsMemberField() + "=" + LDAPUtility.dblBackSlashesForDNInFilters(theEntries[i].getDN()) + "))");
                    }
                    catch (AdminException e)
                    {
                        ...
                        parentGroupEntry = null; // If query failed, set this group as a root group
                    }
