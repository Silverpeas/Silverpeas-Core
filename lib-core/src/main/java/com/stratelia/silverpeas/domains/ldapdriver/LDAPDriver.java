...
* IMPORTANT : For the moment, this is a read-only driver (not possible to add, remove or update a group or a user.
* 
* @author tleroi
*/
public class LDAPDriver extends AbstractDomainDriver
{
   ...
   { 
       SilverTrace.info("admin","LDAPDriver.isSynchroOnLoginEnabled","root.MSG_GEN_ENTER_METHOD","Enabled = " + driverSettings.isSynchroAutomatic() + " - Synchro In Process = " + synchroInProcess);
       return driverSettings.isSynchroAutomatic(); 
   }

   public boolean isSynchroOnLoginRecursToGroups() 
   { 
       SilverTrace.info("admin","LDAPDriver.isSynchroOnLoginRecursToGroups","root.MSG_GEN_ENTER_METHOD","RecursToGroups = " + driverSettings.isSynchroRecursToGroups() + " - Synchro In Process = " + synchroInProcess);
       return driverSettings.isSynchroRecursToGroups(); 
   }

   public boolean isGroupsInheritProfiles() 
   { 
       SilverTrace.info("admin","LDAPDriver.isGroupsInheritProfiles","root.MSG_GEN_ENTER_METHOD","GroupsInheritProfiles = " + driverSettings.isGroupsInheritProfiles() + " - Synchro In Process = " + synchroInProcess);
       return driverSettings.isGroupsInheritProfiles(); 
   }
   
   public boolean mustImportUsers()
   {
	   SilverTrace.info("admin","LDAPDriver.mustImportUsers", "root.MSG_GEN_ENTER_METHOD", "MustImportUsers = " + driverSettings.mustImportUsers() + " - Synchro In Process = " + synchroInProcess);
   ...
   { 
        ...
        try
        {
            if (driverSettings.getTimeStampVar().length() > 0)
            {
                usersReturned = userTranslator.getAllUsers(ld,"(&(" + driverSettings.getTimeStampVar() + ">=" + fromTimeStamp + ")(" + driverSettings.getTimeStampVar() + "<=" + toTimeStamp + "))");
            }
            else
            {
   ...
   public Group[] getAllChangedGroups(String fromTimeStamp, String toTimeStamp) throws Exception
   {
        ...
        try
        {
            if (driverSettings.getTimeStampVar().length() > 0)
            {
                groupsReturned = groupTranslator.getAllChangedGroups(ld,"(&(" + driverSettings.getTimeStampVar() + ">=" + fromTimeStamp + ")(" + driverSettings.getTimeStampVar() + "<=" + toTimeStamp + "))");
            }
            else
            {
