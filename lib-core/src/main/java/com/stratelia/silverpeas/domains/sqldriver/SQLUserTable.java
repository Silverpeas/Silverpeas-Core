...
public class SQLUserTable
{
	   ...
	   public int createUser(Connection c, UserDetail user) throws AdminException
	   {
			...
			try
			{
				...
				nextId = DBUtil.getNextId(drvSettings.getUserTableName(), drvSettings.getUserSpecificIdColumnName());
			...
			{
			   throw new AdminException("SQLUserTable.createUser", SilverpeasException.ERROR, "root.EX_SQL_QUERY_FAILED", "Query = " + theQuery, e);
			}
			finally
			{
	   ...
	   public void deleteUser(Connection c, int userId) throws AdminException
	   {
			...
			{
			   throw new AdminException("SQLUserTable.deleteUser", SilverpeasException.ERROR, "root.EX_SQL_QUERY_FAILED", "Query = " + theQuery, e);
			}
			finally
			{
			     DBUtil.close(statement);
	   ...
	   public void updateUser(Connection c, UserDetail ud) throws AdminException
	   {
			...
			String theQuery = "update " + drvSettings.getUserTableName() + " set " 
							+ drvSettings.getUserFirstNameColumnName() + " = ?," + drvSettings.getUserLastNameColumnName()+ " = ?,"  
							+ drvSettings.getUserEMailColumnName() + " = ?," + drvSettings.getUserLoginColumnName() + " = ? " 
							+ " where " + drvSettings.getUserSpecificIdColumnName() + " = ?";
			...
			{
			   throw new AdminException("SQLUserTable.updateUser", SilverpeasException.ERROR, "root.EX_SQL_QUERY_FAILED", "Query = " + theQuery, e);
			}
			finally
			{
			     DBUtil.close(statement);
	   ...
	   public void updateUserPassword(Connection c, int userId, String value) throws AdminException
	   {
	   		if (drvSettings.isUserPasswordAvailable())
	   		{
				PreparedStatement statement = null;
				String theQuery = "update " + drvSettings.getUserTableName() + " set " 
								  + drvSettings.getUserPasswordColumnName() + " = ?" 
								  + " where " + drvSettings.getUserSpecificIdColumnName() + " = ?";
				...
				{
				   throw new AdminException("SQLUserTable.updateUserPassword", SilverpeasException.ERROR, "root.EX_SQL_QUERY_FAILED", "Query = " + theQuery, e);
				}
				finally
				{
				     DBUtil.close(statement);
	   ...
	   public void updateUserPasswordValid(Connection c, int userId, boolean value) throws AdminException
	   {
	   		if (drvSettings.isUserPasswordValidAvailable())
	   		{
				PreparedStatement statement = null;
				String theQuery = "update " + drvSettings.getUserTableName() + " set " 
								  + drvSettings.getUserPasswordValidColumnName() + " = ?" 
								  + " where " + drvSettings.getUserSpecificIdColumnName() + " = ?";
				...
				{
				   throw new AdminException("SQLUserTable.updateUserPasswordValid", SilverpeasException.ERROR, "root.EX_SQL_QUERY_FAILED", "Query = " + theQuery, e);
				}
				finally
				{
				     DBUtil.close(statement);
	   ...
	   public ArrayList getAllUserIds(Connection c) throws AdminException
	   {
			...
			{
			   throw new AdminException("SQLUserTable.getDirectGroupIdsOfUser", SilverpeasException.ERROR, "root.EX_SQL_QUERY_FAILED", "Query = " + theQuery, e);
			}
			finally
			{
	   ...
	   public ArrayList getAllUsers(Connection c) throws AdminException
	   {
			...
			PreparedStatement statement = null;
		    ArrayList theResult = new ArrayList();
		    String theQuery = "select " + getColumns() + " from " + drvSettings.getUserTableName();
			...
			{
			   throw new AdminException("getAllUsers", SilverpeasException.ERROR, "root.EX_SQL_QUERY_FAILED", "Query = " + theQuery, e);
			}
			finally
			{
			     DBUtil.close(rs,statement);
	    ...
	    * Returns all the groups in a given userRole (not recursive).
	    */
	   public ArrayList getUsersBySpecificProperty(Connection c, String propertyName, String value) throws AdminException
	   {
			ResultSet rs = null;
			PreparedStatement statement = null;
		    ArrayList theResult = new ArrayList();
			...
			{
			   throw new AdminException("getUsersBySpecificProperty", SilverpeasException.ERROR, "root.EX_SQL_QUERY_FAILED", "Query = " + theQuery, e);
			}
			finally
			{
			     DBUtil.close(rs,statement);
   ...
   public UserDetail getUser(Connection c, int userId) throws AdminException
   {
		ResultSet rs = null;
		PreparedStatement statement = null;
		String theQuery = "select " + getColumns() + " from " + drvSettings.getUserTableName() + " where id = ?";
		...
		{
		   throw new AdminException("SQLUserTable.getUser", SilverpeasException.ERROR, "root.EX_SQL_QUERY_FAILED", "Query = " + theQuery, e);
		}
		finally
		{
		     DBUtil.close(rs,statement);
   ...
   public String getUserSpecificProperty(Connection c, int userId, DomainProperty dp) throws AdminException
   {
		ResultSet rs = null;
		PreparedStatement statement = null;
		String theQuery = "select " + dp.getMapParameter() + " from " + drvSettings.getUserTableName() + " where id = ?";
		...
		{
		   throw new AdminException("SQLUserTable.getUserSpecificProperty", SilverpeasException.ERROR, "root.EX_SQL_QUERY_FAILED", "Query = " + theQuery, e);
		}
		finally
		{
		     DBUtil.close(rs,statement);
		...
		if (drvSettings.isUserPasswordAvailable())
   		{
			ResultSet rs = null;
			PreparedStatement statement = null;
			String theQuery = "select " + drvSettings.getUserPasswordColumnName() + " from " + drvSettings.getUserTableName() + " where id = ?";
			...
			{
			   throw new AdminException("SQLUserTable.getUserPassword", SilverpeasException.ERROR, "root.EX_SQL_QUERY_FAILED", "Query = " + theQuery, e);
			}
			finally
			{
			     DBUtil.close(rs,statement);
		...
		if (drvSettings.isUserPasswordValidAvailable())
   		{
			ResultSet rs = null;
			PreparedStatement statement = null;
			String theQuery = "select " + drvSettings.getUserPasswordValidColumnName() + " from " + drvSettings.getUserTableName() + " where id = ?";
			...
			{
			   throw new AdminException("SQLUserTable.getUserPasswordValid", SilverpeasException.ERROR, "root.EX_SQL_QUERY_FAILED", "Query = " + theQuery, e);
			}
			finally
			{
			     DBUtil.close(rs,statement);
