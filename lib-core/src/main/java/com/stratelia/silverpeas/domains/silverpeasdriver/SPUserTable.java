...
public class SPUserTable extends Table {
  ...
  static final private String USER_COLUMNS = "id,firstName,lastName,phone,homePhone,"
   ...
   */
  protected SPUserRow fetchUser(ResultSet rs) throws SQLException {
    String toBoolean;
  ...
  public SPUserRow[] getDirectUsersOfGroup(int groupId)
      throws AdminPersistenceException {
    return (SPUserRow[]) getRows(SELECT_USERS_IN_GROUP, groupId).toArray(
        new SPUserRow[0]);
  }
  ...
  static final private String SELECT_USER_IDS_IN_GROUP = "select DomainSP_User.id"
      + " from DomainSP_User,DomainSP_Group_User_Rel"
      + " where id = userId and groupId = ?";
  ...
  public SPUserRow[] getAllMatchingUsers(SPUserRow sampleUser)
      throws AdminPersistenceException {
    String[] columns = new String[] { "firstName", "lastName", "phone",
        "homePhone", "cellPhone", "fax", "address", "title", "company",
        "position", "email", "loginMail" };
    String[] values = new String[] { sampleUser.firstName, sampleUser.lastName,
        sampleUser.phone, sampleUser.homePhone, sampleUser.cellPhone,
        sampleUser.fax, sampleUser.address, sampleUser.title,
        sampleUser.company, sampleUser.position, sampleUser.email,
  ...
  static final private String INSERT_USER = "insert into DomainSP_User" + "("
      + " id,firstName,lastName,phone,homePhone,"
      + " cellPhone,fax,address,title,company,position,boss,"
      + " email, loginMail, login,password,passwordValid" + ") values ("
      + " ?,?,?,?,?," + " ?,?,?,?,?,?,?," + " ?,?,?,?,?" + ")";
   ...
   * Update a user row.
   */
  public void updateUserDetailOnly(SPUserRow user)
      throws AdminPersistenceException {
    ...
    try {
      ...
      throw new AdminPersistenceException("SPUserTable.updateUserDetailOnly",
  ...
  static final private String UPDATE_USER_DETAIL_ONLY = "update DomainSP_User set"
      + " firstName = ?,"
      + " lastName = ?,"
      + " email = ?,"
      + " login = ?"
  ...
  static final private String UPDATE_USER = "update DomainSP_User set"
      + " firstName = ?," + " lastName = ?," + " phone = ?,"
      + " homePhone = ?," + " cellPhone = ?," + " fax = ?," + " address = ?,"
      + " title = ?," + " company = ?," + " position = ?," + " boss = ?,"
      + " email = ?," + " loginMail = ?," + " login = ?," + " password = ?,"
  ...
  static final private String DELETE_USER = "delete from DomainSP_User where id = ?";
