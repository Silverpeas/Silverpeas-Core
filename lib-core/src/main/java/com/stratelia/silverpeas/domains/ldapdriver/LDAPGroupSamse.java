 ...
 * Example (with the attribute 'memberUid') :
 * memberUid 003478
 * memberUid 004512
 * ....
...
public class LDAPGroupSamse extends AbstractLDAPGroup
{
    protected Vector getMemberGroupIds(String lds, String memberId, boolean isGroup) throws AdminException
    {
        ...
        if (isGroup)
        {
            memberEntry = LDAPUtility.getFirstEntryFromSearch(lds, driverSettings.getGroupsSpecificGroupsBaseDN(), driverSettings.getScope(), driverSettings.getGroupsIdFilter(memberId));
        }
        else
        {
        ...
        theEntries = LDAPUtility.search1000Plus(lds, driverSettings.getGroupsSpecificGroupsBaseDN(), driverSettings.getScope(), "(&" + driverSettings.getGroupsFullFilter() + "(" + driverSettings.getGroupsMemberField() + "=" + memberId + "))", driverSettings.getGroupsNameField());
    ...
    {
    	SilverTrace.info("admin", "LDAPGroupSamse.getTRUEUserIds()", "root.MSG_GEN_ENTER_METHOD", "lds = "+lds+", group = "+groupEntry.getDN());
        ...
        {
        	SilverTrace.info("admin", "LDAPGroupSamse.getTRUEUserIds()", "root.MSG_GEN_PARAM_VALUE", "stringVals["+i+"] = "+stringVals[i]);
        	usersVector.add(stringVals[i]);
            /*try
            {
        		//Case of most common OpenLDAP implementation.
        		//memberUid = specificId
        		//Verify that the user exist in the scope. Is it necessary for Samse ?
        		userEntry = LDAPUtility.getFirstEntryFromSearch(lds, driverSettings.getLDAPUserBaseDN(), driverSettings.getScope(), driverSettings.getUsersIdFilter(stringVals[i]));
                if (userEntry != null)
                {
                	usersVector.add(stringVals[i]);
                }
            ...
            {
                SilverTrace.error("admin", "LDAPGroupSamse.getTRUEUserIds()", "admin.MSG_ERR_LDAP_GENERAL", "USER NOT FOUND : " + LDAPUtility.dblBackSlashesForDNInFilters(stringVals[i]), e);
            }*/
        }
        stringVals = null;
    ...
    protected LDAPEntry[] getChildGroupsEntry(String lds, String parentId, String extraFilter) throws AdminException
    {
        ...
        else
        {
            ...
            try
            {
                SilverTrace.info("admin", "LDAPGroupSamse.getChildGroupsEntry()", "root.MSG_GEN_PARAM_VALUE", "Root Group Search");
				...
				SynchroReport.debug("LDAPGroupSamse.getChildGroupsEntry()", "Récupération de " +  theEntries.length + " groupes racine",null);
            }
            catch (AdminException e)
            {
                if (synchroInProcess)
                {
                    SilverTrace.warn("admin", "LDAPGroupSamse.getChildGroupsEntry()", "admin.EX_ERR_CHILD_GROUPS", "ParentGroupId=" + parentId, e);
                    synchroReport.append("PB getting Group's subgroups : " + parentId + "\n");
					SynchroReport.error("LDAPGroupSamse.getChildGroupsEntry()", "Erreur lors de la récupération des groupes racine (parentId = "+parentId+")", e);
                }
                else
                {
                    throw e;
    ...
    protected Vector getTRUEChildGroupsEntry(String lds, String parentId, LDAPEntry theEntry)
    {
        ...
        if ((parentId != null) && (parentId.length() > 0))
        {
            SilverTrace.info("admin", "LDAPGroupSamse.getTRUEChildGroupsEntry()", "root.MSG_GEN_PARAM_VALUE", "Root Group Search : " + parentId);
            ...
            for (i = 0; i < stringVals.length; i++)
            {
                try
                {
                    ...
                    if (childGroupEntry != null)
                    {
                        ...
                        if (LDAPUtility.getFirstEntryFromSearch(lds, driverSettings.getGroupsSpecificGroupsBaseDN(), driverSettings.getScope(), driverSettings.getGroupsIdFilter(groupSpecificId)) != null)
                        {
                ...
                {
                    SilverTrace.error("admin", "LDAPGroupSamse.getTRUEChildGroupsEntry()", "admin.MSG_ERR_LDAP_GENERAL", "GROUP NOT FOUND : " + stringVals[i], e);
                }
            }
        }
        return entryVector;
    ...
    public Group[] getAllChangedGroups(String lds, String extraFilter) throws AdminException
    {
        ...
        while (groupsIdsSet.size() > 0)
        {
            ...
            while (it.hasNext())
            {
                ...
                SilverTrace.info("admin", "LDAPGroupSamse.getAllChangedGroups()", "root.MSG_GEN_PARAM_VALUE", "GroupTraite2="+theGroup.getDN().toString());
                ...
                for (i = 0; i < les.length; i++)
                {
                    SilverTrace.info("admin", "LDAPGroupSamse.getAllChangedGroups()", "root.MSG_GEN_PARAM_VALUE", "GroupFound2="+les[i].getDN());
                    if (!groupsManaged.containsKey(les[i].getDN().toString()))
                    {
                        SilverTrace.info("admin", "LDAPGroupSamse.getAllChangedGroups()", "root.MSG_GEN_PARAM_VALUE", "GroupAjoute2="+les[i].getDN().toString());
                        groupsCur.add(les[i]);
                        groupsManaged.put(les[i].getDN().toString(),translateGroup(lds,les[i]));
                    }
                }
