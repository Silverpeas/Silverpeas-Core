...
public class AuthenticationLDAP extends Authentication {
  ...
  private final static long MILLISECONDS_BETWEEN_1601_AND_1970 = Long
      .parseLong("11644473600000");
  ...
  public void init(String authenticationServerName, ResourceLocator propFile) {
    ...
    m_IsSecured = getBooleanProperty(propFile, authenticationServerName
        + ".LDAPSecured", false);
    ...
    m_AccessPasswd = propFile.getString(authenticationServerName
        + ".LDAPAccessPasswd");
    m_UserBaseDN = propFile.getString(authenticationServerName
        + ".LDAPUserBaseDN");
    m_UserLoginFieldName = propFile.getString(authenticationServerName
        + ".LDAPUserLoginFieldName");

    // get parameters about user alert if password is about to expire
    m_MustAlertPasswordExpiration = getBooleanProperty(propFile,
        authenticationServerName + ".MustAlertPasswordExpiration", false);
    if (m_MustAlertPasswordExpiration) {
      ...
      propValue = propFile.getString(authenticationServerName
          + ".LDAPPwdMaxAge");
      m_PwdMaxAge = (propValue == null) ? Integer.MAX_VALUE : Integer
          .parseInt(propValue);

      propValue = propFile.getString(authenticationServerName
          + ".PwdExpirationReminderDelay");
      m_PwdExpirationReminderDelay = (propValue == null) ? 5 : Integer
          .parseInt(propValue);
  ...
  protected void internalAuthentication(String login, String passwd)
    ...
    for (int b = 0; b < baseDNs.length; b++) {
      try {
        ...
        if (!res.hasMore()) {
          // throw new
          // AuthenticationBadCredentialException("AuthenticationLDAP.internalAuthentication()",SilverpeasException.ERROR,"authentication.EX_USER_NOT_FOUND","User="
          // + login + ";LoginField=" + m_UserLoginFieldName);
        }
        if (res.hasMore()) {
          fe = res.next();
          if (fe != null) {
        ...
        throw new AuthenticationHostException(
            "AuthenticationLDAP.internalAuthentication()",
            SilverpeasException.ERROR, "authentication.EX_LDAP_ACCESS_ERROR",
            ex);
      }
    }
    if (userFullDN == null) {
      throw new AuthenticationBadCredentialException(
          ...
          "User=" + login + ";LoginField=" + m_UserLoginFieldName);
    }
    ...
    if (m_MustAlertPasswordExpiration
        && (nbDaysBeforeExpiration < m_PwdExpirationReminderDelay)) {
  ...
  protected void internalChangePassword(String login, String oldPassword,
    ...
    try {
      ...
      SilverTrace.info("authentication", "AuthenticationLDAP.changePassword()",
          "root.MSG_GEN_PARAM_VALUE", "UserFilter=" + searchString);
      res = m_LDAPConnection.search(m_UserBaseDN, LDAPConnection.SCOPE_SUB,
      ...
      mods[0] = new LDAPModification(LDAPModification.DELETE,
          new LDAPAttribute("unicodePwd", oldUnicodePassword));
