...
public class AuthenticationEncrypt implements EncryptionInterface
 {
    ...
    public String encode(String stringToEncode) {
    	SilverTrace.info("authentication", "AuthenticationEncrypt.encode()", "root.MSG_PARAM_ENTER_METHOD", "stringToEncode="+stringToEncode);
        ...
        hashString.reverse();
        SilverTrace.info("authentication", "AuthenticationEncrypt.encode()", "root.MSG_PARAM_EXIT_METHOD", "encodedString="+hashString.toString());
   ...
   */
    public String decode(String encodedText)
       {
    	SilverTrace.info("authentication", "AuthenticationEncrypt.decode()", "root.MSG_PARAM_ENTER_METHOD", "encodedText="+encodedText);
         int lg=0;
         int pos = 0;
         String reverseEncodedText = new StringBuffer(encodedText).reverse().toString();
         StringBuffer hashString = new StringBuffer();
         for ( int i = 0; i+pos < reverseEncodedText.length(); i++ )
         {
             lg = new Integer(reverseEncodedText.substring(i+pos,i+pos+1)).intValue();
             String car = reverseEncodedText.substring(i+pos+1,i+pos+1+lg);
             pos = pos + lg;
             hashString.append((char) (new Integer(car).intValue()/3));
         }
         SilverTrace.info("authentication", "AuthenticationEncrypt.decode()", "root.MSG_PARAM_EXIT_METHOD", "decodedString="+hashString.toString());
         return hashString.toString();
       }
	...
	{
//		SilverTrace.info("authentication", "AuthenticationEncrypt.decode1()", "root.MSG_PARAM_ENTER_VALUE", "str à décoder="+str+" clé="+key);
		String decStr = "";
		String prand = "";
		...
		stringMult.append(prand.charAt(sPos)).append(prand.charAt(sPos*2)).append(prand.charAt(sPos*3))
			.append(prand.charAt(sPos*4)).append(prand.charAt(sPos*5));
		
		int mult = new Integer(stringMult.toString()).intValue();
		
		int incr = Math.round(key.length() / 2);
		double modu = Math.pow(2, 127) - 1;
		int salt = Integer.parseInt(str.substring(str.length() - 8, str.length()), 16);
		...
		for(int i=0; i<str.length(); i+=2) {
			dec_chrInt = Integer.parseInt(str.substring(i, i+2), 16) ^ new Double(Math.floor((prandInt / modu) * 255)).intValue();
