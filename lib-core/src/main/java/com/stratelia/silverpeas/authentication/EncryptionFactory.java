...
public class EncryptionFactory
{
	...
	{
        ResourceLocator settingsFile = new ResourceLocator("com.silverpeas.authentication.settings.authenticationSettings.properties", "");
        EncryptionInterface encryptionInterface = null;
        try {
            Class encryptionClass = Class.forName(settingsFile.getString("encryptionClass"));
            encryptionInterface = (EncryptionInterface) encryptionClass.newInstance();
          }
          catch (Exception e) {
      		SilverTrace.info("authentication", "EncryptionFactory.getCustomEncryption()", "root.MSG_PARAM_ENTER_VALUE", "Encrypt/Decrypt Custom Class not found");
          }        
          return encryptionInterface;
	}
}