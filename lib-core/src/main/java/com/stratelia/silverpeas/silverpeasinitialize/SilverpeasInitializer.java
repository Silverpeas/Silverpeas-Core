...
public class SilverpeasInitializer
{
	...
	public void startInitialize()
	{
        ...
        {
            // Add properties
            _silverpeasinitializeSettings = java.util.ResourceBundle.getBundle("com.stratelia.silverpeas._silverpeasinitialize.settings._silverpeasinitializeSettings", new Locale("fr", ""));
            if (_silverpeasinitializeSettings.getString("initialize").equalsIgnoreCase("true") == true)
            {
            ...
            {
                LogMsg(this, LOG_DEBUG, "SilverpeasInitializer", "Do not initialize because of _silverpeasinitializeSettings.initialize=false.");
            }
            // Add SSL support
	...
	{
		File[] result = null;
		File   pathInitialize = new File(_silverpeasinitializeSettings.getString("pathInitialize"));

		if (pathInitialize == null)
		{
			LogMsg(this, LOG_ERROR, "getListInitializeSettingsFile", "Repository Initialize for InitializeSettings.xml files is not defined in Settings.");
		}
		else
		{
	...
	private void initPropertySystem()
	{
		...
		if (pathInitialize == null)
		{
			LogMsg(this, LOG_ERROR, "initPropertySystem", "Repository Initialize for systemSettings.properties file is not defined in Settings.");
		}
		else
		{
			try
			{
				FileInputStream fis = new FileInputStream(new File(pathInitialize, "systemSettings.properties"));
				Properties		systemFileProperties = new Properties();

				systemFileProperties.load(fis);
				Properties systemProperties = System.getProperties();

				for (Enumeration e = systemFileProperties.propertyNames(); e.hasMoreElements(); )
				{
					String key = (String) e.nextElement();

					if (systemProperties.getProperty(key) != null)
					{
						System.out.println("_SilverpeasInitialize : override system property [" + key + "] = " + "(" + systemProperties.getProperty(key) + ") -> (" + systemFileProperties.getProperty(key) + ")");
					}
			...
			{
				LogMsg(this, LOG_ERROR, "initPropertySystem", "File systemSettings.properties in directory " + pathInitialize + " not found.");
			}
			catch (IOException e)
			{
				LogMsg(this, LOG_ERROR, "initPropertySystem", "Unable to read systemSettings.properties.");
			}
		}
	}
