...
/**
 * This is the base class of all scheduler job classes. This class is abstract. If you will implement
 * your own special job class, you have to overrite the method 'execute' and add your own job generation
 * method in the class 'SimpleScheduler'
 */
...
//	private String						sJobLogFileName;
	...
	// Get next schedule time
        ...
        SilverTrace.info("scheduler", "SchedulerJob.run", "root.MSG_GEN_PARAM_VALUE", ": Job '" + sJobName + "' starts without errors.");
        SilverTrace.info("scheduler", "SchedulerJob.run", "root.MSG_GEN_PARAM_VALUE", ": Next schedule time: " + logDateFormat.format (new Date (nextTimeStamp)));
        /*
    	// Write a header into the log file
		try
		{
            logStream = new PrintStream (new FileOutputStream (theLogBaseFile.getAbsolutePath () + System.getProperties ().getProperty ("file.separator") + sJobName + ".log", true));
			logStream.println (logDateFormat.format (new Date ()) + ": Job '" + sJobName + "' starts without errors.");
			logStream.println (logDateFormat.format (new Date ()) + ": Next schedule time: " + logDateFormat.format (new Date (nextTimeStamp)) + "\n");
			logStream.close ();
		}
		catch (Exception aException)
		{
			System.out.println ("SchedulerJob.run: Exception while writing the log header occured (Reason: " + aException.toString () + ")");
		}
        */
		...
		while (bRunnable)
		{
			...
			// Calculate the delay time
				...
				if (sleepTime < 0)
				{
				// Yields if there are problems with the date. Should normaly not occour.
					sleep (0);
				}
				else
				{
			...
			{
			    /*
                // Suppress mesages while shutdown
				if (bRunnable)
				{
					System.out.println ("SchedulerJob.run: InterruptedException while sleeping occured (Reason: " + aException.toString () + ")");
				}
                */
			}
			...
			if (bRunnable && ((new Date ()).getTime () >= nextTimeStamp))
			{
				try
				{
        		 	SilverTrace.info("scheduler", "SchedulerJob.run", "root.MSG_GEN_PARAM_VALUE", ": ---------------- Start of job '" + sJobName + "' -------------------");
                    // Open a new Stream to the log file
                    /*
					logStream = new PrintStream (new FileOutputStream (theLogBaseFile.getAbsolutePath () + System.getProperties ().getProperty ("file.separator") + sJobName + ".log", true));
					logStream.println (logDateFormat.format (new Date ()) + ": ---------------- Start of job '" + sJobName + "' -------------------");
                    */
				   ...
				   try
					{
                        ...
                        execute (new Date ());
						//logStream.flush ();
						theOwner.handleSchedulerEvent (new SchedulerEvent (SchedulerEvent.EXECUTION_SUCCESSFULL, this));
					}
					catch (SchedulerException aException)
					{
						theOwner.handleSchedulerEvent (new SchedulerEvent (SchedulerEvent.EXECUTION_NOT_SUCCESSFULL, this));
					}

        		 	SilverTrace.info("scheduler", "SchedulerJob.run", "root.MSG_GEN_PARAM_VALUE", ": ---------------- End of job '" + sJobName + "' -------------------");
        		 	SilverTrace.info("scheduler", "SchedulerJob.run", "root.MSG_GEN_PARAM_VALUE", ": Next schedule time: " + logDateFormat.format (new Date (nextTimeStamp)));
                    /*
					logStream.println (logDateFormat.format (new Date ()) + ": ---------------- End of job '" + sJobName + "' -------------------");
					logStream.println (logDateFormat.format (new Date ()) + ": Next schedule time: " + logDateFormat.format (new Date (nextTimeStamp)) + ".\n");
					logStream.close ();
                    */
				}
				...
				{
					//System.out.println ("SchedulerJob.run: Exception while job execution occured (Reason: " + aException.toString () + ")");
		...
		{
            logStream = new PrintStream (new FileOutputStream (theLogBaseFile.getAbsolutePath () + System.getProperties ().getProperty ("file.separator") + sJobName + ".log", true));
			logStream.println (logDateFormat.format (new Date ()) + ": Job '" + sJobName + "' terminates without errors.\n\n");
			logStream.close ();
		}
		catch (Exception aException)
		{
			System.out.println ("SchedulerJob.run: Exception while writing the log footer occured (Reason: " + aException.toString () + ")");
		}
        */
	 ...
	 */
	abstract protected void execute (Date theExecutionDate) throws SchedulerException;
	 ...
	 */
	protected SchedulerJob (SimpleScheduler aController, SchedulerEventHandler aOwner, String aJobName) throws SchedulerException
	{
		if (aController == null)
		{
		...
		currentDayOfMonth	= new Integer (1);
		currentMonth		= new Integer (0);
	...
	/**
	 * This method sets the scheduling parameter. The time settings are given by vectors.
	 * Each vector holds a list of Integer objects (currently ordered). Every Integer
	 * represents a element of a timestamp (cron like).
	 *
	...
	// Check minute values
		...
		for (vectorEnumerator = startMinutes.elements (); vectorEnumerator.hasMoreElements();)
		{
			try
			{
	...
	// Check hour values
		...
		for (vectorEnumerator = startHours.elements (); vectorEnumerator.hasMoreElements();)
		{
			try
			{
				workInt = ((Integer) vectorEnumerator.nextElement()).intValue ();
	...
	// Check day of month values
		...
		for (vectorEnumerator = startDaysOfMonth.elements (); vectorEnumerator.hasMoreElements();)
		{
			try
			{
				workInt = ((Integer) vectorEnumerator.nextElement()).intValue ();
	...
	// Check month values and normalize them for internal usage
		...
		for (vectorEnumerator = startMonths.elements (); vectorEnumerator.hasMoreElements();)
		{
			try
			{
				workInt = ((Integer) vectorEnumerator.nextElement()).intValue ();
	...
	// Check day of week values
		...
		workVector = new Vector ();
		for (vectorEnumerator = startDaysOfWeek.elements (); vectorEnumerator.hasMoreElements();)
		{
			try
			{
				workInt = ((Integer) vectorEnumerator.nextElement()).intValue ();
			...
			// Conversion not realy necessary, but what if SUN changes the implementation .... :-))
				switch (workInt)
				{
					case 0:
	...
	/**
	 * This method sets the scheduling parameter. It is given by a cron like string (currently
	 * ranges are not allowed). So the string '* 3,21 * 3 0' starts the execution
	 * every Sunday in March on 03:00 and 21:00. The allowed ranges are: minutes (0-59), hours (0-23),
	 * days of a month (1-31), months (1-12; starts with 1 for January), day of a week (0-6; starts
	 * with 0 for Sunday). Currently the parsing of the cron string ist not done by a state machine
	 * but by StringTokenizers so this method is <B>very</B> sensitive for syntax failures!
	 *
	 */
	...
	protected long getNextTimeStamp ()
	{
        ...
        SilverTrace.debug("scheduler", "SchedulerJob.getNextTimeStamp", "Current TimeStamp: " + logDateFormat.format (new Date (getMillisecondsOfCalendar (calcCalendar))));
        ...
        SilverTrace.debug("scheduler", "SchedulerJob.getNextTimeStamp", "Start TimeStamp: " + logDateFormat.format (new Date (getMillisecondsOfCalendar (calcCalendar))));
	...
	// !!!!!! The values must be ordered ascend !!!!!
		...
		while (!validTimeStamp)
		{
			...
			if (vMinutes.size () == 0) // Default ('*') -> Hit every minute
			{
				...
				{
					currentMinute = new Integer (currentMinute.intValue () + 1);
					carryMinute 		= false;
					carryHour			= false;
					carryDayOfMonth	= false;
					carryMonth			= false;
				...
				{
					currentMinute = new Integer (calcCalendar.getActualMinimum (Calendar.MINUTE));
					carryMinute = true;
				}*/

				// If the cron setting for minutes is *, we don't have to care about incrementing minutes
				// So do a carryHour
				carryMinute 		= true;
			}
			else
			...
			calcCalendar.set (Calendar.MINUTE, currentMinute.intValue ());
//            SilverTrace.debug("scheduler", "SchedulerJob.getNextTimeStamp", "MINUTE: " + Integer.toString(currentMinute.intValue()));
			...
			if (carryMinute)
			{
				...
				calcCalendar.set (Calendar.HOUR_OF_DAY, currentHour.intValue ());
//                SilverTrace.debug("scheduler", "SchedulerJob.getNextTimeStamp", "HOUR: " + Integer.toString(currentHour.intValue()));
			}
			...
			if (carryHour)
			{
				...
				// Special handling for lists with one element
					...
					else
					{
						...
						{
							currentDayOfMonth = (Integer) vDaysOfMonth.elementAt (indexOfMonths + 1);
							carryDayOfMonth 	= false;
							carryMonth			= false;
						}
					}
				...
				calcCalendar.set (Calendar.DAY_OF_MONTH, currentDayOfMonth.intValue ());
//                SilverTrace.debug("scheduler", "SchedulerJob.getNextTimeStamp", "DAY_OF_MONTH: " + Integer.toString(currentDayOfMonth.intValue()));
			}
			...
			if (carryDayOfMonth)
			{
				...
				calcCalendar.set (Calendar.MONTH, currentMonth.intValue ());
//                SilverTrace.debug("scheduler", "SchedulerJob.getNextTimeStamp", "MONTH: " + Integer.toString(currentMonth.intValue()));
			}
		...
		// Get new year
			...
			// Prevent Check for the 'ever carry' of one element lists
				...
				// Hit every year
					currentYear = new Integer (currentYear.intValue () + 1);
					calcCalendar.set (Calendar.YEAR, currentYear.intValue ());
//                    SilverTrace.debug("scheduler", "SchedulerJob.getNextTimeStamp", "YEAR: " + Integer.toString(currentYear.intValue()));
				}
		...
		// If time stamp is greater than the current time check the day of week
			...
			// Check eventualy day movement while calculations
				...
				// Check for correct day of week
					...
					{
						for (Enumeration vectorEnumerator = vDaysOfWeek.elements (); vectorEnumerator.hasMoreElements();)
						{
							if (calcCalendar.get (Calendar.DAY_OF_WEEK) == ((Integer) vectorEnumerator.nextElement()).intValue ())
							{
								validTimeStamp = true;
								break;
							}
						}
//                        SilverTrace.debug("scheduler", "SchedulerJob.getNextTimeStamp", "!!! Invalid Day Of Week" + " : " + calcCalendar.get(Calendar.DAY_OF_WEEK));
					}
				}
                else
                {
//                    SilverTrace.debug("scheduler", "SchedulerJob.getNextTimeStamp", "!!! calcCalendar.get(Calendar.DAY_OF_MONTH) != currentDayOfMonth.intValue()" + " : " + calcCalendar.get(Calendar.DAY_OF_MONTH) + " != " + currentDayOfMonth.intValue());
                }
			}
            else
            {
//                SilverTrace.debug("scheduler", "SchedulerJob.getNextTimeStamp", "!!! getMillisecondsOfCalendar (calcCalendar) <= currentTime" + " : " + getMillisecondsOfCalendar(calcCalendar) + " <= " + currentTime);
            }
		} // while (getMillisecondsOfCurrentTimeStamp (calcCalendar) < currentTime)

        SilverTrace.debug("scheduler", "SchedulerJob.getNextTimeStamp", "New TimeStamp: " + logDateFormat.format (new Date (getMillisecondsOfCalendar (calcCalendar))));
	...
	/**
	 * Sorts the internal cron vectors nad remove doubled entries. This is necessary to calculate
	 * the correct schedule time
	 */
