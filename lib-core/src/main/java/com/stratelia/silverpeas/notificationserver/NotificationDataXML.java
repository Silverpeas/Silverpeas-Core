...
public class NotificationDataXML implements DocumentHandler
{
  ...
  private NotificationData mData;
	...
	public NotificationDataXML() throws NotificationServerException
	{
		...
		{
			mParser = null;
			throw new NotificationServerException("NotificationDataXML()", SilverpeasException.ERROR, "notificationServer.EX_ERROR_IN_XML_PARSING", e);
		}
	}
	...
	public void ParseXML(String pFileName) throws NotificationServerException
	{
		...
		{
			throw new NotificationServerException("NotificationDataXML.ParseXML()", SilverpeasException.ERROR, "notificationServer.EX_ERROR_IN_XML_PARSING", "FileName=" + pFileName,e);
		}
	}
	 ...
	 * @see
	 */
	public void ParseXML(InputSource pInputSource) throws NotificationServerException
	{
		try
		{
		...
		{
			throw new NotificationServerException("NotificationDataXML.ParseXML()", SilverpeasException.ERROR, "notificationServer.EX_ERROR_IN_XML_PARSING", "inputSource=" + pInputSource.toString(),e);
		}
	}
			...
			if (PRIORITY.equals(mTagPath.get(1)))
			{
				if (mCurrentAttributeList != null)
				{
					for (i = 0; i < mCurrentAttributeList.getLength(); i++)
					{
						...
						{
							mData.setPrioritySpeed(EncodeHelper.htmlStringToJavaString(mCurrentAttributeList.getValue(i)));
						}
					}
				}
			}
			else if (TARGET.equals(mTagPath.get(1)))
			{
				if (mCurrentAttributeList != null)
				{
					for (i = 0; i < mCurrentAttributeList.getLength(); i++)
					{
						...
						{
							mData.setTargetChannel(EncodeHelper.htmlStringToJavaString(mCurrentAttributeList.getValue(i)));
						}
					}
				}
			}
					...
					if (STATUS.equals(mTagPath.get(3)))
					{
						if (mCurrentAttributeList != null)
						{
							for (i = 0; i < mCurrentAttributeList.getLength(); i++)
							{
								...
								{
									mData.setReportToSenderStatus((mData.getReportToSenderStatus() != null ? (mData.getReportToSenderStatus() + ";") : "") + EncodeHelper.htmlStringToJavaString(mCurrentAttributeList.getValue(i)));
								}
							}
						}
					}
					else if (TARGET.equals(mTagPath.get(3)))
					{
						if (mCurrentAttributeList != null)
						{
							for (i = 0; i < mCurrentAttributeList.getLength(); i++)
							{
								if (CHANNEL.equals(mCurrentAttributeList.getName(i)))
								{
									mData.setReportToSenderTargetChannel(EncodeHelper.htmlStringToJavaString(mCurrentAttributeList.getValue(i)));
								}
							}
						}
					}
					...
					if (STATUS.equals(mTagPath.get(3)))
					{
						if (mCurrentAttributeList != null)
						{
							for (i = 0; i < mCurrentAttributeList.getLength(); i++)
							{
								if (NAME.equals(mCurrentAttributeList.getName(i)))
								{
									mData.setReportToLogStatus((mData.getReportToLogStatus() != null ? (mData.getReportToLogStatus() + ";") : "") + EncodeHelper.htmlStringToJavaString(mCurrentAttributeList.getValue(i)));
								}
							}
						}
					}
	...
	{
		String theContents = EncodeHelper.htmlStringToJavaString(mContents.toString());
		...
		case 3:
			...
			else if (TARGET.equals(mTagPath.get(1)))
			{
				...
				{
					mData.setTargetParam(NotificationServerUtil.unpackKeyValues(theContents));
				}
			}
			break;
		case 5:
