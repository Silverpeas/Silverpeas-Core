...
{	
		...
		String m_sContainerInterface = null;						// The class to call that implements the ContainerInterface
		ContainerInterface m_containerInterface = null;	// The object (class.forName(m_sContainerInterface))
		List	 m_asUserRoles = null;										// User roles of the container
		String m_sReturnURL = null;											// URL to call when the content want to get back on the container
		String m_sSessionControlBeanName = null;				// Name of the bean in the session
		URLIcone m_ClassifyURLIcone = null;							// URLIcone on the classify JSP to call for classifying a SilverContent
		...
		public ContainerPeas(String sContainerDescriptorPath)
		{
				...
				if(sContainerDescriptorPath.equals("containerPDC"))
				{
						...
						this.setContainerInterface("com.stratelia.silverpeas.pdc.control.PdcBmImpl");
						...
						urlIcone.setIconePath(GeneralPropertiesManager.getGeneralResourceLocator().getString("ApplicationURL")+"/pdcPeas/jsp/icons/pdcPeas_classify_in_pdc.gif");
