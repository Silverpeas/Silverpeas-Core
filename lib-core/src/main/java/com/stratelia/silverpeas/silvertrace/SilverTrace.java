...
/**
 * SilverTrace is the trace tool used in silverpeas to trace debug, running infos and errors.
 * This is a 'fully' static class. All functions could be called directly and is thread-safe.
 * The trace functions are : debug, info, warn, error, fatal.
 *
...
public class SilverTrace
{
    ...
    /**
     * The trace file will be copied every 1st day of a mounth with the name : FileName.ext.year-mounth
     * A new file named FileName.ext is the created and will contains the next mounth's traces
     * Example : MyFile.txt.2001-07
     * @see #addAppenderRollingFile
     */
    public static String        ROLLING_MODE_MOUNTH = "'.'yyyy-MM";

    /**
     * The trace file will be copied every 1st day of a week with the name : FileName.ext.year-week
     * A new file named FileName.ext is the created and will contains the next week's traces
     * Example : MyFile.txt.2001-34
     * @see #addAppenderRollingFile
     */
    public static String        ROLLING_MODE_WEEK = "'.'yyyy-ww";

    /**
     * The trace file will be copied every day at midnight with the name : FileName.ext.year-mounth-day
     * A new file named FileName.ext is the created and will contains the next day's traces
     * Example : MyFile.txt.2001-07-23
     * @see #addAppenderRollingFile
     */
    public static String        ROLLING_MODE_DAILY = "'.'yyyy-MM-dd";

    /**
     * The trace file will be copied every hour with the name : FileName.ext.year-mounth-day-hour
     * A new file named FileName.ext is the created and will contains the next hour's traces
     * Example : MyFile.txt.2001-07-23-18
     * @see #addAppenderRollingFile
     */
    ...
    protected static String     layoutFullDebug = "%-15.15r [%-26.26t] - %d{dd/MM/yy-HH:mm:ss,SSS} - %-5p : %m%n";
     ...
     * @param classe  the short name of the classe that call this function (ex : "MyFavoriteClass") Could be followed by the function name (ex : "MyFavoriteClass.myFunction()")
    ...
    /**
     * Trace some debug informations. The programmer is free to display the message he wants...
     * This function have one extra parameter :
     * extraInfos to add additional informations
     *
     *
     * @param module  the module name (ex : root, bus, outlook, ...)
     * @param classe  the short name of the classe that call this function (ex : "MyFavoriteClass") Could be followed by the function name (ex : "MyFavoriteClass.myFunction()")
    ...
    /**
     * Trace some debug informations. The programmer is free to display the message he wants...
     * This function have one extra parameters :
     * ex to display an exception
     *
     *
     * @param module  the module name (ex : root, bus, outlook, ...)
     * @param classe  the short name of the classe that call this function (ex : "MyFavoriteClass") Could be followed by the function name (ex : "MyFavoriteClass.myFunction()")
    ...
    /**
     * Trace some debug informations. The programmer is free to display the message he wants...
     * This function have two extra parameters :
     * extraInfos to add additional informations
     * ex to display an exception
     *
     *
    ...
    /**
     * Trace some 'info' informations. The message MUST BE one of the predefined in the property files.
     * To add some extra infos, use the function with the 4th parameter : extraInfos
     *
     *
     * @param module  the module name (ex : root, bus, outlook, ...)
     * @param classe  the short name of the classe that call this function (ex : "MyFavoriteClass") Could be followed by the function name (ex : "MyFavoriteClass.myFunction()")
        ...
        if (initFinished)
        {
            try
            {
                ...
                if (cat != null)
                {
                    ...
                    {
                        cat.info(formatTraceMessage(module, classe, messageID, traceMessages.getMsgString(messageID), extraInfos), ex);
                    }
                }
            }
            catch (RuntimeException e)
    ...
    /**
     * Trace some 'warning' informations. The message MUST BE one of the predefined in the property files.
     * To add some extra infos, use the function with the 4th parameter : extraInfos
     *
     *
     * @param module  the module name (ex : root, bus, outlook, ...)
        ...
        if (initFinished)
        {
            try
            {
                ...
                if (cat != null)
                {
                    ...
                    {
                        cat.warn(formatTraceMessage(module, classe, messageID, traceMessages.getMsgString(messageID), extraInfos), ex);
                    }
                }
            }
            catch (RuntimeException e)
    ...
    /**
     * Trace some 'error' informations. The message MUST BE one of the predefined in the property files.
     * To add some extra infos, use the function with the 4th parameter : extraInfos
     *
     *
     * @param module  the module name (ex : root, bus, outlook, ...)
        ...
        if (initFinished)
        {
            try
            {
                ...
                if (cat != null)
                {
                    ...
                    {
                        cat.error(formatTraceMessage(module, classe, messageID, traceMessages.getMsgString(messageID), extraInfos), ex);
                    }
                }
                ...
                if (cat != null)
                {
                    if (cat.isEnabledFor(Priority.ERROR))
                    {
                        cat.error(formatErrorAndFatalMessage(module, classe, messageID, extraInfos, ex));
                    }
                }
            }
            catch (RuntimeException e)
    ...
    /**
     * Trace some 'fatal error' informations. The message MUST BE one of the predefined in the property files.
     * To add some extra infos, use the function with the 4th parameter : extraInfos
     *
     *
     * @param module  the module name (ex : root, bus, outlook, ...)
        ...
        if (initFinished)
        {
            try
            {
                ...
                if (cat != null)
                {
                    ...
                    {
                        cat.fatal(formatTraceMessage(module, classe, messageID, traceMessages.getMsgString(messageID), extraInfos), ex);
                    }
                }
                // Error and Fatal traces
                cat = getCategory(MODULE_ERROR_AND_FATAL, null);
    ...
    /**
     * Trace some actions (create, delete, update) done by a user on an object
     * of an instance in a space.
     *
     *
     * @param module  the module name (ex : root, bus, outlook, ...)
     * @param classe  the short name of the classe that call this function (ex : "MyFavoriteClass") Could be followed by the function name (ex : "MyFavoriteClass.myFunction()")
     ...
     *
     */
    static public void spy(String module, String classe, String spaceId, String instanceId, String objectId, String userId, String actionId)
    {
        if (initFinished)
        {
            try
    ...
    static public void resetAll()
    {
        ...
        try
        {
            resources = java.util.ResourceBundle.getBundle("com.stratelia.silverpeas.silvertrace.settings.silverTrace", new Locale("", ""));
            ...
            for (i = 0; i < nbFiles; i++)
            {
                ...
                catch (IOException e)
                {
                    if (initFinished)
                    {
                        SilverTrace.error("silvertrace", "SilverTrace.resetAll()", "silvertrace.ERR_INIT_TRACE_FROM_PROP", "File:[" + ((File) theFiles.get(i)).getAbsolutePath() + "]", e);
                    }
                    else
                    {
                        emergencyTrace("Error in SilverTrace initialization : Cant load property file : '" + ((File) theFiles.get(i)).getAbsolutePath() + "'", e);
                    }
                }
            }
        }
        catch (Exception e)
        {
            if (initFinished)
            {
                SilverTrace.error("silvertrace", "SilverTrace.resetAll()", "silvertrace.ERR_INIT_TRACE", e);
            }
            else
            {
    ...
    static public void initFromProperties(Properties fileProperties)
    {
        ...
        try
        {
            ...
            if (MsgTrace.stringValid(enumModulesName) && MsgTrace.stringValid(enumModulesPath))
            {
            ...
            while (appenderTypeStr != null)
            {
                ...
                appenderEnabled = MsgTrace.getBooleanProperty(fileProperties, "appender" + Integer.toString(i) + ".enabled", true);
            ...
            // Third, enumerate all modules and look if there are special trace levels on them
            // -------------------------------------------------------------------------------
            ...
            while (enumModulesNames.hasMoreElements())
            {
                ...
                traceLevel = fileProperties.getProperty("traceLevel." + enumModulesName);
                if (traceLevel != null)
                {
     ...
     *
     * @param module the module name (ex : root, bus, outlook, ...)
     * @param val the trace level : could be one of the TRACE_LEVEL_... values. Use TRACE_LEVEL_UNKNOWN to remove the level condition for the module.
     *
     */
     ...
     *
     * @param module the module name (ex : root, bus, outlook, ...)
     * @param chained if false : return a trace level only if the module have been set with one. Else, return TRACE_LEVEL_UNKNOWN. If true, look for the inherited trace level.
     *
     ...
     * If an appender with the same type have been previously set, delete it and replace it with the new created one.
     *
     *
     * @param module the module name (ex : root, bus, outlook, ...)
     * @param patternLayout the things displayed in this appender, could be one of the LAYOUT_... constants
     ...
     * @param appendOnFile true to append at the end of the existing file (if ther is one), false to remove old file before writting
     *
     */
     ...
     * @param patternLayout the things displayed in this appender, could be one of the LAYOUT_... constants
     * @param fileName full-path name of the file where the trace are written
     * @param rollingMode frequency of the rolling file, could be one of the ROLLING_MODE_... constants
     *
     */
    static public void addAppenderRollingFile(String module, String patternLayout, String fileName, String rollingMode)
    {
        Category cat = getCategory(module, null);
        ...
        if (cat != null)
        {
            try
            {
                DailyRollingFileAppender a1 = new DailyRollingFileAppender(getLayout(patternLayout), fileName, rollingMode);
     ...
     * If an appender with the same type have been previously set, delete it and replace it with the new created one.
     * How it works : mails are only sent when an ERROR or FATAL occur. The mail contains the error and the 512 last traces
     * taken into account (ie, higher than the trace level).
     *
     *
     * @param module the module name (ex : root, bus, outlook, ...)
     * @param patternLayout the things displayed in this appender, could be one of the LAYOUT_... constants
     ...
     *
     */
    static public void addAppenderMail(String module, String patternLayout, String mailHost, String mailFrom, String mailTo, String mailSubject)
    {
        Category     cat = getCategory(module, null);
     ...
     *
     * @param module the module name (ex : root, bus, outlook, ...)
     * @param typeOfAppender could be a mask of APPENDER_... values or APPENDER_ALL to remove all appenders attached to the module
     *
     */
    ...
    /**
     * The purpose of this function is just to return the list of available modules to the JSP - Exploitation page
     * This function is subject to change and SHOULD NOT BE USED by any other page or java class...
     *
     *
    ...
    /**
     * The purpose of this function is just to return the available appenders for the JSP - Exploitation page
     * This function is subject to change and SHOULD NOT BE USED by any other page or java class...
     *
     *
     * @param module the module name (ex : root, bus, outlook, ...)
    ...
    /**
     * The purpose of this function is just to return informations about an appender for the JSP - Exploitation page
     * This function is subject to change and SHOULD NOT BE USED by any other page or java class...
     *
     *
     * @param module the module name (ex : root, bus, outlook, ...)
     ...
     *
     * @return A set of properties discribing the attached appender or null if there is no such appender attached to this module
     */
     ...
     * @see
     */
    static protected String formatSpyMessage(String spaceId, String instanceId, String objectId, String userId, String actionId)
    {
     ...
     *
     */
    static protected String formatErrorAndFatalMessage(String module, String classe, String messageID, String extraInfos, Throwable ex)
    {
        String extraParams;
     ...
     *
     */
    static protected String formatTraceMessage(String module, String classe, String messageID, String message, String extraInfos)
    {
        StringBuffer valret = new StringBuffer("");
     ...
     *
     */
    static protected void addAppenderFromProperties(Properties fileProperties, int appenderNumber, int appenderType)
    {
        String  module;
        String  layout;
        String  fileName;
        ...
        // Retrieve the properties of the current appender and call the function that will create and attach it
        //
        module = fileProperties.getProperty("appender" + Integer.toString(appenderNumber) + ".module");
        if (module == null)
        {
            module = MODULE_ROOT;
        }
        layout = fileProperties.getProperty("appender" + Integer.toString(appenderNumber) + ".layout");
        if (layout == null)
        {
            layout = LAYOUT_SHORT;
        }
        ...
        case APPENDER_CONSOLE:
            consoleName = fileProperties.getProperty("appender" + Integer.toString(appenderNumber) + ".consoleName");
            addAppenderConsole(module, layout, consoleName);
            break;
        case APPENDER_FILE:
            fileName = translateFileName(fileProperties.getProperty("appender" + Integer.toString(appenderNumber) + ".fileName"));
            append = MsgTrace.getBooleanProperty(fileProperties, "appender" + Integer.toString(appenderNumber) + ".append", true);
            addAppenderFile(module, layout, fileName, append);
            break;
        case APPENDER_ROLLING_FILE:
            fileName = translateFileName(fileProperties.getProperty("appender" + Integer.toString(appenderNumber) + ".fileName"));
            rollingModeName = fileProperties.getProperty("appender" + Integer.toString(appenderNumber) + ".rollingMode");
        ...
        case APPENDER_MAIL:
            ...
            mailSubject = fileProperties.getProperty("appender" + Integer.toString(appenderNumber) + ".mailSubject");
    ...
    static protected Category getCategory(String module, String classToAppend)
    {
        ...
        else
        {
            ...
            /*
             * FOR THE MOMENT, GO TO THE MODULES' LEVEL (level 3)
             * if (MsgTrace.stringValid(classToAppend))
             * {
             * return Category.getInstance(modulePath + "." + classToAppend);
             * }
             * else
             * {
             * return Category.getInstance(modulePath);
     ...
     * @see
     */
    static protected void emergencyTrace(String module, String classe, String msgToTrace, String extraInfos, Throwable ex)
    {
        StringBuffer sb = new StringBuffer("SilverTrace can't display normaly the message : ");
