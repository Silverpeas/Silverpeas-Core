...
public class NotifPreferenceTable extends AbstractTable {
  ...
  static final private String SELECT_NOTIFPREFERENCE_BY_ID
    = "select " + NOTIFPREFERENCE_COLUMNS + " from ST_NotifPreference where id = ?";

  /**
   * Returns the unique NotifPreference row having the given userId,componentInstanceId,messageType
   */
  public NotifPreferenceRow getByUserIdAndComponentInstanceIdAndMessageType(int userId, int componentInstanceId, int messageType)
    throws UtilException
  {
    int[] intArgs = {userId, componentInstanceId, messageType};
    return (NotifPreferenceRow) getUniqueRow(SELECT_NOTIFPREFERENCE_BY_USERID_AND_COMPONENTINSTANCEID_AND_MESSAGETYPE, intArgs);
  }

  static final private String SELECT_NOTIFPREFERENCE_BY_USERID_AND_COMPONENTINSTANCEID_AND_MESSAGETYPE
    = "select " + NOTIFPREFERENCE_COLUMNS + " from ST_NotifPreference where "
  ...
  public NotifPreferenceRow[] getAllByComponentInstanceId(int componentInstanceId)
    throws UtilException
  {
    return (NotifPreferenceRow[]) getRows(SELECT_ALL_NOTIFPREFERENCE_WITH_GIVEN_COMPONENTINSTANCEID,componentInstanceId)
           .toArray(new NotifPreferenceRow[0]);
  }

  static final private String SELECT_ALL_NOTIFPREFERENCE_WITH_GIVEN_COMPONENTINSTANCEID = "select " + NOTIFPREFERENCE_COLUMNS + " from ST_NotifPreference where componentInstanceId=?";
  ...
  public NotifPreferenceRow[] getAllByUserId(int userId)
    throws UtilException
  {
    return (NotifPreferenceRow[]) getRows(SELECT_ALL_NOTIFPREFERENCE_WITH_GIVEN_USERID,userId)
           .toArray(new NotifPreferenceRow[0]);
  }

  static final private String SELECT_ALL_NOTIFPREFERENCE_WITH_GIVEN_USERID = "select " + NOTIFPREFERENCE_COLUMNS + " from ST_NotifPreference where userId=?";
  ...
  {
    return (NotifPreferenceRow[]) getRows(SELECT_ALL_NOTIFPREFERENCE).toArray(new NotifPreferenceRow[0]);
  }

  static final private String SELECT_ALL_NOTIFPREFERENCE
    = "select " + NOTIFPREFERENCE_COLUMNS + " from ST_NotifPreference";
  ...
  {
    return (NotifPreferenceRow[]) getRows(query)
           .toArray(new NotifPreferenceRow[0]);
  }
  ...
  static final private String INSERT_NOTIFPREFERENCE
    = "insert into"
  ...
  static final private String UPDATE_NOTIFPREFERENCE
    = "update ST_NotifPreference set"
    + " notifAddressId = ?,"
    + " componentInstanceId = ?,"
    + " userId = ?,"
    + " messageType = ?"
  ...
  /**
   * Updates theNotifPreference row.
     ...
     throws UtilException
  {
    ...
    static final private String DELETE_NOTIFPREFERENCE
       = "delete from ST_NotifPreference where id=?";
  ...
  protected Object fetchRow(ResultSet rs)
     throws SQLException
  {
    return new NotifPreferenceRow(
      rs.getInt("id"),
      rs.getInt("notifAddressId"),
      rs.getInt("componentInstanceId"),
      rs.getInt("userId"),
