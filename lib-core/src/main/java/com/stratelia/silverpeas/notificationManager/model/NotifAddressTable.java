...
public class NotifAddressTable extends AbstractTable {
  ...
  static final private String SELECT_NOTIFADDRESS_BY_ID
    = "Select " + NOTIFADDRESS_COLUMNS + " from ST_NotifAddress Where id = ?";
  ...
  public NotifAddressRow[] getAllByUserId(int aUserId, String orderField)
    throws UtilException
  {
    String req = "select " + NOTIFADDRESS_COLUMNS +
                 " from ST_NotifAddress" +
  ...
  public NotifAddressRow[] getAllByNotifChannelId(int notifChannelId)
    throws UtilException
  {
    return (NotifAddressRow[]) getRows(SELECT_ALL_NOTIFADDRESS_WITH_GIVEN_NOTIFCHANNELID,notifChannelId)
           .toArray(new NotifAddressRow[0]);
  }

  static final private String SELECT_ALL_NOTIFADDRESS_WITH_GIVEN_NOTIFCHANNELID = "select " + NOTIFADDRESS_COLUMNS + " from ST_NotifAddress where notifChannelId=?";
  ...
  public NotifAddressRow[] getAllByUserId(int userId)
    throws UtilException
  {
    return (NotifAddressRow[]) getRows(SELECT_ALL_NOTIFADDRESS_WITH_GIVEN_USERID,userId)
           .toArray(new NotifAddressRow[0]);
  }

  static final private String SELECT_ALL_NOTIFADDRESS_WITH_GIVEN_USERID = "select " + NOTIFADDRESS_COLUMNS + " from ST_NotifAddress where userId=?";
  ...
  {
    return (NotifAddressRow[]) getRows(SELECT_ALL_NOTIFADDRESS).toArray(new NotifAddressRow[0]);
  }

  static final private String SELECT_ALL_NOTIFADDRESS
    = "select " + NOTIFADDRESS_COLUMNS + " from ST_NotifAddress";
  ...
  {
    return (NotifAddressRow[]) getRows(query)
           .toArray(new NotifAddressRow[0]);
  }
  ...
  static final private String INSERT_NOTIFADDRESS
    = "insert into"
  ...
  static final private String UPDATE_NOTIFADDRESS
    = "update ST_NotifAddress set"
    + " userId = ?,"
    + " notifName = ?,"
    + " notifChannelId = ?,"
    + " address = ?,"
     ...
     throws UtilException
  {
    ...
    static final private String DELETE_NOTIFADDRESS
       = "delete from ST_NotifAddress where id=?";
    ...
    throws UtilException
  {
    NotifAddressRow[] notifAddressToBeDeleted
       = getAllByNotifChannelId(notifChannelId);
  ...
  protected Object fetchRow(ResultSet rs)
     throws SQLException
  {
    return new NotifAddressRow(
      rs.getInt("id"),
      rs.getInt("userId"),
      rs.getString("notifName"),
      rs.getInt("notifChannelId"),
