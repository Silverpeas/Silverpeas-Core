...
public class NotifChannelTable extends AbstractTable {
  ...
  static final private String SELECT_NOTIFCHANNEL_BY_ID
    = "select " + NOTIFCHANNEL_COLUMNS + " from ST_NotifChannel Where id = ?";
  ...
  {
    return (NotifChannelRow[]) getRows(SELECT_ALL_NOTIFCHANNEL).toArray(new NotifChannelRow[0]);
  }

  static final private String SELECT_ALL_NOTIFCHANNEL
    = "select " + NOTIFCHANNEL_COLUMNS + " from ST_NotifChannel";
  ...
  {
    return (NotifChannelRow[]) getRows(query)
           .toArray(new NotifChannelRow[0]);
  }
  ...
  static final private String INSERT_NOTIFCHANNEL
    = "insert into"
  ...
  static final private String UPDATE_NOTIFCHANNEL
    = "Update ST_NotifChannel set"
    + " name = ?,"
    + " description = ?,"
    + " couldBeAdded = ?,"
    + " fromAvailable = ?,"
     ...
     throws UtilException
  {
    ...
    static final private String DELETE_NOTIFCHANNEL
       = "delete from ST_NotifChannel where id=?";
  ...
  protected Object fetchRow(ResultSet rs)
     throws SQLException
  {
    return new NotifChannelRow(
      rs.getInt("id"),
      rs.getString("name"),
      rs.getString("description"),
      rs.getString("couldBeAdded"),
      rs.getString("fromAvailable"),
      rs.getString("subjectAvailable")) ;
  }
