 ...
 * If an assertion failed, then an IllegalArgumentException is thrown.
 *
 * Using a such class to check the arguments before any code execution in a method ensures at the
 * begining the published <i>contract</i> of a method is well satisfied. It permits to detect in an
 * explicit way the violation of that <i>contract</i> and thus to fix it in a fast way. Without any
 * argument checking, the violation detection could be done in a more or less deep stacktrace execution
 * of the method and in an implicit way, so letting it difficult to identify and to fix it.
 */
