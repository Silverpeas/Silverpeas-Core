 ...
 * components to schedule some jobs at given moments in time or at regular time.
 *
 * A Scheduler object should maintain a registry of Job and Trigger instances. Once registered, the
 * scheduler is responsible for executing the jobs when their associated trigger fires (when their
 * scheduled time arrives). A job can be executed in a two ways: by an event listener at trigger
 * firing event reception or by a Job object carrying the operation to run.
 * A listener of scheduler's events can be specified at job registering to handle, for example,
 * the status of the job execution or to perform some non-business operations out of the job.
 *
 * Scheduler instances are produced by a SchedulerFactory that encapsulates the actual backend used
 * to implement the scheduling system. Whatever the instances returned by the factory, they share
 * a single entry point to the scheduling backend so that a job scheduled with an instance can be
 * found with another one.
 */
public interface Scheduler {

  /**
   * Schedules a job under the specified name, that will be fired with the specified
   * trigger, and by setting the specified listener to recieve the events mapped with the job
   * execution state.
   *
   ...
   * trigger firing).
   * If a job is already scheduled under the specified name, then a SchedulerException exception
   * is thrown.
   ...
   * event listener will recieve the events mapped with the state of the job execution.
   *
   * The specified job will be registered as a scheduled job in the scheduler under its name and its
   * execution will be fired by the specified trigger.
   * In the case an event listener is specified, it will recieve the events mapped with the state of
   * the job execution.
   * If a job is already scheduled under the same name of the specified job, then a
  ...
  /**
   * Schedules the specified job. It will be fired with the specified trigger.
   *
   * The specified job will be registered as a scheduled job in the scheduler under its name and its
   * execution will be fired by the specified trigger.
   * If a job is already scheduled under the same name of the specified job, then a
   ...
   * The firing of triggers are halted and the jobs removed. All of the resources are cleaned up.
   * Once shutdown done, the scheduler cannot be restarted and used for scheduling new jobs.
