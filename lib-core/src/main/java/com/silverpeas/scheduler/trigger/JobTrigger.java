 ...
 *
 * A job trigger is a schema about the triggering of job executions. It is is used for actually
 * scheduling a job in the scheduling backend.
 *
 * This class is an abstract one from which new job trigger can be defined. Currently two types of
 * trigger is supported: a trigger with an Unix-like cron expression and another one with a simple
 * periodicity.
 */
public abstract class JobTrigger {
   ...
   *
   * Null means no explicit starting date and the first fire of the trigger will be done according
   * to the triggering definition.
   * It may or not be the first actual fire time of the trigger,  depending on the type of this
   * trigger and its triggering definition. However, the first actual fire time won't be done after
   * this date.
