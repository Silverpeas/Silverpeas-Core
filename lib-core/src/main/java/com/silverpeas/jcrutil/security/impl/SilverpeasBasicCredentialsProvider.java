...
public class SilverpeasBasicCredentialsProvider implements CredentialsProvider {
   ...
   * {@inheritDoc}
   *
   * Build a {@link Credentials} object for the given authorization header. The creds may be used to
   * login to the repository. If the specified header string is
   * <code>null</code> the behaviour depends on the {@link #defaultHeaderValue} field:<br> <ul> <li>
   * if this field is
   * <code>null</code>, a LoginException is thrown. This is suiteable for clients (eg. webdav
   * clients) for with sending a proper authorization header is not possible, if the server never
   * send a 401. <li> if this an empty string, null-credentials are returned, thus forcing an null
   * login on the repository <li> if this field has a 'user:password' value, the respective simple
   * credentials are generated. </ul>
   * <p/>
