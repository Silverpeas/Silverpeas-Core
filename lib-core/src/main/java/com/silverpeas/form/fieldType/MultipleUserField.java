...
public class MultipleUserField implements Field
{
  ...
  {
	SilverTrace.info("form", "MultipleUserField.setUserIds", "root.MSG_GEN_ENTER_METHOD", "userIds = "+userIds);
  ...
  {
	SilverTrace.info("form", "MultipleUserField.getValue", "root.MSG_GEN_PARAM_VALUE", "userIds = "+getUserIds());
     if (getUserIds() == null) return null;
	 if (getUserIds().length == 0) return "";
  ...
  {
     if (value instanceof UserDetail[]) return ! isReadOnly();
     else return false;
  }
