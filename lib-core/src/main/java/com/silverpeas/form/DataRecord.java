...
/**
 * A DataRecord is the interface used by all the form components to exchange,
 * display and save a set of named and typed fields which are unknown at compile
 * time but defined by a silverpeas end user in a workflow process model or a
 * publication model.
 * 
 * A DataRecord is modelized by a RecordTemplate giving all the fields names and
 * types.
 * 
...
public interface DataRecord extends Serializable {
   ...
   * 
   * This id is unique within the RecordSet from witch this DataRecord has been
   * extracted.
   * 
   * This id is null when the DataRecord has been built from a RecordTemplate
