...
public class MyLinksPeasRequestRouter extends ComponentRequestRouter
{
    ...
    public String getDestination(String function, ComponentSessionController componentSC, HttpServletRequest request)
    {
        ...
        SilverTrace.info("myLinksPeas", "MyLinksPeasRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "User=" + componentSC.getUserId() + " Function=" + function);
        ...
        try
        {
			...
			{
				// récupération des paramètres venus de l'écran de saisie et création de l'objet LinkDetail
			...
			{
				// récupération des paramètres transmis et création de l'objet LinkDetail
				LinkDetail link = generateLink(request);
				myLinksSC.createLink(link);
        ...
        SilverTrace.info("myLinks", "MyLinksPeasRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "Destination=" + destination);
        return destination;
    }
    ...
    private LinkDetail generateLink(HttpServletRequest request)
    {
		...
		if (instanceId != null)
			return new LinkDetail(name, description,url,visible,popup, instanceId);
		else  */
