...
public class AlertUserPeasSessionController extends AbstractComponentSessionController
{
     ...
     * @see
     */
    public AlertUserPeasSessionController(MainSessionController mainSessionCtrl, ComponentContext componentContext)
    {
        super(mainSessionCtrl, componentContext, "com.stratelia.silverpeas.alertUserPeas.multilang.alertUserPeasBundle", "com.stratelia.silverpeas.alertUserPeas.settings.alertUserPeasIcons");
   ...
   // -------------------------------------------------------------------------------------------------------------------
	...
	public String initSelection() 
	{
		...
		//bien que le up s'affiche en popup, le mécanisme de fermeture est assuré par le composant=> il est donc nécessaire d'indiquer
	...
	{
		m_userRecipients = SelectionUsersGroups.getUserDetails(m_Selection.getSelectedElements());
		m_groupRecipients = SelectionUsersGroups.getGroups(m_Selection.getSelectedSets());
	...
	public void prepareNotification(String message)
	{
		...
		notifMetaData.addUserRecipients(SelectionUsersGroups.getUserIds(getUserRecipients()));
		notifMetaData.addGroupRecipients(SelectionUsersGroups.getGroupIds(getGroupRecipients()));
        if (message != null && message.trim().length() > 0 && (getUserRecipients().length > 0 || getGroupRecipients().length > 0))
		{
			/*StringBuffer content = new StringBuffer(notifMetaData.getContent("fr"));
			content.append(getString("AuthorMessage")).append(" : \"").append(message).append("\"\n\n\n");
	...
	{
		StringBuffer content = new StringBuffer(getNotificationMetaData().getContent(language));
		SilverTrace.info("alertUserPeas", "AlertUserPeasSessionController.setNotificationContent()", "root.MSG_GEN_PARAM_VALUE", "language = "+language+", content = "+content.toString());
		content.append("\n\n").append(getString("AuthorMessage")).append(" : \n\"").append(message).append("\"");
		getNotificationMetaData().setContent(content.toString(), language);
		SilverTrace.info("alertUserPeas", "AlertUserPeasSessionController.setNotificationContent()", "root.MSG_GEN_EXIT_METHOD", "content = "+getNotificationMetaData().getContent(language));
	}
	...
	public void sendNotification() 
	{
		try 
		{
			SilverTrace.info("alertUserPeas", "AlertUserPeasSessionController.sendNotification()", "root.MSG_GEN_PARAM_VALUE", "content_en = "+getNotificationMetaData().getContent("en"));
			m_NotificationSender.notifyUser(getNotificationMetaData());
		} 
		catch (NotificationManagerException e) 
		{
			SilverTrace.warn("alertUserPeas","AlertUserPeasSessionController.sendNotification()", "root.EX_NOTIFY_USERS_FAILED",e);
		}
    }
