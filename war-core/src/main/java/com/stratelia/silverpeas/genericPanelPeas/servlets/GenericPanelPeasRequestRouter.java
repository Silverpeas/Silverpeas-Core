...
public class GenericPanelPeasRequestRouter extends ComponentRequestRouter
{
    ...
    public String getDestination(String function, ComponentSessionController componentSC, HttpServletRequest request)
    {
        ...
        SilverTrace.info("genericPanelPeas", "getDestination()", "root.MSG_GEN_PARAM_VALUE", "Function=" + function);
        ...
        try
        {
            ...
            else if (function.startsWith("DoOperation"))
            {
                ...
                {
                    genericPanelPeasSC.setSelectedElements(getSelected(request, genericPanelPeasSC.getNbMaxDisplayed()));
                }

                SilverTrace.info("genericPanelPeas", "getDestination()", "root.MSG_GEN_PARAM_VALUE", "Operation=" + op);
                ...
                {
                    request.setAttribute("HostUrl",genericPanelPeasSC.getZoomToItemURL() + "?elementId=" + request.getParameter("userId"));
                    destination = "goBack.jsp";
                }
                else if ((op != null) && (op.startsWith("GENERICPANELMINIFILTER")))
                {
                    genericPanelPeasSC.setMiniFilter(request.getParameter("miniFilter" + op.substring("GENERICPANELMINIFILTER".length())), op.substring("GENERICPANELMINIFILTER".length()));
                    destination = "genericPanelPeas.jsp";
                }
                else // Go...
                {
            ...
            {
                request.setAttribute("isZoomToItemValid",new Boolean(genericPanelPeasSC.isZoomToItemValid()));
                request.setAttribute("isFilterValid",new Boolean(genericPanelPeasSC.isFilterValid()));
                request.setAttribute("isMultiSelect",new Boolean(genericPanelPeasSC.isMultiSelect()));
                request.setAttribute("isSelectable",new Boolean(genericPanelPeasSC.isSelectable()));
            ...
            request.setAttribute("ToPopup",new Boolean(genericPanelPeasSC.isPopupMode()));
        ...
        SilverTrace.info("genericPanelPeas", "getDestination()", "root.MSG_GEN_PARAM_VALUE", "Destination=" + destination);
        return destination;
    }
