...
public class NotificationUserSessionController extends AbstractComponentSessionController
{
     ...
     * @see
     */
    public NotificationUserSessionController(MainSessionController mainSessionCtrl, ComponentContext componentContext)
    {
        super(mainSessionCtrl, componentContext,"com.stratelia.silverpeas.notificationUser.multilang.notificationUserBundle");
    ...
    public ArrayList getDefaultAddresses() throws NotificationManagerException
    {
        ...
        al.add(nm.getNotificationAddress(NotificationParameters.ADDRESS_BASIC_POPUP, uId));
        al.add(nm.getNotificationAddress(NotificationParameters.ADDRESS_BASIC_SILVERMAIL, uId));
        al.add(nm.getNotificationAddress(NotificationParameters.ADDRESS_BASIC_SMTP_MAIL, uId));
        return al;
    }
    ...
    public ArrayList getAvailableUsers()
    {
        ...
        for (i = 0; i < allUsers.length; i++)
        {
            ...
            p.setProperty("name", allUsers[i].getLastName() + " " + allUsers[i].getFirstName());
            ar.add(p);
        }
        return ar;
    }
    ...
    public void sendMessage(String compoId, String notificationId, String priorityId, String txtTitle, String txtMessage, String[] selectedUsers, String[] selectedGroups) throws NotificationManagerException
    {
        ...
        int                notifTypeId = NotificationParameters.ADDRESS_COMPONENT_DEFINED;
				...
				SilverTrace.debug("notificationUser", "NotificationUsersessionController.sendMessage()", "root.MSG_GEN_PARAM_VALUE", "  AVANT CONTROLE priorityId="+priorityId);
				...
				if ((notificationId != null) && (notificationId.length() > 0))
        {
            notifTypeId = Integer.parseInt(notificationId);
				...
				//pb sous bea, quand prirityId est vide, le Integer.parseInt(priorityId) cause une exception
				if(priorityId == null || priorityId.length()==0)
					priorityId = "0";

			NotificationMetaData notifMetaData = new NotificationMetaData(Integer.parseInt(priorityId), txtTitle, txtMessage);
  ...
  public String buildOptions(ArrayList ar, String selectValue, String selectText, boolean bSorted)
  {
        ...
        if (selectText != null)
        {
            ...
            valret.append("<option value=\"\" " + selected + ">" + EncodeHelper.javaStringToHtmlString(selectText) + "</option>\n");
        }
        if (bSorted)
        {
        ...
        if (arToDisplay != null)
        {
            for (i=0 ; i<arToDisplay.size() ; i++) 
            {
                ...
                valret.append("<option value=\"" + elmt.getProperty("id") + "\" " + selected + ">" + EncodeHelper.javaStringToHtmlString(elmt.getProperty("name")) + "</option>\n");
            }
        }
		...
		//JCG
		public String initSelectionPeas(String[] idUsers, String[] idGroups, String paramValues)
		{
			SilverTrace.debug("notificationUser", "NotificationUsersessionController.initSelectionPeas()", "root.MSG_GEN_PARAM_VALUE", "ENTER METHOD");

    	    String m_context = GeneralPropertiesManager.getGeneralResourceLocator().getString("ApplicationURL");
			String hostUrl = m_context+URLManager.getURL(URLManager.CMP_NOTIFICATIONUSER)+"GetTarget"+paramValues;
			String cancelUrl = m_context + URLManager.getURL(URLManager.CMP_NOTIFICATIONUSER)+"GetTarget"+paramValues;
		...
		public String[] initTargetsUsers(String theTargetsUsers){
			...
			if (theTargetsUsers!=null && theTargetsUsers.length()>0){
				if(theTargetsUsers.equals("Administrators"))
					idUsers = this.getOrganizationController().getAdministratorUserIds(getUserId());
				else
		...
		public ArrayList getSelectedUsers(String[] selectedUersId)throws NotificationUserException
    {
        Properties   p;
        int          i;
        ArrayList    ar = new ArrayList();
				...
				if(selectedUersId!=null && selectedUersId.length>0){
					...
					if ((selectedUsers != null) && (selectedUsers.length > 0))
					{
						...
						{
							throw new NotificationUserException("NotificationUserSessionControl.getSelectedUsers()", SilverpeasException.ERROR, "notificationUser.EX_CANT_GET_SELECTED_USERS_INFOS");
						}
					}
				}
        return ar;
		...
		public ArrayList getSelectedGroups(String[] selectedGroupsId) throws NotificationUserException
    {
				SilverTrace.debug("notificationUser", "NotificationUsersessionController.getSelectedGroups()", "root.MSG_GEN_PARAM_VALUE", "ENTER METHOD");
        ...
        ArrayList  ar = new ArrayList();
				...
				if(selectedGroupsId !=null && selectedGroupsId.length>0){
					...
					if ((selectedGroups != null) && (selectedGroups.length > 0))
					{
						...
						{							
							throw new NotificationUserException("NotificationUserSessionControl.getSelectedGroups()", SilverpeasException.ERROR, "notificationUser.EX_CANT_GET_SELECTED_GROUPS_INFOS");
						}
					}
				}
        return ar;
		...
		private UserDetail[] getUserDetailList(String[] idUsers){
			SilverTrace.debug("notificationUser", "NotificationUsersessionController.getUserDetailList()", "root.MSG_GEN_PARAM_VALUE", "Enter Method");
