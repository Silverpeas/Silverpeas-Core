...
abstract public class CacheManager
{
    ...
    abstract public BrowsePanelProvider getSearchPanelProvider(int what, CacheManager cm, SelectionExtraParams sep);
    abstract public BrowsePanelProvider getBrowsePanelProvider(int what, CacheManager cm, SelectionExtraParams sep);
    abstract public PanelProvider getCartPanelProvider(int what, CacheManager cm, SelectionExtraParams sep);
    ...
    {
        SilverTrace.info("selectionPeas", "CacheManager.setInfos", "root.MSG_GEN_PARAM_VALUE", "What = " + what + ", Id=" + id + ", Name = " + pl.m_Values[0] + ", Selected=" + pl.m_Selected);
    ...
    public PanelLine getInfos(int what, String id)
    {
        ...
        if (valret != null)
        {
            SilverTrace.info("selectionPeas", "CacheManager.getInfos", "root.MSG_GEN_PARAM_VALUE", "What = " + what + ", Id=" + id + ", Name = " + valret.m_Values[0] + ", Selected=" + valret.m_Selected);
        }
        else
        {
            SilverTrace.info("selectionPeas", "CacheManager.getInfos", "root.MSG_GEN_PARAM_VALUE", "What = " + what + ", Id=" + id + ", NULL");
    ...
    public PanelLine[] getSelectedLines(int what)
    {
        ...
        Arrays.sort(valret, new Comparator()
        {
            ...
            {
                return ((PanelLine) o1).m_Values[0].toUpperCase().compareTo(((PanelLine) o2).m_Values[0].toUpperCase());
            }
