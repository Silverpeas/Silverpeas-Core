...
public class CacheManagerUsersGroups extends CacheManager
{
    ...
    public String[] getColumnsNames(int what)
    {
        if (what == CM_SET)
        {
            ...
            if (SelectionPeasSettings.m_DisplayNbUsersByGroup)
            {
                if (SelectionPeasSettings.m_DisplayDomains)
                {
                    ...
                    columnsHeader[COL_GROUP_DESCRIPTION] = m_Global.getString("GML.description");
                    columnsHeader[COL_GROUP_NBUSERS] = m_Global.getString("GML.users");
                    columnsHeader[COL_GROUP_NBUSERS+1] = m_Local.getString("selectionPeas.domain");
                }
                else
                {
            ...
            else
            {
                if (SelectionPeasSettings.m_DisplayDomains)
                {
                    ...
                    columnsHeader[COL_GROUP_NAME] = m_Global.getString("GML.nom");
                    columnsHeader[COL_GROUP_DESCRIPTION] = m_Global.getString("GML.description");
                    columnsHeader[COL_GROUP_DESCRIPTION+1] = m_Local.getString("selectionPeas.domain");
                }
                else
                {
        ...
        else if (what == CM_ELEMENT)
        {
            ...
            if (SelectionPeasSettings.m_DisplayDomains)
            {
                ...
                columnsHeader[COL_USER_DOMAIN] = m_Local.getString("selectionPeas.domain");
            }
            else
            {
                columnsHeader = new String[3];
    ...
    public PanelMiniFilterSelect getSelectMiniFilter(int what)
    {
        if (what == CM_SET)
        {
            return new PanelMiniFilterSelect(999, Integer.toString(what), "set", m_Context + m_Icon.getString("selectionPeas.selectAll"), m_Context + m_Icon.getString("selectionPeas.unSelectAll"), m_Local.getString("selectionPeas.selectAll"), m_Local.getString("selectionPeas.unSelectAll"), m_Local.getString("selectionPeas.selectAll"), m_Local.getString("selectionPeas.unSelectAll"));
        }
        else if (what == CM_ELEMENT)
        {
            return new PanelMiniFilterSelect(999, Integer.toString(what), "element", m_Context + m_Icon.getString("selectionPeas.selectAll"), m_Context + m_Icon.getString("selectionPeas.unSelectAll"), m_Local.getString("selectionPeas.selectAll"), m_Local.getString("selectionPeas.unSelectAll"), m_Local.getString("selectionPeas.selectAll"), m_Local.getString("selectionPeas.unSelectAll"));
        }
        else
        {
            return null;
    ...
    public PanelMiniFilterToken[] getPanelMiniFilters(int what)
    {
        if (what == CM_SET)
        {
            ...
            theArray[0] = new PanelMiniFilterEdit(0, Integer.toString(what), "", m_Context + m_Icon.getString("selectionPeas.filter"), m_Local.getString("selectionPeas.filter"), m_Local.getString("selectionPeas.filter"));
            return theArray;
        }
        else if (what == CM_ELEMENT)
        {
    ...
    protected PanelLine getLineFromId(int what, String id)
    {
        ...
        if (what == CM_SET)
        {
            ...
            SilverTrace.info("selectionPeas", "CacheManagerUsersGroups.getSetLineFromId()", "root.GEN_MSG_PARAM_VALUE", "id=" + id);
            if (SelectionPeasSettings.m_DisplayNbUsersByGroup)
            {
                if (SelectionPeasSettings.m_DisplayDomains)
                {
                    theValues = new String[4];
                    theValues[COL_GROUP_NAME] = EncodeHelper.javaStringToHtmlString(theGroup.getName());
                    theValues[COL_GROUP_DESCRIPTION] = EncodeHelper.javaStringToHtmlString(theGroup.getDescription());
                    theValues[COL_GROUP_NBUSERS] = Integer.toString(m_oc.getAllSubUsersNumber(theGroup.getId()));
                    theValues[COL_GROUP_NBUSERS+1] = EncodeHelper.javaStringToHtmlString(m_ac.getDomain(theGroup.getDomainId()).getName());
                }
                else
                {
            ...
            else
            {
                if (SelectionPeasSettings.m_DisplayDomains)
                {
                    ...
                    theValues[COL_GROUP_NAME] = EncodeHelper.javaStringToHtmlString(theGroup.getName());
                    theValues[COL_GROUP_DESCRIPTION] = EncodeHelper.javaStringToHtmlString(theGroup.getDescription());
                    theValues[COL_GROUP_DESCRIPTION+1] = EncodeHelper.javaStringToHtmlString(m_ac.getDomain(theGroup.getDomainId()).getName());
                }
                else
                {
        ...
        else if (what == CM_ELEMENT)
        {
            ...
            SilverTrace.info("selectionPeas", "CacheManagerUsersGroups.getElementLineFromId()", "root.GEN_MSG_PARAM_VALUE", "id=" + id);
            if (SelectionPeasSettings.m_DisplayDomains)
            {
                theValues = new String[4];
                theValues[COL_USER_LASTNAME] = EncodeHelper.javaStringToHtmlString(theUser.getLastName());
                theValues[COL_USER_FIRSTNAME] = EncodeHelper.javaStringToHtmlString(theUser.getFirstName());
                theValues[COL_USER_EMAIL] = EncodeHelper.javaStringToHtmlString(theUser.geteMail());
                theValues[COL_USER_DOMAIN] = EncodeHelper.javaStringToHtmlString(m_ac.getDomain(theUser.getDomainId()).getName());
            }
            else
            {
                theValues = new String[3];
    ...
    public BrowsePanelProvider getSearchPanelProvider(int what, CacheManager cm, SelectionExtraParams sep)
    {
        if (what == CM_SET)
        {
            return new SearchGroupPanel(m_Language, m_Local, cm, getSureExtraParams(sep));
        }
        else if (what == CM_ELEMENT)
        {
            return new SearchUserPanel(m_Language, m_Local, cm, getSureExtraParams(sep));
        }
        else
        {
            return null;
    ...
    public BrowsePanelProvider getBrowsePanelProvider(int what, CacheManager cm, SelectionExtraParams sep)
    {
        if (what == CM_SET)
        {
            return new BrowseGroupPanel(m_Language, m_Local, cm, getSureExtraParams(sep));
        }
        else if (what == CM_ELEMENT)
        {
            return new BrowseUserPanel(m_Language, m_Local, cm, getSureExtraParams(sep));
        }
        else
        {
            return null;
    ...
    public PanelProvider getCartPanelProvider(int what, CacheManager cm, SelectionExtraParams sep)
    {
        if (what == CM_SET)
        {
            return new CartGroupPanel(m_Language, m_Local, cm, getSureExtraParams(sep));
        }
        else if (what == CM_ELEMENT)
        {
    ...
    public PanelOperation getPanelOperation(String operation)
    {
        ...
        {
            return new PanelOperation(m_Local.getString("selectionPeas.helpBrowse"), m_Context + m_Icon.getString("selectionPeas.browseArb"), operation);
        }
        else if ("DisplaySearchElement".equals(operation))
        {
            return new PanelOperation(m_Local.getString("selectionPeas.helpSearchElement"), m_Context + m_Icon.getString("selectionPeas.userSearc"), operation);
        }
        else if ("DisplaySearchSet".equals(operation))
        {
            return new PanelOperation(m_Local.getString("selectionPeas.helpSearchSet"), m_Context + m_Icon.getString("selectionPeas.groupSearc"), operation);
        }
        else
        {
            return null;
    ...
    protected SelectionUsersGroups getSureExtraParams(SelectionExtraParams sep)
    {
    	...
    	if (m_ud.isDomainAdminRestricted() && ((valret.getDomainId() == null) || (valret.getDomainId().length() <= 0)))
    	{
