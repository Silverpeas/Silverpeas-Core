...
public class AuthenticationNTLMServlet extends HttpServlet
{
        ...
        if (msg != null && (msg.startsWith("NTLM ") ||
            ...
            if (ssn == null || ssn.getAttribute("NtlmHttpAuth") == null) {
            	...
            	if (authenticationSettings.getBoolean("ntlm.useOnlyWithIE", false))
            	{
                	...
                	if (StringUtil.isDefined(userAgent) && userAgent.indexOf("MSIE") == -1)
                		doNTLM = false;
            	}
            	if (doNTLM)
            	{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException
    {
        ...
        NtlmPasswordAuthentication ntlm = (NtlmPasswordAuthentication) session.getAttribute("NtlmHttpAuth");
        if (ntlm == null)
        ...
        if (!StringUtil.isDefined(redirectURL))
        {
			...
			String defaultDomainId 	= SilverpeasSettings.readString(authenticationSettings, "ntlm.defaultDomainId", "0");
			String domainId 		= SilverpeasSettings.readString(authenticationSettings, "ntlm.domainId."+domain, defaultDomainId);
			
			SilverTrace.info("authentication", "AuthenticationNTLMServlet.doPost", "root.MSG_GEN_PARAM_VALUE", "login = "+login+", domain = "+domain+", domainId = "+domainId+", defaultDomainId = "+defaultDomainId);
			
			//Used by hyperlink and to be coherent with AuthenticationServlet
			session.setAttribute("Silverpeas_pwdForHyperlink", "UnavailablePasswordWithNTLM");
			
			// Authenticate current user on his domain
			String authentificationKey = lpAuth.authenticate(login, domainId, request);
			
			if ((authentificationKey == null || authentificationKey.startsWith("Error")) && !defaultDomainId.equals(domainId))
			{
				//Authentication failed on domainId
				//Try to authenticate on defaultDomain (which is different from domainId)
    ...
    {
    	return new ResourceLocator("com.silverpeas.authentication.settings.authenticationSettings", "");
    }
    ...
    private String getStartURL(HttpServletRequest request)
    {
        ...
        if(m_sContext.length()>0 && m_sContext.charAt(m_sContext.length()-1) == '/')
        {
