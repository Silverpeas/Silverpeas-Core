...
public class AuthenticationServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException
    {
    	...
    	boolean cookieEnabled = SilverpeasSettings.readBoolean(authenticationSettings, "cookieEnabled", false);
        ...
        SilverTrace.debug("authentication", "AuthenticationServlet.doPost()", "root.MSG_GEN_PARAM_VALUE",
        	"sCryptedPassword = " + sCryptedPassword);
        ...
        if (isNewEncryptMode)
        {
			...
			decodedLogin = decode(sLogin, stringKey, false);
        	sDecodedPassword = ((sCryptedPassword == null || sCryptedPassword.length() == 0)
        		? decode(sPassword, stringKey, false) : decode(sPassword, stringKey, true));
			if (cookieEnabled)
			{
	    		if (sCryptedPassword != null && sCryptedPassword.length() > 0)
	    		{
		...
		{
			authentificationKey = lpAuth.authenticate(sLogin, sDecodedPassword, sDomainId, request); // Throws Specific Exception
		}
		else
		{
			/*try
			...
			catch (AuthenticationException e)
			{
				SilverTrace.error("authentication", "AuthenticationServlet.doPost()",
					"root.CANT_STORE_AUTHENTICATION_TESTKEY", "sLogin = " + sLogin + ", sDomainId = " + sDomainId
					+ ", testKey = " + testKey, e);
			}
        ...
        if (authentificationKey != null && !authentificationKey.startsWith("Error"))
        {
        	...
        	{
        		SilverTrace.debug("authentication", "AuthenticationServlet.doPost()", "root.MSG_GEN_ENTER_METHOD", "Ok");
...
//		SilverTrace.debug("authentication", "AuthenticationServlet.convert2Alpha()", "root.MSG_GEN_ENTER_METHOD","alphaString="+alphaString);
		return alphaString;
	}
