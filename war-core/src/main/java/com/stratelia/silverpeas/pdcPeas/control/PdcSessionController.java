...
public class PdcSessionController extends AbstractComponentSessionController
{
	...
	private final static java.text.SimpleDateFormat formatter = new java.text.SimpleDateFormat("yyyy/MM/dd");
	...
	{
		return getUserDetail().isAccessAdmin() || getUserDetail().isAccessKMManager();
	}
	...
	public void deleteAxis(String axisId) throws PdcException {
		...
		try {
			...
			throw new PdcException("PdcSessionController.deleteAxis",SilverpeasException.ERROR,"pdcPeas.CANNOT_DELETE_AXIS","",e);
		} finally {
			closeConnection(con);
		}
	}
	...
	public Value getAxisValue(String valueId, boolean setAsCurrentValue) throws PdcException {
		Value value = getPdcBm().getAxisValue(valueId, new Integer(getCurrentAxis().getAxisHeader().getRootId()).toString());
		value.setAxisId(Integer.parseInt(getCurrentAxis().getAxisHeader().getPK().getId()));
	...
	public void setValuesToPdcAddAPosteriori(String values) {
		SilverTrace.info("Pdc", "PdcSessionController.setValuesToPdcAddAPosteriori", "root.MSG_GEN_PARAM_VALUE", "values = " + values);
        this.values = values;
    }
	...
	public int moveCurrentValueToNewFatherId(String newFatherId, int orderNumber) throws PdcException {
		int status = getPdcBm().moveValueToNewFatherId(getCurrentAxis() , getCurrentValue(), newFatherId, orderNumber);
		return status;
	}
	...
	public int createDaughterValue(Value value) throws PdcException {
		...
		value.setCreatorId(getUserId());
		value.setCreationDate(formatter.format(new Date()));
		int daughterId = getPdcBm().createDaughterValue(value, currentValueId, new Integer(getCurrentAxis().getAxisHeader().getRootId()).toString());
		refreshCurrentAxis(getAxisDetail(getCurrentAxis().getAxisHeader().getPK().getId()));
		return daughterId;
	}

	public int updateValue(Value value) throws PdcException {
		int status = getPdcBm().updateValue(value, new Integer(getCurrentAxis().getAxisHeader().getRootId()).toString());
		refreshCurrentValueAndCurrentAxis(value, getAxisDetail(getCurrentAxis().getAxisHeader().getPK().getId()));
		return status;
	}
	...
	public void deleteValueAndSubtree(String valueId) throws PdcException {
		...
		try {
			...
			ArrayList values = (ArrayList) getPdcBm().getDaughterValues(new Long(treeId).toString(), valueId);
			//ajoute le valueId
			values.add(valueId);

			con = getConnection();

			getPdcBm().deleteValueAndSubtree(con, valueId, getCurrentAxis().getAxisHeader().getPK().getId(), new Integer(getCurrentAxis().getAxisHeader().getRootId()).toString());

			getThBm().deleteSynonymsTerms(con, treeId, values);
// dans le PdcBmImpl : on efface les droits liées aux valeurs du subtree effacé
			commitConnection(con);

			refreshCurrentValueAndCurrentAxis(null, getAxisDetail(getCurrentAxis().getAxisHeader().getPK().getId()));

		} catch (Exception e) {
			rollbackConnection(con);
			throw new PdcException("PdcSessionController.deleteValueAndSubtree",SilverpeasException.ERROR,"pdcPeas.CANNOT_DELETE_VALUES","",e);
		} finally {
			closeConnection(con);
		}
	}
	...
	public String deleteValue(String valueId) throws PdcException {
		...
		try {			
			...
			daughterValueName = getPdcBm().deleteValue(con, valueId, getCurrentAxis().getAxisHeader().getPK().getId(), new Integer(getCurrentAxis().getAxisHeader().getRootId()).toString());
			...
			rollbackConnection(con);
			throw new PdcException("PdcSessionController.deleteValue",SilverpeasException.ERROR,"pdcPeas.CANNOT_DELETE_VALUE","",e);
		} finally {
			closeConnection(con);
		}
	...
	*/
	public List getFullPath(String valueId) throws PdcException{
		return getPdcBm().getFullPath(valueId, new Integer(getCurrentAxis().getAxisHeader().getRootId()).toString());
	}

	private void refreshCurrentValueAndCurrentAxis(Value value, Axis axis) throws PdcException {
	...
	private Connection getConnection() throws PdcException {
        ...
        {
            throw new PdcException("PdcSessionController.getConnection()", SilverpeasException.ERROR, "root.EX_CONNECTION_OPEN_FAILED", e);
        }
		return con;
	}
	...
	private void closeConnection(Connection con) throws PdcException {
		if (con != null) {
			...
			{
				 throw new PdcException("pdcPeas.closeConnection",SilverpeasException.ERROR,"root.EX_CONNECTION_CLOSE_FAILED","",e);
			}
		}
	}
	...
	private void rollbackConnection(Connection con) {
        if (con != null) {
            try {
                ...
                SilverTrace.error("pdcPeas", "rollbackConnection()", "root.EX_CONNECTION_CLOSE_FAILED", "", e);
            }
        }
    }
    ...
    private void commitConnection(Connection con) {
        if (con != null) {
            try {
                ...
                SilverTrace.error("pdcPeas", "commitConnection()", "root.EX_CONNECTION_CLOSE_FAILED", "", e);
            }
        }
    }
    ...
    public String initUserPanelForPdcManager() throws RemoteException, PdcException, SQLException
    {
		...
		String name = getCurrentAxis().getAxisHeader().getName(getCurrentLanguage());
	...
	public Value getValue(String axisId, String valueId) throws PdcException {
        ...
        List values = getPdcBm().getPertinentDaughterValuesByInstanceIds(new SearchContext(), axisId, valueId, getComponentList());
		if (values != null) {
	...
	public List getManagers() throws PdcException, SQLException
	{
		...
		List managers = pdcBm.getManagers(getCurrentAxis().getAxisHeader().getPK().getId(), valueId);
	...
	public List getRights() throws PdcException, SQLException
	{
		...
		for (int i = 0; i<axisValues.size(); i++) 
		{
			...
			usersAndGroups = getManagers(getCurrentAxis().getAxisHeader().getPK().getId(), valueId);
	...
	{
		return isValueManager(getCurrentAxis().getAxisHeader().getPK().getId(), getCurrentValue().getPK().getId());
	}
	...
	public void setManagers(List userIds, List groupIds) throws PdcException, SQLException
	{
		...
		pdcBm.setManagers(userIds, groupIds, getCurrentAxis().getAxisHeader().getPK().getId(), valueId);
	}
	...
	public void eraseManagers() throws PdcException, SQLException
	{
		...
		pdcBm.setManagers(null, null, getCurrentAxis().getAxisHeader().getPK().getId(), valueId);
	}
