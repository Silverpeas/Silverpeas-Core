...
public class PdcRequestRouter extends ComponentRequestRouter
{
    ...
    {
        return new PdcSessionController(mainSessionCtrl, componentContext, "com.stratelia.silverpeas.pdcPeas.multilang.pdcBundle", "com.stratelia.silverpeas.pdcPeas.settings.pdcPeasIcons");
    }
	...
	/**
	* This is the separator string between the name of axis or values and their order in the option html tag
	*/
    ...
    {
		PdcSessionController pdcSC = (PdcSessionController) componentSC; // get the session controller to inform the request
		String destination = "";
		...
		try
		{
			...
			if (function.startsWith("Main")) {
				...
				SilverTrace.info("Pdc", "PdcRequestRouter.Main", "root.MSG_GEN_PARAM_VALUE", "creationAllowed = " + creationAllowed);
				...
				request.setAttribute("ViewType", pdcSC.getCurrentView()); // the type of the axe
				...
				SilverTrace.info("Pdc", "PdcRequestRouter.Main", "root.MSG_GEN_PARAM_VALUE", "list.size() = " + list.size());
				...
				// get URL parameters
				String type = request.getParameter("ViewType"); // get the id of the selected axe
				...
				request.setAttribute("SecondaryAxis", pdcSC.getSecondaryAxis()); // set a sorted list
				request.setAttribute("PrimaryAxis", pdcSC.getPrimaryAxis()); // set a sorted list
				request.setAttribute("ViewType", pdcSC.getCurrentView()); // the type of the axe
				...
				//get URL parameters
				String axeId = request.getParameter("Id"); // get the id of the selected axe
				...
				if (!pdcSC.isPDCAdmin() && !pdcSC.isAxisManager(axeId))
					throw new AccessForbiddenException("PdcRequestRouter.EditAxit", SilverpeasException.WARNING, null);

				String viewType		= pdcSC.getCurrentView(); // get the type of the current axe
				...
				// get values from session controller to set into the request
				request.setAttribute("SecondaryAxis", secondaryAxis); // set a sorted list
				request.setAttribute("PrimaryAxis", primaryAxis); // set a sorted axis header list 
				...
				if (translation == null || translation.equals("null") || translation.length() != 0)
				...
				if (!pdcSC.isPDCAdmin())
					throw new AccessForbiddenException("PdcRequestRouter."+function, SilverpeasException.WARNING, null);
				...
				// get URL parameters
				String axeId 	= request.getParameter("Id"); // get the id of the selected axe
				String valueId 	= request.getParameter("ValueId"); // get the id of the selected value
				...
				// get URL parameters
				String axeName	= request.getParameter("Name").trim(); // get the name of the axe
				String axeType	= request.getParameter("Type");
				String axeOrder = extractOrder(request.getParameter("Order"));
				String axeDescription	= request.getParameter("Description").trim(); // get the description of the axe

				// create the axe
				AxisHeader axisHeader = new AxisHeader(new AxisPK("unknown"),axeName,axeType,(new Integer(axeOrder)).intValue(), null,null,-1,axeDescription);
				SilverTrace.info("Pdc", "PdcRequestRouter.CreateAxis", "root.MSG_GEN_PARAM_VALUE", "Before creation");
				...
				SilverTrace.info("Pdc", "PdcRequestRouter.CreateAxis", "root.MSG_GEN_PARAM_VALUE", "After creation");

				switch (status){
					case 1:	request.setAttribute("MaxAxis","1"); // max
							request.setAttribute("ViewType", pdcSC.getCurrentView());
							...
							destination = "/pdcPeas/jsp/axisManager.jsp";
							break;
					case 2:	request.setAttribute("AxisHeader", axisHeader);
							request.setAttribute("AlreadyExist","1");
							request.setAttribute("ViewType", pdcSC.getCurrentView());
							request.setAttribute("SecondaryAxis", pdcSC.getSecondaryAxis());
							request.setAttribute("PrimaryAxis", pdcSC.getPrimaryAxis());
							destination = getDestination("AxisAlreadyExist", componentSC, request);
							break;
					default : destination = "/pdcPeas/jsp/reloadPdc.jsp";
				}
				...
				// update axis
				AxisHeader axisHeader = new AxisHeader(currentAxisPK,axeName,axeType,order, null,null,-1, axeDescription);
				...
				switch (status){
					case 2:	request.setAttribute("AxisHeader", axisHeader); // already exist
							request.setAttribute("AlreadyExist","1");
							request.setAttribute("ViewType", pdcSC.getCurrentView());
							request.setAttribute("SecondaryAxis", pdcSC.getSecondaryAxis());
							request.setAttribute("PrimaryAxis", pdcSC.getPrimaryAxis());
				...
				// get URL parameters
				String valueId	= request.getParameter("Id"); // get the id of the selected value
				...
				if (currentValue == null) {//il n'y a pas de documents classés dans cette valeur
				    currentValue = pdcSC.getAxisValue(valueId);
				    currentValue.setNbObjects(0);
				}
				...
				boolean isAdmin = pdcSC.isPDCAdmin() || pdcSC.isAxisManager() || pdcSC.isInheritedManager();
				...
				// get rights for this axis and this user and if user is admin or kmAdmin
				Boolean KMadmin = new Boolean( ( pdcSC.getUserDetail().isAccessKMManager() || pdcSC.getUserDetail().isAccessAdmin() )  );
				...
				if (!newFatherId.equals(currentValue.getFatherId()))
				{
					...
					List newSisters = getDaughterValues(pdcSC.getCurrentAxis(), pdcSC.getAxisValue(newFatherId, false));
				...
				destination = getDestination("ToMoveValueChooseMother", componentSC, request);
			}
				...
				switch (status){
					case 1:	
						...
						destination = getDestination("ToMoveValueChooseMother", componentSC, request);					
						break;
					default : destination = "/pdcPeas/jsp/reloadAxis.jsp";
				}
			}
				...
				boolean isAdmin = pdcSC.isPDCAdmin() || pdcSC.isAxisManager() || pdcSC.isInheritedManager();
				if (!isAdmin)
					throw new AccessForbiddenException("PdcRequestRouter.DeleteValue", SilverpeasException.WARNING, null);
				...
				if (daughterName == null) {
					...
					//String rootId = new Integer(currentAxis.getAxisHeader().getRootId()).toString();
					if (currentValue.getLevelNumber() == 0)
						request.setAttribute("Root" , "1");
					else
						request.setAttribute("Root" , "0");
				...
				// set into the request
				request.setAttribute("Value", currentValue); 
				request.setAttribute("Sisters", getSisterValues(currentAxis,currentValue)); 
				...
				request.setAttribute("Sisters", getDaughterValues(pdcSC.getCurrentAxis(),pdcSC.getCurrentValue())); 
				...
				Value updatedValue = new Value(valueId, "unknown", valueName,valueDescription,null,null,null,-1,(new Integer(valueOrder)).intValue(),null);
				...
				switch (status){
					case 1:	
							...
							destination = "/pdcPeas/jsp/editValue.jsp";//destination = getDestination("EditValue", componentSC, request);
							break;
					default : destination = "/pdcPeas/jsp/reloadAxis.jsp";
				}
				...
				// get URL parameters
				String valueName	= request.getParameter("Name").trim(); // get the name of the axe
				String valueDescription	= request.getParameter("Description").trim(); // get the description of the axe
				String valueOrder	= extractOrder(request.getParameter("Order"));

				// create the axe
				Value value = new Value("UNKNOWN", "unknown", valueName,valueDescription,null,null,null,-1,(new Integer(valueOrder)).intValue(),null);
				int status = pdcSC.insertMotherValue(value);

				switch (status){
					case 1:	request.setAttribute("ValueToCreate", value); 

							//String rootId = new Integer(pdcSC.getCurrentAxis().getAxisHeader().getRootId()).toString();
							Value currentValue = pdcSC.getCurrentValue();
							if (currentValue.getLevelNumber() == 0)
								request.setAttribute("Root" , "1");
							else
				...
				switch (status){
					case 1:	request.setAttribute("ValueToCreate", value); 
							destination = getDestination("NewDaughterValue", componentSC, request);
						break;
					default : 
			...
			else if (function.startsWith("EraseManager"))
			{
				...
				String valueId	= (String) request.getParameter("valueId"); // get the id of the selected axe
				...
				if (res != null && !"".equals(res)) {
					...
					while (st.hasMoreTokens()) {
					  val		= st.nextToken();
					  Value newValue = new Value("UNKNOWN","unknown", val,val,null,null,null,-1,-1,null);
	...
	*/
	private List getSisterValues(Axis axis, Value value){
		...
		while (it.hasNext()){
			...
			if ( daughterValue.getMotherId().equals(motherId) && !(daughterValue.getPK().getId()).equals(valueId) ){
	...
	*/
	...
	private void setLanguageAsAttribute(PdcSessionController pdcSC, HttpServletRequest request)
	{
		//récupération de la langue et passage en paramètre à la jsp
