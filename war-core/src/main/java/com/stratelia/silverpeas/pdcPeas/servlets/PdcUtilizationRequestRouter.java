...
public class PdcUtilizationRequestRouter extends ComponentRequestRouter
{
    ...
    {
        return new PdcUtilizationSessionController(mainSessionCtrl, componentContext, "com.stratelia.silverpeas.pdcPeas.multilang.pdcBundle", "com.stratelia.silverpeas.pdcPeas.settings.pdcPeasIcons");
    ...
    public String getDestination(String function, ComponentSessionController componentSC, HttpServletRequest request)
    {
		...
		PdcUtilizationSessionController pdcSC = (PdcUtilizationSessionController) componentSC; // get the session controller to inform the request
		try{
			...
			if (function.startsWith("Main")) {
				...
				UsedAxis usedAxis = new UsedAxis(-1, "unknown", -1, new Integer(baseValue).intValue(), mandatory, variant);

				int status = pdcSC.addUsedAxis(usedAxis);

				switch (status) {
					case 1 : request.setAttribute("UsedAxis", usedAxis);
							...
							String axisId = request.getParameter("Id");
							 ...
							 destination = "/pdcPeas/jsp/utilizationAdd.jsp";
							 break;
					default : destination = getDestination("Main", componentSC, request);
				}
				...
				Axis axis = pdcSC.getAxisDetail(new Integer(usedAxis.getAxisId()).toString());
				...
				UsedAxis usedAxis = new UsedAxis(usedAxisId, "unknown", -1, new Integer(baseValue).intValue(), mandatory, variant);
				...
				switch (status) {
					case 1 : request.setAttribute("UsedAxis", usedAxis);
							 ...
							 destination = "/pdcPeas/jsp/utilizationEdit.jsp";
							 break;
					case 2 : request.setAttribute("UsedAxis", usedAxis);
							 request.setAttribute("AxisDetail", pdcSC.getCurrentAxis());
	...
	private void setBrowseContextInRequest(PdcUtilizationSessionController pdcSC, HttpServletRequest request) {
		request.setAttribute("browseContext", new String[]{pdcSC.getSpaceLabel(), pdcSC.getComponentLabel()});
	}
