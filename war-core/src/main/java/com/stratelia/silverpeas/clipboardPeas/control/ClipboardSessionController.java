...
/**
 * A servlet ClipboardSessionControler acts as a proxy
 * for a ClipboardBm EJB.
 */
public class ClipboardSessionController extends AbstractComponentSessionController
{
    ...
    /**
     * The ClipboardSessionController is built empty
     * and will be later initialized.
     */
    public ClipboardSessionController(MainSessionController mainSessionCtrl, ComponentContext context) 
    {
		super(mainSessionCtrl, context, "com.stratelia.webactiv.clipboard.multilang.clipboard", "com.stratelia.webactiv.clipboard.settings.clipboardIcons");
    ...
    public String getHF_JavaScriptTask(HttpServletRequest request)
    {
        ...
        if (message != null)
        {
            SilverTrace.info("clipboardPeas", "ClipboardSessionController.getDestination()", "root.MSG_GEN_PARAM_VALUE", " message = " + message);
            ...
            {	
                ...
                str.append("top.ClipboardWindow = window.open('../../Rclipboard/jsp/clipboardRefresh.jsp','Clipboard','width=350,height=300,alwaysRaised');");
                str.append("}");
								str.append("}");
            ...
            else if (message.equals("REFRESH"))
            {
                ...
                str.append("document.refreshform.action = '../.." + URLManager.getURL(null,Space,Component) + JSPPage + "';");
            ...
            {
                com.stratelia.silverpeas.notificationserver.channel.server.SilverMessage serverMessage = com.stratelia.silverpeas.notificationserver.channel.server.SilverMessageFactory.read(getUserId(), sessionId);

                if (serverMessage != null)
                {
                    SilverTrace.info("clipboardPeas", "ClipboardSessionController.getDestination()", "root.MSG_GEN_PARAM_VALUE", " serverMessage = " + serverMessage);
                    if (serverMessage.getWhat().equals("ALERT"))
                    {
                        str.append("alert ('");
                        str.append(EncodeHelper.javaStringToJsString(serverMessage.getContent()));
                        str.append("');");
                        str.append("self.location.href = '../../Rclipboard/jsp/Idle.jsp?message=DELMSG&messageTYPE=SERVER&messageID=" + serverMessage.getID() + "';");
                    }
					...
					{
	                    SilverTrace.info("clipboardPeas", "ClipboardSessionController.getDestination()", "root.MSG_GEN_PARAM_VALUE", " popupMessage = " + popupMessage.getWhat());
						if (popupMessage.getWhat().equals("ALERT"))
						{
		                    SilverTrace.info("clipboardPeas", "ClipboardSessionController.getDestination()", "root.MSG_GEN_PARAM_VALUE", " URLManager.getURL(URLManager.CMP_POPUP) = "+URLManager.getURL(URLManager.CMP_POPUP));
		                    SilverTrace.info("clipboardPeas", "ClipboardSessionController.getDestination()", "root.MSG_GEN_PARAM_VALUE", " MessageID = "+popupMessage.getID());
							str.append("msgPopup = SP_openWindow('../.."+URLManager.getURL(URLManager.CMP_POPUP)+"ReadMessage.jsp?MessageID=" + popupMessage.getID() + "','popupmsg"+new Long(new Date().getTime()).toString()+"',500,260,'scrollbars=yes');");
						}
						...
						{
		                    SilverTrace.info("clipboardPeas", "ClipboardSessionController.getDestination()", "root.MSG_GEN_PARAM_VALUE", " URLManager.getURL(URLManager.CMP_COMMUNICATIONUSER) = "+URLManager.getURL(URLManager.CMP_COMMUNICATIONUSER));
		                    SilverTrace.info("clipboardPeas", "ClipboardSessionController.getDestination()", "root.MSG_GEN_PARAM_VALUE", " userId = "+popupMessage.getSenderId());
							request.setAttribute("MessageID", popupMessage.getID());
		                    str.append("OpenDiscussion('../.."+URLManager.getURL(URLManager.CMP_COMMUNICATIONUSER)+"OpenDiscussion?userId=" + popupMessage.getSenderId() + "&MessageID=" + popupMessage.getID()+"','popupDiscussion"+popupMessage.getSenderId()+"',650,400,'menubar=no,scrollbars=no,statusbar=no');");
						}
						//CBO : FIN ADD
					}
				}
            ...
            else if (message.equals("DELMSG"))
            {
				...
				if (TYPE != null) {
					if (TYPE.equals("SERVER")) {
						com.stratelia.silverpeas.notificationserver.channel.server.SilverMessageFactory.del(ID);
					} else if (TYPE.equals("POPUP")) {
						com.stratelia.silverpeas.notificationserver.channel.popup.SilverMessageFactory.del(ID);
					}
				}
            }
        }
    ...
    public String getMessageError()
    {
        ...
        {
            SilverTrace.info("clipboardPeas", "ClipboardSessionController.getMessageError()", "clipboardPeas.EX_CANT_GET_MESSAGE", "", e);
        }
        return message;
    }
    ...
    /**
     * --------------------------------------------------------------------------------------------------------
     * Return the list of object (IndexEntry format) in clipbord.
     * 
     * @return
     * 
     * @see
    ...
    {
        SilverTrace.info("clipboardPeas", "ClipboardSessionController.getStrateliaReferenceObjects()", "root.MSG_GEN_ENTER_METHOD");
        ClipboardBm clipboard = getClipboard();
        ...
        while (qi.hasNext())
        {
            ...
            if ((clipObject != null) && (clipObject.isDataFlavorSupported(ClipboardSelection.IndexFlavor)))
            {
                // ce qu'il faut faire
                ...
                {
                    SilverTrace.error("clipboardPeas", "ClipboardSessionController.getIndexEntryObjects()", "root.EX_CLIPBOARD_PASTE_FAILED", "", e);
                }
                // result.add (clipObject);
            }
        }
        SilverTrace.info("clipboardPeas", "ClipboardSessionController.getStrateliaReferenceObjects()", "root.MSG_GEN_EXIT_METHOD");
        return result;
    }
     ...
     * @see
     */
    public Collection getObjects() throws java.rmi.RemoteException, javax.naming.NamingException, java.sql.SQLException
    {
        ClipboardBm clipboard = getClipboard();
        ArrayList   result = new ArrayList();
        Collection clipObjects = clipboard.getObjects();
    ...
    public ResourceLocator getSettings()
    {
        if (settings == null)
        {
            settings = new ResourceLocator("com.stratelia.webactiv.clipboard.settings.clipboardSettings", "");
        }
        return settings;
    }
    ...
    public String getComponentLabel(String componentId)
    {
        ...
        {
            SilverTrace.error("clipboardPeas", "ClipboardSessionController.getComponentLabel()", "clipboardPeas.EX_CANT_GET_COMPO_LABEL");
