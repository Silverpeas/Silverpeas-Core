...
public class ClipboardRequestRouter extends ComponentRequestRouter
{
    ...
    {
        SilverTrace.info("clipboardPeas", "ClipboardRequestRooter.getDestination()", "root.MSG_GEN_ENTER_METHOD", " componentName = " + componentSC.getClass().getName() + "; function = " + function);
        ...
        else if (function.startsWith("paste"))
        {
            ...
            {
                SilverTrace.info("clipboardPeas", "ClipboardRequestRooter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "compR is null" + componentName);
        ...
        else if (function.startsWith("delete"))
        {
            try
            {
                ...
                for (int i = 0; i < max; i++)
                {
                    String removedValue = request.getParameter("clipboardId" + String.valueOf(i));

                    if (removedValue != null)
                    {
                        SilverTrace.info("clipboardPeas", "ClipboardRequestRooter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "clipboardId" + String.valueOf(i) + " = " + removedValue);
            ...
            {
                SilverTrace.error("clipboardPeas", "ClipboardRequestRooter.getDestination()", "clipboardPeas.EX_CANT_WRITE", "delete.jsp");
            }
            destination = "/clipboard/jsp/clipboard.jsp";
        }
        else if (function.startsWith("selectObject"))
        {
            try
            {
                ...
                {
                    SilverTrace.info("clipboardPeas", "ClipboardRequestRooter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "selectObject " + objectIndex + " -> " + objectStatus);
                    userClipboard.setSelected(Integer.valueOf(objectIndex).intValue(), Boolean.valueOf(objectStatus).booleanValue());
                }
            }
            catch (Exception e)
            {
        ...
        else if (function.startsWith("selectionpaste"))
        {
            ...
            {
                SilverTrace.error("clipboardPeas", "ClipboardRequestRooter.getDestination()", "clipboardPeas.EX_CANT_WRITE", "selectionpaste.jsp");
            }
            String componentName = clipboardSC.getComponentRooterName();
        ...
        SilverTrace.info("clipboardPeas", "ClipboardRequestRooter.getDestination()", "root.MSG_GEN_EXIT_METHOD","Destination="+destination);
        return destination;
    }

    public void updateSessionManagement(HttpSession session, String destination)
    {
        SilverTrace.info("clipboardPeas", "ClipboardRequestRouter.updateSessionManagement", "root.MSG_GEN_PARAM_VALUE", "dest=" +destination);
