...
public abstract class AbstractPieChartBuilder  
{
	...
	private void buildStatsByInstance()
	{
		...
		Hashtable cmpStats = getCmpStats(); //Hashtable key=componentId, value=new String[3] {tout, groupe, user}
		...
		while (it.hasNext())
		{
			...
			{
				SilverTrace.error("silverStatisticsPeas", "AbstractPieChartBuilder.buildStatsByInstance()", "root.EX_SQL_QUERY_FAILED", e);
			}
			if (cmp != null)
			{
	...
	public PieChart2D getChart(String spaceId, String currentUserId, Vector currentStats)
	{
		...
		try 
		{
			...
			if ( (spaceId == null) || (spaceId.length()==0) )
			{
				if(userDetail.getAccessLevel().equals("A")) {//Admin
					...
					tabSpaceIds = (String[]) listSpaceIds.toArray(new String[listSpaceIds.size()]);
				}
			...
			for (int i=0; i<tabSpaceIds.length; i++)
			{
				...
				legend.add("["+ ( ( space.getName().length()>LEGEND_MAX_LENGTH ) ?  space.getName().substring(0, LEGEND_MAX_LENGTH) : space.getName() +"]") );
				currentStats.add( new String[]{"SPACE", tabSpaceIds[i], space.getName(), String.valueOf(count1), String.valueOf(count2), String.valueOf(count3)} );
			}
			...
			for (int i=0; i<componentIds.length; i++)
			{
				...
				if (item != null)
				{
					...
					legend.add( ( item.getName().length()>LEGEND_MAX_LENGTH ) ?  item.getName().substring(0, LEGEND_MAX_LENGTH) : item.getName() );
					currentStats.add( new String[]{"CMP", componentIds[i], item.getName(), String.valueOf(count1), String.valueOf(count2), String.valueOf(count3)} );
				}
			}
			
			pieChart = ChartUtil.buildPieChart(getChartTitle(), 
											   buildDoubleArrayFromStringCollection(counts),
											   (String[]) legend.toArray(new String[0])
											   );
		}
		catch (Exception e) 
		{
			SilverTrace.error("silverStatisticsPeas", "AbstractPieChartBuilder.getChart()", "root.EX_SQL_QUERY_FAILED", e);
		}
	...
	abstract Hashtable getCmpStats(); //Hashtable key=componentId, value=new String[3] {tout, groupe, user}
}
