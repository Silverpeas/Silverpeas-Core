...
/**
 * Class declaration
 * Get cumul datas from database to access and Volume
 *
 * @author
 */
...
// DLE
	...
	* @throws SQLException
	*/
	public static Hashtable getStatsUserVentil(String dateStat, String currentUserId, String filterIdGroup, String filterIdUser) throws SQLException
	{
		SilverTrace.info(
		...
		Hashtable resultat = new Hashtable(); //key=componentId, value=new String[3] {tout, groupe, user}
		
		//Query Tout
		String selectQuery =
			"SELECT componentId, SUM(countAccess)"
			+ " FROM SB_Stat_accessCumul"
			+ " WHERE dateStat='" + dateStat + "'"
		...
		while (it.hasNext())
		{
			...
			//filtre les composants autorisés selon les droits de l'utilisateur (Admin ou Gestionnaire d'espace)
	...
	*/
	public static Collection getStatsUserEvolution(String entite, String entiteId, String filterIdGroup, String filterIdUser) throws SQLException, ParseException
	{
		SilverTrace.info(
			"silverStatisticsPeas",
		...
		if("SPACE".equals(entite)) {
			...
			if (!filterIdGroup.equals("")) {
				...
				selectQuery =
					"SELECT A.dateStat, SUM(A.countAccess)"
					+ " FROM SB_Stat_accessCumul A, ST_Group_User_Rel C"
					+ " WHERE A.spaceId ='" + entiteId + "'"
					+ " AND A.userId = C.userId"
					+ " AND C.groupId ='" + filterIdGroup + "'"
					+ " GROUP BY A.dateStat"
					+ " ORDER BY A.dateStat ASC";
			}
	 ...
	 * @see
	 */
	private static Collection getStatsUserFromQuery(String selectQuery) throws SQLException, ParseException
	{
		SilverTrace.debug("silverStatisticsPeas", "SilverStatisticsPeasDAOAccessVolume.getStatsUserFromQuery", "selectQuery=" + selectQuery);
		Statement  stmt = null;
		ResultSet  rs = null;
	 ...
	 * @see
	 */
	private static Collection getStatsUserFromResultSet(ResultSet rs) throws SQLException, ParseException
	{
	 ...
	 * @see
	 */
	private static Hashtable getHashtableFromQuery(String selectQuery) throws SQLException
	{
		SilverTrace.debug(
			"silverStatisticsPeas",
	 ...
	 * @see
	 */
	private static Hashtable getHashtableFromResultset(ResultSet rs) throws SQLException
	{
	...
	* @throws SQLException
	*/
	public static Hashtable getStatsPublicationsVentil(String dateStat, String currentUserId, String filterIdGroup, String filterIdUser) throws SQLException
	{
		SilverTrace.info(
			"silverStatisticsPeas",
		...
		//Query Tout
		String selectQuery =
			"SELECT componentId, SUM(countVolume)"
			+ " FROM SB_Stat_VolumeCumul"
			+ " WHERE dateStat='" + dateStat + "'"
			+ " GROUP BY dateStat, componentId"
	...
	{
		SilverTrace.info("silverStatisticsPeas", "SilverStatisticsPeasDAOAccesVolume.getConnection()", "root.MSG_GEN_ENTER_METHOD");
		...
		{
			SilverTrace.error("silverStatisticsPeas", "SilverStatisticsPeasDAOAccesVolume.freeConnection()", "root.EX_CONNECTION_CLOSE_FAILED", "", e);
		}
		return con;
	}
	...
	{
		SilverTrace.info("silverStatisticsPeas", "SilverStatisticsPeasDAOAccesVolume.freeConnection()", "root.MSG_GEN_ENTER_METHOD");
