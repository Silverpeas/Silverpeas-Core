...
public class FileServer extends HttpServlet {
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res)
    ...
    if (typeUpload != null) {
      ...
      if (dirType != null) {
        if (dirType.equals(GeneralPropertiesManager.getGeneralResourceLocator()
            .getString("RepositoryTypeTemp"))) {
          filePath = FileRepositoryManager.getTemporaryPath("useless",
              componentId)
              + sourceFile;
        }
      } else {
        // the file to download is not in a temporary directory
        filePath = FileRepositoryManager.getAbsolutePath(componentId)
    ...
    SilverTrace.debug("peasUtil", "FileServer.doPost()",
        "root.MSG_GEN_PARAM_VALUE", " zip=" + zip);
    if (zip != null) {
      res.setContentType("application/x-zip-compressed");
      tempFile = File
          .createTempFile("zipfile", ".zip", new File(tempDirectory));
      SilverTrace.debug("peasUtil", "FileServer.doPost()",
    ...
    if (tempFile != null) {
      ...
      tempFile.delete();
      // the tool which transforms pdf file to html, creates 3 files with the -c
      // option
      // resultPage1234.html, resultPage1234-1.html and resultPage1234_ind.html
      // in the solution with pdf files, the files resultPage1234.html and
      // resultPage1234_ind.html are not deleted
      // we test the mime-type and we delete the files resultPage1234.html and
      // resultPage1234_ind.html too.
      /*
       * if (mimeType.equals("application/pdf")) { String canonicalPath =
       * tempFile.getCanonicalPath(); String name = canonicalPath.substring(0,
       * canonicalPath.indexOf("-1.html")); File indexFileName = new File(name + "_ind.html"); File
       * pureFileName = new File(name + ".html"); // delete indexFileName.delete();
       * pureFileName.delete(); // expurge name = null; canonicalPath = null; }
       */
    }
