...
public class FileUploader extends HttpServlet
{
    ...
    public void init(ServletConfig config)
    {
        ...
        {
            SilverTrace.fatal("peasUtil", "FileUploader.init", "peasUtil.CANNOT_ACCESS_SUPERCLASS");
        }
    }
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
    {
        ...
        try
        {
    		...
    		if (privateOrPublic != null && privateOrPublic.length()>0 && !"null".equals(privateOrPublic))
    		{
    			int versionType = (privateOrPublic.equals("publique") ? DocumentVersion.TYPE_PUBLIC_VERSION:DocumentVersion.TYPE_DEFAULT_VERSION);
        ...
        {
        	SilverTrace.debug("peasUtil", "FileUploader.doPost", "root.MSG_GEN_PARAM_VALUE", e);
        }
    }
    
    private void processAttachment(String fileId, FileItem file, String language) throws Exception
    {
    	SilverTrace.debug("peasUtil", "FileUploader.processAttachment()", "root.MSG_GEN_ENTER_METHOD", "fileId = "+fileId);
    	...
    	String destFile 	= FileRepositoryManager.getAbsolutePath(componentId) + AttachmentController.CONTEXT_ATTACHMENTS + attachmentDetail.getPhysicalName(language);
		
		//String uploadFileFullName = AttachmentController.CONTEXT_OFFICE_ATTACHMENTS+fileName;
    ...
    private void processVersion(String documentId, int versionType, String comment, FileItem file, String userId, String language) throws Exception
    {
    	VersioningUtil versioningUtil = new VersioningUtil();
    	
    	DocumentVersion documentVersion = versioningUtil.getLastVersion(new DocumentPK(new Integer(documentId).intValue()));
    	    	
		//DocumentVersion newDocumentVersion = versioningUtil.createNewDocumentVersion(documentVersion);
		...
		//SilverTrace.debug("peasUtil", "FileUploader.processVersion()", "root.MSG_GEN_PARAM_VALUE", "newDocumentVersion.getId() = "+newDocumentVersion.getPk().getId());
    	...
    	String newVersionFile = FileRepositoryManager.getAbsolutePath(componentId) + DocumentVersion.CONTEXT_VERSIONING + newPhysicalName;
