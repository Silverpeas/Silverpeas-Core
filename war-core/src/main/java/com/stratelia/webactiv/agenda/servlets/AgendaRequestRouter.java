...
public class AgendaRequestRouter extends ComponentRequestRouter
{
    ...
    {
        ComponentSessionController component = (ComponentSessionController) new AgendaSessionController(mainSessionCtrl, context);
        ...
        String destination = "";
       	...
       	try {
	        ...
	        else if (function.startsWith("ViewOtherAgenda"))
	        {
		        ...
		        SilverTrace.info("agenda", "AgendaRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE",selectedUser.getDisplayedName());
	        ...
	        {
		        SilverTrace.info("agenda", "AgendaRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE",scc.getUserDetail().getDisplayedName());
	...
	private File processFormUpload(AgendaSessionController agendaSc, HttpServletRequest request) throws FileUploadException
	{
		String logicalName = "";
		...
		try {
			...
			if (fileItem != null)
		    {
			      ...
			      if (logicalName != null)
			      {
			          ...
			          tempFolderPath = FileRepositoryManager.getAbsolutePath(agendaSc.getComponentId()) + GeneralPropertiesManager.getGeneralResourceLocator().getString("RepositoryTypeTemp") + File.separator + tempFolderName;
			          if (!new File(tempFolderPath).exists())
			              FileRepositoryManager.createAbsolutePath(agendaSc.getComponentId(),GeneralPropertiesManager.getGeneralResourceLocator().getString("RepositoryTypeTemp") + File.separator + tempFolderName);

			          //Creation of the file in the temp folder
			          fileUploaded = new File(FileRepositoryManager.getAbsolutePath(agendaSc.getComponentId())+GeneralPropertiesManager.getGeneralResourceLocator().getString("RepositoryTypeTemp")+ File.separator + tempFolderName + File.separator +logicalName);
			          fileItem.write(fileUploaded);
			
			          //Is a real file ?
			          if (fileSize > 0)
			              SilverTrace.debug("agenda","AgendaRequestRouter.processFormUpload()","root.MSG_GEN_PARAM_VALUE","fileUploaded = "+fileUploaded+" fileSize="+fileSize+" fileType="+fileType);
			      }
		    }
		}
		catch (Exception e)
