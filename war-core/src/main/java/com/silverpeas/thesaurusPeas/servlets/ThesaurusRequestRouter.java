...
public class ThesaurusRequestRouter extends ComponentRequestRouter
{
    ...
    {
        return new ThesaurusSessionController(mainSessionCtrl, componentContext, "com.silverpeas.thesaurusPeas.multilang.thesaurusPeasBundle", "com.silverpeas.thesaurusPeas.settings.thesaurusPeasIcons");
    }
    ...
    {
		SilverTrace.info("thesaurusPeas", "ThesaurusRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", function);
		ThesaurusSessionController scc = (ThesaurusSessionController) componentSC; // get the session controller to inform the request
		String destination = "";       
		...
		try{
			if (function.startsWith("Main")) {
				...
				if (scc.getCurrentAxis() != null) {
					request.setAttribute("idAxis", scc.getCurrentAxis().getAxisHeader().getPK().getId());
    ...
    private void processSynonyms(HttpServletRequest request, String termId, ThesaurusSessionController scc) throws ThesaurusException {
    ...
    private void loadSynonyms(HttpServletRequest request, ThesaurusSessionController scc) throws ThesaurusException{
       Iterator it =  scc.getAxisTerms().iterator();
       Iterator itSynonyms;
       String id;
       ArrayList namesWithStatus;
