...
public class ThesaurusSessionController extends AbstractComponentSessionController
{
	...
	private Collection jargonsSelected = new ArrayList(); // Collection de Jargon correspondant aux usersSelected et groupsSelected
	private Collection newJargonsSelected = new ArrayList(); // Collection de Jargon correspondant aux usersSelected et groupsSelected n'ayant pas de jargon
	private Vocabulary currentUpdVoca = null; // vocabulaire en cours de création ou de modification
	private PdcBm pdcBm;
	private ThesaurusBm thBm;

	// gestions des objets en session
	private void setCurrentVoca(Vocabulary voca) {currentVoca = voca;}
	public void setCurrentUpdVoca(String nom, String desc) {Vocabulary voca = new Vocabulary(); voca.setName(nom); voca.setDescription(desc); currentUpdVoca = voca;}
	private void setCurrentAxis(Axis axis) {currentAxis = axis;}
	private void setUsersSelected(Collection userIds) {usersSelected = userIds;}
	//private void setGroupsSelected(Collection groupIds) {groupsSelected = groupIds;}
	private void setPdcBm() {pdcBm = new PdcBmImpl();}
	private void setThBm() {thBm  = ThesaurusBm.getInstance();}

	public Vocabulary getCurrentVoca() {return currentVoca;}
	public Vocabulary getCurrentUpdVoca() {return currentUpdVoca;}
	public Axis getCurrentAxis() {return currentAxis;}
	private Collection getUsersSelected() {return usersSelected;}
	private Collection getGroupsSelected() {return groupsSelected;}
	public  Collection getUserSelectedJargons() {return jargonsSelected;}
	public  Collection getUserSelectedNewJargons() {return newJargonsSelected;}
	private PdcBm getPdcBm() {return pdcBm;}
	private ThesaurusBm getThBm() {return thBm;}
	...
	*/
	public Collection getListAxis() throws ThesaurusException
	{
        ...
        {
            throw new ThesaurusException("ThesaurusSessionController.getListAxis", SilverpeasException.ERROR, "thesaurusPeas.EX_CANT_GET_AXIS_LIST", e);
        }
		return axis;
	...
	*/
	public Axis getAxis(String idAxis) throws ThesaurusException
	{
        ...
        {
            throw new ThesaurusException("ThesaurusSessionController.getAxis", SilverpeasException.ERROR, "thesaurusPeas.EX_CANT_GET_AXIS", e);
        }
	}
	/**
	...
	public Value getTerm(String idTerm) throws ThesaurusException
	{
		try
		{
			...
			if ((getCurrentAxis() != null)&&(!idTerm.equals("-1"))) {
				term = getPdcBm().getAxisValue(idTerm, new Integer(getCurrentAxis().getAxisHeader().getRootId()).toString());
				if (term == null)
					setCurrentAxis(null);
			}
        ...
        {
            throw new ThesaurusException("ThesaurusSessionController.getAxis", SilverpeasException.ERROR, "thesaurusPeas.EX_CANT_GET_TERM", e);
        }
	}
	...
	*/
	public Collection getListVoca() throws ThesaurusException
	{
        ...
        {
            throw new ThesaurusException("ThesaurusSessionController.getListVoca", SilverpeasException.ERROR, "thesaurusPeas.EX_CANT_GET_VOCABULARY_LIST", e);
        }
		return vocas;
	}
	/**
	...
	*/
	...
	/**
	* Crée en base un vocabulaire à partir du nom et de la description du vocabulaire
	* Retourne l'id du vocabulaire créé ou -1 si le vocabulaire ne peut pas être créé car un vocabulaire de même nom existe
	* @param name
	* @param desc
	* @return long
	* @throws ThesaurusException
	...
	* Retourne -1 si le vocabulaire ne peut pas être modifié car un vocabulaire de même nom existe, 0 sinon
	* @param name
	* @param desc
	* @return
	* @throws ThesaurusException
	...
	*/
	...
	{
		if ((!getCurrentVoca().getName().toLowerCase().equals(name.toLowerCase()))&&(existVocabulary(name)))
			return -1;
	...
	*/
	private Synonym getNewSynonym(String name, String termId)
	{
		...
		{
			long idVoca = ((IdPK) getCurrentVoca().getPK()).getIdAsLong();
			long idTree = new Integer(getCurrentAxis().getAxisHeader().getRootId()).longValue();
	...
	* @see
	*/
	public void updateSynonyms(Collection names, String termId) throws ThesaurusException
	{
		...
		if (it.hasNext()) {
           ...
           getThBm().deleteSynonyms(currentVoca.getPK().getId(), currentAxis.getAxisHeader().getPK().getId(), termId);
        }
	}
	/**
	...
	*/
	public String initSelectedUserPanel() throws ThesaurusException
	{
        ...
        PairObject hostComponentName = new PairObject(getString("thesaurus.componentName"),  m_context + "/Rthesaurus/jsp/Main");
        ...
        hostPath[1] = new PairObject(getString("thesaurus.BBlistAffectations")+nomVoca,  "/Rthesaurus/jsp/EditAssignments");
	...
	* met en session la liste des users et la liste des groupes sélectionnés au travers du userPanel
	* @param
	* @return
	* @throws ThesaurusException
	* @see
	*/
	...
	{
        Selection sel = getSelection();
        String[] userIds = SelectionUsersGroups.getDistinctUserIds(sel.getSelectedElements(),sel.getSelectedSets());
		Collection users = new ArrayList();
	...
	*/
	...
	/**
	* Set la liste des jargons pour la liste des users et groupes sélectionnés au travers du userPanel ayant un jargon
	* @param
	* @return Collection
	* @throws ThesaurusException
	* @see
	*/
	...
	/**
	* Set la liste des jargons pour la liste des users et groupes sélectionnés au travers du userPanel n'ayant pas de jargons
	* @param
	* @return
	* @see
	*/
	...
	/**
	* Enregistre l'affectation au vocabulaire courant de tous les users et groupes sélectionnés au travers du userPanel s'il n'existe pas de conflits
	...
	*/
	...
	/**
	* Enregistre l'affectation au vocabulaire passé en paramètre de tous les users et groupes sélectionnés au travers du userPanel s'il n'existe pas de conflits
	* Retourne  le nombre de conflits
	* @param idVoca
	* @return int
	* @throws ThesaurusException
	...
	*/
	...
	/**
	* Enregistre en base les affectations au vocabulaire courant de tous les users et groupes sélectionnés au travers du userPanel et ...
	* ... n'ayant pas déjà une affectation autre que celle au vocabulaire courant
	* @param
	* @return
	* @throws ThesaurusException
	* @see getNewJargons
	...
	*/
	...
	/**
	* Enregistre en base les affectations au vocabulaire passé en paramètre de tous les users et groupes sélectionnés au travers du userPanel et ...
	...
	*/
	...
	/**
	* Retourne la liste des nouvelles affectations (à enregistrer) pour la liste des users et groupes sélectionnés au travers du userPanel
	* i.e. vérifie pour chaque user et groupe s'ils n'ont pas un jargon différent de idVoca dans la liste des jargons ...
	* ... des users et groupes sélectionnés (jargonsSelected)
	* @param idVoca
	* @return Collection
	* @see
	*/
	...
	/**
	* Retourne la liste des affectations (à enregistrer) portant sur le vocabulaire pour la liste des users et groupes sélectionnés au travers du userPanel
	* @param idVoca
	* @return Collection
	* @see
	*/
	...
	/**
	* Retourne la liste de toutes les affectations (à enregistrer) pour la liste des users et groupes sélectionnés au travers du userPanel
	* @param idVoca
	* @return Collection
	* @see
	*/
