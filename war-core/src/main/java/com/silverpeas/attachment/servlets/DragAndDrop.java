...
public class DragAndDrop extends HttpServlet
{
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
    {
        ...
        try
        {
    		...
    		SilverTrace.info("attachment", "DragAndDrop.doPost", "root.MSG_GEN_PARAM_VALUE", "componentId = "+componentId);
    		...
    		for (int i=0; i<items.size(); i++)
    		{
    			...
    			SilverTrace.info("attachment", "DragAndDrop.doPost", "root.MSG_GEN_PARAM_VALUE", "item #"+i+" = "+item.getFieldName());
    			SilverTrace.info("attachment", "DragAndDrop.doPost", "root.MSG_GEN_PARAM_VALUE", "item #"+i+" = "+item.getName());
    			...
    			if (!item.isFormField())
    			{
    				...
    				if (fullFileName != null && runOnUnix)
        			{
        				...
        				SilverTrace.info("attachment", "DragAndDrop.doPost", "root.MSG_GEN_PARAM_VALUE", "fullFileName on Unix = "+fullFileName);
        			}
    				...
    				SilverTrace.info("attachment", "DragAndDrop.doPost", "root.MSG_GEN_PARAM_VALUE", "item #"+i+" size = "+size);
        			
        			String type = fileName.substring(fileName.lastIndexOf(".")+1, fileName.length());
					String physicalName = new Long(new Date().getTime()).toString() + "." +type;
					
        			item.write(new File(AttachmentController.createPath(componentId, context)+physicalName));
        			String mimeType 		= AttachmentController.getMimeType(fileName);

        			//create AttachmentDetail Object
    				AttachmentDetail ad = new AttachmentDetail(atPK, physicalName, fileName, null, mimeType, size, context, new Date(), foreignKey);
    				ad.setAuthor(userId);
					...
					if (actifyPublisherEnable)
					{
	    				...
	    				SilverTrace.info("attachment", "DragAndDrop.doPost", "root.MSG_GEN_PARAM_VALUE", "nb tokenizer ="+tokenizer.countTokens());
						...
						if (fileForActify)
						{
							...
							String destFile			= FileRepositoryManager.getTemporaryPath() + actifyWorkingPath + File.separator + fileName;
