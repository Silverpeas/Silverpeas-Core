...
public class MyCheckOutFiles extends GenericPortlet implements FormNames {
	...
	public void doView(RenderRequest request, RenderResponse response) throws PortletException, IOException
	{
		...
		try {
			attachments = (new AttachmentBmImpl()).getAttachmentsByWorkerId(m_MainSessionCtrl.getUserId()).iterator();
			documents = getVersioningBm().getAllFilesReserved(Integer.parseInt(m_MainSessionCtrl.getUserId())).iterator();
		} catch (Exception e) {
    ...
    private void include(RenderRequest request, RenderResponse response, String pageName) throws PortletException 
    {
        ...
        try {
            PortletRequestDispatcher dispatcher = getPortletContext().getRequestDispatcher("/portlets/jsp/myCheckOutFiles/"+pageName);
