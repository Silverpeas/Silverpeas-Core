...
public class SPDesktopServlet extends HttpServlet {
  ...
  private String getSpaceHomepageURL(String spaceId, final HttpServletRequest request)
    ...
    if (spaceStruct != null) {
      ...
      if (StringUtil.isDefined(defaultSpaceHomepageURL)) {
        defaultSpaceHomepageURL = URLManager.getApplicationURL() + defaultSpaceHomepageURL;
        addSpaceIdToURL(defaultSpaceHomepageURL, spaceId);
      }
      ...
      if (spaceStruct.getFirstPageType() == SpaceInst.FP_TYPE_HTML_PAGE
        ...
        addSpaceIdToURL(destination, spaceId);
  ...
  private void addSpaceIdToURL(String url, String spaceId) {
    if (url.indexOf("?") == -1) {
      url += "?SpaceId=" + spaceId;
    } else {
      url += "&SpaceId=" + spaceId;
