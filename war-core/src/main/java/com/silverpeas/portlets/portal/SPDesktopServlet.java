...
public class SPDesktopServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
		...
		else
		{
	        ...
	        SilverTrace.debug("portlet", "SPDesktopServlet.doGetPost", "root.MSG_GEN_PARAM_VALUE", "DesktopMessages initialized !");
	        DriverUtil.init(request);
	        SilverTrace.debug("portlet", "SPDesktopServlet.doGetPost", "root.MSG_GEN_PARAM_VALUE", "DriverUtil initialized !");
	        response.setContentType("text/html;charset=ISO-8859-1");
	        
	        // Get the list of visible portlets(sorted by the row number)
	        try {
	            ServletContextThreadLocalizer.set(context);
	            PortletRegistryContext portletRegistryContext = DriverUtil.getPortletRegistryContext(spContext);
	            SilverTrace.debug("portlet", "SPDesktopServlet.doGetPost", "root.MSG_GEN_PARAM_VALUE", "portletRegistryContext retrieved !");
	        	...
	        	SilverTrace.error("portlet", "SPDesktopServlet.doGetPost", "root.MSG_GEN_PARAM_VALUE", "Portlets exception !", e);
	        } finally {
    ...
    private String getSpaceId(HttpServletRequest request)
    {
    	...
    	if (!StringUtil.isDefined(spaceId))
    	{
    		...
    		if (StringUtil.isDefined(spaceId))
    		{
    	    	...
    	    	SpaceInst spaceStruct = m_MainSessionCtrl.getOrganizationController().getSpaceInstById(spaceId);
    ...
    private String getSpaceHomepage(String spaceId, HttpServletRequest request) throws UnsupportedEncodingException
    {
    	...
    	if (spaceStruct != null)
    	{
    		...
    		// Maintenance Mode
			if (m_MainSessionCtrl.isSpaceInMaintenance(spaceId) && m_MainSessionCtrl.getUserAccessLevel().equals("U"))
				return GeneralPropertiesManager.getGeneralResourceLocator().getString("ApplicationURL")+"admin/jsp/spaceInMaintenance.jsp";
			
			//Page d'accueil de l'espace = Composant
			if (spaceStruct.getFirstPageType() == SpaceInst.FP_TYPE_COMPONENT_INST && StringUtil.isDefined(spaceStruct.getFirstPageExtraParam()))
			{
				String componentId = spaceStruct.getFirstPageExtraParam();
				if (organizationCtrl.isComponentAvailable(componentId, userId))
				{
					return GeneralPropertiesManager.getGeneralResourceLocator().getString("ApplicationURL")+URLManager.getURL("useless", componentId)+"Main";
				}
			}
			
			//Page d'accueil de l'espace = URL
			if (spaceStruct.getFirstPageType() == SpaceInst.FP_TYPE_HTML_PAGE && StringUtil.isDefined(spaceStruct.getFirstPageExtraParam()))
			{
				...
				destination = getParsedDestination(destination, s_sUserLogin, m_MainSessionCtrl.getCurrentUserDetail().getLogin());
				destination = getParsedDestination(destination, s_sUserFullName, URLEncoder.encode(m_MainSessionCtrl.getCurrentUserDetail().getDisplayedName(), "ISO-8859-1"));
				destination = getParsedDestination(destination, s_sUserId, URLEncoder.encode(m_MainSessionCtrl.getUserId(), "ISO-8859-1"));
				destination = getParsedDestination(destination, s_sSessionId, URLEncoder.encode(request.getSession().getId(), "ISO-8859-1"));
				destination = getParsedDestination(destination, s_sUserEmail, m_MainSessionCtrl.getCurrentUserDetail().geteMail());
				destination = getParsedDestination(destination, s_sUserFirstName, URLEncoder.encode(m_MainSessionCtrl.getCurrentUserDetail().getFirstName(), "ISO-8859-1"));
				destination = getParsedDestination(destination, s_sUserLastName, URLEncoder.encode(m_MainSessionCtrl.getCurrentUserDetail().getLastName(), "ISO-8859-1"));
		  
		  		// !!!! Add the password : this is an uggly patch that use a session variable set in the "AuthenticationServlet" servlet
		  		destination = this.getParsedDestination(destination, s_sUserPassword, (String)request.getSession().getAttribute("Silverpeas_pwdForHyperlink"));
