...
public class FileSharingCallBack extends CallBack {
	...
	{ 
		SilverTrace.info("fileSharing", "FileSharingCallback.doInvoke()", "root.MSG_GEN_ENTER_METHOD", "action = "+action+", iParam = "+iParam+", componentId = "+componentId+", extraParam = "+extraParam.toString());
		
		if (iParam == -1)
		{
			SilverTrace.info("fileSharing", "FileSharingCallback.doInvoke()", "root.MSG_GEN_PARAM_VALUE", "fileId is null. Callback stopped ! action = "+action+", componentId = "+componentId+", extraParam = "+extraParam.toString());
			return;
		}
		...
		if (componentId != null)
		{
			...
			{
				throw new FileSharingRuntimeException("FileSharingCallback.doInvoke()", SilverpeasRuntimeException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
			}
		}
	}
