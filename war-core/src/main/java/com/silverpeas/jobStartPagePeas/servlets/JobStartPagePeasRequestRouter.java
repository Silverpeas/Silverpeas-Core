...
public class JobStartPagePeasRequestRouter extends ComponentRequestRouter {
  ...
  public String getDestinationStartPage(String function, JobStartPagePeasSessionController jobStartPageSC,
    ...
    if (function.equals("UpdateJobStartPage")) //modif page d'accueil
    {
      ...
      if (jobStartPageSC.isJSR168Used()) {
        ...
        request.setAttribute("fullURL",
            GeneralPropertiesManager.getGeneralResourceLocator().getString("ApplicationURL") + "/dt?dt.SpaceId=" + jobStartPageSC.
            getManagedSpaceId() + "&dt.Role=Admin");
      } else {
        request.setAttribute("fullURL",
            GeneralPropertiesManager.getGeneralResourceLocator().getString("ApplicationURL") + "/Rportlet/jsp/admin?spaceId=" + jobStartPageSC.
            getManagedSpaceId());
      }
  ...
  public String getDestinationNavBar(String function, JobStartPagePeasSessionController jobStartPageSC,
  ...
  public String getDestinationComponent(String function, JobStartPagePeasSessionController jobStartPageSC,
    ...
    if (function.equals("GoToComponent")) {
      ...
      SilverTrace.info("jobStartPagePeas", "JobStartPagePeasRequestRouter.GoToComponent", "root.MSG_GEN_PARAM_VALUE",
          "compoId = " + compoId);
      ...
      SPParameter hiddenParam = new SPParameter("HiddenComponent", resource.getString("JSPP.hiddenComponent"), "", false,
      ...
      if (JobStartPagePeasSettings.isPublicParameterEnable) {
        SPParameter publicParam = new SPParameter("PublicComponent", resource.getString("JSPP.publicComponent"), "",
      ...
      SPParameter hiddenParam = new SPParameter("HiddenComponent", resource.getString("JSPP.hiddenComponent"), isHidden,
          false, "always", "checkbox", null, "useless", "no", null);
      visibleParameters.add(0, hiddenParam);
      ...
      if (JobStartPagePeasSettings.isPublicParameterEnable) {
        ...
        SPParameter publicParam = new SPParameter("PublicComponent", resource.getString("JSPP.publicComponent"),
            isPublic, false, "always", "checkbox", null, "useless", "no", null);
        visibleParameters.add(0, publicParam);
      }
      ...
      SilverTrace.info("jobStartPagePeas", "JobStartPagePeasRequestRouter.PlaceComponentAfter()",
      ...
      ComponentInst compoint1 = jobStartPageSC.getComponentInst(jobStartPageSC.getManagedInstanceId());
      SilverTrace.info("jobStartPagePeas", "JobStartPagePeasRequestRouter.EffectivePlaceComponent()",
          "root.MSG_GEN_PARAM_VALUE", "Managed spaceId= " + jobStartPageSC.getManagedSpaceId());
      SpaceInst destinationSpace = jobStartPageSC.getSpaceInstById(request.getParameter("DestinationSpace"));
      SilverTrace.info("jobStartPagePeas", "JobStartPagePeasRequestRouter.EffectivePlaceComponent()",
          "root.MSG_GEN_PARAM_VALUE", "espace source: " + currentSpace.getId() + " espace dest=" + destinationSpace.
          getId());
      //Moving only if destination space and current space are different
      if (!destinationSpace.getId().equals(currentSpace.getId())) {
        SilverTrace.info("jobStartPagePeas", "JobStartPagePeasRequestRouter.EffectivePlaceComponent()",
            "root.MSG_GEN_PARAM_VALUE", "espace source: " + jobStartPageSC.isSpaceInMaintenance(currentSpace.getId().
            substring(2)) + " espace dest=" + jobStartPageSC.isSpaceInMaintenance(destinationSpace.getId().substring(2)));
        ...
        if ((jobStartPageSC.isSpaceInMaintenance(destinationSpace.getId().substring(2)) && jobStartPageSC.
            isSpaceInMaintenance(currentSpace.getId().substring(2))) || jobStartPageSC.isAppInMaintenance()) {
          //Moving component to space
          SilverTrace.info("jobStartPagePeas", "JobStartPagePeasRequestRouter.EffectivePlaceComponent()",
              "root.MSG_GEN_PARAM_VALUE", "component = " + compoint1.getId() + " espace dest:" + destinationSpace.getId().
              substring(2) + " componentBefore=" + request.getParameter("ComponentBefore"));
          jobStartPageSC.setMoveComponentToSpace(compoint1, request.getParameter("DestinationSpace").substring(2),
          ...
          SilverTrace.info("jobStartPagePeas", "JobStartPagePeasRequestRouter.EffectivePlaceComponent()",
              "root.MSG_GEN_PARAM_VALUE", "Spaces not in maintenance Espace destination= " + spaceDest);
      ...
      SilverTrace.info("jobStartPagePeas", "JobStartPagePeasRequestRouter.ChangeDestinationSpace()",
          "root.MSG_GEN_PARAM_VALUE", "currentSpaceId=" + currentSpace.getId() + "selectedSpaceId=" + selectedSpace.
          getId());
      request.setAttribute("jobStartPageSC", jobStartPageSC);
      request.setAttribute("currentComponentName", compoint1.getLabel());
      request.setAttribute("currentSpace", currentSpace);
      ...
      try {
        ...
        throw new AdminException("JobStartPagePeasRequestRouter.getDestination()", SilverpeasException.ERROR,
            "jobStartPagePeas.CANT_COPY_COMPONENT", e);
      }
      ...
      try {
        ...
        throw new AdminException("JobStartPagePeasRequestRouter.getDestination()", SilverpeasException.ERROR,
            "jobStartPagePeas.CANT_PAST_COMPONENT", e);
      }
      refreshNavBar(jobStartPageSC, request);
      destination = "/jobStartPagePeas/jsp/startPageInfo.jsp";
    }
  ...
  /*********************** Gestion des espaces *****************************************/
  public String getDestinationSpace(String function, JobStartPagePeasSessionController jobStartPageSC,
      HttpServletRequest request) throws AdminException {
    String destination = null;
    ...
    if (function.equals("StartPageInfo")) {
      ...
      if (request.getParameter("NameObject") != null && request.getParameter("NameObject").length() > 0) {
        jobStartPageSC.setCreateSpaceParameters(request.getParameter("NameObject"), request.getParameter("Description"),
            request.getParameter("SousEspace"), spaceTemplate, I18NHelper.getSelectedLanguage(request), request.
            getParameter("SelectedLook"));
      }

      if (spaceTemplate != null && spaceTemplate.length() > 0 && jobStartPageSC.getCurrentSpaceTemplateProfiles() != null && jobStartPageSC.
          getCurrentSpaceTemplateProfiles().length > 0) {
        setSpacesNameInRequest(jobStartPageSC, request);
      ...
      destination = jobStartPageSC.initUserPanelForTemplateProfile((String) request.getAttribute("myComponentURL"),
          request.getParameter("profileIndex"));
      ...
      if (profile != null) {
        SilverTrace.info("jobStartPagePeas", "JobStartPagePeasRequestRouter.SpaceManager()", "root.MSG_GEN_PARAM_VALUE",
            "profileName=" + profile.getName() + " profileName=" + profile.getLabel());
        request.setAttribute("Profile", profile);
      }
      ...
      try {
        ...
        throw new AdminException("JobStartPagePeasRequestRouter.AddFileToLook", SilverpeasException.ERROR,
            "jobStartPagePeas.CANT_UPLOAD_FILE", e);
      }
  ...
  public String getDestination(String function, ComponentSessionController componentSC, HttpServletRequest request) {
    ...
    SilverTrace.info("jobStartPagePeas", "JobStartPagePeasRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE",
        "User=" + jobStartPageSC.getUserId() + " Function=" + function);
    ...
    try {
      ...
      if (destination.equals("/jobStartPagePeas/jsp/jobStartPageNav.jsp")) {
        ...
        SPParameter hiddenParam = new SPParameter("HiddenComponent", jobStartPageSC.getString("JSPP.hiddenComponent"),
            isHidden, false, "always", "checkbox", null, "useless", "no", null);
        visibleParameters.add(0, hiddenParam);
        ...
        if (JobStartPagePeasSettings.isPublicParameterEnable) {
          ...
          SPParameter publicParam = new SPParameter("PublicComponent", jobStartPageSC.getString("JSPP.publicComponent"),
              isPublic, false, "always", "checkbox", null, "useless", "no", null);
          visibleParameters.add(0, publicParam);
        }
  ...
  private void setSpacesNameInRequest(SpaceInst spaceint1, JobStartPagePeasSessionController jobStartPageSC,
      HttpServletRequest request) {
    if (spaceint1 != null) {
  ...
  private List getVisibleParameters(List parameters) {
    ...
    for (int p = 0; p < parameters.size(); p++) {
      ...
      if (parameter.getUpdatable().toLowerCase().equals("never") || parameter.getUpdatable().toLowerCase().equals(
          "hidden")) {
  ...
  private void request2ComponentInst(ComponentInst componentInst, HttpServletRequest request,
      JobStartPagePeasSessionController jobStartPageSC) {
    String name = request.getParameter("NameObject");
    String desc = request.getParameter("Description");
    if (desc == null) {
    ...
    SPParameter parameter = null;
    String value = null;
    SilverTrace.info("jobStartPagePeas", "JobStartPagePeasRequestRouter.EffectiveCreateInstance",
