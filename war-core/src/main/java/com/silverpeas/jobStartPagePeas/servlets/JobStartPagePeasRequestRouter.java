...
public class JobStartPagePeasRequestRouter extends ComponentRequestRouter
{
    ...
    {
        return new JobStartPagePeasSessionController(mainSessionCtrl, componentContext);
    }
    ...
    public String getDestinationStartPage(String function, JobStartPagePeasSessionController jobStartPageSC, HttpServletRequest request)
    {
        ...
        {
            destination= getDestination((String)request.getParameter("choix"),jobStartPageSC,request);
        }
        ...
        else if(function.equals("Portlet"))
        {
        	...
        	if (jobStartPageSC.isJSR168Used())
        	{
	            ...
	            request.setAttribute("fullURL", GeneralPropertiesManager.getGeneralResourceLocator().getString("ApplicationURL") + "/dt?dt.SpaceId="+jobStartPageSC.getManagedSpaceId()+"&dt.Role=Admin");
        	}
        	else
        	{
    ...
    public String getDestinationNavBar(String function, JobStartPagePeasSessionController jobStartPageSC, HttpServletRequest request)
    {
        String destination = null;
        ...
        {
            if ((request.getParameter("Espace") != null) && (request.getParameter("Espace").length() > 0))
            {
        ...
        {
			request.setAttribute("isUserAdmin",new Boolean(jobStartPageSC.isUserAdmin()));
			request.setAttribute("globalMode",new Boolean(jobStartPageSC.isAppInMaintenance()));
			request.setAttribute("IsBackupEnable", jobStartPageSC.isBackupEnable());
			request.setAttribute("IsBasketEnable", new Boolean(JobStartPagePeasSettings.isBasketEnable));
    ...
    public String getDestinationComponent(String function, JobStartPagePeasSessionController jobStartPageSC, HttpServletRequest request) throws AdminException
    {
        String destination = null;
        //NEWD DLE
       	...
       	if(function.equals("GoToComponent"))
        {
            ...
            if (idFather == null || (idFather != null && idFather.equals("0"))) {//l'instance appartient à un espace
            	jobStartPageSC.setSpaceId(spaceint1.getId());
            }
			SilverTrace.info("jobStartPagePeas", "JobStartPagePeasRequestRouter.GoToComponent", "root.MSG_GEN_PARAM_VALUE", "compoId = "+compoId);
        ...
        else if(function.equals("CreateInstance"))
        {
            ...
            WAComponent componentInstSelected = jobStartPageSC.getComponentByNum(Integer.parseInt(componentNum));
           
            setSpacesNameInRequest(jobStartPageSC, request);
            
            List visibleParameters = getVisibleParameters(componentInstSelected.getSortedParameters());
            
            SPParameter hiddenParam = new SPParameter("HiddenComponent", resource.getString("JSPP.hiddenComponent"), "", false, "always", "checkbox", null, "useless", "no", null);
            visibleParameters.add(0, hiddenParam);
            
            if (JobStartPagePeasSettings.isPublicParameterEnable)
			{
				SPParameter publicParam = new SPParameter("PublicComponent", resource.getString("JSPP.publicComponent"), "", false, "always", "checkbox", null, "useless", "no", null);
				visibleParameters.add(0, publicParam);
			}
            
			request.setAttribute("Parameters", visibleParameters);
			request.setAttribute("HiddenParameters", getHiddenParameters(componentInstSelected.getSortedParameters()));
        ...
        {
        	ComponentInst compoint1 = jobStartPageSC.getComponentInst(jobStartPageSC.getManagedInstanceId());
			...
			List visibleParameters = getVisibleParameters(parameters.getSortedParameters());
			
			String isHidden = "";
			if (compoint1.isHidden())
				isHidden = "yes";
			
			SPParameter hiddenParam = new SPParameter("HiddenComponent", resource.getString("JSPP.hiddenComponent"), isHidden, false, "always", "checkbox", null, "useless", "no", null);
			visibleParameters.add(0, hiddenParam);
			...
			if (JobStartPagePeasSettings.isPublicParameterEnable)
			{
				...
				SPParameter publicParam = new SPParameter("PublicComponent", resource.getString("JSPP.publicComponent"), isPublic, false, "always", "checkbox", null, "useless", "no", null);
				visibleParameters.add(0, publicParam);
			}
			
			request.setAttribute("Parameters", visibleParameters);
			request.setAttribute("HiddenParameters", getHiddenParameters(parameters.getSortedParameters()));
			
			request.setAttribute("JobPeas", sCompoName);
			request.setAttribute("ComponentInst", compoint1);
			
			request.setAttribute("IsInheritanceEnable", new Boolean(JobStartPagePeasSettings.isInheritanceEnable));
        ...
        {
        	ComponentInst componentInst = jobStartPageSC.getComponentInst(jobStartPageSC.getManagedInstanceId());
            ...
            {
            	refreshNavBar(jobStartPageSC, request);
                request.setAttribute("urlToReload", "GoToComponent?ComponentId="+componentInst.getId());
            	destination = "/jobStartPagePeas/jsp/closeWindow.jsp";
            }
            else
            {
        ...
        {
        	try {
            	jobStartPageSC.initUserPanelInstanceForGroupsUsers((String) request.getAttribute("myComponentURL"));
            }
            catch(Exception e){
        ...
        else if (function.equals("EffectiveCreateInstanceProfile"))
        {
        	...
        	request.setAttribute("urlToReload", "RoleInstance?IdProfile="+profileId); //le NameProfile et le LabelProfile n'est pas indispensable
        	destination = "/jobStartPagePeas/jsp/closeWindow.jsp";
        }
        ...
        else if(function.equals("PlaceComponentAfter"))
        {
			...
			SilverTrace.info("jobStartPagePeas", "JobStartPagePeasRequestRouter.PlaceComponentAfter()", "root.MSG_GEN_PARAM_VALUE", "validLicense= " + validLicense);
			request.setAttribute("validLicense",validLicense);
			// NEWF DLE
            destination = "/jobStartPagePeas/jsp/placeComponentAfter.jsp";
        }
		//NEWD DLE
/*		else if(function.equals("EffectivePlaceComponentAfter"))
		{
			jobStartPageSC.setComponentPlace(request.getParameter("ComponentBefore"));
			refreshNavBar(jobStartPageSC, request);
		...
		else if(function.equals("EffectivePlaceComponent"))
		{
			...
			ComponentInst compoint1 = jobStartPageSC.getComponentInst(jobStartPageSC.getManagedInstanceId());
			SilverTrace.info("jobStartPagePeas", "JobStartPagePeasRequestRouter.EffectivePlaceComponent()", "root.MSG_GEN_PARAM_VALUE", "Managed spaceId= " + jobStartPageSC.getManagedSpaceId());
			SpaceInst destinationSpace = jobStartPageSC.getSpaceInstById(request.getParameter("DestinationSpace"));
			SilverTrace.info("jobStartPagePeas","JobStartPagePeasRequestRouter.EffectivePlaceComponent()","root.MSG_GEN_PARAM_VALUE","espace source: "+currentSpace.getId()+" espace dest="+destinationSpace.getId());
			//Moving only if destination space and current space are different
			if (!destinationSpace.getId().equals(currentSpace.getId()))
			{
				SilverTrace.info("jobStartPagePeas","JobStartPagePeasRequestRouter.EffectivePlaceComponent()","root.MSG_GEN_PARAM_VALUE","espace source: "+jobStartPageSC.isSpaceInMaintenance(currentSpace.getId().substring(2))+" espace dest="+jobStartPageSC.isSpaceInMaintenance(destinationSpace.getId().substring(2)));
				//Destination and source space in maintenance = OK
				if ((jobStartPageSC.isSpaceInMaintenance(destinationSpace.getId().substring(2)) && jobStartPageSC.isSpaceInMaintenance(currentSpace.getId().substring(2))) || jobStartPageSC.isAppInMaintenance())
				{
					//Moving component to space
					SilverTrace.info("jobStartPagePeas","JobStartPagePeasRequestRouter.EffectivePlaceComponent()","root.MSG_GEN_PARAM_VALUE","component = "+compoint1.getId()+" espace dest:"+destinationSpace.getId().substring(2)+" componentBefore="+request.getParameter("ComponentBefore"));
					jobStartPageSC.setMoveComponentToSpace(compoint1, request.getParameter("DestinationSpace").substring(2), request.getParameter("ComponentBefore"));
					SilverTrace.info("jobStartPagePeas","JobStartPagePeasRequestRouter.EffectivePlaceComponent()","root.MSG_GEN_PARAM_VALUE","Retour OK");
					refreshNavBar(jobStartPageSC, request);
					request.setAttribute("urlToReload", "StartPageInfo");
					destination = "/jobStartPagePeas/jsp/closeWindow.jsp";
				}
				else
				{
					...
					SilverTrace.info("jobStartPagePeas", "JobStartPagePeasRequestRouter.EffectivePlaceComponent()", "root.MSG_GEN_PARAM_VALUE", "Spaces not in maintenance Espace destination= " + spaceDest);
					...
					request.setAttribute("jobStartPageSC",jobStartPageSC);
					Boolean validLicense = new Boolean(getLicense(licenseCoupercoller));
					SilverTrace.info("jobStartPagePeas", "JobStartPagePeasRequestRouter.EffectivePlaceComponent()", "root.MSG_GEN_PARAM_VALUE", "validLicense= " + validLicense);
					request.setAttribute("validLicense",validLicense);
		...
		else if(function.equals("ChangeDestinationSpace"))
		{
			...
			SilverTrace.info("jobStartPagePeas", "JobStartPagePeasRequestRouter.ChangeDestinationSpace()", "root.MSG_GEN_PARAM_VALUE", "currentSpaceId=" + currentSpace.getId()+"selectedSpaceId=" + selectedSpace.getId());
			request.setAttribute("jobStartPageSC",jobStartPageSC);
			request.setAttribute("currentComponentName",compoint1.getLabel());
			request.setAttribute("currentSpace",currentSpace);
			request.setAttribute("selectedSpace",selectedSpace);
	...
	/*********************** Gestion des espaces *****************************************/
    public String getDestinationSpace(String function, JobStartPagePeasSessionController jobStartPageSC, HttpServletRequest request) throws AdminException
    {
        String destination = null;
        ...
        else if(function.equals("SetSpaceTemplateProfile"))
        {
            ...
            if (request.getParameter("NameObject") != null && request.getParameter("NameObject").length() > 0)
            {
                jobStartPageSC.setCreateSpaceParameters(request.getParameter("NameObject"), request.getParameter("Description"), request.getParameter("SousEspace"), spaceTemplate, I18NHelper.getSelectedLanguage(request), request.getParameter("SelectedLook"));
            }

            if (spaceTemplate != null && spaceTemplate.length() > 0 && jobStartPageSC.getCurrentSpaceTemplateProfiles() != null && jobStartPageSC.getCurrentSpaceTemplateProfiles().length > 0)
            {                
                setSpacesNameInRequest(jobStartPageSC, request);
                
                request.setAttribute("SpaceBefore",request.getParameter("SpaceBefore"));
                request.setAttribute("SpaceTemplateProfiles", jobStartPageSC.getCurrentSpaceTemplateProfiles());
                request.setAttribute("SpaceTemplateProfilesGroups", jobStartPageSC.getCurrentSpaceTemplateProfilesGroups());
                request.setAttribute("SpaceTemplateProfilesUsers", jobStartPageSC.getCurrentSpaceTemplateProfilesUsers());
        ...
        {
            destination = jobStartPageSC.initUserPanelForTemplateProfile((String) request.getAttribute("myComponentURL"), request.getParameter("profileIndex"));
        }
        else if(function.equals("ReturnUserPanelForTemplateProfile"))
        {
            jobStartPageSC.returnUserPanelForTemplateProfile(request.getParameter("profileIndex"));
            destination = getDestinationSpace("SetSpaceTemplateProfile", jobStartPageSC, request);
        }
        ...
        else if(function.equals("DeleteSpace"))
        {
			...
			refreshNavBar(jobStartPageSC, request);
            if ((jobStartPageSC.getManagedSpaceId() != null) && (jobStartPageSC.getManagedSpaceId().length() > 0))
            {
                destination = "/jobStartPagePeas/jsp/startPageInfo.jsp";
            }
            else
        ...
        else if(function.equals("SpaceManager"))
        {
			...
			if (profile != null)
			{
				SilverTrace.info("jobStartPagePeas", "JobStartPagePeasRequestRouter.SpaceManager()", "root.MSG_GEN_PARAM_VALUE", "profileName=" + profile.getName()+" profileName=" + profile.getLabel());
	            request.setAttribute("Profile", profile);
			}
            ...
            if (inheritedProfile != null)
            {
	            ...
	            request.setAttribute("listInheritedGroups", jobStartPageSC.groupIds2groups(inheritedProfile.getAllGroups()));
	            request.setAttribute("listInheritedUsers", jobStartPageSC.userIds2users(inheritedProfile.getAllUsers()));
            }
        ...
        else if (function.equals("SelectUsersGroupsSpace"))
        {
        	String role = request.getParameter("Role");
        	try {
            	jobStartPageSC.initUserPanelSpaceForGroupsUsers((String) request.getAttribute("myComponentURL"), role);
            }
            catch(Exception e){
				SilverTrace.warn("jobStartPagePeas","JobStartPagePeasRequestRouter.getDestination()","root.EX_USERPANEL_FAILED","function = "+function, e);
			}
        ...
        else if (function.equals("SpaceManagerDescription"))
        {
			...
			String name = "";
			String desc = "";
			if (i < m_ListSpaceProfileInst.size()) {//seulement le premier profil (manager)
        ...
        else if (function.equals("SpaceLook"))
        {
			...
			SpaceLookHelper slh = new SpaceLookHelper("Space"+jobStartPageSC.getManagedSpaceId());
			slh.setFiles(files);
        ...
        else if (function.equals("UpdateSpaceLook"))
        {
        	try {
				...
				if (file != null && StringUtil.isDefined(file.getName()))
				{
					...
					FileRepositoryManager.createAbsolutePath("Space"+jobStartPageSC.getManagedSpaceId(), "look");
					
					String[] dir = new String[1];
					dir[0] = "look";
					
					String path = FileRepositoryManager.getAbsolutePath("Space"+jobStartPageSC.getManagedSpaceId(), dir);
				...
				throw new AdminException("JobStartPagePeasRequestRouter.AddFileToLook", SilverpeasException.ERROR, "jobStartPagePeas.CANT_UPLOAD_FILE", e);
			}
    ...
    {
    	return FileRepositoryManager.getAbsolutePath("Space"+sc.getManagedSpaceId(), new String[]{"look"});
    }
    ...
    public String getDestination(String function, ComponentSessionController componentSC, HttpServletRequest request)
    {
        ...
        SilverTrace.info("jobStartPagePeas", "JobStartPagePeasRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "User=" + jobStartPageSC.getUserId() + " Function=" + function);
        ...
        try
        {
            ...
            else if (destination.equals("/jobStartPagePeas/jsp/componentInfo.jsp"))
            {
				...
				SPParameter hiddenParam = new SPParameter("HiddenComponent", jobStartPageSC.getString("JSPP.hiddenComponent"), isHidden, false, "always", "checkbox", null, "useless", "no", null);
				visibleParameters.add(0, hiddenParam);
				...
				if (JobStartPagePeasSettings.isPublicParameterEnable)
				{
					...
					SPParameter publicParam = new SPParameter("PublicComponent", jobStartPageSC.getString("JSPP.publicComponent"), isPublic, false, "always", "checkbox", null, "useless", "no", null);
					visibleParameters.add(0, publicParam);
				}
        ...
        SilverTrace.info("jobStartPagePeas", "JobStartPagePeasRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "Destination=" + destination);
        return destination;
    }

    protected void refreshNavBar(JobStartPagePeasSessionController jobStartPageSC, HttpServletRequest request)
    {
        jobStartPageSC.refreshCurrentSpaceCache();
        request.setAttribute("haveToRefreshNavBar",new Boolean(true));
    }
    
    private void setSpacesNameInRequest(JobStartPagePeasSessionController jobStartPageSC, HttpServletRequest request)
    {
    	setSpacesNameInRequest(jobStartPageSC.getSpaceInstById(), jobStartPageSC, request);
    }
    
    private void setSpacesNameInRequest(SpaceInst spaceint1, JobStartPagePeasSessionController jobStartPageSC, HttpServletRequest request)
    {
    	if (spaceint1 != null)
        {
	        if (!spaceint1.isRoot()) {//je suis sur un ss-espace
	        	...
	        	request.setAttribute("currentSpaceName",spaceFather.getName(jobStartPageSC.getLanguage()));
	        	request.setAttribute("nameSubSpace", spaceint1.getName(jobStartPageSC.getLanguage()));
	        }
	        else {
	        	request.setAttribute("currentSpaceName",spaceint1.getName(jobStartPageSC.getLanguage()));
    ...
    private List getVisibleParameters(List parameters) 
    {
    	...
    	for (int p=0; p<parameters.size(); p++) {
    		...
    		if (parameter.getUpdatable().toLowerCase().equals("never") || parameter.getUpdatable().toLowerCase().equals("hidden")){
    ...
    {
    	return param != null && param.length() > 0 && !"null".equalsIgnoreCase(param); 
    }
    
    private void request2SpaceInst(SpaceInst spaceInst, HttpServletRequest request, JobStartPagePeasSessionController jobStartPageSC)
    {
    	String name 		= request.getParameter("NameObject");
        String desc 		= request.getParameter("Description");
    ...
    private void request2ComponentInst(ComponentInst componentInst, HttpServletRequest request, JobStartPagePeasSessionController jobStartPageSC)
    {
    	String name = request.getParameter("NameObject");
        String desc = request.getParameter("Description");
        if (desc == null)
        ...
        if (StringUtil.isDefined(pInheritance))
        	componentInst.setInheritanceBlocked(new Boolean(pInheritance).booleanValue());
		...
		SPParameters	xmlParameters	= (SPParameters) componentInstSelected.getSPParameters().clone();
		List			parameters	= xmlParameters.getParameters();
		SPParameter		parameter	= null;
		String			value		= null;
		SilverTrace.info("jobStartPagePeas", "JobStartPagePeasRequestRouter.EffectiveCreateInstance", "root.MSG_GEN_PARAM_VALUE", "nb parameters = "+parameters.size());
