...
(function() {
   ...
   * 
   */
angular.module('silverpeas.directives').directive('silverpeasRating', ['Rating', 'context', '$timeout', function(Rating, context, $timeout) {
  return {
    ...
    link : function postLink(scope, element, attrs) {$timeout(function() {
      if (attrs.rating) {
      ...
      _rating = function(rating) {
        ...
        if (!readonly || rating.notesCount > 0) {
          ...
          if (readonly) {
            angular.element(".spOverRating", element).attr('title', displayTitleCounter(rating, param_userAnonymous));
          }

          // setting titles over stars and reset buttons
          $rating.find(".rateit-reset").attr('title', label_RatingVoteDelete + ' : ' + userRating);
          ...
          $rating.bind('rated reset', function(e) {
            ...
            rating.rate(value).then(function(newRating) {
              notySuccess(label_RatingVoteOK);
              ...
              ri.find(".rateit-reset").attr('title', label_RatingVoteDelete + ' : ' + newRating.userRating);
      ...
      function displayTitleCounter(rating, anonymous) {
        ...
        if (!anonymous) {
          if (rating.userRating > 0) {
            label += label_RatingVoteYours + rating.userRating;
          } else {
