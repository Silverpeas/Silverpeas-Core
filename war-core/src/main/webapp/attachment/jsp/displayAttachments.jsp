...
<%
      ...
      if (itAttachments.hasNext() || (StringUtil.isDefined(profile) && !profile.equals("user"))) {
        ...
        while (itAttachments.hasNext()) {
          ...
          if (ViewFactory.getPreviewService().isItPossibleGettingPreview(new File(attachmentDetail.getAttachmentPath(contentLanguage)))) {
            %>
            <img onclick='javascript:preview(this, <%=attachmentDetail.getPK().getId()%>);' class="preview" src='<c:url value="/util/icons/preview.png" />' />
            <%
          }
          out.println("</span>");
