...
public final class CalendarResourceURIs {
   ...
   */
  static URI buildCalendarURI(Calendar calendar) {
    if (calendar == null || !calendar.isPersisted()) {
      return null;
    }
    return UriBuilder.fromUri(URLUtil.getApplicationURL())
        .path(RESTWebService.REST_WEB_SERVICES_URI_BASE).path(CALENDAR_BASE_URI)
   ...
   */
  static URI buildCalendarEventURI(CalendarEvent event) {
    if (event == null) {
      return null;
    }
    return UriBuilder.fromUri(URLUtil.getApplicationURL())
        .path(RESTWebService.REST_WEB_SERVICES_URI_BASE).path(CALENDAR_BASE_URI)
        .path(event.getComponentInstanceId())
