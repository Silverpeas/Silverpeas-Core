...
public class MassiveWebSecurityFilterOnReportedXssIT {
   ...
   * <p>
   * Be aware the callbacks are named one by one by the detection, so any of them missing from that
   * list goes through. Naming them spares in return the false rejections a mere {@code on} prefix
   * followed by any word characters would cause, on a text such as {@code "a value only=42"}.
   * </p>
   */
  @Test
