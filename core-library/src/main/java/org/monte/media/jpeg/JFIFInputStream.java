...
/**
 * JFIFInputStream. <p> This InputStream uses two special marker values which do not exist in the
 * JFIF stream: <ul> <li><b>-1</b>: marks junk data at the beginning of the file.</li> <li><b>0</b>:
 * marks entropy encoded image data.</li> </ul> <p> The junk data at the beginning of the file can
 * be accessed by calling the read-methods immediately after opening the stream. Call nextSegment()
 * immediately after opening the stream if you are not interested into this junk data. <p> Junk data
 * at the end of the file is delivered as part of the EOI_MARKER segment. Finish reading after
 * encountering the EOI_MARKER segment if you are not interested in this junk data.
 *
...
public class JFIFInputStream extends FilterInputStream {
  ...
  /**
   * This hash set holds the Id's of markers which stand alone, respectively do no have a data
   * segment.
   */
  private final HashSet<Integer> standaloneMarkers = new HashSet<Integer>();
  /**
   * This hash set holds the Id's of markers which have a data segment followed by a entropy-coded
   * data segment.
   */
  ...
  public static class Segment {
    ...
    /**
     * If the marker starts a marker segment, holds the length of the data in the data segment. If
     * the marker starts a entropy-coded data segment, holds the value -1.
     */
    ...
    public String toString() {
      return "Segment marker=0x" + Integer.toHexString(marker) + " offset=" + offset + "=0x" + Long
        .toHexString(offset);
    }
  }
  private Segment segment;
  /**
   * This variable is set to true, if a 0xff byte has been found in entropy-code data.
   */
  ...
  public Segment getNextSegment() throws IOException {
    ...
    if (standaloneMarkers.contains(marker)) {
      ...
      if (length < 2) {
        throw new IOException("JFIFInputStream found illegal segment length " + length + " after marker " + Integer
          .toHexString(marker) + " at offset " + offset + ".");
      }
  ...
  /**
   * Reads up to
   * <code>len</code> b of data from this input stream into an array of b. This method blocks until
   * some input is available. <p> This method simply performs
   ...
   * <code>n</code> b of data from the input stream. The
   * <code>skip</code> method may, for a variety of reasons, end up skipping over some smaller
   * number of b, possibly
   * <code>0</code>. The actual number of b skipped is returned. <p> This method simply performs
   * <code>in.skip(n)</code>.
   *
   ...
   * subsequent reads re-read the same b. <p> The
   * <code>readlimit</code> argument tells this input stream to allow that many b to be read before
   * the mark position gets invalidated. <p> This method simply performs
   * <code>in.mark(readlimit)</code>.
   *
   * @param readlimit the maximum limit of b that can be read before the mark position becomes
   * invalid.
   * @see java.io.FilterInputStream#in
   ...
   * If this happens within readlimit b, it allows the outer code to reset the stream and try
   * another parser.
   *
