...
public class PublicationDAO extends AbstractDAO {
  ...
  public static int getNbPubByFatherPath(Connection con, NodePK fatherPK,
    ...
    if (!fatherPath.isEmpty()) {
      ...
      try (PreparedStatement prepStmt = con.prepareStatement(selectStatement)) {
        ...
        setDateVisibility(prepStmt, dateNow, hourNow);
  ...
  private static void setDateVisibility(PreparedStatement prepStmt, String dateNow,
      String hourNow) throws SQLException {
  ...
  private static List<String> getRemovedInstanceIdsLinkedToInstance(final Connection con,
    ...
    final JdbcSqlQuery query = JdbcSqlQuery
        .select("distinct sb_publication_publi.instanceId AS instanceId")
        .from("sb_node_node")
  ...
  private static Map<String, Integer> loadDistributionTree(final Connection con,
    ...
    final JdbcSqlQuery query = JdbcSqlQuery
        .select(
            "sb_node_node.nodeId, sb_node_node.nodefatherid, COUNT(sb_publication_publi.pubName) " +
                "AS nbPubli")
        .from("sb_node_node")
  ...
  private static PublicationDetail resultSet2PublicationDetail(ResultSet rs, boolean getSort)
    ...
    try {
      ...
      throw new SQLException(PUBLICATION_DAO_RESULT_SET_2_PUBLICATION_DETAIL_INTERNAL_ERROR +
          "a date format unknown for publication.pk = " + pk + " : " + e);
    }
  ...
  public static List<PublicationDetail> selectByFatherIds(Connection con,
    ...
    selectStatement.append(
        "select distinct P.pubId, P.infoId, P.pubName, P.pubDescription, P.pubCreationDate, P" +
            ".pubBeginDate, ");
    selectStatement.append(
        "P.pubEndDate, P.pubCreatorId, P.pubImportance, P.pubVersion, P.pubKeywords, P" +
            ".pubContent, ");
    selectStatement.append(
    ...
    selectStatement.append("where ").append(whereClause);
    ...
    if (status != null && !status.isEmpty()) {
      ...
      selectStatement.append(" and ").append(statusBuffer);
    }
  ...
  public static List<PublicationDetail> getByIds(final Connection con,
    ...
    try {
      ...
      JdbcSqlQuery.executeBySplittingOn(publicationIds, (idBatch, ignore) -> JdbcSqlQuery
          .select(QueryStringFactory.getLoadRowFields())
  ...
  private static JdbcSqlQuery prepareSelectPksByCriteria(final PublicationCriteria criteria) {
    ...
    if (criteria.mustJoinOnNodeFatherTable()) {
      ...
      query = JdbcSqlQuery.select("P.pubId");
    }
    ...
    criteria.getOrderByList().stream()
        .filter(o -> o != QUERY_ORDER_BY.BEGIN_VISIBILITY_DATE_ASC &&
            o != QUERY_ORDER_BY.BEGIN_VISIBILITY_DATE_DESC)
        .filter(
            o -> o != QUERY_ORDER_BY.CREATION_DATE_ASC && o != QUERY_ORDER_BY.CREATION_DATE_DESC)
  ...
  private static void configureSelectByCriteria(final JdbcSqlQuery query,
    ...
    criteria.getOrderByList().stream()
        ...
        .ifPresent(o -> query.addSqlPart(
            ", CASE WHEN P.pubUpdateDate > P.pubBeginDate THEN P.pubUpdateDate ELSE P" +
                ".pubBeginDate END AS " +
                o.getPropertyName())
        );
  }
  ...
  private static void configureFromByCriteria(final JdbcSqlQuery query,
    ...
    if (!criteria.getComponentInstanceIds().isEmpty()) {
      ...
      query.join("ST_ComponentInstance I")
          .on(instanceSelector +
              ".instanceid = CONCAT(I.componentname , CAST(I.id AS VARCHAR(20)))");
    }
  }
  ...
  private static void visibleFilter(final JdbcSqlQuery sqlQuery, final String conjunction,
    ...
    sqlQuery
        .addSqlPart(conjunction + f("((? > %spubBeginDate", p), dateNow)
        .and(f("? < %spubEndDate)", p), dateNow)
        .or(f("(? = %spubBeginDate", p), dateNow).and(f("? < %spubEndDate", p), dateNow)
        .and(f("? > %spubBeginHour)", p), hourNow)
        .or(f("(? > %spubBeginDate", p), dateNow).and(f("? = %spubEndDate", p), dateNow)
        .and(f("? < %spubEndHour)", p), hourNow)
        .or(f("(? = %spubBeginDate", p), dateNow).and(f("? = %spubEndDate", p), dateNow)
        .and(f("? > %spubBeginHour", p), hourNow).and(f("? < %spubEndHour))", p), hourNow);
  }
  ...
  private static void configureOrderingByCriteria(final JdbcSqlQuery query,
    ...
    if (!orderBies.isEmpty()) {
      query.orderBy(orderBies.stream()
          .map(o -> (o.isComplex() ? "" : "P.") + o.getPropertyName() + " " +
              (o.isAsc() ? "asc" : "desc"))
          .collect(joining(",")));
    }
  }
  ...
  public static void changeInstanceId(Connection con, PublicationPK pubPK,
    ...
    final String updateQuery = "update " + PublicationDAO.PUBLICATION_TABLE_NAME +
        " set instanceId = ? " +
        " where pubId = ? ";
  ...
  public static void storeRow(Connection con, PublicationDetail detail)
    ...
    try (PreparedStatement prepStmt = con.prepareStatement(UPDATE_PUBLICATION)) {
      ...
      setDateParameter(prepStmt, 13, detail.getLastUpdateDate(),
          formatDate(detail.getCreationDate()));
  ...
  public static Collection<PublicationDetail> selectBetweenDate(Connection con, String beginDate,
    ...
    final String selectStatement = SELECT_FROM + PUBLICATION_TABLE_NAME +
        " where instanceId = ? " +
        AND +
        "( ? <= pubCreationDate AND ? >= pubCreationDate ) " +
        " ) " +
        " order by pubCreationDate DESC, pubId DESC";
  ...
  public static SilverpeasList<SocialInformationPublication> getAllPublicationsIDbyUserid(
    ...
    final PaginationCriterion pagination =
        new PaginationCriterion(1, 500).setOriginalSizeRequired(false);
  ...
  public static Collection<PublicationDetail> getDraftsByUser(Connection con, String userId)
    ...
    final String sb = SELECT_FROM_SB_PUBLICATION_PUBLI +
        ...
        "and ((pubStatus = ? " +
        "and pubcloneid = -1 " +
        "and pubclonestatus is null) " +
        "or " +
        "(pubcloneid <> -1 " +
        "and pubclonestatus = ? )) " +
        "order by pubUpdateDate desc";

    List<PublicationDetail> publications = new ArrayList<>();
    try (PreparedStatement prepStmt = con.prepareStatement(sb)) {
