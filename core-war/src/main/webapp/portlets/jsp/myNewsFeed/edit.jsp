...
%>
    ...
    <form name="inputForm" target="_self" method="POST" action="<%=actionURL.toString()%>">
        <table border="0" width="100%" style="align: center">
            ...
            <!-- START "url" text box -->
            <tr>
                <td class="txtlibform"><fmt:message key="portlets.portlet.myNewsFeed.pref.notDisplayMyActivity" /> :</td>
                <% if ("checked".equalsIgnoreCase(pref.getValue("notDisplayMyActivity",""))) { %>
					<td><input name="notDisplayMyActivity" type="checkbox" value="<%=pref.getValue("notDisplayMyActivity","")%>" checked="checked"/></td>
				<% } else { %>
					<td><input name="notDisplayMyActivity" type="checkbox" value="checked" /></td>
				<% } %>
            </tr>
