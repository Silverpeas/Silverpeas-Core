(function (root, factory) {
  ...
  converse.plugins.add("actions", {
    ...
    'initialize': function () {
      ...
      __ = function(key) {
        switch (key) {
          case 'Reply':
            return sp.i18n.get('chat.client.web.replyTo.button');
          case 'Like':
            return sp.i18n.get('chat.client.web.reactionTo.like.button');
          case 'Dislike':
            return sp.i18n.get('chat.client.web.reactionTo.dislike.button');
          default:
      ...
      _converse.api.listen.on('connected', function() {
        ...
        sp.log.debug("actions plugin is ready");
    }
  });
  ...
  async function parseStanza(stanza, attrs) {
    ...
    if (reactions) {
      ...
      attrs.reaction_emoji = reactions.querySelector('reaction').innerHTML;
        sp.log.debug("parseStanza", stanza, attrs);
    }
    return attrs;
  }

  function handleReplyAction(model) {
      sp.log.debug('handleReplyAction', model)
  ...
  function handleReactionAction(model, emoji) {
      sp.log.debug('handleReactionAction', model, emoji);
    ...
    if (msgId) {
      ...
      _converse.api.send($msg({to: target, from: _converse.api.connection.get().jid, type}).c('body').t(body).up().c("reactions", {'xmlns': 'urn:xmpp:reactions:0', 'id': msgId}).c('reaction').t(emoji).up().up().c('origin-id', {'xmlns': 'urn:xmpp:sid:0', 'id': originId}));
    }
  }

  function replyChat(model, nick, text) {

    sp.log.debug("replyChat", model, nick, text);
  ...
  function getChatBoxFromMessageModel(model) {
    const view = _converse.chatboxviews.get(getTargetJidFromMessageModel(model));
      ...
      sp.log.debug("getChatBoxFromMessageModel", view);

    return view;
  }

  function setupTimer() {
    //  sp.log.debug("setupTimer render");
    renderReactions();
  ...
  function renderReactions() {
    if (!converse.emojis.initialized) {
      sp.log.debug('ConverseJS\'s Emojis are not initialized');
      return;
    }
    const models = _converse.chatboxes.models;
    //  sp.log.debug("rections render", models);
    ...
    for (model of models)
    {
      if (model.messages)
      {
        for (message of model.messages.models)
        {
          ...
          if (reactionId)
          {
            //  sp.log.debug("renderReactions", model.get('id'), reactionId, reactionEmoji);
    ...
    for (const reaction of msgReactions.values()) {
      //  sp.log.debug("rections item", reaction);
      ...
      if (el) {
        ...
        for (const emoji of reaction.emojis.values()) {
          //  sp.log.debug("rections emoji", emoji);
