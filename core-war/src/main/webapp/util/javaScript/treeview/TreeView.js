...
/*---------------Display control - Affichage du contr�le------------------------*/
...
/*---------------Design control - Construction du contr�le----------------------*/

TreeView.prototype.built_control = function ( node ) {
	try {
	var node_is_root = ( 0 == node.E_Num ) ;

	// CREATION DE LA BASE DU CONTR�LE ET DE LA RACINE
	if ( node_is_root ) {
		...
		image_plus.src = this.context+"/util/icons/treeview/1px.gif"
		...
		image_racine.src = this.context+"/util/icons/treeview/1px.gif"
		...
		// CREATION DE LA BASE D'UN ITEM DU CONTR�LE
		} else {
			var childrens_node = document.createElement ( "ul" )
			childrens_node.id = this.id + "_childrens_Folder_" + node.E_Num
			childrens_node.className = "childrens"
		...
		for ( var i = 0 ; i < childrens_count ; i++ ) {
			...
			switch ( cas ) {
				...
				case 6:	this.built_last_node_file ( parentNode , node )
					break		// Fils = fichier
				default : alert(" Cas ind�fini : " + cas )
				}
...
TreeView.prototype.built_node_folder_without_children = function ( parentNode , node ) {
	...
	image_branche.src = this.context+"/util/icons/treeview/1px.gif"
	image_branche.className = "branche"
	image_dossier.src = this.context+"/util/icons/treeview/1px.gif"
...
// Features Add - Ajout de fonctionnalit�s
...
/*----------------------- Alternate Reduce all folder & Collapse all folder  - R�duit tous les dossier ------------------------------*/
...
/*----------------------- Reduce all folder - R�duit tous les dossier ------------------------------*/
...
/*----------------------Ouvre le lien d'un �l�ment du contr�le-------------------*/
