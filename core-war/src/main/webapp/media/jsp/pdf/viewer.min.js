...
var PDFViewerApplication = {
  ...
  initialize: function initialize(appConfig) {
    ...
    configurePDFJS(_pdfjsLib.PDFJS);
  ...
  get supportsFullscreen() {
    var support = spFscreen.supportsFullscreen() && !_pdfjsLib.PDFJS.disableFullscreen;
  ...
  load: function load(pdfDocument, scale) {
    ...
    pdfDocument.getMetadata().then(function (_ref5) {
      ...
      sp.log.debug('PDF ' + pdfDocument.fingerprint + ' [' + info.PDFFormatVersion + ' ' + (info.Producer || '-').trim() + ' / ' + (info.Creator || '-').trim() + ']' + ' (PDF.js: ' + (_pdfjsLib.version || '-') + (!_pdfjsLib.PDFJS.disableWebGL ? ' [WebGL]' : '') + ')');
      var pdfTitle = void 0;
...
//    }, failureCallback);
//  }
  function parseTextPropertyResource(bundleAsTextProperties, lang, successCallback) {
    function evalString(text) {
      if (text.lastIndexOf('\\') < 0) return text;
      return text.replace(/\\\\/g, '\\').replace(/\\n/g, '\n').replace(/\\r/g, '\r').replace(/\\t/g, '\t').replace(/\\b/g, '\b').replace(/\\f/g, '\f').replace(/\\{/g, '{').replace(/\\}/g, '}').replace(/\\"/g, '"').replace(/\\'/g, "'");
    }
    ...
    gTextData += bundleAsTextProperties;
    parseProperties(bundleAsTextProperties, function (data) {
      for (var key in data) {
        var id,
            prop,
            index = key.lastIndexOf('.');
  ...
  function loadLocale(lang, callback) {
    ...
    clear();
    gLanguage = lang;
    parseTextPropertyResource(PdfViewerBundle.getAsTextProperties(), lang);
    callback();
...
//        sp.log.debug('using the embedded JSON directory, early way out');
...
//        callback();
//      } else {
//        sp.log.debug('no resource to load, early way out');
//      }
//      fireL10nReadyEvent(lang);
//      gReadyState = 'complete';
//      return;
...
//    }
  ...
  function substArguments(str, args, key) {
    ...
    return str.replace(reArgs, function (matched_text, arg) {
      ...
      sp.log.debug('argument {{' + arg + '}} for #' + key + ' is undefined.');
      return matched_text;
    });
  }
...
var PDFPresentationMode = function () {
  ...
  _createClass(PDFPresentationMode, [{
    ...
    value: function request() {
      ...
      if (!spFscreen.requestFullscreen(this.container)) {
        return false;
      }
      this.args = {
    ...
    value: function _addFullscreenChangeListeners() {
      ...
      spFscreen.addEventListener('fullscreenchange', this.fullscreenChangeBind);
    }
  }, {
    key: '_removeFullscreenChangeListeners',
    value: function _removeFullscreenChangeListeners() {
      spFscreen.removeEventListener('fullscreenchange', this.fullscreenChangeBind);
      delete this.fullscreenChangeBind;
    }
  }, {
    key: 'isFullscreen',
    get: function get() {
      return !!(spFscreen.fullscreenElement());
    }
  }]);
  ...
  _createClass(SecondaryToolbar, [{
    ...
    value: function _bindCursorToolsListener(buttons) {
      this.eventBus.on('cursortoolchanged', function (evt) {
        ...
        if (window.SP_DOWNLOAD_ENABLED && evt.tool === _pdf_cursor_tools.CursorTool.SELECT) {
          buttons.cursorSelectToolButton.classList.add('toggled');
        } else if (evt.tool === _pdf_cursor_tools.CursorTool.HAND) {
...
function webViewerLoad() {
  ...
  pdfjsWebApp.PDFViewerApplication.run(config).then(function() {
    window.SP_PDF_VIEWER_DEFERRED.resolve(pdfjsWebApp);
  });
}
