...
<%!
   ...
   */
  String truncatePath(String completPath, ArrayList<Value> list, boolean isLinked, int withLastValue) {
    ...
    for (int nb = 0; nb < nbShowedEltAuthorized; nb++) {
      ...
      completPathBuilder.append(linkedNode(value, isLinked)).append(separatorPath);
    }

    // colle ici les points de suspension
    completPathBuilder.append(troncateSeparator + separatorPath);
    ...
    return completPathBuilder.toString();
  }
   ...
   * @return completPath - le chemin fabrique
   */
  String buildCompletPath(ArrayList<Value> list, boolean isLinked, int withLastValue) {
    ...
    if (list.size() > maxEltAuthorized) {
      completPath = new StringBuilder(truncatePath(completPath.toString(), list, isLinked, withLastValue));
    } else {
      for (int nb = 0; nb < list.size() - withLastValue; nb++) {
        Value value = list.get(nb);
        completPath.append(linkedNode(value, isLinked)).append(separatorPath);
      }
    }

    if ((completPath.toString().isEmpty()) || (completPath.toString().equals("/"))) {
      completPath = new StringBuilder();
    } else {
      completPath = new StringBuilder(completPath.substring(0, completPath.length() - separatorPath.length())); // retire le dernier separateur
    }

    return completPath.toString();
  }

  String buildCompletPath(ArrayList<Value> list) {
    return buildCompletPath(list, false, 0);
  }

  String buildCompletPath(ArrayList<Value> list, boolean isLinked) {
