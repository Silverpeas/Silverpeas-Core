...
( function() {
  ...
  CKEDITOR.plugins.add( 'autolink', {
    ...
    init: function( editor ) {
      ...
      function getHtmlToInsert( text ) {
        ...
        if (typeof editor.config.autolink_setupLink === 'function') {
          editor.config.autolink_setupLink( link );
        }
  ...
  /**
   * The [Auto Link](https://ckeditor.com/cke4/addon/autolink) plugin keystrokes used to finish
   * link completion.
   *
   * ```javascript
  ...
  /**
   * Regex used by the [Auto Link](https://ckeditor.com/cke4/addon/autolink) plugin to match URL
   * adresses.
   *
  ...
  /**
   * Regex used by the [Auto Link](https://ckeditor.com/cke4/addon/autolink) plugin to match email
   * adresses.
   *
  ...
  /**
   * Function used by the [Auto Link](https://ckeditor.com/cke4/addon/autolink) plugin to allow
   * specific setup about generated HTML link.
   *
   * @cfg {function} [autolink_setupLink]
   * @since 4.20.1 (silveryocha)
   * @member CKEDITOR.config
   */
  CKEDITOR.config.autolink_setupLink = function( link ) {
    var isInternalUrl = false;
    var linkUrl = link.getAttribute( 'href' );
    if ( linkUrl ) {
      isInternalUrl = linkUrl.startsWith( webContext );
      if ( !isInternalUrl ) {
        var exploded = linkUrl.split( webContext );
        isInternalUrl = exploded.length > 1 && exploded[0].startsWith( 'http' );
      }
    }
    if ( !isInternalUrl ) {
