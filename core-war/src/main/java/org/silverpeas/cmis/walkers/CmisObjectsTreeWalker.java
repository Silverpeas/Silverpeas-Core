 ...
 * the CMIS objects tree) depends on its type, the implementation of the walker is left to more
 * concrete walkers specifically designed to handle each object type. And consequently as each type
 * of Silverpeas objects is mapped to a peculiar type of CMIS object, those concrete walkers are
 * also dedicated to handle, in their functional view, a peculiar type of CMIS objects.
 *
 * @author mmoquillon
 */
public interface CmisObjectsTreeWalker {
   ...
   * must be not null, otherwise, for any other object types, it is ignored even if it is set. The
   * type of the child to create is provided by the
   ...
   * objects tree. Only the data satisfying the given filtering rules are returned. The root of the
   * CMIS objects tree is the {@link CmisFilePath#PATH_SEPARATOR}.
   *
  ...
  /**
   * Gets a list of CMIS data of the Silverpeas objects that are the direct parents of the CMIS
   * object uniquely identified by the specified identifier. Only the data satisfying the given
   * filtering rules are returned. A file-able CMIS object can have one or more parents.
   * Nevertheless, in Silverpeas, only publications in an EDM can have more than one parent (through
   * the aliasing mechanism); so unless the object referred by the specified identifier is an
   * aliased publication, this method returns usually a list with one element.
   *
   ...
   * folder ({@link ObjectInFolderContainer} instances) is returned with, for each of them, if any,
   * their own direct children and so on. Each child is described by their CMIS data (a decorator
   * {@link ObjectInFolderData} of an {@link ObjectData} instance) filtered with the given filtering
   * rules. If this method is invoked for a document instead of a folder, then an empty list should
   * be returned.
   *
   * @param folderId the unique identifier of a Silverpeas resource.
   * @param filtering the filtering rules to apply on the CMIS data to return.
   ...
   * is a container of others {@link ObjectInFolderContainer} objects (recursive walk of children).
   * They are described by an {@link ObjectInFolderData} instance that is a decorator of an
   ...
   * be returned (see {@link I18n#getSupportedLanguageCodes()}).
