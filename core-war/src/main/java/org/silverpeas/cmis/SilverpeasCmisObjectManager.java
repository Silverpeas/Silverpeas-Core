 ...
 * the CMIS service's operation by invoking one of the manager's method with the correct parameter
 * correspondence.
 * </p>
 * <p>
 * The walking through the tree of CMIS objects and the modification operations on those objects
 * (and therefore their corresponding Silverpeas resources and contributions) are performed by a
 * {@link AbstractCmisObjectsTreeWalker} instance. Such an object knows both how to walk through a
 * given subtree and how to perform CRUD operations on the objects of that subtree according to the
 * type of these objects. For example, the walking down a subtree isn't done identically when rooted
 * to a space than when rooted to an application (as they don't accept the same children type) and
 * don't support necessary the same operations.
 * </p>
 *
 * @author mmoquillon
 */
...
public class SilverpeasCmisObjectManager {
   ...
   * {@link org.apache.chemistry.opencmis.commons.PropertyIds#OBJECT_TYPE_ID} property. If the
   * creation of instances of such concrete type isn't supported, then a
   * {@link org.apache.chemistry.opencmis.commons.exceptions.CmisNotSupportedException} exception is
   * thrown.
   *
   ...
   * given language. The concrete type of the CMIS object to create is given by the
   * {@link org.apache.chemistry.opencmis.commons.PropertyIds#OBJECT_TYPE_ID} property and must be
   * {@link org.silverpeas.core.cmis.model.TypeId#SILVERPEAS_DOCUMENT} otherwise a
   * {@link org.apache.chemistry.opencmis.commons.exceptions.CmisNotSupportedException} exception is
   * thrown. If an error occurs while storing the content stream, then a
   * {@link CmisStorageException} exception is thrown.
   *
   * @param folderId the unique identifier of the parent folder.
   ...
   * then a
   * {@link org.apache.chemistry.opencmis.commons.exceptions.CmisContentAlreadyExistsException} is
   * thrown.
   ...
   *
   * @param path the path of the object from the root. The path is made up of segments, each of them
   * separated by the '/' character. A segment can be either the name of a CMIS fileable object or
   * its unique identifier. For better performance we recommend strongly to use object's identifiers
   * for segments.
   * @param filtering parameters to apply on the characteristics to include with the object.
  ...
  /**
   * Gets the file-able objects that are the descendents of the specified folder down to the given
   * depth of its offspring tree.
   *
   * @param folderId the unique identifier of a folder, whatever its concrete type in Silverpeas.
   * @param filtering the filtering parameters to apply on the characteristics to include with each
   * object.
   * @param depth the depth of the offspring tree to walk across. If 1, only the direct children of
   * the specified folder are returned. If negative all descendant objects at all depth levels in
   * the CMIS hierarchy are returned. Any positive value means only objects that are children of the
   * folder and descendants down to the given levels deep.
   ...
   *
   * @param objectId the unique identifier of an object in Silverpeas, whatever its concrete type in
   * Silverpeas.
