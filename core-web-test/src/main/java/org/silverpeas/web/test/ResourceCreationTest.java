...
/**
 * Unit tests on the creation of a new resource in Silverpeas through a REST web service. This class
 * is an abstract one, and it implements some tests that are redondant over all web resources in
...
public abstract class ResourceCreationTest extends RESTWebServiceTest
   ...
   * @param entity the web entity to post.
   * @param atURI the URI at which the entity has to be posted.
   * @param authId the authentication identification to use to identify the user behind the
   * request.
   * @return the response of the post.
   */
  ...
  public void creationOfANewResourceByANonAuthenticatedUser() {
    try (Response response = post(aResource(), at(aResourceURI()), withAsAuthId(AuthId.NONE))) {
  ...
  public void creationOfANewResourceWithADeprecatedSession() {
    ...
    try (Response response = post(aResource(), at(aResourceURI()), withAsAuthId(authId))) {
      int receivedStatus = response.getStatus();
      int unauthorized = Status.UNAUTHORIZED.getStatusCode();
      assertThat(receivedStatus, is(unauthorized));
    }
  ...
  public void creationOfANewResourceByANonAuthorizedUser() {
    ...
    try (Response response = post(aResource(), at(aResourceURI()))) {
      int receivedStatus = response.getStatus();
  ...
  public void postAnInvalidResourceState() {
    try (Response response =
             post("{\"uri\": \"https://toto.chez-les-papoos.com/invalid/resource\"}",
                 at(aResourceURI()))) {
      int receivedStatus = response.getStatus();
