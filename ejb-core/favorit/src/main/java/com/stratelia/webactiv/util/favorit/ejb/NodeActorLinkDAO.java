...
public class NodeActorLinkDAO {
  ...
  public static void removeByNodePath(Connection con, String tableName,
    ...
    // NEWD DLE
    /*
     * String deleteStatement = "delete from " + tableName + " where space = ? and " +
     * " componentName = ? " + " and nodeId in ( " + " select  nodeId from " + node.getTableName() +
   ...
   */
  /*
   * public static Collection getNodePKsByActorSpaceAndComponent( Connection con, String tableName,
   * String userId, String space, String componentName) throws SQLException { SilverTrace.info(
   * "favorit", "NodeActorLinkDAO.getNodePKsByActorSpaceAndComponent", "root.MSG_GEN_ENTER_METHOD",
   * "tableName = " + tableName + ", userId = " + userId + ", space = " + space +
   * ", componentName = " + componentName); String selectStatement =
   * "select nodeId, space, componentName from " + tableName +
   * " where actorId = ? and space = ? and componentName = ?"; PreparedStatement prepStmt = null;
   * ResultSet rs = null; try { prepStmt = con.prepareStatement(selectStatement);
   * prepStmt.setString(1, userId); prepStmt.setString(2, space); prepStmt.setString(3,
   * componentName); rs = prepStmt.executeQuery(); ArrayList list = new ArrayList(); while
   * (rs.next()) { String id = String.valueOf(rs.getInt(1)); String sp = rs.getString(2); String
   ...
   */
  /*
   * public static Collection getActorPKsByNodePK(Connection con, String tableName, NodePK node)
   * throws SQLException { SilverTrace.info( "favorit", "NodeActorLinkDAO.getActorPKsByNodePK",
   * "root.MSG_GEN_ENTER_METHOD", "tableName = " + tableName + ", node = " + node.toString());
   * String selectStatement = "select actorId from " + tableName +
   * " where nodeId = ? and space = ? and componentName = ?"; PreparedStatement prepStmt = null;
   * ResultSet rs = null; try { prepStmt = con.prepareStatement(selectStatement); prepStmt.setInt(1,
   * new Integer(node.getId()).intValue()); prepStmt.setString(2, node.getSpace());
   * prepStmt.setString(3, node.getComponentName()); rs = prepStmt.executeQuery(); ArrayList list =
