...
/*	public static void createTable(Connection con, String tableName) throws SQLException
	{
		...
		try
		{
			prepStmt = con.prepareStatement(createStatement);
...
*/
	...
	public static void add(Connection con, String tableName, String userId, NodePK node) throws SQLException
	{
		SilverTrace.info(
			"favorit",
			"NodeActorLinkDAO.add",
			"root.MSG_GEN_ENTER_METHOD",
			"tableName = " + tableName + ", userId = " + userId + ", node = " + node.toString());
	...
	public static void remove(Connection con, String tableName, String userId, NodePK node) throws SQLException
	{
		SilverTrace.info(
			"favorit",
			"NodeActorLinkDAO.remove",
			"root.MSG_GEN_ENTER_METHOD",
			"tableName = " + tableName + ", userId = " + userId + ", node = " + node.toString());

// NEWD DLE
		//String insertStatement = "delete from " + tableName + " where actorId = ? and nodeId = ? and space = ? and componentName = ?";
...
//			NEWF DLE
	...
	public static void removeByNodePath(Connection con, String tableName, NodePK node, String path) throws SQLException
	{
		SilverTrace.info(
			"favorit",
			"NodeActorLinkDAO.removeByNodePath",
			"root.MSG_GEN_ENTER_METHOD",
			"tableName = " + tableName + ", node = " + node.toString() + ", path = " + path);
...
//			NEWF DLE
	...
	public static Collection getNodePKsByActor(Connection con, String tableName, String userId) throws SQLException
	{
		SilverTrace.info(
			"favorit",
			"NodeActorLinkDAO.getNodePKsByActor",
			"root.MSG_GEN_ENTER_METHOD",
			"tableName = " + tableName + ", userId = " + userId);
// NEWD DLE
	//	String selectStatement = "select nodeId, space, componentName from " + tableName + " where actorId = ?";
...
// NEWD DLE
	 ...
	 * @see
	 */
/*	public static Collection getNodePKsByActorSpaceAndComponent(
		Connection con,
		String tableName,
		String userId,
		String space,
		...
		throws SQLException
	{
		...
		String selectStatement =
			"select nodeId, space, componentName from "
				+ tableName
				+ " where actorId = ? and space = ? and componentName = ?"; 
		...
		try
		{
			...
			while (rs.next())
			{
				...
				String cptName = rs.getString(3);
				NodePK nodePK = new NodePK(id, sp, cptName);
				list.add(nodePK);
			}
			return list;
		}
...
/*	
	public static Collection getActorPKsByNodePK(Connection con, String tableName, NodePK node) throws SQLException
	{
		SilverTrace.info(
			"favorit",
			"NodeActorLinkDAO.getActorPKsByNodePK",
			"root.MSG_GEN_ENTER_METHOD",
			"tableName = " + tableName + ", node = " + node.toString());

		String selectStatement =
			"select actorId from " + tableName + " where nodeId = ? and space = ? and componentName = ?";
		PreparedStatement prepStmt = null;
		ResultSet rs = null;
		try
		{
			...
			prepStmt.setString(2, node.getSpace());
			prepStmt.setString(3, node.getComponentName());
			rs = prepStmt.executeQuery();
			ArrayList list = new ArrayList();
			while (rs.next())
...
// NEWD DLE
	...
	public static Collection getActorPKsByNodePK(Connection con, String tableName, NodePK node) throws SQLException
	{
		...
		String selectStatement =
			"select actorId from " + tableName + " where nodeId = ? and componentName = ?";
		PreparedStatement prepStmt = null;
		ResultSet rs = null;
		try
		{
