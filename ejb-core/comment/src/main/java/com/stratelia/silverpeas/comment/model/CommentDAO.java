...
public class CommentDAO
{
	...
	public static CommentPK createComment(Connection con, Comment cmt) throws SQLException
	{
		String insert_query = "insert into " + COMMENT_TABLENAME + " values ( ?, ?, ?, ?, ?, ?, ? )";
		PreparedStatement prep_stmt = null;
		prep_stmt = con.prepareStatement(insert_query);
		int newId = 0;
		try
		{
			newId = DBUtil.getNextId(cmt.getCommentPK().getTableName(), new String("commentId"));
		}
		catch (Exception e)
		{
	...
	{
		String delete_query = "delete from " + COMMENT_TABLENAME + " where commentId=" + pk.getId();
		PreparedStatement prep_stmt = null;
	...
	public static void moveComments(Connection con, ForeignPK fromPK, ForeignPK toPK) throws SQLException
	{
		String update_query =
			"update " + COMMENT_TABLENAME
			...
			if (rs.next())
			{
				...
				cmt =
					new Comment(
						...
						rs.getString("commentCreationDate"),
	...
	public static ArrayList getMostCommentedAllPublications(Connection con) throws SQLException
	{
		String select_query =
			"select count(commentId) as nb_comment, foreignId, instanceId from "
				+ COMMENT_TABLENAME
		...
		try
		{
			...
			while (rs.next())
			{
				...
				listPublisCommentsCount.add(new CommentInfo(countComment.intValue(),instanceId,foreignId.toString()));
			}
		}
		catch (Exception e) {
			e.printStackTrace();
	...
	public static int getCommentsCount(Connection con, WAPrimaryKey foreign_pk) throws SQLException
	{
		String select_query =
			"select count(commentId) as nb_comment from "
				+ COMMENT_TABLENAME
				+ " where instanceId = '"
	...
	public static Vector getAllComments(Connection con, WAPrimaryKey foreign_pk) throws SQLException
	{
		String select_query =
	...
	public static void deleteAllComments(Connection con, ForeignPK pk) throws SQLException
	{
		String delete_query = "delete from " + COMMENT_TABLENAME + " where foreignId = ? And instanceId = ? ";
		PreparedStatement prep_stmt = null;
		prep_stmt = con.prepareStatement(delete_query);
		try
		{
