...
public class JournalDAO
{
		...
		throws SQLException, UtilException, CalendarException
	{
		String insertStatement =
			"insert into CalendarJournal (" + COLUMNNAMES + ") " + " values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		...
		try
		{
			...
			if (prepStmt.executeUpdate() == 0)
				throw new CalendarException(
					"JournalDAO.Connection con,  addJournal(Connection con, JournalHeader journal)",
					SilverpeasException.ERROR,
					"calendar.EX_EXCUTE_UPDATE_EMPTY");
		}
		finally
		{
			DBUtil.close(prepStmt);
		...
		try
		{
			...
			if (prepStmt.executeUpdate() == 0)
				throw new CalendarException(
					"JournalDAO.Connection con,  addJournal(Connection con, JournalHeader journal)",
					SilverpeasException.ERROR,
					"calendar.EX_EXCUTE_DELETE_EMPTY");
		}
		finally
		{
			DBUtil.close(prepStmt);
	...
	public static boolean hasTentativeJournalsForUser(Connection con, String userId)
	...
	public static Collection getTentativeJournalHeadersForUser(Connection con, String userId)
		throws SQLException, java.text.ParseException
	{
		PreparedStatement prepStmt = null;
		ResultSet rs = null;
		...
		throws SQLException, java.text.ParseException
	{
		...
		selectStatement.append("select distinct ").append(JournalDAO.JOURNALCOLUMNNAMES).append(" from CalendarJournal, CalendarJournalAttendee ");
		if (categoryId != null)
		...
		{
			selectStatement.append("union ").append("select distinct ").append(JournalDAO.JOURNALCOLUMNNAMES).append(" from CalendarJournal ");
			if (categoryId != null)
				selectStatement.append(", CalendarJournalCategory ");
	...
	public static Collection getDayJournalHeadersForUser(
		Connection con,
		String day,
		String userId,
		String categoryId,
		...
		throws SQLException, java.text.ParseException
	{
		return getJournalHeadersForUser(con, day, userId, categoryId, participation, "=");
	}
	
	public static Collection getNextJournalHeadersForUser(
			Connection con,
			String day,
			String userId,
			String categoryId,
	...
	public static Collection countMonthJournalsForUser(
		Connection con,
		String month,
		String userId,
		String categoryId,
	...
	public static Collection getPeriodJournalHeadersForUser(
		Connection con,
		String begin,
		String end,
		String userId,
		...
		throws SQLException, java.text.ParseException
	{
		...
		StringBuffer selectStatement = new StringBuffer(200); 
		selectStatement.append("select distinct ").append(JournalDAO.COLUMNNAMES).append(" from CalendarJournal, CalendarJournalAttendee ");
		if (categoryId != null)
			selectStatement.append(", CalendarJournalCategory ");
		...
		if (participation.equals(ParticipationStatus.ACCEPTED))
		{
			selectStatement.append(" union select distinct ").append(JournalDAO.COLUMNNAMES).append(" from CalendarJournal ");
			if (categoryId != null)
				selectStatement.append(", CalendarJournalCategory ");
			...
			/*selectStatement.append(" and ( (startDay >= '"
				+ begin
				+ "' and startDay <= '"
				+ end
				+ "') or "
		...
		throws SQLException, CalendarException, java.text.ParseException
	{
		...
		String selectStatement = "select " + JournalDAO.COLUMNNAMES + " from CalendarJournal " + "where id = ?";
	...
	public static Collection getOutlookJournalHeadersForUser(Connection con, String userId)
	throws SQLException, CalendarException, java.text.ParseException
{

	String selectStatement = "select " + JournalDAO.COLUMNNAMES + " from CalendarJournal " + "where delegatorId = ? and externalId is not null";
	...
	public static Collection getOutlookJournalHeadersForUserAfterDate(Connection con, String userId, java.util.Date startDate)
	throws SQLException, CalendarException, java.text.ParseException
{

	String selectStatement = "select " + JournalDAO.COLUMNNAMES + " from CalendarJournal " + "where delegatorId = ? and startDay >= ? and externalId is not null";
	...
	public static Collection getJournalHeadersForUserAfterDate(Connection con, String userId, java.util.Date startDate, int nbReturned)
	throws SQLException, CalendarException, java.text.ParseException
	{
	
		String selectStatement = "select " + JournalDAO.COLUMNNAMES + " from CalendarJournal " + "where delegatorId = ? "+
