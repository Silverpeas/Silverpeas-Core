...
public abstract class Schedulable implements java.io.Serializable {
  ...
  protected static final java.text.SimpleDateFormat hourFormat = 
  ...
  public Classification getClassification() {
    if (classification == null) classification = new Classification();
  ...
  public java.util.Date getStartDate() {
    if (getStartDay() == null) return null;
    try {
  ...
  public void setStartDate(java.util.Date start) {
    ...
    try {
      ...
      SilverTrace.warn("calendar", "Schedulable.setStartDate(setStartDate(java.util.Date start)", "calendar_MSG_NOT_PARSE_DATE");
    }
  }
  ...
  public String getStringDuration() {
    try {
      java.util.Date startDate = completeFormat.parse(getStartDay() + " " + getStartHour());
      java.util.Date endDate = completeFormat.parse(getEndDay() + " " + getEndHour());
      long ms = endDate.getTime() - startDate.getTime();
      return Schedulable.hourMinuteToString((int)((ms / (60000)) % 60), (int)(ms / 3600000));
    }
    catch (Exception e) {
      SilverTrace.warn("calendar", "Schedulable.getStringDuration", "calendar_MSG_NOT_SCEDULE","return = 00:00");
      return "00:00";
    }
  }
  ...
  public int getMinuteDuration() {
    try {
      ...
      SilverTrace.warn("calendar", "Schedulable.getMinuteDuration() ", "calendar_MSG_NOT_SCEDULE","return = 0");
      return 0;
    }
  }
  
  public boolean isOver(Schedulable schedule) {
    if ((getStartHour() == null) || (getEndHour() == null)) return false;
    if ((schedule.getStartHour() == null) || (schedule.getEndHour() == null)) return false;
    if ((getStartDate().compareTo(schedule.getStartDate()) <= 0) && (getEndDate().compareTo(schedule.getStartDate()) > 0 )) return true;
    if ((getStartDate().compareTo(schedule.getEndDate()) < 0) && (getEndDate().compareTo(schedule.getEndDate()) >= 0 )) return true;
    if ((schedule.getStartDate().compareTo(getStartDate()) <= 0) && (schedule.getEndDate().compareTo(getStartDate()) > 0 )) return true;
    if ((schedule.getStartDate().compareTo(getEndDate()) < 0) && (schedule.getEndDate().compareTo(getEndDate()) >= 0 )) return true;
    //}
    return false;
  }
	...
	public String toString() {
		String result = " id = " + getId() + " name = " + getName()
				...
				+ getDescription() + " startDay = " + getStartDay()
				+ " startHour = " + getStartHour() + " endDay = " + getEndDay()
				+ " endHour = " + getEndHour()
