...
public class SILVERMAILPersistence
{
	...
	public static void addMessage(SILVERMAILMessage p_Msg) throws SILVERMAILException
	{
		...
		if (p_Msg != null)
		{
			try
			{
				dao = SilverpeasBeanDAOFactory.getDAO("com.stratelia.silverpeas.notificationserver.channel.silvermail.SILVERMAILMessageBean");
			...
			{
    			throw new SILVERMAILException("SILVERMAILPersistence.addMessage()", SilverpeasException.ERROR, "silvermail.EX_CANT_WRITE_MESSAGE",e);
			}
		}
	}
	 ...
	 * 
	 */
	public static Collection getMessageOfFolder(int p_UserId, String p_FolderName) throws SILVERMAILException
	{
		...
		whereClause = whereClause + " AND FOLDERID=" + convertFolderNameToId(p_FolderName);
		...
		try
		{
			...
			if (collectionMessageBean.isEmpty() == false)
			{
				...
				while (cmbIterator.hasNext() == true)
				{
					...
					{
						SilverTrace.debug("silvermail", "SILVERMAILListener.getMessageOfFolder()", "PB converting body id to LongText", "Message Body = " + pmb.getBody());
						body = pmb.getBody();
					}
		...
		{
  			throw new SILVERMAILException("SILVERMAILPersistence.getMessageOfFolder()", SilverpeasException.ERROR, "silvermail.EX_CANT_READ_MSG","UserId=" + Long.toString(p_UserId) + ";Folder=" + p_FolderName,e);
		}
	...
	public static SILVERMAILMessage getMessage(long p_Id) throws SILVERMAILException
	{
		...
		try
		{
			...
			if (smb != null)
			{
				...
				{
					SilverTrace.debug("silvermail", "SILVERMAILListener.getMessage()", "PB converting body id to LongText", "Message Body = " + smb.getBody());
					body = smb.getBody();
				}
				result.setBody(body);
		...
		{
  			throw new SILVERMAILException("SILVERMAILPersistence.getMessage()", SilverpeasException.ERROR, "silvermail.EX_CANT_READ_MSG", "MsgId=" + Long.toString(p_Id),e);
		}
	...
	public static void deleteMessage(long p_Id) throws SILVERMAILException
	{
		...
		try
		{
			...
			{
				SilverTrace.debug("silvermail", "SILVERMAILListener.deleteMessage()", "PB converting body id to LongText", "Message Body = " + p_Id);
			}
			dao.remove(pk);
		}
		catch (Exception e)
		{
  			throw new SILVERMAILException("SILVERMAILPersistence.deleteMessage()", SilverpeasException.ERROR, "silvermail.EX_CANT_DEL_MSG", "MsgId=" + Long.toString(p_Id),e);
		}
	}

	public static void deleteAllMessages(int p_UserId, String p_FolderName) throws SILVERMAILException
    {
    	  Iterator messageIterator = getMessageOfFolder(p_UserId, p_FolderName).iterator();
    ...
    private static void markMessageAsReaden(SILVERMAILMessageBean smb) throws SILVERMAILException
	{
		SilverpeasBeanDAO	  dao;
		...
		{
  			throw new SILVERMAILException("SILVERMAILPersistence.markMessageAsReaden()", SilverpeasException.ERROR, "silvermail.EX_CANT_READ_MSG", "MsgId=" + smb.getPK().getId(),e);
		}
	}
	...
	protected static String getUserLogin(long userId) throws SILVERMAILException
	{
        ...
        {
  			throw new SILVERMAILException("SILVERMAILPersistence.getUserLogin()", SilverpeasException.ERROR, "silvermail.EX_CANT_GET_USER_LOGIN", "UserId=" + Long.toString(userId),e);
        }
		return result;
	}
