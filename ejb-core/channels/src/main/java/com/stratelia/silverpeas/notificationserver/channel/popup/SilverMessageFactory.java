...
public class SilverMessageFactory {
	...
	public static SilverMessage read ( String p_UserId ) {
     ...
     whereClause = " ID IN ( SELECT MIN(ID) FROM ST_PopupMessage WHERE USERID=" + p_UserId + " )";
     ...
     try
     {
       ...
       if( collectionPopupMessage.isEmpty() == false )
       {
		...
		{
			SilverTrace.debug("popup", "SilverMessageFactory.read()", "PB converting body id to LongText", "Message Body = " + pmb.getBody());
			msg = pmb.getBody();
		}
        if( msg != null )
        {
     ...
     {
       SilverTrace.error( "popup", "SilverMessageFactory.read()", "popup.EX_CANT_READ_MSG", "UserId=" + p_UserId, e);
     }
  ...
  public static void del( String p_MsgId) {
    try {
      ...
      pk.setId( p_MsgId );
		...
		{
			SilverTrace.debug("popup", "SilverMessageFactory.del()", "PB converting body id to LongText", "Message Body = " + p_MsgId);
		}
      dao.remove( pk );
    }
    catch( PersistenceException e ){
       SilverTrace.error( "popup", "SilverMessageFactory.del()", "popup.EX_CANT_DEL_MSG", "MsgId=" + p_MsgId, e);
    }
  }
	...
	public static void push ( String p_UserId, NotificationData p_Message ) {
	    ...
	    try
	    {
	    	SilverTrace.debug("popup", "SilverMessageFactory.push()", "Message = " + p_Message.toString());
			SilverTrace.debug("popup", "SilverMessageFactory.push()", "Message.isAnswerAllowed = " + p_Message.isAnswerAllowed());
			dao = SilverpeasBeanDAOFactory.getDAO( "com.stratelia.silverpeas.notificationserver.channel.popup.POPUPMessageBean" );
			pmb.setUserId( Long.parseLong( p_UserId  ) );
	      
			//CBO : UPDATE
			//pmb.setBody( Integer.toString(LongText.addLongText(p_Message.getMessage())) );
			if("COMMUNICATION".equals(p_Message.getComment())) {
				pmb.setBody(p_Message.getComment() + Integer.toString(LongText.addLongText(p_Message.getMessage())) );	
			} else {
				pmb.setBody( Integer.toString(LongText.addLongText(p_Message.getMessage())) );
			}
			//CBO : FIN UPDATE
	    ...
	    {
	       SilverTrace.error( "popup", "SilverMessageFactory.push()", "popup.EX_CANT_PUSH_MSG", "UserId=" + p_UserId + ";Msg=" + p_Message, e);
	    }
	}
