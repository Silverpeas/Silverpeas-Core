...
public class POPUPPersistence
{
	...
	public static void addMessage(POPUPMessage p_Msg) throws POPUPException
	{
		...
		if (p_Msg != null)
		{
			try
			{
				...
				SilverTrace.debug("popup", "POPUPPersistence.getMessage()", "Date et time", DateUtil.date2SQLDate(new Date())+ "-" + DateUtil.getFormattedTime(new Date()));
				dao.add(smb);
			}
			catch (Exception e)
			{
    			throw new POPUPException("POPUPPersistence.addMessage()", SilverpeasException.ERROR, "POPUP.EX_CANT_WRITE_MESSAGE",e);
			}
		}
	}
	...
	public static POPUPMessage getMessage(long p_Id) throws POPUPException
	{
		...
		try
		{
			...
			if (smb != null)
			{
				...
				{
					SilverTrace.debug("popup", "POPUPListener.getMessage()", "PB converting body id to LongText", "Message Body = " + smb.getBody());
					body = smb.getBody();
				}
				result.setBody(body);
		...
		{
  			throw new POPUPException("POPUPPersistence.getMessage()", SilverpeasException.ERROR, "POPUP.EX_CANT_READ_MSG", "MsgId=" + Long.toString(p_Id),e);
		}
		return result;
	}
	...
	public static void deleteMessage(long p_Id) throws POPUPException
	{
		...
		try
		{
			...
			{
				SilverTrace.debug("popup", "POPUPListener.deleteMessage()", "PB converting body id to LongText", "Message Body = " + p_Id);
			}
			dao.remove(pk);
		}
		catch (Exception e)
		{
  			throw new POPUPException("POPUPPersistence.deleteMessage()", SilverpeasException.ERROR, "POPUP.EX_CANT_DEL_MSG", "MsgId=" + Long.toString(p_Id),e);
		}
	}
	...
	protected static String getUserLogin(long userId) throws POPUPException
	{
        ...
        {
  			throw new POPUPException("POPUPPersistence.getUserLogin()", SilverpeasException.ERROR, "POPUP.EX_CANT_GET_USER_LOGIN", "UserId=" + Long.toString(userId),e);
        }
		return result;
	}
}
