...
public class SilverMessageFactory {
	...
	public static SilverMessage read (String userId, String sessionId) {
		...
		String whereClause = " ID IN ( SELECT MIN(ID) FROM ST_ServerMessage WHERE USERID=" + userId + " AND SESSIONID='"+sessionId+"')";
		...
		try
		{
			...
			if(!collectionServerMessage.isEmpty())
			{
				...
				{
					SilverTrace.debug("server", "SilverMessageFactory.read()", "PB converting body id to LongText", "Message Body = " + smb.getBody());
					msg = smb.getBody();
				}
				if(msg != null)
				{
		...
		{
			SilverTrace.error("server", "SilverMessageFactory.read()", "server.EX_CANT_READ_MSG", "UserId=" + userId+", sessionId = "+sessionId, e);
		}
	...
	public static void del(String p_MsgId) {
		try {
			...
			{
				SilverTrace.debug("server", "SilverMessageFactory.del()", "PB converting body id to LongText", "Message Body = " + p_MsgId);
			}
			dao.remove(pk);
		}
		catch( PersistenceException e ){
			SilverTrace.error("server", "SilverMessageFactory.del()", "server.EX_CANT_DEL_MSG", "MsgId=" + p_MsgId, e);
		}
	}
	...
	public static void push (String userId, String p_Message, String sessionId) {
		...
		{
			SilverTrace.error("server", "SilverMessageFactory.push()", "server.EX_CANT_PUSH_MSG", "UserId=" + userId + ";Msg=" + p_Message, e);
		}
	}
