...
public class SMTPListener extends AbstractListener
{
	...
	{
		ResourceLocator mailerSettings = new ResourceLocator("com.stratelia.silverpeas.notificationserver.channel.smtp.smtpSettings", "");
	...
	public void onMessage(javax.jms.Message msg)
	{
		try
		{
			SilverTrace.info("smtp", "SMTPListner.onMessage()", "root.MSG_GEN_PARAM_VALUE", "JMS Message = " + msg.toString());
			processMessage(msg);
		}
		catch (NotificationServerException e)
		{
			SilverTrace.error("smtp", "SMTPListner.onMessage()", "smtp.EX_CANT_PROCESS_MSG", "JMS Message = " + msg.toString() + ", Payload = " + m_payload == null ? "" : m_payload, e);
		}
	}
	...
	public void send(NotificationData p_Message) throws NotificationServerException
	{
        ...
        try
        {
            ...
            tmpFromString = (String) keyValue.get("FROM");  // retrieves the FROM key value.
            tmpSubjectString = (String) keyValue.get("SUBJECT");  // retrieves the SUBJECT key value.
            tmpUrlString = (String) keyValue.get("URL");  // retrieves the URL key value.
            tmpLanguageString = (String) keyValue.get("LANGUAGE");  // retrieves the LANGUAGE key value.
        ...
        {
            throw new NotificationServerException("SMTPListner.send()", SilverpeasException.ERROR, "smtp.EX_NO_TARGET_AVAILABLE", e);
        }
        catch (NoSuchElementException e)
        {
            throw new NotificationServerException("SMTPListner.send()", SilverpeasException.ERROR, "smtp.EX_MISSING_PARAMETER", e);
        }
		...
		ResourceLocator messages = new ResourceLocator("com.stratelia.silverpeas.notificationserver.channel.smtp.multilang.smtpBundle", tmpLanguageString);

        // if no FROM field was entered, then add an error. If not, send the email.
        if (tmpFromString == null)
        {
            throw new NotificationServerException("SMTPListner.send()", SilverpeasException.ERROR, "smtp.EX_MISSING_FROM");
        }
        else
        {
			//String body = (tmpUrlString==null) ? p_Message.getMessage() : p_Message.getMessage() + "\n\n" + messages.getString("clickBelow") + "\n" + tmpUrlString;
			...
			if (tmpUrlString!=null)
			{
				...
				body += "<a href=\""+tmpUrlString+"\" target=_blank>" + messages.getString("clickHere") + "</a> "+messages.getString("ToAccessDocument");
			}
			
            if (tmpAttachmentIdString == null)
            {
                sendEmail(tmpFromString, p_Message.getTargetReceipt(), tmpSubjectString, body, (tmpUrlString!=null));
            }
            else
            {
                 ...
                 * tmpFromString,
                 * p_Message.getTargetReceipt(),
                 * tmpSubjectString,
                 * p_Message.getMessage(),
                 * tmpAttachmentIdString,
	...
	/**
	 * send email to destination using SMTP protocol and JavaMail 1.3 API (compliant with MIME format).
	 ...
	 */
	private void sendEmail(String pFrom, String pTo, String pSubject, String pMessage, boolean htmlFormat) throws NotificationServerException
	{
		...
		properties.put("mail.smtp.auth", new Boolean(m_SmtpAuthentication).toString());
		// properties.put("mail.smtp.dsn.notify", "SUCCESS,FAILURE ORCPT=rfc822;" + NSADM );
		...
		try
		{
			...
			{
				SilverTrace.warn("smtp", "SMTPListner.sendEmail()", "root.MSG_GEN_PARAM_VALUE", "From = "+pFrom+", To = "+pTo);
			}
			...
			{			
				...
				mbp1.setDataHandler( new DataHandler( new ByteArrayDataSource(pMessage, "text/html; charset=\"iso-8859-1\"") ) );
			...
			if (m_SmtpAuthentication)
			{
				SilverTrace.info("smtp", "SMTPListner.sendEmail()", "root.MSG_GEN_PARAM_VALUE", "m_Host = " + m_Host+ " m_Port="+m_Port+" m_User="+m_User);
		...
		{
			throw new NotificationServerException("SMTPListner.sendEmail()", SilverpeasException.ERROR, "smtp.EX_CANT_SEND_SMTP_MESSAGE", e);
		}
		finally
		{
			if (transport != null)
	...
	/*
	 * A finir, car il faut modifier le path de l'Upload de Silverpeas
	 * private void sendEmailWithAttachment(
	 * String pFrom,
	 * String pTo,
	 * String pSubject,
	 * String pMessage,
	 * String pAttachmentId,
	 * String pAttachmentName)
	 ...
	 * catch (AddressException e)
	 * {
	 * throw new NotificationServerException(e, "Invalid Address");
	 * }
	 * 
	 * email = new MimeMessage( session );
	 * email.setFrom( fromAddress );
	 ...
	 * DataSource source =
	 * new FileDataSource("C:\\Dev\\web\\Upload\\D"+pAttachmentId+"\\"+pAttachmentName);
