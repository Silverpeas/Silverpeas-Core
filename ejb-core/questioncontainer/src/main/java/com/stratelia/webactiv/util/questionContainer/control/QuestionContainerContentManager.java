...
public class QuestionContainerContentManager
{
	...
	public static int getSilverObjectId(String id, String peasId) throws ContentManagerException {
		SilverTrace.info("questionContainer","QuestionContainerContentManager.getSilverObjectId()", "root.MSG_GEN_ENTER_METHOD", "id = "+id);
   ...
   public static int createSilverContent(Connection con, QuestionContainerHeader qC, String userId, boolean isVisible) throws ContentManagerException
   {
	  ...
	  SilverTrace.info("questionContainer","QuestionContainerContentManager.createSilverContent()", "root.MSG_GEN_ENTER_METHOD", "SilverContentVisibility = "+scv.toString());
	  return getContentManager().addSilverContent(con, qC.getPK().getId(), qC.getPK().getComponentName(), userId, scv);
   }
   ...
   {
	  int	silverContentId = getContentManager().getSilverContentId(qC.getPK().getId(), qC.getPK().getComponentName());
	  if (silverContentId != -1) {
		  SilverContentVisibility scv = new SilverContentVisibility(isVisible);
		  setDateAttributes(scv, qC.getBeginDate(), qC.getEndDate());
		  SilverTrace.info("questionContainer","QuestionContainerContentManager.updateSilverContentVisibility()", "root.MSG_GEN_ENTER_METHOD", "SilverContentVisibility = "+scv.toString());
		  getContentManager().updateSilverContentVisibilityAttributes(scv, qC.getPK().getComponentName(), silverContentId);
   ...
   private static void setDateAttributes(SilverContentVisibility visibility, String startDate, String endDate) {
       String updatableStart = (startDate != null && !startDate.equals(""))? startDate : nullBeginDate;
       String updatableEnd   = (endDate != null &&   !endDate.equals(""))? endDate : nullEndDate;
   ...
   public static void deleteSilverContent(Connection con, QuestionContainerPK pk) throws ContentManagerException
   {
	  ...
	  SilverTrace.info("questionContainer","QuestionContainerContentManager.deleteSilverContent()", "root.MSG_GEN_ENTER_METHOD", "id = "+pk.getId()+", contentId = "+contentId);
	   ...
	   if (contentManager == null)
		{
		   try
			...
			{
			    SilverTrace.fatal("questionContainer", "QuestionContainerContentManager.getContentManager()", "root.EX_UNKNOWN_CONTENT_MANAGER", e);
			}
		}
