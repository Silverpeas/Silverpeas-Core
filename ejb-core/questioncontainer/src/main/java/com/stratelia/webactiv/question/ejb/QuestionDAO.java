...
package com.stratelia.webactiv.question.ejb;
...
public class QuestionDAO {
  ...
  private static Question getQuestionFromResultSet(ResultSet rs,
    ...
    // String instanceId = rs.getString(15); // pas utilisé mais dans la liste
    // des récupérations
    String style = rs.getString(16);

    Question result = new Question(new QuestionPK(id, questionPK), fatherId,
        label, description, clue, image, isQCM, type, isOpen, cluePenalty,
        maxTime, displayOrder, nbPointsMin, nbPointsMax, style);
  ...
  public static Question getQuestion(Connection con, QuestionPK questionPK)
      throws SQLException {
    SilverTrace.info("question", "QuestionDAO.getQuestion()",
        "root.MSG_GEN_ENTER_METHOD", "questionPK = " + questionPK);
    ResultSet rs = null;
    Question question = null;

    String selectStatement = "select " + QUESTIONCOLUMNNAMES + " from "
        + questionPK.getTableName() + " where questionId = ?";
  ...
  public static QuestionPK createQuestion(Connection con, Question question)
      throws SQLException {
    SilverTrace.info("question", "QuestionDAO.createQuestion()",
  ...
  public static void updateQuestion(Connection con, Question question)
      throws SQLException {
    SilverTrace.info("question", "QuestionDAO.updateQuestion()",
        "root.MSG_GEN_ENTER_METHOD", "question = " + question);
