...
public class NodeImportExport {
	...
	public NodeTreesType getTrees(List listComponentId) {
		...
		//Parcours de la liste des id des composants dont on veut récupérer l'arborescence de thèmes
		...
		while (itListComponentId.hasNext()) {
			...
			try {//Récupérarion du thème racine
				...
				throw new NodeRuntimeException("NodeImportExport.getNodeBm()",SilverpeasRuntimeException.ERROR,"node.GETTING_NODES_BY_FATHER_FAILED",ex);
			}
		}
	...
	private Collection getRecursiveTree(NodePK nodePK) throws RemoteException {
		...
		if ((listChildrenDetails != null) && (listChildrenDetails.size() != 0)) {//Il n'y pas d'exception jetée...
		...
		else listChildrenDetails = null;//Il n'y pas d'exception jetée... et en plus un arrayList vide est créé
	...
	private NodeBm getNodeBm() throws NodeRuntimeException {
		...
		if (nodeBm == null) {
			try {
				...
				throw new NodeRuntimeException("NodeImportExport.getNodeBm()",SilverpeasRuntimeException.ERROR,"root.EX_CANT_GET_REMOTE_OBJECT",e);
			}
		}
		return nodeBm;
	}
