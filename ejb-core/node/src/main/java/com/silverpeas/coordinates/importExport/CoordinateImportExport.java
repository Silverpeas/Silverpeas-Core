...
/**
 * Classe gérant la manipulation des axes de coordinates pour le module d'importExport.
 * @author dlesimple
 */
public class CoordinateImportExport {
	 ...
	 */
	public int addPositions(String componentId, String axisPath) throws CoordinateRuntimeException 
	{
		ArrayList combination = getArrayCombination(axisPath);
		SilverTrace.debug("coordinates", "CoordinateImportExport.addPositions", "root.MSG_GEN_PARAM_VALUE", "combination = " + combination);
		...
		try {
		  ...
		  while (it.hasNext()) {
	          ...
	          SilverTrace.info("coordinates","CoordinateImportExport.addPositions()", "root.MSG_GEN_PARAM_VALUE","nodeId = "+nodeId);
		      NodePK nodePK = new NodePK(nodeId, componentId);
	          SilverTrace.info("coordinates","CoordinateImportExport.addPositions()", "root.MSG_GEN_PARAM_VALUE","avant nodeBm.getPath() ! i = "+i);
		      Collection path = nodeBm.getPath(nodePK);
              SilverTrace.info("coordinates","CoordinateImportExport.addPositions()", "root.MSG_GEN_PARAM_VALUE","path for nodeId "+nodeId+" = "+path.toString());
		      ...
		      while (pathIt.hasNext()) {
		          ...
		          if (!anscestorId.equals("0")) {
		               if (anscestorId.equals(nodeId))
		                  point = new CoordinatePoint(-1, new Integer(anscestorId).intValue(), true, nodeLevel, i);
		               else
		                  point = new CoordinatePoint(-1, new Integer(anscestorId).intValue(), false, nodeLevel, i);
		               allnodes.add(point);
		          }
		      }
		      i++;
		  ...
		  coordinateId = getCoordinatesBm().addCoordinate(coordinatePK, allnodes);
                ...
                throw new CoordinateRuntimeException("CoordinateImportExport.addPositions()",SilverpeasRuntimeException.ERROR, "coordinates.ADDING_COORDINATES_COMBINATION_FAILED", e);
        }
		SilverTrace.debug("coordinates", "CoordinateImportExport.addPositions()", "root.MSG_GEN_PARAM_VALUE", "coordinateId = " + coordinateId);
		return coordinateId;
	}
	 ...
	 * @return nothing
	 */
	/*public void addNodesToPublication(PublicationPK pubPK, List nodes)
	{
		try {
		...
		{
            throw new CoordinateRuntimeException("CoordinateImportExport.addNodesToPublication()",SilverpeasRuntimeException.ERROR, "coordinates.ATTACHING_NODES_TO_PUBLICATION_FAILED", e);
		}
	}*/
	...
	{
		SilverTrace.debug("coordinates", "CoordinateImportExport.getNodeDetailByName()", "root.MSG_GEN_PARAM_VALUE", "name = " + name+" nodeRootId="+nodeRootId+" componentId="+componentId);
		try {
			NodeDetail nodeDetail = getNodeBm().getDetailByNameAndFatherId(new NodePK("useless", componentId), name, nodeRootId);
			return nodeDetail;
		} catch (Exception e)
		{
            throw new CoordinateRuntimeException("CoordinateImportExport.addNodesToPublication()",SilverpeasRuntimeException.ERROR, "coordinates.ATTACHING_NODES_TO_PUBLICATION_FAILED", e);
	 ...
	 */
	public ArrayList getCombinationLabels(ArrayList combination, String componentId) throws RemoteException
	{
	...
	public Collection getPathOfNode(NodePK nodePk) {
		...
		throw new NodeRuntimeException("NodeImportExport.getPathOfNode()",SilverpeasRuntimeException.ERROR,"node.GETTING_NODE_PATH_FAILED",ex);
		}
    ...
    public List getAxis(String componentId) {
        ...
        try {
      	  ...
      	  throw new CoordinateRuntimeException("CoordinateImportExport.getAxis()", SilverpeasRuntimeException.ERROR, "coordinates.EX_IMPOSSIBLE_DOBTENIR_LES_AXES", e);
        }
        return axis;
  }
     ...
     * @return
     */
    public List getAxisHeadersWithChildren(String componentId, boolean includeUnclassified, boolean takeAxisInChildrenList) {
		ResourceLocator nodeSettings = new ResourceLocator("com.stratelia.webactiv.util.node.nodeSettings", "");
		String sortField = nodeSettings.getString("sortField", "nodepath");
		String sortOrder = nodeSettings.getString("sortOrder", "asc");
		List axis = new ArrayList();
		try {
		  ...
		  for (int h=0; h<headers.size(); h++)
		  {
			  ...
			  if (new Integer(header.getNodePK().getId()).intValue() > 1 && includeUnclassified)
			  {
		  ...
		  throw new CoordinateRuntimeException("CoordinateImportExport.getAxisHeadersWithChildren()", SilverpeasRuntimeException.ERROR, "coordinates.EX_IMPOSSIBLE_DOBTENIR_LES_AXES", e);
		}
		return axis;
    }
    ...
    public List getAxisChildren(NodePK nodePK, boolean takeAxisInChildrenList) {
		...
		try {
		  ...
		  throw new CoordinateRuntimeException("CoordinateImportExport.getAxisChildren()", SilverpeasRuntimeException.ERROR, "coordinates.EX_IMPOSSIBLE_DOBTENIR_LES_VALEURS_DUN_AXE", e);
		}
		return children;
    }
    ...
    public List getAxisHeaders(String componentId) {
        ...
        try {
            axisHeaders = getNodeBm().getHeadersByLevel(new NodePK("useless",componentId),2);
        } catch (Exception e) {
	          throw new CoordinateRuntimeException("CoordinateImportExport.getAxisHeaders()", SilverpeasRuntimeException.ERROR, "coordinates.EX_IMPOSSIBLE_DOBTENIR_LES_ENTETES_DES_AXES", e);
        }
        return axisHeaders;
  }
	...
	{
		SilverTrace.info("coordinates","CoordinateImportExport.addPosition()", "root.MSG_GEN_PARAM_VALUE","fatherId = "+axisId+" And position = "+position.toString());
        ...
        SilverTrace.info("coordinates","CoordinateImportExport.addPosition()", "root.MSG_GEN_PARAM_VALUE","fatherDetail = "+fatherDetail.toString());
        try {
        	fatherDetail = getNodeHeader(axisId, componentId);
        	positionPK = getNodeBm().createNode(position, fatherDetail);
            SilverTrace.info("coordinates","CoordinateImportExport.addPosition()", "root.MSG_GEN_PARAM_VALUE","positionPK = "+positionPK.toString());
        	positionDetail = getNodeHeader(positionPK);
            SilverTrace.info("coordinates","CoordinateImportExport.addPosition()", "root.MSG_GEN_PARAM_VALUE","positionDetail = "+positionDetail.toString());
        } catch (Exception e) {
	            throw new CoordinateRuntimeException("CoordinateImportExport.addPosition()",SilverpeasRuntimeException.ERROR, "coordinates.EX_IMPOSSIBLE_DAJOUTER_UNE_COMPOSANTE_A_L_AXE", e);
        }
	...
	public NodeDetail getNodeHeader(NodePK pk) {
	    ...
	    try {
	    	...
	    	throw new CoordinateRuntimeException("CoordinateImportExport.getNodeHeader()", SilverpeasRuntimeException.ERROR, "coordinates.EX_IMPOSSIBLE_DOBTENIR_LE_NOEUD", e);
	    }
	    return nodeDetail;
	}
	...
	private CoordinatesBm getCoordinatesBm()
	{
		...
		try {
			CoordinatesBmHome kscEjbHome	=	(CoordinatesBmHome) EJBUtilitaire.getEJBObjectRef(JNDINames.COORDINATESBM_EJBHOME, CoordinatesBmHome.class);
			coordinatesBm			=	kscEjbHome.create();
		}
		catch (Exception e) {
			  throw new CoordinateRuntimeException("CoordinateImportExport.getCoordinatesBm()",SilverpeasRuntimeException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
	...
	private NodeBm getNodeBm() {
	  ...
	  try {
		  ...
		  throw new CoordinateRuntimeException("CoordinateImportExport.getNodeBm()",SilverpeasRuntimeException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
	  }
	  return nodeBm;
	}
	...
	/*private PublicationBm getPublicationBm() {
		...
		try {
		  ...
		  throw new CoordinateRuntimeException("CoordinateImportExport.getPublicationBm()",SilverpeasRuntimeException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
	  }
	...
	/*public Collection getUnbalancedPublications(String componentId) {
	    ...
	    try {
            ...
            throw new CoordinateRuntimeException("CoordinateImportExport.getUnbalancedPublications()",SilverpeasRuntimeException.ERROR, "importExport.EX_IMPOSSIBLE_DOBTENIR_LA_LISTE_DES_PUBLICATIONS_NON_CLASSEES", e);
        }
	    return publications;
    }*/
	 ...
	 * @return
	 */
	public List coupleIds (List filesNames, List nodeIds, int cur, int loop, int nTuple, String prefixeId, int nbAxis)
	{
	   String tmp;
