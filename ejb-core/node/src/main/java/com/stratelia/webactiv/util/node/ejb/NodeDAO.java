...
public class NodeDAO
{
	...
	public static boolean isSameNameSameLevelOnCreation(Connection con, NodeDetail nd) throws SQLException
	{
		...
		catch (SQLException e)
		{
			SilverTrace.error(
				"node",
				"NodeDAO.isSameNameSameLevelOnCreation()",
	 ...
	 * @since 1.0
	 */
	public static boolean isSameNameSameLevelOnUpdate(Connection con, NodeDetail nd) throws SQLException
	{
		int nbItems = 0;
		...
		catch (SQLException e)
		{
			SilverTrace.error(
				"node",
				"NodeDAO.isSameNameSameLevelOnUpdate()",
				"root.EX_SQL_QUERY_FAILED",
				"selectQuery = "
					+ selectQuery.toString()
					+ " id = "
	...
	public static Collection getChildrenPKs(Connection con, NodePK nodePK) throws SQLException
	{
		...
		catch (SQLException e)
		{
			SilverTrace.error(
				"node",
				"NodeDAO.getChildrenPKs()",
				"root.EX_SQL_QUERY_FAILED",
				"childrenStatement = "
					+ childrenStatement.toString()
					+ " id = "
					+ nodePK.getId()
					+ " compo name = "
	...
	*/
	public static Collection getDescendantPKs(Connection con, NodePK nodePK) throws SQLException
	{
		...
		String path = null;
		StringBuffer selectQuery = new StringBuffer();
		selectQuery.append("select nodePath from ").append(nodePK.getTableName()).append(" where nodeId = ? and instanceId = ? ");
		PreparedStatement prepStmt = null;
		ResultSet rs = null;
		...
		if (path != null)
		{
			...
			catch (SQLException e)
			{
				SilverTrace.error(
					...
					"NodeDAO.getDescendantPKs()",
					"root.EX_SQL_QUERY_FAILED",
					"selectQuery = " + selectQuery.toString() + " compo name = " + nodePK.getComponentName(),
					e);
				throw e;
			}
			finally
	...
	public static List getDescendantDetails(Connection con, NodeDetail node) throws SQLException
	{
		...
		{
			SilverTrace.error("node", "NodeDAO.getDescendantDetails()", "root.EX_SQL_QUERY_FAILED", "selectQuery = " + selectQuery.toString() + " compo name = " + node.getNodePK().getComponentName(),	e);
			throw e;
		}
		finally
		{
	 ...
	 * @since 1.6
	 */
	public static List getHeadersByLevel(Connection con, NodePK nodePK, int level) throws SQLException
	{
		...
		catch (SQLException e)
		{
			SilverTrace.error(
				"node",
				"NodeDAO.getHeadersByLevel()",
				"root.EX_SQL_QUERY_FAILED",
				"nodeStatement = " + nodeStatement.toString(),
				e);
			throw e;
		}
		finally
	...
	public static List getAllHeaders(Connection con, NodePK nodePK, String sorting) throws SQLException
	{
		...
		try
		{
			...
			while (rs.next())
			{
	            ...
	            //Add default translation
	            NodeI18NDetail nodeI18NDetail = new NodeI18NDetail(nd.getLanguage(), nd.getName(), nd.getDescription());
	...
	{
		SilverTrace.info("node", "NodeDAO.getSubTree()", "root.MSG_GEN_ENTER_METHOD", "nodePK = " + nodePK+", status = "+status);
	...
	private static List getSubTree(Connection con, List tree, NodePK nodePK, String status) throws SQLException
	{
...
/*
		...
		nodeStatement.append(" on (n1.nodeId = n2.nodeId)");
		nodeStatement.append(" where instanceId ='").append(nodePK.getComponentName()).append("'");
	...
	public static Collection getChildrenDetails(Connection con, NodePK nodePK, String sorting) throws SQLException
	{
...
/*		childrenStatement.append("select * from ").append(nodePK.getTableName()).append(" n1 ");
	...
	public static int getChildrenNumber(Connection con, NodePK nodePK) throws SQLException
	{
		...
		StringBuffer selectQuery = new StringBuffer();
		selectQuery.append("select count(*) from ").append(nodePK.getTableName()).append(" where nodeFatherId = ? and instanceId = ?");
	...
	public static NodePK insertRow(Connection con, NodeDetail nd) throws SQLException
	{
		...
		SilverTrace.info("node", "NodeDAO.insertRow()", "root.MSG_GEN_PARAM_VALUE", "fatherId = "+nd.getFatherPK().getId()+", nbBrothers = " + nbBrothers);
		...
		insertQuery.append("insert into ").append(nd.getNodePK().getTableName()).append(" values ( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?, ? , ?, ? , ?)");
		PreparedStatement prepStmt = null;
		try
		{
	...
	public static NodePK selectByNameAndFatherId(Connection con, NodePK pk, String name, int nodeFatherId) throws SQLException
	{
	 ...
	 * @since 1.0
	 */
	public static NodeDetail loadRow(Connection con, NodePK nodePK, boolean getTranslations) throws SQLException
	{
		SilverTrace.info("node", "NodeDAO.loadRow()", "root.MSG_GEN_PARAM_VALUE", "nodePK = " + nodePK);
		NodeDetail detail = null;
		StringBuffer selectQuery = new StringBuffer();
		selectQuery.append("select * from ").append(nodePK.getTableName());
		selectQuery.append(" where nodeId=").append(nodePK.getId());
		selectQuery.append(" and instanceId='").append(nodePK.getComponentName()).append("'");
		SilverTrace.info("node", "NodeDAO.loadRow()", "root.MSG_GEN_PARAM_VALUE", "selectQuery = " + selectQuery.toString());
		...
		try
		{
			...
			{
				//throw new NoSuchEntityException("Row for id " + nodePK.getId() + " not found in database.");
				throw new NodeRuntimeException(
					"NodeDAO.loadRow()",
					SilverpeasRuntimeException.ERROR,
	...
	public static NodeDetail loadRow(Connection con, NodePK nodePK,String name, int nodeFatherId) throws SQLException
	{
		SilverTrace.info("node", "NodeDAO.loadRow()", "root.MSG_GEN_PARAM_VALUE", "nodePK = " + nodePK);
		NodeDetail detail = null;
		StringBuffer selectQuery = new StringBuffer();
		selectQuery.append("select * from ").append(nodePK.getTableName());
		selectQuery.append(" where lower(nodename)='").append(name.toLowerCase()).append("'");
		selectQuery.append(" and instanceId='").append(nodePK.getComponentName()).append("'");
		selectQuery.append(" and nodefatherid=").append(nodeFatherId);
		SilverTrace.info("node", "NodeDAO.loadRow()", "root.MSG_GEN_PARAM_VALUE", "selectQuery = " + selectQuery.toString());
	...
	public static void storeRow(Connection con, NodeDetail nodeDetail) throws SQLException
	{
		...
		updateStatement.append(" nodeLevelNumber = ? , nodeFatherId = ? , modelId = ? , nodeStatus = ? , orderNumber = ?, lang = ?, rightsDependsOn = ? ");
	...
	public static void moveNode(Connection con, NodeDetail nodeDetail) throws SQLException
	{
		...
		updateStatement.append(" nodeLevelNumber = ? , nodeFatherId = ? , instanceId = ? , orderNumber = ?, rightsDependsOn = ? ");
	...
	public static void updateRightsDependency(Connection con, NodePK pk, int rightsDependsOn) throws SQLException
	{
		SilverTrace.info("node", "NodeDAO.updateRightsDependency()", "root.MSG_GEN_ENTER_METHOD", "nodePK = " + pk.toString()+", rightsDependsOn = "+rightsDependsOn);
