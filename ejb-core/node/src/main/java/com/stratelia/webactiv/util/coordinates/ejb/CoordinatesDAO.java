...
public class CoordinatesDAO
{
	 ...
	 * @see
	 */
	private static ArrayList selectCoordinatePointsByNodeIds(Connection con, ArrayList fatherIds, CoordinatePK pk)
		throws SQLException
	{
		...
		int begin)
	{
		...
		while (i < toCheck.size() && coordinateMatch)
		{
			...
			if (currentRId == currentCoordinateId && currentNId == ((Integer) fatherIds.get(f)).intValue())
			{
	 ...
	 * @see
	 */
	public static Collection selectByFatherIds(Connection con, ArrayList fatherIds, CoordinatePK pk)
		throws SQLException
	{
		...
		SilverTrace.info(
			"coordinates",
			"CoordinatesDAO.selectByFatherIds()",
			"root.MSG_GEN_PARAM_VALUE",
			"fatherIds = " + fatherIds.toString() + " | points = " + points.toString());
		...
		while (i < points.size())
		{
			...
			if (point.getNodeId() == ((Integer) fatherIds.get(0)).intValue())
			{
				...
				nbMatchingPoints = getNbMatchingCoordinates(currentCoordinateId, fatherIds, toCheck, i + 1);
	...
	public static Collection selectByFatherPaths(Connection con, ArrayList fatherPaths, CoordinatePK pk)
		throws SQLException
	{
	...
	private static ArrayList selectCoordinateIdsByNodeIds(Connection con, ArrayList fatherPaths, CoordinatePK pk)
		throws SQLException
	{
		ResultSet rs = null;
	 ...
	 * @see
	 */
	private static void addCoordinatePoint(Connection con, CoordinatePK pk, CoordinatePoint point, int coordinateId)
		throws SQLException
	{
		String insertQuery = "insert into " + pk.getTableName() + " values ( ? , ? , ? , ? , ?)";
		PreparedStatement prepStmt = null;
	 ...
	 * @see
	 */
	public static int addCoordinate(Connection con, CoordinatePK pk, ArrayList coordinatePoints) throws SQLException
	{
		SilverTrace.info(
			"coordinates",
			"CoordinatesDAO.addCoordinate()",
			"root.MSG_GEN_PARAM_VALUE",
		...
		while (it.hasNext())
		{
			...
			SilverTrace.info(
				...
				"CoordinatesDAO.addCoordinate()",
				"root.MSG_GEN_PARAM_VALUE",
				"Try to insert point = " + point.toString());
			addCoordinatePoint(con, pk, point, coordinateId);
			SilverTrace.info(
				"coordinates",
				"CoordinatesDAO.addCoordinate()",
				"root.MSG_GEN_PARAM_VALUE",
				"insertion of point = " + point.toString() + " succeeded !");
		}
		return coordinateId;
	}
	 ...
	 * @see
	 */
	public static void removeCoordinates(Connection con, CoordinatePK pk, ArrayList coordinateIds) throws SQLException
	{
		SilverTrace.info(
			"coordinates",
			"CoordinatesDAO.removeCoordinates()",
			"root.MSG_GEN_PARAM_VALUE",
			"pk = " + pk.toString() + " and coordinateIds = " + coordinateIds.toString());
	 ...
	 * @see
	 */
	public static void removeCoordinatesByPoints(Connection con, CoordinatePK pk, ArrayList coordinatePoints)
		throws SQLException
	{
		SilverTrace.info(
			"coordinates",
	...
	private static int getMaxCoordinateId(Connection con, CoordinatePK pk) throws SQLException
	{
		...
		try
		{
			String nextPKStatement =
				"SELECT MAX(coordinatesId) FROM "
					+ pk.getTableName()
					+ " where instanceId='"
					+ pk.getComponentName()
					+ "'";
	...
	private static int getMaxDisplayOrder(Connection con, CoordinatePK pk) throws SQLException
	{
		...
		{
			String nextPKStatement =
				"SELECT MAX(coordinatesDisplayOrder) FROM "
					+ pk.getTableName()
					+ " where instanceId='"
					+ pk.getComponentName()
					+ "'";
	...
	private static Coordinate selectCoordinateByCoordinatePK(Connection con, CoordinatePK pk) throws SQLException
	{
		...
		String selectQuery =
			"select * "
				...
				+ " and coordinatesLeaf = ? "
				+ " and instanceId = ? "
				+ " order by coordinatesDisplayOrder ";
	 ...
	 * @see
	 */
	public static ArrayList selectCoordinatesByCoordinateIds(Connection con, ArrayList coordinateIds, CoordinatePK pk)
		throws SQLException
	{
		Iterator it = coordinateIds.iterator();
		...
		{
			coordinateId = (String) it.next();
			coordinate = selectCoordinateByCoordinatePK(con, new CoordinatePK(coordinateId, pk));
	 ...
	 * @see
	 */
	private static Collection getCoordinateIds(Connection con, CoordinatePK pk) throws SQLException
	{

		ArrayList coordinateIds = new ArrayList();
		String selectQuery =
			"select distinct(coordinatesId) "
				+ "from "
				+ pk.getTableName()
				+ " where instanceId ='"
				+ pk.getComponentName()
	 ...
	 * @see
	 */
	public static void addPointToAllCoordinates(Connection con, CoordinatePK pk, CoordinatePoint point)
		throws SQLException
	{
		SilverTrace.info(
			"coordinates",
			...
			"root.MSG_GEN_PARAM_VALUE",
			"pk = " + pk.toString() + " and point = " + point.toString());
	 ...
	 * @see
	 */
	public static Collection getCoordinateIdsByNodeId(Connection con, CoordinatePK pk, String nodeId)
		throws SQLException
	{
