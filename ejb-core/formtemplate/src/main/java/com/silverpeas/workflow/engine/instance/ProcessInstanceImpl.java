...
/**
 * This class is one implementation of interface UpdatableProcessInstance. It
 * uses Castor library to read/write process instance information in database
 * 
...
// TimeStampable
{
  ...
  /**
   * Flag that indicates if this instance is in an active state for a long long
   * time
   * 
  ...
  /**
   * Vector of all history step that trace events occured on this process
   * instance
   * 
  ...
  /**
   * the status of this instance regarding 'undo' process while true, the atomic
   * operations are not stored anymore
   */
   ...
   * 
   * @param state
   *          The name of state to be activated
   */
  private void addActiveState(String state) throws WorkflowException {
    ...
    if (this.currentStep != null
        && this.currentStep.getAction().equals("#question#"))
   ...
   * 
   * @param state
   *          The name of state to be desactivated
   */
  ...
  public void removeTimeout(State state) throws WorkflowException {
    ...
    for (int i = 0; i < activeStates.size(); i++) {
      activeState = (ActiveState) activeStates.get(i);
      SilverTrace
          .debug("workflowEngine", "ProcessInstanceImpl.removeTimeout",
   ...
   *          user to add
   * @param state
   *          state for which the user can make an action
   * @param role
   *          role name under which the user can make an action
   */
   ...
   *          user to add
   * @param state
   *          name of state for which the user can make an action
   * @param role
   *          role name under which the user can make an action
   */
   ...
   *          user to remove
   * @param state
   *          state for which the user could make an action
   * @param role
   *          role name under which the user could make an action
   */
   ...
   *          user to remove
   * @param state
   *          name of state for which the user could make an action
   * @param role
   *          role name under which the user could make an action
   */
   ...
   *          user to add
   * @param state
   *          the name of state for which the user is interested
   * @param role
   *          role name under which the user is interested
   */
  ...
  public String create() throws WorkflowException {
    ...
    try {
      ...
      throw new WorkflowException("ProcessInstanceImpl.create",
          "workflowEngine.EX_ERR_CASTOR_CREATE_INSTANCE", pe);
    } finally {
  ...
  public void delete() throws WorkflowException {
    ...
    try {
      ...
      throw new WorkflowException("ProcessInstanceImpl.delete",
          "workflowEngine.EX_ERR_CASTOR_DELETE_INSTANCE", pe);
    } finally {
      WorkflowJDOManager.closeDatabase(db);
    }
  }
  ...
  public void update() throws WorkflowException {
    ...
    try {
      ...
      throw new WorkflowException("ProcessInstanceImpl.update",
          "workflowEngine.EX_ERR_CASTOR_UPDATE_INSTANCE", pe);
    } finally {
      WorkflowJDOManager.closeDatabase(db);
    }
  }
  ...
  public Vector getParticipants() throws WorkflowException {
    ...
    for (int i = 0; i < historySteps.size(); i++) {
      ...
      ParticipantImpl participant = new ParticipantImpl(user, step
          .getUserRoleName(), state, step.getAction());
    ...
    if (folder == null) {
      ...
      try {
        ...
        throw new WorkflowException("ProcessInstanceImpl",
            "workflowEngine.EXP_UNKNOWN_FOLDER", "folder=" + folderId, e);
      }
    }
  ...
  private void createFolder() throws WorkflowException {
    ...
    try {
      ...
      throw new WorkflowException("ProcessInstanceImpl",
          "workflowEngine.EXP_FOLDER_CREATE_FAILED", "folder=" + folderId, e);
    }
  }
  ...
  private void updateFolder(DataRecord actionData) throws WorkflowException {
    try {
      ...
      throw new WorkflowException("ProcessInstanceImpl",
          "workflowEngine.EXP_FOLDER_UPDATE_FAILED", "folder=" + instanceId, e);
    }
  }
  ...
  public Field getField(String fieldName) throws WorkflowException {
    ...
    if (folder == null)
      throw new WorkflowException("ProcessInstanceImpl.getField",
          "workflowEngine.EX_ERR_GET_FOLDER");
  ...
  public DataRecord getFormRecord(String formName, String role, String lang)
    ...
    try {
      ...
      DataRecord data = form.getDefaultRecord(role, lang, getAllDataRecord(
          role, lang));
      ...
      throw new WorkflowException("ProcessInstanceImpl",
          "workflowEngine.EXP_FORM_READ_FAILED", formName, e);
    }
  }
  ...
  /**
   * Get the most recent step where an action has been performed on the given
   * state. If no action has been performed on this state, return the step that
   * activate this state.
   */
  ...
  /**
   * Returns the most recent step where an action was performed on the given
   * state.
   * 
   * @param stateName
   *          name of state for which we want the most recent step
  ...
  private HistoryStep getMostRecentStepOnState(String stateName) {
    ...
    for (int i = 0; i < historySteps.size(); i++) {
      ...
      if (stateName == null || stateName.length() == 0) {
        if (step.getResolvedState() == null
            || step.getResolvedState().length() == 0) {
          stepMatch = true;
        }
      } else if (step.getResolvedState() != null
          && step.getResolvedState().equals(stateName)) {
        stepMatch = true;
      }
  ...
  /**
   * Returns the most recent step where an action caused the activation of the
   * given state
   * 
   * @param stateName
   *          name of state
   * @return the most recent step where an action caused the activation of the
   *         given state
   */
  private HistoryStep getOriginStep(String stateName) throws WorkflowException {
    ...
    try {
      ...
      throw new WorkflowException("ProcessInstanceManagerImpl.undoStep",
          "workflowEngine.EX_ERR_UNDO_STEP", we);
    } catch (PersistenceException pe) {
      throw new WorkflowException(
          "ProcessInstanceManagerImpl.getProcessInstances",
          "workflowEngine.EX_ERR_CASTOR_UNDO_STEP", pe);
    } finally {
      WorkflowJDOManager.closeDatabase(db);
    }
  ...
  public void lock() throws WorkflowException {
    ...
    if (locked)
      throw new WorkflowException("ProcessInstanceImpl.lock()",
          "workflowEngine.EX_ERR_INSTANCE_ALREADY_LOCKED");
  ...
  /**
   * Computes tuples role/user/state (stored in an Actor object) from a
   * QualifiedUsers object
   * 
   ...
   *          action description
   * @param params
   *          params concatenated as "param1##param2...paramN"
   */
  private void addUndoHistoryStep(String action, String params)
    ...
    try {
      ...
      throw new WorkflowException("ProcessInstanceImpl.addUndoHistoryStep",
          "workflowEngine.EX_ERR_CASTOR_CREATE_UNDO_HISTORYSTEP", pe);
    } finally {
      WorkflowJDOManager.closeDatabase(db);
    }
  }
   ...
   * 
   * @param historyStep
   *          the historyStep when the atomic operations had occured
   */
    ...
    try {
      ...
      while (results.hasMore()) {
        ...
        if (action.equals("addActiveState")) {
          ...
          throw new WorkflowException("ProcessInstanceManagerImpl.undoStep",
              "workflowEngine.EXP_UNKNOWN_ACTION");
  ...
  /**
   * Cancel all the atomic operations since the step where first action had
   * occured
   * 
   * @param state
   *          the name of state where ac action has been discussed
   * @param actionDate
   *          date of state re-resolving
   */
  ...
  public HistoryStep[] getBackSteps(User user, String roleName, String stateName)
    ...
    try {
      ...
      throw new WorkflowException("ProcessInstanceImpl.getBackSteps",
          "workflowEngine.EX_ERR_CASTOR_GET_BACKSTEPS", pe);
    } finally {
      WorkflowJDOManager.closeDatabase(db);
    }
  }
  ...
  private HistoryStep getStep(String stepId) throws WorkflowException {
    ...
    if (foundStep == null)
      throw new WorkflowException("ProcessInstanceImpl.getStep",
          "workflowEngine.EX_ERR_HISTORYSTEP_NOT_FOUND", "id : " + stepId);
  ...
  public State addQuestion(String content, String stepId, State fromState,
    ...
    QuestionImpl question = new QuestionImpl((ProcessInstance) this, content,
        fromState.getName(), step.getResolvedState(), fromUser, participant
            .getUser());
   ...
   * 
   * 1 - make a fictive answer 2 - remove active state 3 - remove working user 4
   * - recurse in question target state, if questions have been asked in cascade
   * 
   * @param question
   *          the question to cancel
   */
