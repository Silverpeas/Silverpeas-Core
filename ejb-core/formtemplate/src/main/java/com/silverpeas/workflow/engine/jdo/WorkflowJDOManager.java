...
/**
 * This class offers services about database persistence.
...
public class WorkflowJDOManager
{
	 ...
	 */
	private static ResourceLocator settings = new ResourceLocator("com.silverpeas.workflow.engine.castorSettings", "fr");
	 ...
	 * @return Database object
	 */
	static public Database getDatabase(boolean fast)
		throws WorkflowException
	{
		...
		if (db == null)
		{
			if (jdo == null)
			{
				...
				try {
					...
					throw new WorkflowException("JDOManager.getDatabase", "EX_ERR_CASTOR_DATABASE_MAPPING_ERROR", me);
				}
			}
			...
			{
				throw new WorkflowException("JDOManager.getDatabase", "EX_ERR_CASTOR_DATABASE_NOT_FOUND", dbnfe);
			}
			catch (PersistenceException pe)
			{
				throw new WorkflowException("JDOManager.getDatabase", "EX_ERR_CASTOR_GET_DATABASE", pe);
			}
		}
	...
	static public void closeDatabase(Database db)
	{
		if (db != null)
		{
			...
			{
				SilverTrace.warn("workflowEngine", "JDOManager.closeDatabase", "root.EX_CONNECTION_CLOSE_FAILED", pe);
			}
		}
	}
}