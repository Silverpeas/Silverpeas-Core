...
public class UserManagerImpl implements UserManager
{
	...
	*/
		...
		throws WorkflowException
	{
	   ...
	   try
	   {
	      // the modelId is the peasId.
          ComponentInst peas = admin.getComponentInst(modelId);
          userDetails = organizationController.getUsers(
		     peas.getDomainFatherId(), modelId, roleName);
	   }
	   catch (AdminException e)
	   {
	   ...
	   if (userDetails == null) userDetails = new UserDetail[0];
      ...
      throws WorkflowException
   {
      ...
      if (userDetail == null)		
	  {
	     throw new UnknownUserException("UserManagerImpl.getUserDetail",
		                                userId);
	  }
	  return userDetail;
   }
	...
	*/
      ...
      throws WorkflowException
	{
		...
		if (info==null)
			throw new WorkflowException("UserManagerImpl.getRelatedUser",
		  		                      "workflowEngine.EXP_USERINFO_NOT_FOUND", "user id : " + user.getUserId() + ", info name : " + relation);
	...
	public UserSettings getUserSettings(String userId, String peasId) throws WorkflowException
	{
		...
		if (settings == null)
		{
			try
			{
				...
				query = db.getOQLQuery( "SELECT settings FROM com.silverpeas.workflow.engine.user.UserSettingsImpl settings WHERE userId = $1 AND peasId = $2");
			...
			{
				throw new WorkflowException("UserManagerImpl.getUserSettings", "EX_ERR_CASTOR_GET_USER_SETTINGS", pe);
			}
			finally
			{
