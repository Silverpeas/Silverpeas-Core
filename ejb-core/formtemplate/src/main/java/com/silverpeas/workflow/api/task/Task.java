...
/**
 * A task object is an activity description built by the workflow engine and
 * sent via the taskManager to an external system which will notify the end user
 * and manage the task realisation.
 * 
 * Task objects will be created by the workflow engine when a new task is
 * assigned to a user. Task objects will be created too for the ProcessManager
 * GUI which will be used by the user to do the assigned activity.
 */
public interface Task {
   ...
   * 
   * The id of this workflow internal information must be stored by the external
   * system to be sent to the workflow engine when the activity is done.
   */
   ...
   * 
   * The name of this workflow internal information must be stored by the
   * external system to be sent to the workflow engine when the activity is
   * done.
   */
  public State getState();

  /**
   * Returns the history steps that user can discussed (ask a question to the
   * actor of that step).
   */
  ...
  /**
   * When this Task is done, builds a TaskDoneEvent giving the choosed action
   * name and the filled form.
   */
  public TaskDoneEvent buildTaskDoneEvent(String actionName, DataRecord data);

  /**
   * When this Question is asked for a task, builds a QuestionEvent giving the
   * choosed step that must give the answer.
   */
  public QuestionEvent buildQuestionEvent(String stepId, DataRecord data);

  /**
   * When this Response is answer for a task, builds a ResponseEvent giving the
   * question id that must give the answer.
   */
