 ...
 * @see FieldDisplayer
 */
public class TextFieldDisplayer implements FieldDisplayer
{
		...
		if (! template.getTypeName().equals(TextField.TYPE))
		{
	 	...
	 	if (contentType != null)
	 	{
	 		if (contentType.equals(TextField.CONTENT_TYPE_INT))
	 		{
				...
				out.println("			errorMsg+=\"  - '" + label + "' " + Util.getString("GML.MustContainsNumber", language) + "\\n\";");
				out.println("			errorNb++;");
	 			out.println("		}");
	 		}
	 		else if (contentType.equals(TextField.CONTENT_TYPE_FLOAT))
	 		{
	 			...
	 			out.println("		if (field.value != \"\" && !(/^([+-]?(((\\d+(\\.)?)|(\\d*\\.\\d+))([eE][+-]?\\d+)?))$/.test(field.value))) {");
