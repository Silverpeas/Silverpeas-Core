 ...
 * @see FieldDisplayer
 */
public class WysiwygFCKFieldDisplayer implements FieldDisplayer
{
	public static final String dbKey = "xmlWysiwygField_";
	public static final String dir = "xmlWysiwyg";
	
	private static final ResourceLocator settings = new ResourceLocator("com.stratelia.silverpeas.wysiwyg.settings.wysiwygSettings", "");
  ...
  public void display(PrintWriter out,
			...
			else 
			{
				...
				String configFile = SilverpeasSettings.readString(settings, "configFile", Util.getPath()+"/wysiwyg/jsp/javaScript/myconfig.js");
   ...
   */
   public void update(String newValue, Field field, FieldTemplate template, PagesContext pageContext) throws FormException 
   {
	   if (! field.getTypeName().equals(TextField.TYPE))
	   ...
	   if (field.acceptValue(newValue, pageContext.getLanguage()))
	   {
		   ...
		   try {
			   ...
			   String fileName = setContentIntoFile(pageContext.getComponentId(), pageContext.getObjectId(), template.getFieldName(), newValue, contentLanguage);
   ...
   public void cloneContents(String componentIdFrom, String objectIdFrom, String componentIdTo, String objectIdTo) throws UtilException, IOException
   {
	   String[] dirs = new String[1];
	   dirs[0] = dir;
			   ...
			   if (fileName.startsWith(objectIdFrom+"_"))
			   {
				   ...
				   while (languages.hasNext())
				   {
					   ...
					   if (fieldName.startsWith(language+"_"))
					   {
						   ...
						   FileRepositoryManager.copyFile(fromPath+file.getName(), toPath+getFileName(fieldName, objectIdTo, language));
					   }
				   }
			   }
		   }
