...
public class PublicationTemplateImpl implements PublicationTemplate
{
  ...
  /**
   * Returns the Form witch name is name parameter
   * the records built from this template.
   */
  public Form getEditForm(String name)
     throws PublicationTemplateException
	 {
	 	SilverTrace.info("form", "PublicationTemplateImpl.getEditForm", "root.MSG_GEN_PARAM_VALUE", "name="+name);
		Form form = null;
		...
		if (form == null)
			throw new PublicationTemplateException("PublicationTemplateImpl.getEditForm", "form.EX_CANT_GET_FORM", "name="+name,null);
		return form;
	 }
	...
	throws PublicationTemplateException
  {
		...
		{
			String htmlFileName = PublicationTemplateManager.makePath(PublicationTemplateManager.templateDir, fileName);
  ...
  throws PublicationTemplateException
{
  ...
  public ArrayList getTemplatesObj()
  {
	 ...
	 */
	private RecordTemplate loadRecordTemplate(String xmlFileName) throws PublicationTemplateException
	{
		...
		{
			// Format these url
			xmlFileName 	= PublicationTemplateManager.makePath(PublicationTemplateManager.templateDir, xmlFileName);
			...
			GenericRecordTemplate recordTemplate = (GenericRecordTemplate) unmar.unmarshal(new InputSource(new FileInputStream(xmlFileName)));
		...
		{
			throw new PublicationTemplateException("PublicationTemplateImpl.loadPublicationTemplate", "form.EX_ERR_CASTOR_LOAD_XML_MAPPING", "Publication Template FileName : " + xmlFileName, me);
		}
		catch (MarshalException me)
		{
			throw new PublicationTemplateException("PublicationTemplateImpl.loadPublicationTemplate", "form.EX_ERR_CASTOR_UNMARSHALL_PUBLICATION_TEMPLATE", "Publication Template FileName : " + xmlFileName, me);
		}
		catch (ValidationException ve)
		{
			throw new PublicationTemplateException("PublicationTemplateImpl.loadPublicationTemplate", "form.EX_ERR_CASTOR_INVALID_XML_PUBLICATION_TEMPLATE", "Publication Template FileName : " + xmlFileName, ve);
		}
		catch (IOException ioe)
		{
			throw new PublicationTemplateException("PublicationTemplateImpl.loadPublicationTemplate", "form.EX_ERR_CASTOR_LOAD_PUBLICATION_TEMPLATE", "Publication Template FileName : " + xmlFileName, ioe);
		}
	}
	
	public void saveRecordTemplates() throws PublicationTemplateException
	{
		String subDir = fileName.substring(0, fileName.lastIndexOf("."))+File.separator;
		if (template != null)
	...
	private void saveRecordTemplate(RecordTemplate recordTemplate, String subDir, String xmlFileName) throws PublicationTemplateException
	{
		...
		{
			// Format these url
			String xmlDirPath 	= PublicationTemplateManager.makePath(PublicationTemplateManager.templateDir, subDir);
			File dir = new File(xmlDirPath);
			if (!dir.exists())
				try {
					FileFolderManager.createFolder(dir);
				} catch (UtilException e) {
					throw new PublicationTemplateException("PublicationTemplateImpl.saveRecordTemplate", "form.EX_ERR_CASTOR_SAVE_PUBLICATION_TEMPLATE", "xmlDirPath = " + xmlDirPath, e);
				}
