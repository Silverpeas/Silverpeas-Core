...
public class TagCloudDAO
{
	...
	private static final String ALL_COLUMNS = COLUMN_ID + ", " + COLUMN_TAG + ", " + COLUMN_LABEL
		+ ", " + COLUMN_INSTANCEID + ", " + COLUMN_EXTERNALID + ", " + COLUMN_EXTERNALTYPE;
	...
	public static void createTagCloud(Connection con, TagCloud tagCloud) throws SQLException
	{
		...
		{
			SilverTrace.warn(
				"tagCloud", "TagCloudDAO.createTagCloud", "root.EX_PK_GENERATION_FAILED", e);
		}
	...
	public static void deleteTagCloud(Connection con, TagCloudPK pk, int type) throws SQLException
	{
		String query = new StringBuffer(100)
			...
			.append(" AND ").append(COLUMN_EXTERNALID).append(" = ?")
			.append(" AND ").append(COLUMN_EXTERNALTYPE).append(" = ?")
			.toString();
		PreparedStatement prepStmt = con.prepareStatement(query);
		try
		{
	 ...
	 * @throws SQLException
	 */
	public static Collection getInstanceTagClouds(Connection con, String instanceId)
		throws SQLException
	{
		String query = new StringBuffer(100)
			.append("SELECT ").append(ALL_COLUMNS).append(" FROM ").append(TABLE_NAME)
			.append(" WHERE ").append(COLUMN_INSTANCEID).append(" = ?")
			.append(" ORDER BY ").append(COLUMN_TAG).append(" ASC")
			.toString();
	 ...
	 * @return The list of tagclouds corresponding to the tag and the instance id given as
	 * 		   parameters.
	 * @throws SQLException
	 */
	public static Collection getTagCloudsByTags(Connection con, String tags, String instanceId,
			int type)
		throws SQLException
	{
	 ...
	 * @throws SQLException
	 */
	public static Collection getTagCloudsByElement(Connection con, String instanceId,
			String externalId, int type)
		throws SQLException
	{
		StringBuffer querySb = new StringBuffer(100)
			.append("SELECT ").append(ALL_COLUMNS).append(" FROM ").append(TABLE_NAME)
	...
	{
		StringBuffer querySb = new StringBuffer(100)
			.append("SELECT ").append(COLUMN_TAG).append(" FROM ").append(TABLE_NAME)
			.append(" WHERE ").append(COLUMN_INSTANCEID).append(" = ?")
			.append(" AND ").append(COLUMN_EXTERNALID).append(" = ?")
			.append(" AND ").append(COLUMN_EXTERNALTYPE).append(" = ?")
			.append(" ORDER BY ").append(COLUMN_TAG).append(" ASC");
