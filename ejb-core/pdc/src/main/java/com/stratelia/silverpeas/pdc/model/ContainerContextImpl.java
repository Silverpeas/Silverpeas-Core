...
{
		private int nContainerInstanceId = -1;									// The instance of the container on which the content is going to use
...
//		private String BrowseBar = null;											//implement a BrowseBar object
		private URLIcone uClassifyURLIcone = null;							// URL on the JSP to add classify positions on the container
//		private URLIcone uGenericClassifyURLIcone = null;			// URL on the JSP to add classify positions across containers
		private ContainerPositionInterface curPosition = null;	// Current position in the container
		...
		*/
		public String getClassifyURLWithParameters(String sComponentId, String sSilverContentId)
		{
				try
				{
						return uClassifyURLIcone.getActionURL() + "?" + containerPeas.getContainerInterface().getCallParameters(sComponentId, sSilverContentId);
				}
				catch(Exception e)
				{
						SilverTrace.error("containerManager","ContainerContext.getClassifyURLWithParameters","root.MSG_GEN_PARAM_VALUE","Fatal Error", e);
						return null;
				}
		}
		...
		public ContainerPositionInterface getSilverContentIdSearchContext(int nSilverContentId, String sComponentId)
		{
				...
				{
						SilverTrace.error("containerManager","ContainerContext.getSilverContentIdPositions","root.MSG_GEN_PARAM_VALUE","Fatal Error", e);
						return null;
				}
		}
		...
		*/
		public List getSilverContentIdByPosition(ContainerPositionInterface containerPosition, List alComponentIds)
		{
				...
				{
						ContainerInterface ci = containerPeas.getContainerInterface();
						return ci.findSilverContentIdByPosition(containerPosition, alComponentIds);
				}
				catch(Exception e)
				{
						SilverTrace.error("containerManager","ContainerContext.getSilverContentIdByPosition","root.MSG_GEN_PARAM_VALUE","Fatal Error", e);
						return null;
				}
		}
}
