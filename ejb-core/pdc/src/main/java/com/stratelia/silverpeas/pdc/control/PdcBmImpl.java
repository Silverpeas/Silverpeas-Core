...
{ 
    ...
    public PdcBmImpl()
    {
        try
        {
            dao = SilverpeasBeanDAOFactory.getDAO("com.stratelia.silverpeas.pdc.model.AxisHeaderPersistence");
        }
        catch (PersistenceException exce_DAO)
        {
            SilverTrace.error("Pdc", "PdcBmImpl", "Pdc.CANNOT_CONSTRUCT_PERSISTENCE", exce_DAO);
        }
    }
    ...
    public List getAxisByType(String type) throws PdcException
    {
        ...
        try
        {
            axis = (List) dao.findByWhereClause(new AxisPK("useless"), " AxisType='" + type + "' order by AxisOrder ");
            return persistence2AxisHeaders(axis);
        }
        catch (PersistenceException exce_select)
        {
            throw new PdcException("PdcBmImpl.getAxisByType", SilverpeasException.ERROR, "Pdc.CANNOT_FIND_AXES_TYPE", exce_select);
        }
    }
    
    private List persistence2AxisHeaders(List silverpeasBeans) throws PersistenceException, PdcException
    {
    	...
    	if (silverpeasBeans != null)
    	{
    		...
    		while (it.hasNext())
    		{
    			AxisHeaderPersistence silverpeasBean = (AxisHeaderPersistence) it.next();
    ...
    private void setTranslations(AxisHeader axisHeader)
    {
		...
		try
		{
			...
			List translations = axisHeaderI18NDAO.getTranslations(con, axisId);
    ...
    public List getAxis() throws PdcException
    {
        ...
        try
        {
            axis = (List) dao.findByWhereClause(new AxisPK("useless"), " 1=1 order by AxisType asc, AxisOrder asc ");
            return persistence2AxisHeaders(axis);
        }
        catch (PersistenceException exce_select)
        {
            throw new PdcException("PdcBmImpl.getAxis", SilverpeasException.ERROR, "Pdc.CANNOT_FIND_AXES", exce_select);
        }
    ...
    public int createAxis(AxisHeader axisHeader) throws PdcException
    {
        ...
        else
        {
            ...
            try
            {
                ...
                // recupere les axes de meme type ordonnés qui ont un numéro d'ordre >= à celui de l'axe à inserer
                String     whereClause = "AxisType = '" + type + "' and AxisOrder >= " + order + " ORDER BY AxisOrder ASC";

                // ATTENTION il faut traiter l'ordre des autres axes
                Collection axisToUpdate = dao.findByWhereClause(axisHeader.getPK(), whereClause);
                ...
                // build of the Value
                com.stratelia.silverpeas.pdc.model.Value  value = new com.stratelia.silverpeas.pdc.model.Value("unknown", new Integer(axisHeader.getRootId()).toString(), axisHeader.getName(), axisHeader.getDescription(), axisHeader.getCreationDate(), axisHeader.getCreatorId(), "unknown", -1, -1, "unknown");
                ...
                SilverTrace.info("Pdc", "PdcBmImpl.createAxis()", "root.MSG_GEN_PARAM_VALUE", "axisHeader = " + axisHeader.toString());
            ...
            catch (Exception exce_create)
            {
                ...
                throw new PdcException("PdcBmImpl.createAxis", SilverpeasException.ERROR, "Pdc.CANNOT_CREATE_AXE", exce_create);
            }
            finally
            {
                closeConnection(con);
    ...
    {
        SilverTrace.info("Pdc", "PdcBmImpl.updateAxis()", "root.MSG_GEN_PARAM_VALUE", "axisHeader = " + axisHeader.toString());
        int  status = 0;
        List axis = getAxis();
        ...
        else
        {
            ...
            try
            {
                int order = axisHeader.getAxisOrder();  // si order = -1 alors l'ordre ne doit pas être modifié
                ...
                if (order != -1)
                {
                    ...
                    // ATTENTION il faut traiter l'ordre des autres axes
                    Collection axisToUpdate = dao.findByWhereClause(con, axisHeader.getPK(), whereClause);
                ...
                SilverTrace.info("Pdc", "PdcBmImpl.updateAxis()", "root.MSG_GEN_PARAM_VALUE", "axisHeader.getPK().getId() = " + axisHeader.getPK().getId());
                AxisHeader oldAxisHeader = getAxisHeader(con, axisHeader.getPK().getId());

                SilverTrace.info("Pdc", "PdcBmImpl.updateAxis()", "root.MSG_GEN_PARAM_VALUE", "oldAxisHeader.getRootId() = " + oldAxisHeader.getRootId());
                ...
                boolean axisDescHasChanged = false;
                 ...
                 if (oldAxisHeader.getName() != null && !oldAxisHeader.getName().toLowerCase().equals(axisHeader.getName().toLowerCase()))
                	axisNameHasChanged = true;
                if (oldAxisHeader.getDescription() != null && !oldAxisHeader.getDescription().toLowerCase().equals(axisHeader.getDescription().toLowerCase()))
                ...
                if (axisNameHasChanged || axisDescHasChanged)
                {
                    ...
                    String treeId = (new Integer(oldAxisHeader.getRootId())).toString();
					/*TreeNode root = tree.getRoot(con, treeId);
                    TreeNode node = new TreeNode(root.getPK().getId(), root.getTreeId(), axisHeader.getName(), axisHeader.getDescription(), root.getCreationDate(), root.getCreatorId(), root.getPath(), root.getLevelNumber(), root.getOrderNumber(), root.getFatherId());
                ...
                if (axisHeader.isRemoveTranslation())
                {
                	...
                	{
                		List translations = axisHeaderI18NDAO.getTranslations(con, Integer.parseInt(axisHeader.getPK().getId()));
                ...
                else
                {
    	            if (axisHeader.getLanguage() != null)
    	            {
    	                ...
    	                {
    	            		AxisHeaderI18N newAxis = new AxisHeaderI18N(Integer.parseInt(axisHeader.getPK().getId()), axisHeader.getLanguage(), axisHeader.getName(), axisHeader.getDescription());
            ...
            {
				rollbackConnection(con);
                throw new PdcException("PdcBmImpl.updateAxis", SilverpeasException.ERROR, "Pdc.CANNOT_UPDATE_AXE", exce_update);
            }
            finally
            {
                closeConnection(con);
    ...
    public void deleteAxis(Connection con, String axisId) throws PdcException
    {
        try
        {
            AxisHeader axisHeader = getAxisHeader(con, axisId);  // get the header of the axe to obtain the rootId.
            
            PdcRightsDAO.deleteAxisRights(con, axisId);

            tree.deleteTree(con, new Integer(axisHeader.getRootId()).toString()); // delete data in the tree table
			pdcUtilizationBm.deleteUsedAxisByAxisId(con, axisId); // delete data in the pdc utilization table
            ...
            (new PdcSubscriptionUtil()).checkAxisOnDelete(Integer.parseInt(axisId), axisHeader.getName());
        ...
        {
            throw new PdcException("PdcBmImpl.deleteAxis", SilverpeasException.ERROR, "Pdc.CANNOT_DELETE_AXE", exce_delete);
        }
    }
    ...
    {
		Axis axis = null;
        AxisHeader axisHeader = getAxisHeader(axisId);  // get the header of the axe to obtain the rootId.
        if (axisHeader != null) {
	...
	{
        AxisHeaderPersistence axisHeader = getAxisHeaderPersistence(axisId);  // get the header of the axe to obtain the rootId.
		int        treeId	  = -1;
        if (axisHeader != null) {
     ...
     */
    public com.stratelia.silverpeas.pdc.model.Value getValue(String axisId, String valueId) throws PdcException
    {
        ...
        {
            throw new PdcException("PdcBmImpl.getValue", SilverpeasException.ERROR, "Pdc.CANNOT_ACCESS_VALUE", exce_select);
        }
        finally
        {
            closeConnection(con);
     ...
     * @return the Value object
     */
    public com.stratelia.silverpeas.pdc.model.Value getAxisValue(String valueId, String treeId) throws PdcException
    {
        com.stratelia.silverpeas.pdc.model.Value      value = null;
        ...
        {
            throw new PdcException("PdcBmImpl.getAxisValue", SilverpeasException.ERROR, "Pdc.CANNOT_ACCESS_VALUE", exce_select);
        }
        finally
        {
            closeConnection(con);
    ...
    public List getAxisValuesByName(String valueName) throws PdcException
    {
        ...
        try
        {
            List listTreeNodes = tree.getNodesByName(con, valueName);
            listValues = createValuesList(listTreeNodes);

        }
        catch (Exception exce_select)
        {
            throw new PdcException("PdcBmImpl.getAxisValuesByName", SilverpeasException.ERROR, "Pdc.CANNOT_FIND_VALUES", exce_select);
        }
        finally
        {
            closeConnection(con);
    ...
    public List getDaughterValues(String axisId, String valueId) throws PdcException {
        ...
        try
        {
             ...
             while (i.hasNext()) {
             	com.stratelia.silverpeas.pdc.model.Value value = (com.stratelia.silverpeas.pdc.model.Value) i.next();
             	listValuesString.add(value.getPK().getId());
             }
        }
        catch (Exception exce_select)
        {
            throw new PdcException("PdcBmImpl.getDaughterValues", SilverpeasException.ERROR, "Pdc.CANNOT_RETRIEVE_SUBNODES", exce_select);
        }
        finally
        {
            closeConnection(con);
    ...
    public List getFilteredAxisValues(String rootId, AxisFilter filter) throws PdcException {
        ...
        {
            throw new PdcException("PdcBmImpl.getFilteredAxisValues", SilverpeasException.ERROR, "Pdc.CANNOT_RETRIEVE_SUBNODES", exce_select);
        }
    ...
    public com.stratelia.silverpeas.pdc.model.Value getRoot(String axisId) throws PdcException {
        ...
        try
        {
			AxisHeader axisHeader	= getAxisHeader(axisId, false);  // get the header of the axe to obtain the rootId.
			int        treeId		= axisHeader.getRootId();
        ...
        {
            throw new PdcException("PdcBmImpl.getRoot", SilverpeasException.ERROR, "Pdc.CANNOT_GET_VALUE", e);
        }
        finally
        {
            closeConnection(con);
	...
	private List getAxisValues(int treeId, AxisFilter filter) throws PdcException
	{
        ...
        try
        {
            values = createValuesList(tree.getTree(con, new Integer(treeId).toString(), filter));
        }
        catch (Exception exce_select)
        {
            throw new PdcException("PdcBmImpl.getAxisValues", SilverpeasException.ERROR, "Pdc.CANNOT_ACCESS_LIST_OF_VALUES", exce_select);
        }
        finally
        {
            closeConnection(con);
     ...
     */
    public int insertMotherValue(com.stratelia.silverpeas.pdc.model.Value valueToInsert, String refValue, String axisId) throws PdcException
    {
        int      status = 0;
		AxisHeader axisHeader = getAxisHeader(axisId, false);  // get the header of the axe to obtain the treeId.
		...
		try
		{
			...
			ArrayList oldPath = getPathes(con,refValue, treeId);
		...
		{
			rollbackConnection(con);
			throw new PdcException("PdcBmImpl.insertMotherValue", SilverpeasException.ERROR, "Pdc.Pdc.CANNOT_UPDATE_POSTION", e);
		} finally {
			closeConnection(con);
		}
    ...
    public int moveValueToNewFatherId(Axis axis, Value valueToMove, String newFatherId, int orderNumber) throws PdcException
    {
    	...
    	try
    	{
    		// Avant le déplassement de la valeur, on recupere les vieux chemins afin de reclasser les associations après
    		...
    		else
    		{
    			try
    			{
    				//l'idée : passer en paramètres : des TreeNodePK car le métier est basé sur les Tree
    				tree.moveSubTreeToNewFather(con, new TreeNodePK(valueToMoveId), new TreeNodePK(newFatherId), treeId, orderNumber);
    			}
    			catch (Exception exce_insert)
    			{
    				throw new PdcException("PdcBmImpl.moveValueToNewFatherId", SilverpeasException.ERROR, "Pdc.CANNOT_MOVE_VALUE", exce_insert);
    			}
    		}
    		...
    		if ( (status == 0) && oldPath.size()!=0 ){
    			...
    			// call the ClassifyBm to create oldValue and newValue
    			// and to replace the oldValue by the newValue
    			pdcClassifyBm.createValuesAndReplace(con,new Integer(axis.getAxisHeader().getRootId()).toString(),oldPath,newPath);
    		}
    		commitConnection(con);
    	}
    	catch (Exception e)
    	{
    		rollbackConnection(con);
    		throw new PdcException("PdcBmImpl.moveValueToNewFatherId", SilverpeasException.ERROR, "Pdc.CANNOT_MOVE_VALUE", e);
    	} finally {
    		closeConnection(con);
    	}
    	return status;
   ...
   public List getInheritedManagers(Value value) throws PdcException
   {
	   ...
	   List usersInherited = new ArrayList();
	   List groupsInherited =new ArrayList();
	   for (int i = 1; i < explosedPath.length; i++)
	   {
		   //Value valuePath  = getAxisValue(explosedPath[i], value.getTreeId());
		   List managers =  getManagers(axisId, explosedPath[i]);
		   ...
		   for (int j = 0; j < usersId.size(); j++ )
		   {
			   ...
			   if ( !usersInherited.contains(usersId.get(j)) )  usersInherited.add(usersId.get(j));	
		   }
		   for (int j = 0; j < groupsId.size(); j++ )
		   {
			   //si le groupid n'est pas déjà dans la liste
			   if ( !groupsInherited.contains(groupsId.get(j)) )  groupsInherited.add(groupsId.get(j));
		   } 
	   }
   ...
   public List getManagers(String axisId, String valueId) throws PdcException
   {
	  ...
	  try {
		  ...
		  throw new PdcException("PdcBmImpl.getManagers", SilverpeasException.ERROR, "Pdc.CANNOT_GET_MANAGERS", e);
	  }
	  finally
	  {
		  closeConnection(con);
	  ...
	  return usersAndgroups ;
   	...
   	public boolean isUserManager(String userId) throws PdcException
   	{
		...
		try {
			...
			throw new PdcException("PdcBmImpl.isUserManager", SilverpeasException.ERROR, "Pdc.CANNOT_GET_MANAGERS", e);
		}		
		finally
		{
			closeConnection(con);
   	...
   	private boolean isGroupManager(String[] groupIds) throws PdcException
   	{
   		...
   		try {
			...
			throw new PdcException("PdcBmImpl.isGroupManager", SilverpeasException.ERROR, "Pdc.CANNOT_GET_MANAGERS", e);
		}
		finally
		{
			closeConnection(con);
  ...
  public void setManagers(List userIds, List groupIds, String axisId, String valueId)throws PdcException
  {
	  ...
	  try
	  {
		// supprime tous les droits sur la valeur
		  ...
		  {
			  PdcRightsDAO.insertUserId(con, axisId, valueId, (String) userIds.get(i));
		  }
		  for(int i = 0; i < groupIds.size(); i++)
		  {
			  PdcRightsDAO.insertGroupId(con, axisId, valueId, (String) groupIds.get(i));
		  }
	  }
	  catch (SQLException e)
	  {
		  throw new PdcException("PdcBmImpl.setManagers", SilverpeasException.ERROR, "Pdc.CANNOT_SET_MANAGER", e);
	  }
	  finally
	  {
		  closeConnection(con);
  ...
  public void razManagers(String axisId, String valueId) throws PdcException 
  {
	  ...
	  try {
		  ...
		  throw new PdcException("PdcBmImpl.razManagers", SilverpeasException.ERROR, "Pdc.CANNOT_REMOVE_MANAGER", e);
	  }
	  finally
	  {
		  closeConnection(con);
 ...
 */
  public void deleteManager(String userId) throws PdcException 
  {
	  ...
	  try {
		  ...
		  throw new PdcException("PdcBmImpl.deleteManager", SilverpeasException.ERROR, "Pdc.CANNOT_REMOVE_MANAGER", e);
	  }
	  finally
	  {
		  closeConnection(con);
...
*/
  public void deleteGroupManager(String groupId) throws PdcException 
  {
	  ...
	  try {
		  ...
		  throw new PdcException("PdcBmImpl.deleteGroupManager", SilverpeasException.ERROR, "Pdc.CANNOT_REMOVE_MANAGER", e);
	  }
	  finally
	  {
		  closeConnection(con);
	...
	* @throws PdcException
	*/
	private ArrayList getPathes(Connection con,String refValue, String treeId) throws PdcException {
		TreeNode nodeTree = null;
		ArrayList pathList = new ArrayList();
		TreeNodePK refNodePK = new TreeNodePK(refValue);
		try{
			ArrayList treeList = (ArrayList)tree.getSubTree(con,refNodePK, treeId); // get a list of tree node
     ...
     * @see
     */
    private void insertMotherValueToRootValue(Connection con, com.stratelia.silverpeas.pdc.model.Value valueToInsert, String refValue, String axisId, String treeId) throws PdcException
    {
        try
        {
            // Insertion de la nouvelle racine
            tree.insertFatherToNode(con, (TreeNode) valueToInsert, new TreeNodePK(refValue), treeId);
        }
        catch (Exception exce_insert)
        {
            throw new PdcException("PdcBmImpl.insertMotherValue", SilverpeasException.ERROR, "Pdc.CANNOT_INSERT_VALUE", exce_insert);
        }
    }
     ...
     * @see
     */
    private int insertMotherValueToValue(Connection con, com.stratelia.silverpeas.pdc.model.Value valueToInsert, String refValue, String treeId) throws PdcException
    {
        int        status = 0;
        // get the mother value of the value which have the refValue
        // to find sisters of the valueToInsert
     ...
     * @return 1 if the name already exist 0 otherwise
     */
    public int createDaughterValue(com.stratelia.silverpeas.pdc.model.Value valueToInsert, String refValue, String treeId) throws PdcException
    {
        // get the Connection object
        Connection con = openConnection(true);
        ...
        else
        {
            try
            {
                tree.createSonToNode(con, (TreeNode) valueToInsert, new TreeNodePK(refValue), treeId);
            }
            catch (Exception exce_create)
            {
                throw new PdcException("PdcBmImpl.createDaughterValue", SilverpeasException.ERROR, "Pdc.CANNOT_CREATE_VALUE", exce_create);
            }
            finally
            {
                closeConnection(con);
     ...
     * @return 1 if the name already exist 0 otherwise
     */
    public int updateValue(com.stratelia.silverpeas.pdc.model.Value value, String treeId) throws PdcException
    {
        // get the Connection object
        Connection con = openConnection(true);
		...
		try
		{
			...
			{
				TreeNode node = new TreeNode(value.getPK().getId(), treeId, value.getName(), value.getDescription(), oldValue.getCreationDate(), oldValue.getCreatorId(), oldValue.getPath(), oldValue.getLevelNumber(), value.getOrderNumber(), oldValue.getFatherId());
		...
		{
			new PdcException("PdcBmImpl.updateValue", SilverpeasException.ERROR, "Pdc.CANNOT_UPDATE_VALUE", exce_update);
		}
		finally
		{
			closeConnection(con);
    ...
    {
		SilverTrace.info("Pdc", "PdcBmImpl.deleteValueAndSubtree", "root.MSG_GEN_PARAM_VALUE", "axisId = "+axisId);
        ...
        try
        {
			...
			ArrayList oldPath = getPathes(con,valueId, treeId);

			SilverTrace.info("Pdc", "PdcBmImpl.deleteValueAndSubtree", "root.MSG_GEN_PARAM_VALUE", "oldPath.size() = "+oldPath.size()+", oldPath ="+oldPath.toString());
			...
			SilverTrace.info("Pdc", "PdcBmImpl.deleteValueAndSubtree", "root.MSG_GEN_PARAM_VALUE", "motherId = "+motherId);

            AxisHeader axisHeader = getAxisHeader(con, axisId);
            String axisName = axisHeader.getName();
            List pathInfo = getFullPath(valueId, treeId);

            List subtree  = tree.getSubTree(con, treeNodePK, treeId);
			...
			// Warning, we must update the path of the Value(Classify)
			if ( oldPath.size()!=0 ){
				// Après l'effacement de la valeur et de son arborescence, on recupere les nouveaux chemins
				...
				SilverTrace.info("Pdc", "PdcBmImpl.deleteValueAndSubtree", "root.MSG_GEN_PARAM_VALUE", "newPath.size() = "+newPath.size());
				// call the ClassifyBm to create oldValue and newValue
				// and to replace the oldValue by the newValue

                (new PdcSubscriptionUtil()).checkValueOnDelete(Integer.parseInt(axisId), axisName, oldPath, newPath, pathInfo);
				pdcClassifyBm.createValuesAndReplace(con,axisId,oldPath,newPath);
			}
        ...
        {
			//rollbackConnection(con);
            throw new PdcException("PdcBmImpl.deleteValueAndSubtree", SilverpeasException.ERROR, "Pdc.CANNOT_DELETE_VALUE_SUBTREE", exce_delete);
        }
        /*finally
        {
            closeConnection(con);
    ...
    public String deleteValue(Connection con, String valueId, String axisId, String treeId) throws PdcException
    {
        ...
        try
        {
			com.stratelia.silverpeas.pdc.model.Value	valueToDelete				= getAxisValue(valueId, treeId);
			List										daughtersOfMother			= getDaughters(con, valueToDelete.getMotherId(), treeId); //filles de la mère = soeurs des filles de la valeur à supprimer
			List										daughtersOfValueToDelete	= getDaughters(con, valueId, treeId); //filles de la valeur à supprimer
			...
			if (possibleDaughterName == null) {
				...
				if ( oldPath.size()!=0 ){
					...
					String pattern = "/"+valueId; // motif que l'on doit rechercher dans l'ancien chemin pour le supprimer
					...
					for (int i=0; i<oldPath.size();i++ ) {
						...
						path = path.substring(0,pattern_idx)+path.substring(pattern_idx+lenOfPattern); // retire le motif
						newPath.add(path);
					}
        ...
        {
            throw new PdcException("PdcBmImpl.deleteValue", SilverpeasException.ERROR, "Pdc.CANNOT_DELETE_VALUE", exce_delete);
        }
     ...
     * @return the complet path - It's a List of ArrayList. Each ArrayList contains the name, the id and the treeId of the value in the path.
     */
    ...
    private boolean isAxisNameExist(List axis, AxisHeader axisToCheck)
    {
        ...
        while (it.hasNext())
        {
            ...
            if (((axisHeader.getName()).toLowerCase()).equals(axisNameToCheck.toLowerCase()))
            {
     ...
     */
    private boolean isValueNameExist(List values, com.stratelia.silverpeas.pdc.model.Value valueToCheck)
    {
        ...
        while (it.hasNext())
        {
            ...
            if ((value.getName().toLowerCase()).equals(valueNameToCheck.toLowerCase()))
            {
     ...
     */
    private String isValueNameExist(List values, List valuesToCheck)
    {
        Iterator it = valuesToCheck.iterator();
        com.stratelia.silverpeas.pdc.model.Value    valueToCheck = null;
		String valueName = null;

        while (it.hasNext())
        {
            valueToCheck = (com.stratelia.silverpeas.pdc.model.Value) it.next();
    ...
    /*public AxisHeader getAxisHeader(String axisId)
    {
        ...
        try
        {
            axisHeader = (AxisHeader) dao.findByPrimaryKey(new AxisPK(axisId));
        }
        catch (PersistenceException err_select)
        {
            SilverTrace.info("PDC", "PdcBmImpl.getAxisHeader", "Pdc.CANNOT_RETRIEVE_HEADER_AXE", err_select);
        }
    ...
    public AxisHeader getAxisHeader(String axisId, boolean setTranslations)
    {
        ...
        if (axisHeader == null)
        {
        	try
            {
                AxisHeaderPersistence axisHeaderPersistence = (AxisHeaderPersistence) dao.findByPrimaryKey(new AxisPK(axisId));
    ...
    private AxisHeaderPersistence getAxisHeaderPersistence(String axisId)
    {
        ...
        {
            SilverTrace.error("PDC", "PdcBmImpl.getAxisHeaderPersistence", "Pdc.CANNOT_RETRIEVE_HEADER_AXE", "axisId = "+axisId, err_select);
        }
        return null;
    }
    ...
    private AxisHeader getAxisHeader(Connection connection, String axisId)
    {
        ...
        try
        {
        	AxisHeaderPersistence axisHeaderPersistence = (AxisHeaderPersistence) dao.findByPrimaryKey(connection, new AxisPK(axisId));
            axisHeader = new AxisHeader(axisHeaderPersistence);
        }
        catch (PersistenceException err_select)
        {
    ...
    private List getDaughters(Connection con, String refValue, String treeId)
    {
        ...
        try
        {
            daughters = (ArrayList) createValuesList(tree.getSonsToNode(con, new TreeNodePK(refValue), treeId));
        }
	...
	public List getSubAxisValues(String axisId, String valueId)
	{
        ...
        try {
            ...
            daughters = (ArrayList) createValuesList(tree.getSubTree(con, new TreeNodePK(valueId), new Integer(tId).toString()));
        } catch (Exception err_list)  {
    ...
    private com.stratelia.silverpeas.pdc.model.Value createValue(TreeNode treeNode)
    {
    	if (treeNode != null)
    	{
    		Value value = new com.stratelia.silverpeas.pdc.model.Value(treeNode.getPK().getId(), treeNode.getTreeId(), treeNode.getName(), treeNode.getDescription(), treeNode.getCreationDate(), treeNode.getCreatorId(), treeNode.getPath(), treeNode.getLevelNumber(), treeNode.getOrderNumber(), treeNode.getFatherId());
    ...
    {
		AxisHeader axisHeader = getAxisHeader(Integer.toString(usedAxis.getAxisId()), false);  // get the header of the axe to obtain the treeId.
        String   treeId = new Integer(axisHeader.getRootId()).toString();
    ...
    public int updateUsedAxis(UsedAxis usedAxis) throws PdcException
    {
		...
		// on recherche si la nouvelle valeur de base est une valeur ascendante à la valeur de base originelle
		...
		List objectIdList = pdcClassifyBm.getObjectsByInstance(usedAxis.getInstanceId());

		// on vérifie d'abord que la nouvelle valeur de base est une valeur ascendante
	...
	/**
	* recherche si la valeur de base de l'axe est une valeur ascendante
	* par rapport aux valeurs se trouvant dans SB_Classify...
	...
	*/
	private boolean isAscendanteBaseValue(List objectIdList,UsedAxis usedAxis) throws PdcException{
	...
	*/
	private void updateBaseValueInInstances(Connection con, String baseValueToUpdate, String axisId, String treeId) throws PdcException{
		...
		SilverTrace.info("Pdc", "PdcBmImpl.updateBaseValueInInstances", "root.MSG_GEN_PARAM_VALUE", "newBaseValue = "+newBaseValue);

		pdcUtilizationBm.updateOrDeleteBaseValue(con, new Integer(baseValueToUpdate).intValue(), new Integer(newBaseValue).intValue(), new Integer(axisId).intValue(), treeId);
	}
	...
	*/
	private void updateBaseValuesInInstances(Connection con, String baseValueToUpdate, String axisId, String treeId) throws PdcException{
		...
		try
		{
			descendants = tree.getSubTree(con, new TreeNodePK(baseValueToUpdate), treeId);
		}
		catch (Exception e)
		{
			throw new PdcException("PdcBmImpl.updateBaseValuesInInstances", SilverpeasException.ERROR, "Pdc.CANNOT_DELETE_VALUE", e);
		}
		...
		SilverTrace.info("Pdc", "PdcBmImpl.updateBaseValuesInInstances", "root.MSG_GEN_PARAM_VALUE", "newBaseValue = "+newBaseValue);
		...
		for (int i = 0; i<descendants.size(); i++) {
			...
			SilverTrace.info("Pdc", "PdcBmImpl.updateBaseValuesInInstances", "root.MSG_GEN_PARAM_VALUE", "descendantId = "+descendantId);

			pdcUtilizationBm.updateOrDeleteBaseValue(con, new Integer(descendantId).intValue(), new Integer(newBaseValue).intValue(), new Integer(axisId).intValue(), treeId);
		}
	}
	...
	public List getUsedAxisToClassify(String instanceId, int silverObjectId) throws PdcException {
		...
		//Il faut enrichir chaque axe utilisé par l'axe (pour l'i18n) et ses valeurs
		UsedAxis axis = null;
		int axisRootId = -1;
	...
	/* (non-Javadoc)
	 * @see com.stratelia.silverpeas.pdc.control.PdcBm#copyPositions(int, java.lang.String, int, java.lang.String)
	 */
	...
	private ClassifyPosition checkClassifyPosition(ClassifyPosition position, List usedAxis)
	{
    ...
    public int addPosition(int silverObjectId, ClassifyPosition position, String sComponentId, boolean alertSubscribers) throws PdcException {
        ...
        if (positionId == -1)
        {
	        ...
	        if (alertSubscribers)
	        {
		        ...
		        try {
		            (new PdcSubscriptionUtil()).checkSubscriptions(position.getValues(), sComponentId, silverObjectId);
		        }
		        catch (RemoteException e) {
		            throw new PdcException("PdcBmImpl.addPosition", PdcException.ERROR, "pdcPeas.EX_CHECK_SUBSCRIPTION", e);
		        }
	        }
        }
    ...
    public int updatePosition(ClassifyPosition position,String instanceId, int silverObjectId, boolean alertSubscribers) throws PdcException {
		...
		{
	        try {
	            (new PdcSubscriptionUtil()).checkSubscriptions(position.getValues(), instanceId, silverObjectId);
	        }
	        catch (RemoteException e) {
	            throw new PdcException("PdcBmImpl.updatePosition", PdcException.ERROR, "pdcPeas.EX_CHECH_SUBSCRIPTION", e);
	        }
		}
    ...
    public List getPositions(int silverObjectId, String sComponentId) throws PdcException {
		...
		com.stratelia.silverpeas.classifyEngine.Position position = null;
		for (int i = 0; i<positions.size(); i++) {
			position = (com.stratelia.silverpeas.classifyEngine.Position) positions.get(i);
			...
			for (int p = 0; p<values.size(); p++) {
				...
				if (value.getAxisId() != -1) {
					...
					/*AxisHeader axisHeader	= getAxisHeader(Integer.toString(value.getAxisId()), false);  // get the header of the axe to obtain the rootId.
					int        treeId		= axisHeader.getRootId();*/
					...
					if (valuePath != null) {
						...
						classifyValue.setFullPath(createValuesList(getFullPath(valueId, new Integer(treeId).toString())));
	...
	public List getPertinentAxisByInstanceIds(SearchContext searchContext, String axisType, List instanceIds, AxisFilter filter) throws PdcException {
		...
		List axis = pdcUtilizationBm.getAxisHeaderUsedByInstanceIds(instanceIds, filter);
		SilverTrace.info("Pdc", "PdcBmImpl.getPertinentAxisByInstanceIds", "root.MSG_GEN_PARAM_VALUE", axis.size()+" axis used !");
		...
		List pertinentAxis = pdcClassifyBm.getPertinentAxis(searchContext, axisIds, pdcClassifyBm.getPositionsJoinStatement(instanceIds));
		SilverTrace.info("Pdc", "PdcBmImpl.getPertinentAxisByInstanceIds", "root.MSG_GEN_EXIT_METHOD", pertinentAxis.size()+" pertinent axis !");
		return transformPertinentAxisIntoSearchAxis(pertinentAxis, axis);
	}

	private List transformPertinentAxisIntoSearchAxis(List pertinentAxisList, List axis) throws PdcException {
		...
		for (int i = 0; i < pertinentAxisList.size(); i++) {
			...
			for (int j = 0; j < axis.size(); j++) {
				...
				if (axisHeader.getPK().getId().equals(axisId)) {
					...
					searchAxis.setAxisRootId(new Integer(getRootId(axisHeader.getPK().getId())).intValue());
	...
	public List getPertinentDaughterValuesByInstanceId(SearchContext searchContext, String axisId, String valueId, String instanceId) throws PdcException {
		return getPertinentDaughterValuesByInstanceId(searchContext, axisId, valueId, instanceId, new AxisFilter());
	}
	...
	public List getPertinentDaughterValuesByInstanceId(SearchContext searchContext, String axisId, String valueId, String instanceId, AxisFilter filter) throws PdcException {
		List instanceIds = (List)new ArrayList();
		instanceIds.add(instanceId);
		return getPertinentDaughterValuesByInstanceIds(searchContext,axisId,valueId,instanceIds, filter);
	}
	...
	public List getPertinentDaughterValuesByInstanceIds(SearchContext searchContext, String axisId, String valueId, List instanceIds) throws PdcException {
		return getPertinentDaughterValuesByInstanceIds(searchContext, axisId, valueId, instanceIds, new AxisFilter());
	}
	...
	{
		SilverTrace.info("Pdc", "PdcBmImpl.getPertinentDaughterValuesByInstanceIds", "root.MSG_GEN_ENTER_METHOD", "axisId = "+axisId+", valueId = "+valueId+", userId = "+searchContext.getUserId());
		
		//List pertinentValues = pdcClassifyBm.getPertinentValues(searchContext, new Integer(axisId).intValue(), pdcClassifyBm.getPositionsJoinStatement(instanceIds));
		List pertinentValues = new ArrayList();

		List pertinentDaughters = filterValues(searchContext, axisId, valueId, pertinentValues, instanceIds, filter);
		
		com.stratelia.silverpeas.pdc.model.Value value = null;
		for (int d=0; d<pertinentDaughters.size(); d++)
		{
			value = (com.stratelia.silverpeas.pdc.model.Value) pertinentDaughters.get(d);
			SilverTrace.debug("pdcPeas", "PdcSearchSessionController.getPertinentDaughterValuesByInstanceIds()", "root.MSG_GEN_PARAM_VALUE", "valueId = "+value.getPK().getId()+", valueName = "+value.getName()+", nbObjects = "+value.getNbObjects());
		}
	...
	public List getFirstLevelAxisValuesByInstanceIds(SearchContext searchContext, String axisId, List instanceIds) throws PdcException {
		SilverTrace.info("Pdc", "PdcBmImpl.getFirstLevelAxisValuesByInstanceIds", "root.MSG_GEN_ENTER_METHOD", "axisId = "+axisId);
		...
		List pertinentDaughters = filterValues(searchContext, axisId, rootId, pertinentValues, instanceIds);
		
		com.stratelia.silverpeas.pdc.model.Value value = null;
		for (int d=0; d<pertinentDaughters.size(); d++)
		{
			value = (com.stratelia.silverpeas.pdc.model.Value) pertinentDaughters.get(d);
			SilverTrace.debug("pdcPeas", "PdcSearchSessionController.getFirstLevelAxisValuesByInstanceIds()", "root.MSG_GEN_PARAM_VALUE", "valueId = "+value.getPK().getId()+", valueName = "+value.getName()+", nbObjects = "+value.getNbObjects());
		}
	...
	private String getRootId(String axisId) throws PdcException {
		Connection con = openConnection(true);
		SilverTrace.info("Pdc", "PdcBmImpl.getRootId", "root.MSG_GEN_PARAM_VALUE", "axisId = "+axisId);
		String rootId = null;
        try
        {
			AxisHeader axisHeader	= getAxisHeader(axisId, false);  // get the header of the axe to obtain the rootId.
        ...
        {
            throw new PdcException("PdcBmImpl.getRootId", SilverpeasException.ERROR, "Pdc.CANNOT_GET_VALUE", e);
        }
        finally
        {
            closeConnection(con);
        }
		SilverTrace.info("Pdc", "PdcBmImpl.getRootId", "root.MSG_GEN_PARAM_VALUE", "rootId = "+rootId);
		return rootId;
	}
	private List filterValues(SearchContext searchContext, String axisId, String motherId, List valuesToFilter, List instanceIds) throws PdcException
	{
			return filterValues(searchContext, axisId, motherId, valuesToFilter, instanceIds, new AxisFilter());
	}

	private List filterValues(SearchContext searchContext, String axisId, String motherId, List valuesToFilter, List instanceIds, AxisFilter filter) throws PdcException
	{
		SilverTrace.info("Pdc", "PdcBmImpl.filterValues", "root.MSG_GEN_ENTER_METHOD", "axisId = "+axisId+", motherId = "+motherId);
		Connection con = openConnection(true);
		...
		List countedObjects = null;
		
		SilverTrace.info("Pdc", "PdcBmImpl.filterValues", "root.MSG_GEN_PARAM_VALUE", "apres getHeader()");
		...
		try
		{
			...
			SilverTrace.info("Pdc", "PdcBmImpl.filterValues", "root.MSG_GEN_PARAM_VALUE", "apres getAxisValues()");
					
			JoinStatement joinStatement = pdcClassifyBm.getPositionsJoinStatement(instanceIds);
			
			SilverTrace.info("Pdc", "PdcBmImpl.filterValues", "root.MSG_GEN_PARAM_VALUE", "apres getPositionsJoinStatement()");
			
			List pertinentValues = pdcClassifyBm.getPertinentValues(searchContext, new Integer(axisId).intValue(), joinStatement);
			 
			SilverTrace.info("Pdc", "PdcBmImpl.filterValues", "root.MSG_GEN_PARAM_VALUE", "apres getPertinentValues()");
			...
			for (int nI = 0; nI < descendants.size(); nI++)
			{
				...
				descendant = (com.stratelia.silverpeas.pdc.model.Value) descendants.get(nI);
				descendantPath = descendant.getFullPath();
					
				//check if it's a leaf or not
				if (nI+1 < descendants.size())
				{
					nextDescendant = (com.stratelia.silverpeas.pdc.model.Value) descendants.get(nI+1);
				...
				else
				{
					...
					{
						/*PertinentAxis pertinentAxis = pdcClassifyBm.getPertinentAxis(searchContext, axisId, descendantPath, joinStatement);
						SilverTrace.info("Pdc", "PdcBmImpl.filterValues", "root.MSG_GEN_PARAM_VALUE", "apres getPertinentAxis()");
						if(pertinentAxis.getNbObjects() > 0)
							descendant.setNbObjects(pertinentAxis.getNbObjects());
						else
						{
							emptyValues.add(descendantPath);
							descendants.remove(nI--);
						...
						if (objectValuePairs == null)
							objectValuePairs = pdcClassifyBm.getObjectValuePairs(searchContext, Integer.parseInt(axisId), joinStatement);
						...
						for (int o=0; o<objectValuePairs.size(); o++)
						{
							...
							if (ovp.getValuePath().startsWith(descendantPath) && !countedObjects.contains(objectId))
							{
								...
								if (instanceId.startsWith("kmelia"))
								{
									...
									{
										componentSecurity = (ComponentSecurity) Class.forName("com.stratelia.webactiv.kmelia.KmeliaSecurity").newInstance();
										componentSecurity.enableCache();
									}
									
									if (componentSecurity.isObjectAvailable(instanceId, searchContext.getUserId(), objectId.toString(), "Publication"))
									{
										nbObjects++;
		...
		{
			throw new PdcException("PdcBmImpl.getPertinentDaughterValues", SilverpeasException.ERROR, "Pdc.CANNOT_FILTER_VALUES", e);
		}
		finally
		{
			closeConnection(con);
	 ...
	 * @return true if at least one axis has been selected on component AND at least one axis is mandatory 
	 * @throws PdcException
	 */
	public boolean isClassifyingMandatory(String componentId) throws PdcException
	{
		List axisUsed = getUsedAxisByInstanceId(componentId);
		if (axisUsed == null)
		{
			return false;
		}
	...
	{
		Iterator 	axis 	= getAxis().iterator();
		AxisHeader	a 		= null;
		Connection	con		= openConnection();
		try
		{
		...
		{
			throw new PdcException("PdcBmImpl.indexAllAxis()", SilverpeasException.ERROR, "Pdc.INDEXING_AXIS_FAILED", e);
		}
		finally
		{
			closeConnection(con);
    ...
    private Connection openConnection() throws PdcException
    {
        ...
        {
            throw new PdcException("PdcBmImpl.openConnection()", SilverpeasException.ERROR, "root.EX_CONNECTION_OPEN_FAILED", e);
        }
    }
    ...
    private void closeConnection(Connection con)
    {
        if (con != null)
        {
            ...
            {
                SilverTrace.error("Pdc", "PdcBmImpl.closeConnection()", "root.EX_CONNECTION_CLOSE_FAILED", "", e);
            }
        }
    }
    ...
    private Connection openConnection(boolean autoCommit)
    {
		...
		{
			SilverTrace.error("Pdc", "PdcBmImpl.openConnection()", "root.EX_CONNECTION_OPEN_FAILED", "", e);
		}
		return con;
    }
    ...
    private void rollbackConnection(Connection con)
    {
        if (con != null)
        {
            ...
            {
                SilverTrace.error("Pdc", "PdcBmImpl.rollbackConnection()", "root.EX_CONNECTION_CLOSE_FAILED", "", e);
            }
        }
    }
    ...
    private void commitConnection(Connection con)
    {
        if (con != null)
        {
            ...
            {
                SilverTrace.error("Pdc", "PdcBmImpl.commitConnection()", "root.EX_CONNECTION_CLOSE_FAILED", "", e);
            }
        }
    }
...
/**
     ...
     */
		...
		public List removePosition(Connection connection, int nSilverContentId) throws ContainerManagerException
		{
				...
				{
            throw new ContainerManagerException("PdcBmImpl.removePosition",SilverpeasException.ERROR,"containerManager.EX_INTERFACE_REMOVE_FUNCTIONS", e);
				}
		...
		public ContainerPositionInterface getSilverContentIdSearchContext(int nSilverContentId, String sComponentId) throws ContainerManagerException
		{
			...
			{
				// Get the positions
				List alPositions = pdcClassifyBm.getPositions(nSilverContentId, sComponentId);
				...
				if(alPositions != null && alPositions.size() > 0)
				{
					...
					{
						com.stratelia.silverpeas.classifyEngine.Value value = (com.stratelia.silverpeas.classifyEngine.Value) alValues.get(nI);
			...
			{
				throw new ContainerManagerException("PdcBmImpl.getSilverContentIdPositions",SilverpeasException.ERROR,"containerManager.EX_INTERFACE_FIND_FUNCTIONS", e);
			}
		}
		...
		{
			return findSilverContentIdByPosition(containerPosition, alComponentId, authorId, afterDate, beforeDate, true, true);
		}
		...
		public List findSilverContentIdByPosition(ContainerPositionInterface containerPosition, List alComponentId, String authorId, String afterDate, String beforeDate, boolean recursiveSearch, boolean visibilitySensitive) throws ContainerManagerException
		{
			...
			{
				// Get the objects
				List alSilverContentId = pdcClassifyBm.findSilverContentIdByPosition(containerPosition, alComponentId, authorId, afterDate, beforeDate, recursiveSearch, visibilitySensitive);

				return alSilverContentId;
			}
			catch(Exception e)
			{
				throw new ContainerManagerException("PdcBmImpl.findSilverContentIdByPosition",SilverpeasException.ERROR,"containerManager.EX_INTERFACE_FIND_FUNCTIONS", e);
			}
		}
		...
		{
			return findSilverContentIdByPosition(containerPosition, alComponentId,null,null,null, recursiveSearch, visibilitySensitive);
		}
