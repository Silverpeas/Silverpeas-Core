...
/**
 * This class represents the ClassifyEngine API
 * It gives access to functions for classifying, unclassifying and searching SilverObjetIds
 *
 * Assumption : The SilverObjetIds processed are int values from 0 to n
 *							The axis processed are int values from 0 to n
 */
...
public class ClassifyEngine extends Object
{
	...
	static
	{
		...
		ResourceLocator res = new ResourceLocator("com.stratelia.silverpeas.classifyEngine.ClassifyEngine", "");
		...
		try {
			...
			SilverTrace.error("classifyEngine", "ClassifyEngine.initStatic", "root.EX_CLASS_NOT_INITIALIZED", "m_anRegisteredAxis initialization failed !", e);
		}  
	}
	...
	*/
	public void registerAxis(Connection connection, int nLogicalAxisId) throws ClassifyEngineException
	{
		...
		if (this.AxisAlreadyRegistered(nLogicalAxisId))
			throw new ClassifyEngineException(
				"ClassifyEngine.registerAxis",
				SilverpeasException.ERROR,
				"classifyEngine.EX_AXIS_ALREADY_REGISTERED",
			...
			synchronized (m_anRegisteredAxis)
			{
				...
				if (nNextAvailableAxis == -1)
					throw new ClassifyEngineException(
						"ClassifyEngine.registerAxis",
						SilverpeasException.ERROR,
						"classifyEngine.EX_NOMORE_AVAILABLE_AXIS",
						"nLogicalAxisId: " + nLogicalAxisId);

				// build the statement to classify
				String sSQLStatement = SQLStatement.buildRegisterAxisStatement(nNextAvailableAxis, nLogicalAxisId);
		...
		catch (Exception e)
		{
			throw new ClassifyEngineException(
				"ClassifyEngine.registerAxis",
				SilverpeasException.ERROR,
				"classifyEngine.EX_CANT_REGISTER_AXIS",
				"nLogicalAxisId= " + nLogicalAxisId,
				e);
		}
		finally
	...
	/*
	*		Unregister an axis
	*   Return the list of the deleted positionsIds
	*/
	public List unregisterAxis(Connection connection, int nLogicalAxisId) throws ClassifyEngineException
	{
		PreparedStatement prepStmt = null;
		...
		catch (Exception e)
		{
			throw new ClassifyEngineException(
				"ClassifyEngine.unregisterAxis",
				SilverpeasException.ERROR,
				"classifyEngine.EX_CANT_UNREGISTER_AXIS",
				"nLogicalAxisId= " + nLogicalAxisId,
				e);
		}
		finally
	...
	private static int[] loadRegisteredAxis() throws ClassifyEngineException
	{
		...
		catch (Exception e)
		{
			throw new ClassifyEngineException(
				"ClassifyEngine.loadRegisteredAxis",
				SilverpeasException.ERROR,
				"classifyEngine.EX_CANT_LOAD_REGISTERED_AXIS",
				e);
		}
		finally
		{
	...
	private int getPhysicalAxisId(int nLogicalAxisId) throws ClassifyEngineException
	{
		...
		SilverTrace.error(
			"classifyEngine",
			"ClassifyEngine.getPhysicalAxisId",
			"root.MSG_GEN_PARAM_VALUE",
			"Can't get physical axis Id, nLogicalAxisId: " + nLogicalAxisId+", m_anRegisteredAxis : "+printRegisteredAxis());
		throw new ClassifyEngineException(
			"ClassifyEngine.getPhysicalAxisId",
			SilverpeasException.ERROR,
			"classifyEngine.EX_CANT_GET_PHYSICAL_AXIS",
			"nLogicalAxisId: " + nLogicalAxisId);
	}
	...
	private int getLogicalAxisId(int nPhysicalAxisId) throws ClassifyEngineException
	{
		if (nPhysicalAxisId < 0 || nPhysicalAxisId > m_anRegisteredAxis.length)
			throw new ClassifyEngineException(
				"ClassifyEngine.getLogicalAxisId",
				SilverpeasException.ERROR,
				"classifyEngine.EX_CANT_GET_LOGICAL_AXIS",
	...
	private boolean AxisAlreadyRegistered(int nLogicalAxisId) throws ClassifyEngineException
	{
		for (int nI = 0; nI < nbMaxAxis; nI++)
			if (m_anRegisteredAxis[nI] == nLogicalAxisId)
				return true;
	...
	public int isPositionAlreadyExists(int nSilverObjectId, Position position) throws ClassifyEngineException
	{
		...
		try
		{
			...
			SilverTrace.info("classifyEngine", "ClassifyEngine.classifySilverObject", "root.MSG_GEN_PARAM_VALUE", "sSQLStatement= " + sSQLStatement);
			prepStmt = connection.prepareStatement(sSQLStatement);
		...
		catch (Exception e)
		{
			throw new ClassifyEngineException(
				...
				SilverpeasException.ERROR,
				"classifyEngine.EX_CANT_CLASSIFY_SILVEROBJECTID",
				"sSQLStatement= " + sSQLStatement,
				e);
		}
		finally
		{
			...
			{
				SilverTrace.error("classifyEngine",	"ClassifyEngine.isPositionAlreadyExists", "root.EX_CONNECTION_CLOSE_FAILED", e);
			}
		}
	}

	/*
	*		Classify the given SilverObjectid	within the classifyEngine
	*		If the given connection is null, then we have to open a connection and close it
	*   Return the PositionId
	*/
	public int classifySilverObject(Connection connection, int nSilverObjectId, Position position)
		throws ClassifyEngineException
	{
		...
		try
		{
			...
			// build the statement to classify
			int newPositionId = DBUtil.getNextId(m_sClassifyTable, m_sPositionIdColumn);
			String sSQLStatement = SQLStatement.buildClassifyStatement(nSilverObjectId, position, newPositionId);
		...
		catch (Exception e)
		{
			throw new ClassifyEngineException(
				...
				SilverpeasException.ERROR,
				"classifyEngine.EX_CANT_CLASSIFY_SILVEROBJECTID",
				"nSilverObjectId= " + nSilverObjectId,
				e);
		}
		finally
		{
			...
			{
				DBUtil.close(prepStmt);
				if (bCloseConnection && connection != null && !connection.isClosed()) connection.close();
			}
			catch (Exception e)
			{
				SilverTrace.error(
	...
	/*
	*		Remove the given SilverObjectId	at th egiven position within the classifyEngine
	*		If the given connection is null, then we have to open a connection and close it
	*/
	public void unclassifySilverObjectByPosition(Connection connection, int nSilverObjectId, Position position)
		throws ClassifyEngineException
	{
		boolean bCloseConnection = false;
		...
		try
		{
			...
			String sSQLStatement = SQLStatement.buildRemoveByPositionStatement(nSilverObjectId, position);
	...
	*/
	public void unclassifySilverObject(Connection connection, int nSilverObjectId) throws ClassifyEngineException
	{
		...
		try
		{
			...
			SilverTrace.info(
				"classifyEngine",
				"ClassifyEngine.unclassifySilverObject",
				"root.MSG_GEN_PARAM_VALUE",
				"sSQLStatement= " + sSQLStatement);
			prepStmt = connection.prepareStatement(sSQLStatement);
			prepStmt.executeUpdate();
	...
	/*
	*		Remove the SilverObject referenced with the given positionId	within the classifyEngine
	*		If the given connection is null, then we have to open a connection and close it
	*/
	public void unclassifySilverObjectByPositionId(Connection connection, int nPositionId)
		throws ClassifyEngineException
	{
		boolean bCloseConnection = false;
		PreparedStatement prepStmt = null;
	...
	/*
	*		update the given new position within the classifyEngine
	*		If the given connection is null, then we have to open a connection and close it
	*/
	public void updateSilverObjectPosition(Connection connection, Position newPosition) throws ClassifyEngineException
	{
		boolean bCloseConnection = false;
		...
		catch (Exception e)
		{
			throw new ClassifyEngineException(
				...
				"nPositionId= " + newPosition.getPositionId(),
				e);
		}
		finally
		{
	...
	/*
	*		update several position with the given new position within the classifyEngine
	*		If the given connection is null, then we have to open a connection and close it
	*		add by SAN
	*/
	public void updateSilverObjectPositions(Connection connection, List classifyValues, int nSilverObjectId)
		throws ClassifyEngineException
	{
		boolean bCloseConnection = false;
		...
		try
		{
			...
			for (int i = 0; i < classifyValues.size(); i++)
			{
				...
				// faut utiliser l'instruction suivante on a deja fait toute cette operation
				// dans la méthode updateSilverObjectPosition.
				//value.setAxisId(this.getPhysicalAxisId(value.getAxisId()));

				// build the statement to update the position
				String sSQLStatement = SQLStatement.buildUpdateByObjectIdStatement(value, nSilverObjectId);
	...
	public List findSilverOjectByCriterias(
		List alGivenCriterias,
		...
		throws ClassifyEngineException
	{
		return findSilverOjectByCriterias(
			alGivenCriterias,
			joinStatementContainer,
			joinStatementContent,
			afterDate,
			beforeDate,
			true,
			true);
	...
	*/
		...
		throws ClassifyEngineException
	{
		...
		for (int nI = 0; nI < alGivenCriterias.size(); nI++)
		{
			...
			alCriterias.add(new Criteria(this.getPhysicalAxisId(criteria.getAxisId()), criteria.getValue()));
		}
		PreparedStatement prepStmt = null;
		ResultSet resSet = null;
		try
		{
			...
			String sSQLStatement =
				SQLStatement.buildFindByCriteriasStatementByJoin(
					alCriterias,
					joinStatementContainer,
					joinStatementContent,
					today,
			...
			SilverTrace.debug("classifyEngine", "ClassifyEngine.findSilverOjectByCriterias", "root.MSG_GEN_PARAM_VALUE", "sSQLStatement= " + sSQLStatement);
			resSet = prepStmt.executeQuery();
			SilverTrace.debug("classifyEngine", "ClassifyEngine.findSilverOjectByCriterias", "root.MSG_GEN_PARAM_VALUE", "query executed !");
		...
		catch (Exception e)
		{
			throw new ClassifyEngineException(
				"ClassifyEngine.findSilverOjectByCriterias",
				SilverpeasException.ERROR,
				"classifyEngine.EX_CANT_FIND_SILVEROBJECTID",
				e);
		}
		finally
		{
	...
	*/
	public List getSilverContentIdsByPositionIds(List alPositionids) throws ClassifyEngineException
	{
		...
		catch (Exception e)
		{
			throw new ClassifyEngineException(
				"ClassifyEngine.getSilverContentIdsByPositionIds",
				SilverpeasException.ERROR,
				"classifyEngine.EX_CANT_GET_SILVERCONTENTIDS_BYSILVEROBJECTIDS",
				e);
		}
		finally
		{
	...
	*/
	public List findPositionsBySilverOjectId(int nSilverObjectId) throws ClassifyEngineException
	{
		Connection connection = null;
	...
	private void removeAllPositionValuesOnAxis(Connection connection, int nAxisId) throws ClassifyEngineException
	{
		...
		catch (Exception e)
		{
			throw new ClassifyEngineException(
				"ClassifyEngine.removeAllPositionValuesOnAxis",
				SilverpeasException.ERROR,
				"classifyEngine.EX_CANT_REMOVE_ALLPOSITIONVALUES",
				e);
		}
		finally
		{
	...
	private List removeEmptyPositions(Connection connection) throws ClassifyEngineException
	{
		//-----------------------------
		...
		SilverTrace.info(
			"classifyEngine",
			"ClassifyEngine.removeEmptyPositions",
			"root.MSG_GEN_PARAM_VALUE",
			"sSQLStatement= " + sSQLStatement);
		PreparedStatement prepStmt = null;
		ResultSet resSet = null;
		...
		catch (Exception e)
		{
			throw new ClassifyEngineException(
				"ClassifyEngine.removeEmptyPositions",
				SilverpeasException.ERROR,
				"classifyEngine.EX_CANT_REMOVE_EMPTYPOSITIONS",
				e);
		}
		finally
		{
	...
	*		
	*/
	public void replaceValuesOnAxis(Connection connection, List oldValue, List newValue) throws ClassifyEngineException
	{
		boolean bCloseConnection = false;
		...
		for (int nI = 0; nI < oldValue.size(); nI++)
		{
			...
			if (oldV.getValue().equals(newV.getValue()))
				throw new ClassifyEngineException(
					"ClassifyEngine.replaceValuesOnAxis",
					SilverpeasException.ERROR,
					"classifyEngine.EX_VALUES_IDENTICAL");
		}
		...
		try
		{
			...
			while (nIndex < oldValue.size())
			{
				// build the statement to get the SilverObjectIds
				String sSQLStatement =
					SQLStatement.buildReplaceValuesStatement(
						(Value) oldValue.get(nIndex),
						(Value) newValue.get(nIndex));
		...
		catch (Exception e)
		{
			throw new ClassifyEngineException(
				"ClassifyEngine.replaceValuesOnAxis",
				SilverpeasException.ERROR,
				"classifyEngine.EX_CANT_REPLACE_VALUES",
				e);
		}
		finally
		{
	...
	private void checkSilverObjectId(int nSilverObjectId) throws ClassifyEngineException
	{
		if (nSilverObjectId < 0)
			throw new ClassifyEngineException(
				"ClassifyEngine.checkParameters",
				SilverpeasException.ERROR,
	...
	private void checkPosition(Position position) throws ClassifyEngineException
	{
		if (position != null)
			...
			throw new ClassifyEngineException(
				"ClassifyEngine.checkParameters",
				SilverpeasException.ERROR,
				"classifyEngine.EX_POSITION_NULL");
	}

	private void checkParameters(int nSilverObjectId, Position position) throws ClassifyEngineException
	{
		this.checkSilverObjectId(nSilverObjectId);
	...
	private void checkCriterias(List alCriterias) throws ClassifyEngineException
	{
		...
		if (alCriterias == null)
			throw new ClassifyEngineException(
				"ClassifyEngine.checkCriterias",
				SilverpeasException.ERROR,
				"classifyEngine.EX_CRITERIAS_ARRAY_NULL");
	...
	/*
	 * Return a List of PertinentAxis corresponding to the given criterias for the given AxisIds
	 * The return list is ordered like the given one considering the AxisId
	 *
	 */
	public List getPertinentAxis(List alGivenCriterias, List alAxisIds) throws ClassifyEngineException
	{
		Connection connection = null;
		...
		{
			throw new ClassifyEngineException(
				"ClassifyEngine.getPertinentAxis",
				SilverpeasException.ERROR,
				"classifyEngine.EX_CANT_GET_PERTINENT_AXIS",
				e);
		}
		finally
		{
	...
	private PertinentAxis getSinglePertinentAxis(
		...
		throws SQLException, ClassifyEngineException
	{
		...
		String sSQLStatement = SQLStatement.buildGetPertinentAxisStatement(alCriterias, nAxisId, todayFormatted);

		// Execute the finding
		SilverTrace.info(
			"classifyEngine",
			"ClassifyEngine.getSinglePertinentAxis",
			"root.MSG_GEN_PARAM_VALUE",
			"sSQLStatement= " + sSQLStatement);
		PreparedStatement prepStmt = null;
		ResultSet resSet = null;
		...
		try
		{
			prepStmt = connection.prepareStatement(sSQLStatement);
			resSet = prepStmt.executeQuery();
			SilverTrace.info("classifyEngine", "ClassifyEngine.getSinglePertinentAxis", "root.MSG_GEN_PARAM_VALUE", "Query executed !");
			// Fetch the results
	...
	/*
	 * Return a List of PertinentAxis corresponding to the given criterias for the given AxisIds and given Join Statement
	 * The return list is ordered like the given one considering the AxisId
	 *
	 */
	public List getPertinentAxisByJoin(List alGivenCriterias, List alAxisIds, JoinStatement joinStatementAllPositions)
		throws ClassifyEngineException
	{
		Connection connection = null;
		...
		try
		{
			...
			for (int nI = 0; nI < alAxisIds.size(); nI++)
			{
				int nAxisId = this.getPhysicalAxisId(((Integer) alAxisIds.get(nI)).intValue());
				alPertinentAxis.add(
					this.getSinglePertinentAxisByJoin(
						connection,
						alCriterias,
						nAxisId,
						"",
	...
	public PertinentAxis getSinglePertinentAxisByJoin(
		Connection connection,
		List alCriterias,
		int nAxisId,
		String sRootValue,
	...
	{
		String today = formatter.format(new java.util.Date());
		return getSinglePertinentAxisByJoin(
			connection,
			alCriterias,
			nAxisId,
			sRootValue,
	...
	public PertinentAxis getSinglePertinentAxisByJoin(
		...
		throws SQLException, ClassifyEngineException
	{
		SilverTrace.info("classifyEngine", "ClassifyEngine.getSinglePertinentAxisByJoin", "root.MSG_GEN_ENTER_METHOD", "axisId = "+nAxisId);
		boolean bCloseConnection = false;
		...
		try
		{
			...
			// build the statements
			String sSQLStatement =
				SQLStatement.buildGetPertinentAxisStatementByJoin(
					alCriterias,
					nAxisId,
					sRootValue,
					joinStatementAllPositions,
			...
			{
				// Execute the finding
				SilverTrace.info("classifyEngine", "ClassifyEngine.getSinglePertinentAxisByJoin", "root.MSG_GEN_PARAM_VALUE", "sSQLStatement= " + sSQLStatement);
				prepStmt = connection.prepareStatement(sSQLStatement);
				resSet = prepStmt.executeQuery();
				SilverTrace.info("classifyEngine", "ClassifyEngine.getSinglePertinentAxisByJoin", "root.MSG_GEN_PARAM_VALUE", "Query executed !");
		...
		finally
		{
			...
			SilverTrace.info("classifyEngine", "ClassifyEngine.getSinglePertinentAxisByJoin", "root.MSG_GEN_EXIT_METHOD");
		}
	}

	/*
	 * Return a List of PertinentValues corresponding to the givenAxisId
	 * The return list is ordered like the given one considering the AxisId
	 *
	 */
	public List getPertinentValues(List alGivenCriterias, int nLogicalAxisId) throws ClassifyEngineException
	{
		SilverTrace.info("classifyEngine", "ClassifyEngine.getPertinentValues", "root.MSG_GEN_ENTER_METHOD", "nLogicalAxisId = "+nLogicalAxisId);
		...
		try
		{
			...
			// Build the statement
			String sSQLStatement =
				SQLStatement.buildGetPertinentValueStatement(
					alCriterias,
					this.getPhysicalAxisId(nLogicalAxisId),
					today);
		...
		catch (Exception e)
		{
			throw new ClassifyEngineException(
				"ClassifyEngine.getPertinentValues",
				SilverpeasException.ERROR,
				"classifyEngine.EX_CANT_GET_PERTINENT_VALUES",
				e);
		}
		finally
		{
	 ...
	 *
	 */
	public List getPertinentValuesByJoin(
		List alGivenCriterias,
		int nLogicalAxisId,
		JoinStatement joinStatementAllPositions)
		throws ClassifyEngineException
	{
		SilverTrace.info("classifyEngine", "ClassifyEngine.getPertinentValuesByJoin", "root.MSG_GEN_ENTER_METHOD", "nLogicalAxisId = "+nLogicalAxisId);
		Connection connection = null;
		...
		try
		{
			...
			// Build the statement
			String sSQLStatement =
				SQLStatement.buildGetPertinentValueByJoinStatement(
					alCriterias,
					this.getPhysicalAxisId(nLogicalAxisId),
					joinStatementAllPositions,
					today);
			...
			// Execute the finding
			SilverTrace.info("classifyEngine", "ClassifyEngine.getPertinentValuesByJoin", "root.MSG_GEN_PARAM_VALUE", "sSQLStatement= " + sSQLStatement);
			prepStmt = connection.prepareStatement(sSQLStatement);
			resSet = prepStmt.executeQuery();
			SilverTrace.info("classifyEngine", "ClassifyEngine.getPertinentValuesByJoin", "root.MSG_GEN_PARAM_VALUE", "Query executed !");
	...
	/*
	 * Return a List of ObjectValuePair corresponding to the givenAxisId
	 * The return list is ordered like the given one considering the AxisId
	 *
	 */
	public List getObjectValuePairsByJoin(
		List alGivenCriterias,
		int nLogicalAxisId,
		JoinStatement joinStatementAllPositions)
		throws ClassifyEngineException
	{
		SilverTrace.info("classifyEngine", "ClassifyEngine.getObjectValuePairsByJoin", "root.MSG_GEN_ENTER_METHOD", "nLogicalAxisId = "+nLogicalAxisId);
		Connection connection = null;
		...
		try
		{
			...
			// Build the statement
			String sSQLStatement =
				SQLStatement.buildGetObjectValuePairsByJoinStatement(
					alCriterias,
					this.getPhysicalAxisId(nLogicalAxisId),
					joinStatementAllPositions,
					today, true);
			...
			// Execute the finding
			SilverTrace.info("classifyEngine", "ClassifyEngine.getObjectValuePairsByJoin", "root.MSG_GEN_PARAM_VALUE", "sSQLStatement= " + sSQLStatement);
			prepStmt = connection.prepareStatement(sSQLStatement);
			resSet = prepStmt.executeQuery();
			SilverTrace.info("classifyEngine", "ClassifyEngine.getObjectValuePairsByJoin", "root.MSG_GEN_PARAM_VALUE", "Query executed !");

			// Fetch the results
			List objectValuePairs = new ArrayList();
			while (resSet.next())
			{
				ObjectValuePair ovp = new ObjectValuePair(resSet.getInt(1), resSet.getString(2), resSet.getString(3));
