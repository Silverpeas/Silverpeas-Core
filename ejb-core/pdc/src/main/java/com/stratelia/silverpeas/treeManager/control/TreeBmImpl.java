...
public class TreeBmImpl implements TreeBm {
	...
	public TreeNode getRoot(Connection con, String treeId) throws TreeManagerException {
		SilverTrace.info("treeManager", "TreeManagerBmImpl.getRoot()", "root.MSG_GEN_PARAM_VALUE", "treeId = "+treeId);
		String whereClause = "treeId = "+treeId+" and levelNumber = 0";
		TreeNode root = null;
		try
		{
			List roots = (List) getDAO().findByWhereClause(new TreeNodePK("useless"), whereClause);
			if (roots.size()>0) {
				SilverTrace.info("treeManager", "TreeManagerBmImpl.getRoot()", "root.MSG_GEN_PARAM_VALUE", "roots.size() = "+roots.size());
		...
		{
			throw new TreeManagerException("TreeBmImpl.deleteTree()", SilverpeasException.ERROR, "treeManager.DELETING_TREE_FAILED", e);
		}
		return root;
	}

	public String createRoot(Connection con, TreeNode root) throws TreeManagerException {
		SilverTrace.info("treeManager", "TreeManagerBmImpl.createRoot()", "root.MSG_GEN_PARAM_VALUE", "root = "+root.toString());
		...
		{
			throw new TreeManagerException("TreeBmImpl.createRoot()", SilverpeasException.ERROR, "treeManager.CREATING_ROOT_FAILED", e);
		}
	...
	{
		SilverTrace.info("treeManager", "TreeManagerBmImpl.updateNode()", "root.MSG_GEN_PARAM_VALUE", "node = "+node.toString());
		String nodeId = node.getPK().getId();
		String treeId = node.getTreeId();
		int order = node.getOrderNumber();
		//recupere les noeuds freres ordonnés qui ont un numéro d'ordre >= à celui du noeud à modifier
		String whereClause = "path = (SELECT path FROM SB_Tree_Tree WHERE treeId = "+treeId+" and id = "+nodeId+") and treeId = "+treeId+" and orderNumber >= "+order+" ORDER BY orderNumber ASC";
		//String whereClause = "path = '"+node.getPath()+"' and treeId = "+treeId+" and orderNumber >= "+order+" ORDER BY orderNumber ASC";

		try
		{
			Collection nodesToUpdate = getDAO().findByWhereClause(con, node.getPK(), whereClause);
			...
			SilverTrace.info("treeManager", "TreeManagerBmImpl.updateNode()", "root.MSG_GEN_PARAM_VALUE", "Avant updateNode()");
            ...
            if (node.isRemoveTranslation())
            {
            	...
            	{
            		List translations = treeI18NDAO.getTranslations(con, node.getTreeId(), node.getPK().getId());
            ...
            else
            {
	            if (node.getLanguage() != null)
	            {
	                ...
	                {
	            		TreeNodeI18N newNode = new TreeNodeI18N(Integer.parseInt(node.getPK().getId()), node.getLanguage(), node.getName(), node.getDescription());
			...
			SilverTrace.info("treeManager", "TreeManagerBmImpl.updateNode()", "root.MSG_GEN_PARAM_VALUE", "Apres updateNode()");

			//Le noeud a changé de place, on décale les noeuds dont l'ordre est supérieur ou égal
			if (nodeHasMoved) {
				it = nodesToUpdate.iterator();
		...
		{
			throw new TreeManagerException("TreeBmImpl.updateNode()", SilverpeasException.ERROR, "treeManager.UPDATING_NODE_FAILED", e);
		}
	...
	public void updateRoot(Connection con, TreeNode node) throws TreeManagerException {
		SilverTrace.info("treeManager", "TreeManagerBmImpl.updateRoot()", "root.MSG_GEN_PARAM_VALUE", "node = "+node.toString());
		try
		{	
			String treeId = node.getTreeId();
		...
		{
			throw new TreeManagerException("TreeBmImpl.updateRoot()", SilverpeasException.ERROR, "treeManager.UPDATING_ROOT_FAILED", e);
		}
	...
	public void deleteSubTree(Connection con, TreeNodePK rootPK, String treeId) throws TreeManagerException {
		SilverTrace.info("treeManager", "TreeManagerBmImpl.deleteSubTree()", "root.MSG_GEN_PARAM_VALUE", "rootPK = "+rootPK.toString()+", treeId = "+treeId);
		...
		String whereClause = "treeId = "+treeId+" and (path LIKE '"+node.getPath()+rootId+"/%' or id = "+rootId+")";
		try
		{
			...
			for (int i=0; i<subTree.size(); i++) {
				...
				treeI18NDAO.deleteNodeTranslations(con, nodeToDelete.getPK().getId(), treeId);
		...
		{
			throw new TreeManagerException("TreeBmImpl.deleteSubTree()", SilverpeasException.ERROR, "treeManager.DELETING_TREE_FAILED", e);
		}
		...
		{
			throw new TreeManagerException("TreeBmImpl.deleteTree()", SilverpeasException.ERROR, "treeManager.DELETING_NODE_FAILED", e);
		}
		TreeCache.unvalidateTree(treeId);
	}

	public void deleteTree(Connection con, String treeId) throws TreeManagerException {
		SilverTrace.info("treeManager", "TreeManagerBmImpl.deleteTree()", "root.MSG_GEN_PARAM_VALUE", "treeId = "+treeId);
	...
	public List getTree(Connection con, String treeId) throws TreeManagerException {
		SilverTrace.info("treeManager", "TreeManagerBmImpl.getTree()", "root.MSG_GEN_PARAM_VALUE", "treeId = "+treeId);
	...
	public List getTree(Connection con, String treeId, AxisFilter filter) throws TreeManagerException
    {
		...
		if (sortedList == null) {
			...
			if (root != null) {
				SilverTrace.info("treeManager", "TreeManagerBmImpl.getTree()", "root.MSG_GEN_PARAM_VALUE", "root = "+root.toString());
	...
	private void setTranslations(Connection con, TreeNode node) throws TreeManagerException
	{
		if (I18NHelper.isI18N)
		{
			...
			TreeNodeI18N translation = new TreeNodeI18N(Integer.parseInt(node.getPK().getId()), node.getLanguage(), node.getName(), node.getDescription());
			...
			try
			{
				translations = treeI18NDAO.getTranslations(con, node.getTreeId(), node.getPK().getId());
			}
			catch (SQLException e)
			{
				throw new TreeManagerException("TreeBmImpl.setTranslations()", SilverpeasException.ERROR, "treeManager.GETTING_TRANSLATIONS_FAILED", e);
			}
	...
	public List getSubTree(Connection con, TreeNodePK rootPK, String treeId) throws TreeManagerException {
		SilverTrace.info("treeManager", "TreeManagerBmImpl.getSubTree()", "root.MSG_GEN_PARAM_VALUE", "rootPK = "+rootPK.toString()+", treeId = "+treeId);
		TreeNode root = null;
	...
	private int whereInsertNodeToCorrectPlaceInList(ArrayList sortedList, TreeNode nodeToInsert) {
		SilverTrace.info("treeManager", "TreeManagerBmImpl.whereInsertNodeToCorrectPlaceInList()", "root.MSG_GEN_PARAM_VALUE", "nodeToInsert = "+nodeToInsert.toString());
		int	order = nodeToInsert.getOrderNumber();
		String fatherId = nodeToInsert.getFatherId();

		int i = 0;
		//recherche le pere
		while (i < sortedList.size() && !((TreeNode) sortedList.get(i)).getPK().getId().equals(fatherId)) {
			i++;
		}
		if (i == sortedList.size())	{
			...
			while (i < sortedList.size() && ((TreeNode) sortedList.get(i)).getFatherId().equals(fatherId)) {
	 ...
	 */
	private List getDescendants(Connection con, TreeNode root) throws TreeManagerException {
		SilverTrace.info("treeManager", "TreeManagerBmImpl.getDescendants()", "root.MSG_GEN_PARAM_VALUE", "root = "+root.toString());
	 ...
	 * @throws TreeManagerException
	 */
	private List getDescendants(Connection con, TreeNode root, AxisFilter filter) throws TreeManagerException {
		String rootId = root.getPK().getId();
		String treeId = root.getTreeId();
		SilverTrace.info("treeManager", "TreeManagerBmImpl.getDescendants()", "root.MSG_GEN_PARAM_VALUE", "rootId = " + rootId+", treeId = "+treeId);
		String path = root.getPath();
		//String whereClause = "path LIKE (SELECT path + '%' FROM SB_Tree_Tree WHERE id = "+rootId+") ORDER BY path ASC, orderNumber ASC";
		StringBuffer whereClause = new StringBuffer();
		whereClause.append("treeId = ").append(treeId).append(" and (path LIKE '").append(path).append(rootId).append("/%' or id = ").append(rootId).append(")");
        ...
        if ( filter.size() > 0 )
        {
            ...
            for ( int i=0; i<filter.size(); i++ )
            {
                ...
                if ( AxisFilter.NAME.equals(property) )
                {
                    if ( first_condition )
                    {
                        whereClause.append(" and (LOWER(name) like LOWER('").append(condition.getValue()).append("')");
                        first_condition = false;
                    }
                    else
                    {
                        whereClause.append(" or LOWER(name) like LOWER('").append(condition.getValue().toLowerCase()).append("')");
                    }
                }
                else if ( AxisFilter.DESCRIPTION.equals(property) )
                {
                    if ( first_condition )
                    {
                        whereClause.append(" and (LOWER(description) like LOWER('").append(condition.getValue().toLowerCase()).append("')");
                        first_condition = false;
                    }
                    else
                    {
                        whereClause.append(" or LOWER(description) like LOWER('").append(condition.getValue().toLowerCase()).append("')");
                    }
                }
            }
        }
		...
		SilverTrace.info("treeManager", "TreeManagerBmImpl.getDescendants()", "root.MSG_GEN_PARAM_VALUE", "whereClause = "+whereClause.toString());

		List list = null;
		try
		{
			list = (List) getDAO().findByWhereClause(con, root.getPK(), whereClause.toString());
		}
		catch (PersistenceException pe)
		{
			throw new TreeManagerException("TreeBmImpl.getDescendants()", SilverpeasException.ERROR, "treeManager.GETTING_TREE_FAILED", pe);
		}
		return list;
	}

	public TreeNode getNode(Connection con, TreeNodePK nodePK, String treeId) throws TreeManagerException  {
		SilverTrace.info("treeManager", "TreeManagerBmImpl.getNode()", "root.MSG_GEN_PARAM_VALUE", "id = " + nodePK.getId()+", treeId = "+treeId);
		TreeNode node = null;
		try
		{
		...
		{
			throw new TreeManagerException("TreeBmImpl.getNode()", SilverpeasException.ERROR, "treeManager.GETTING_NODE_FAILED", pe);
		}
		return node;
	}
	...
	public List getNodesByName(Connection con, String nodeName) throws TreeManagerException  {
		SilverTrace.info("treeManager", "TreeManagerBmImpl.getNodesByName()", "root.MSG_GEN_PARAM_VALUE", "nodeName = " + nodeName);
		List nodes = null;
		List result = null;
		try
		{
			String whereClause = "name = '"+encode(nodeName)+"'";
			nodes = (List) getDAO().findByWhereClause(con, new TreeNodePK("useless"), whereClause);
			result = persistence2TreeNode(con, nodes);
		}
		catch (PersistenceException pe)
		{
			throw new TreeManagerException("TreeBmImpl.getNodesByName()", SilverpeasException.ERROR, "treeManager.GETTING_NODES_BY_NAME_FAILED", pe);
		}
		return result;
	}	
	...
	public String insertFatherToNode(Connection con, TreeNode nodeToInsert, TreeNodePK refNodePK, String treeId) throws TreeManagerException {
		SilverTrace.info("treeManager", "TreeManagerBmImpl.insertFatherToNode()", "root.MSG_GEN_PARAM_VALUE", "nodeToInsert = " + nodeToInsert.toString()+", refNodePK = "+refNodePK.toString()+", treeId = "+treeId);
		TreeNode refNode = getNode(con, refNodePK, treeId);

		//Mémoriser le père actuel P1 de la fille
		String refPath = refNode.getPath();

		SilverTrace.info("treeManager", "TreeManagerBmImpl.insertFatherToNode()", "root.MSG_GEN_PARAM_VALUE", "refPath = " + refPath);
		...
		{
			throw new TreeManagerException("TreeBmImpl.insertFatherToNode()", SilverpeasException.ERROR, "treeManager.INSERTING_FATHER_FAILED", e);
		}
		...
		SilverTrace.info("treeManager", "TreeManagerBmImpl.insertFatherToNode()", "root.MSG_GEN_PARAM_VALUE", "newPath = " + newPath);
		//refNode.setPath(newPath); //Ici ??????
		updateNode(con, refNode);
		SilverTrace.info("treeManager", "TreeManagerBmImpl.insertFatherToNode()", "root.MSG_GEN_PARAM_VALUE", "Apres updateNode()");
		
		//Modifier le chemin de tous les descendants en insérant le nouveau père juste un remplacement de sous chaine
		...
		if (list.size() > 0) {
			...
			for (int i = 0; i<list.size(); i++)	{
				...
				SilverTrace.info("treeManager", "TreeManagerBmImpl.insertFatherToNode()", "root.MSG_GEN_PARAM_VALUE", "pathToUpdate = " + pathToUpdate);
				endOfPath = pathToUpdate.substring(refPath.length(), pathToUpdate.length());
				SilverTrace.info("treeManager", "TreeManagerBmImpl.insertFatherToNode()", "root.MSG_GEN_PARAM_VALUE", "endOfPath = " + endOfPath);
	...
	public void moveSubTreeToNewFather(Connection con, TreeNodePK nodeToMovePK, TreeNodePK newFatherPK, String treeId, int orderNumber) throws TreeManagerException 
	{
		...
		// idée le nouveau niveau = niveauActuel - niveauAncienPere + niveauNouveauPere
		//Mémoriser le niveau du père actuel soit le niveauActuel -1 de la valeur déplacée 
		...
		SilverTrace.info("treeManager", "TreeManagerBmImpl.moveSubTreeToNewFather()", "root.MSG_GEN_PARAM_VALUE", "newPath = " + newRacine );
		...
		if (list.size() > 1) {
			...
			for (int i = 1; i<list.size(); i++)	{
				...
				SilverTrace.info("treeManager", "TreeManagerBmImpl.moveSubTreeToNewFather()", "root.MSG_GEN_PARAM_VALUE", "pathToUpdate = " + pathToUpdate);
				nodeToUpdate.setPath(pathToUpdate );
				updateNode(con, new TreeNode(nodeToUpdate));
			}
		}
	...
	public String createSonToNode(Connection con, TreeNode nodeToInsert, TreeNodePK refNode, String treeId) throws TreeManagerException  {
		SilverTrace.info("treeManager", "TreeManagerBmImpl.createSonToNode()", "root.MSG_GEN_PARAM_VALUE", "nodeToInsert = " + nodeToInsert.toString()+", refNode = "+refNode.toString()+", treeId = "+treeId);
		...
		if (order == -1) {
			//the order is not specified. We are going to insert the new node following the alphabetical order
			ArrayList	brothers			= (ArrayList) getSonsToNode(con, (TreeNodePK) father.getPK(), treeId);
			...
			SilverTrace.info("treeManager", "TreeManagerBmImpl.createSonToNode()", "root.MSG_GEN_PARAM_VALUE", "order was equal to -1 so orderId will be "+order+" following the alphabetic order");
		}
		
		//recupere les noeuds freres ordonnés qui ont un numéro d'ordre >= à celui du noeud à modifier
		String whereClause = "treeId = "+treeId+" and fatherId = "+father.getPK().getId()+" and orderNumber >= "+order+" ORDER BY orderNumber ASC";

		try
		{
			//ATTENTION il faut traiter l'ordre des frères
			Collection nodesToUpdate = getDAO().findByWhereClause(con, father.getPK(), whereClause);
			...
			while (it.hasNext()) {
				...
				//On modifie l'ordre du noeud en ajoutant 1 par rapport au nouveau noeud
				order++;
		...
		{
			throw new TreeManagerException("TreeBmImpl.createSonToNode()", SilverpeasException.ERROR, "treeManager.UPDATING_NODE_FAILED", e);
		}
		...
		{
			throw new TreeManagerException("TreeBmImpl.createSonToNode()", SilverpeasException.ERROR, "treeManager.CREATING_SON_FAILED", e);
		}
		TreeCache.unvalidateTree(treeId);
		return pk.getId();
	}
	...
	public List getSonsToNode(Connection con, TreeNodePK treeNodePK, String treeId) throws TreeManagerException  {
		SilverTrace.info("treeManager", "TreeManagerBmImpl.getSonsToNode()", "root.MSG_GEN_PARAM_VALUE", "treeNodePK = " + treeNodePK.toString()+", treeId = "+treeId);
		String whereClause = "treeId = "+treeId+" and fatherId = "+treeNodePK.getId();
		...
		{
			throw new TreeManagerException("TreeBmImpl.getSonsToNode()", SilverpeasException.ERROR, "treeManager.GETTING_SONS_FAILED", pe);
		}
		return (List) result;	
	}

	public void deleteNode(Connection con, TreeNodePK nodePK, String treeId) throws TreeManagerException  {
		SilverTrace.info("treeManager", "TreeManagerBmImpl.deleteNode()", "root.MSG_GEN_PARAM_VALUE", "nodePK = " + nodePK.toString()+", treeId = "+treeId);
		...
		try
		{
			...
			TreeDAO.changeFatherAndPath(con, new Integer(nodeId).intValue(), new Integer(newFather).intValue(), path, treeId);
		...
		{
			throw new TreeManagerException("TreeBmImpl.deleteNode()", SilverpeasException.ERROR, "treeManager.DELETING_NODE_FAILED", se);
		}
		try
		{
			//Supprime le noeud
		...
		{
			throw new TreeManagerException("TreeBmImpl.deleteNode()", SilverpeasException.ERROR, "treeManager.DELETING_NODE_FAILED", e);
		}
		TreeCache.unvalidateTree(treeId);
	}
	...
	public SilverpeasBeanDAO getDAO() throws TreeManagerException {
		...
		try {
			treeDao = SilverpeasBeanDAOFactory.getDAO("com.stratelia.silverpeas.treeManager.model.TreeNodePersistence");
		}
		catch (PersistenceException pe) {
			throw new TreeManagerException("TreeBmImpl.getDAO()", SilverpeasException.ERROR, "treeManager.GETTING_SILVERPEASBEANDAO_FAILED", pe);
		}
		return treeDao;
	}

	public List getFullPath(Connection con, TreeNodePK nodePK, String treeId) throws TreeManagerException  {
		SilverTrace.info("treeManager", "TreeManagerBmImpl.getFullPath()", "root.MSG_GEN_PARAM_VALUE", "nodePK = " + nodePK.toString()+", treeId = "+treeId);
		...
		try{
			...
			//if (path.length() > 1){
				...
				whereClause += " or id = "+nodePK.getId()+") order by levelNumber ASC";
				SilverTrace.info("treeManager", "TreeManagerBmImpl.getFullPath()", "root.MSG_GEN_PARAM_VALUE", "whereClause = " + whereClause);
				Collection tree = getDAO().findByWhereClause(con, nodePK, whereClause);

				list.addAll(persistence2TreeNode(con, tree));
			//}
		} catch (Exception e) {
			throw new TreeManagerException("TreeBmImpl.getFullPath()", SilverpeasException.ERROR, "treeManager.CREATING_SON_FAILED", e);
		} 
		
		return list;
	}
	
	private List persistence2TreeNode(Connection con, Collection silverpeasBeans) throws TreeManagerException
    {
	...
	public String getPath(Connection con, TreeNodePK nodePK, String treeId) throws TreeManagerException  {
		SilverTrace.info("treeManager", "TreeManagerBmImpl.getPath()", "root.MSG_GEN_PARAM_VALUE", "nodePK = " + nodePK.toString()+", treeId = "+treeId);
		String path = "";
		TreeNode node = null;
		try{
		...
		{
			throw new TreeManagerException("TreeBmImpl.getPath()", SilverpeasException.ERROR, "treeManager.CREATING_SON_FAILED", e);
		}
		return path;
	}
	...
	{
		TreeNode nodeToIndex = getNode(con, (TreeNodePK) node.getPK(), node.getTreeId());
    ...
    {
		SilverTrace.info("treeManager", "TreeBmImpl.createIndex()", "root.MSG_GEN_ENTER_METHOD", "node = " + node.toString());
        FullIndexEntry indexEntry = null;

        if (node != null)
        {
            // Index the Node
            indexEntry = new FullIndexEntry("pdc", "TreeNode", node.getPK().getId()+"_"+node.getTreeId());
    ...
    {
        SilverTrace.info("treeManager", "TreeBmImpl.deleteIndex()", "root.MSG_GEN_ENTER_METHOD", "pk = " + pk);
        IndexEntryPK indexEntry = new IndexEntryPK("pdc", "TreeNode", pk.getId()+"_"+treeId);
