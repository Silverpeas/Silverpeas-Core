...
public class PdcUtilizationDAO
{
	 ...
	 * @see
	 */
	public static List getUsedAxisByInstanceId(Connection con, String instanceId) throws SQLException
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				axis = new UsedAxis(id, instanceId, axisId, baseValue, mandatory, variant);
	...
	*/
	public static int updateBaseValue(
		Connection con,
		int oldBaseValue,
		int newBaseValue,
		int axisId,
		...
		throws SQLException
	{
		//String updateQuery = " update "+PdcUtilizationTable+" set baseValue = ( select fatherId from "+TreeTable+" where treeId = "+treeId+" and id = ? ) where instanceId = ? and axisId = ? and baseValue = ? ";
		String updateQuery =
			" update "
				+ PdcUtilizationTable
	...
	/**
	* Test if the base value can be updated.
	* No new sisters ou new "niece"s are used in the utilisation
	* @param con - the connection to the database
	...
	*/
	public static boolean canUpdateBaseValue(
		Connection con,
		int baseValue,
		String axisId,
		String treeId,
		...
		throws SQLException
	{
		...
		String selectQuery = " select path from " + TreeTable + " where treeId = " + treeId + " and id = " + baseValue;
		Statement stmt = null;
		ResultSet rs = null;
		String motherPath = "";
		String valuePath = "";
	 ...
	 * @see
	 */
	public static boolean isAlreadyAdded(
		Connection con,
		String instanceId,
		int usedAxisId,
		int axisId,
		...
		throws SQLException
	{
		Vector forbiddenValues = new Vector(); // il s'agit du vecteur contenant toutes les valeurs interdites
		...
		if (isAdded)
			SilverTrace.info(
				"Pdc",
				"PdcBmImpl.isAlreadyAdded",
				"root.MSG_GEN_PARAM_VALUE",
				"baseValue "
					+ baseValue
					+ " is already exist for instanceId = "
					+ instanceId
					+ " and axisId = "
					+ axisId);
		else
			SilverTrace.info(
				...
				"baseValue "
					+ baseValue
					+ " does not exist for instanceId = "
					+ instanceId
					+ " and axisId = "
					+ axisId);

		// ensuite, pour chaque valeur de base récupérée, on cherche toute la filiation
		// de celle-ci. Si le vecteur est vide ou si la valeur que l'on reçoit est déja utilisée alors on peut retourner faux
		if ((!allBaseValues.isEmpty()) && (!isAdded))
		{
			...
			String selectQuery = " select path, id from " + TreeTable + " " + whereClause + ")";
			...
			whereClause = "where treeId = " + treeId + " and (1=0 "; // on prépare la prochaine clause Where
	...
	/**
	* Updates into the SB_Pdc_Utilization table all usedAxis.
	* Set the mandatory and variant values
	* @param con - the connection to the database
	* @param usedAxis - the new or modified used axis
	*/
	public static void updateAllUsedAxis(Connection con, UsedAxis usedAxis) throws SQLException
	{
		String updateQuery =
			" update " + PdcUtilizationTable + " set mandatory = ?, variant = ? where instanceId = ? and axisId = ? ";
		PreparedStatement prepStmt = null;
	...
	public static List getAxisUsedByInstanceId(Connection con, List instanceIds) throws SQLException
	{
		return getAxisUsedByInstanceId(con, instanceIds, new AxisFilter());
	}

	public static List getAxisUsedByInstanceId(Connection con, List instanceIds, AxisFilter filter) throws SQLException
	{
		SilverTrace.info(
			"Pdc",
			"PdcBmImpl.getAxisUsedByInstanceId",
			"root.MSG_GEN_PARAM_VALUE",
		...
		for (int i = 0; i < filter.size(); i++)
		{
			...
			if (AxisFilter.NAME.equals(property))
			{
				...
				if (AxisFilter.DESCRIPTION.equals(property))
				{
					if (first_condition)
					{
	...
	/**
	* Returns all basevalue from the SB_Pdc_Utilization table for an axis and an instance of the component
	* @param con - The connection to the database
	...
	*/
	private static Vector getAllBaseValues(Connection con, int usedAxisId, String instanceId, int axisId)
		throws SQLException
	{

		String selectQuery =
			"select baseValue from " + PdcUtilizationTable + " where instanceId = ? and axisId = ? and id <> ? ";

		SilverTrace.info(
			"Pdc",
			"PdcBmImpl.getAllBaseValues",
			"root.MSG_GEN_PARAM_VALUE",
			"selectQuery = select baseValue from "
				+ PdcUtilizationTable
				+ " where instanceId = "
				+ instanceId
				+ " and axisId = "
	...
	*/
	private static Vector getAllDaughterValues(Connection con, Vector forbiddenValues, String whereClause)
		throws SQLException
	{
		String selectQuery = "select id from " + TreeTable + " " + whereClause;
		SilverTrace.info(
			"Pdc",
			"PdcBmImpl.getAllDaughterValues",
			"root.MSG_GEN_PARAM_VALUE",
			"selectQuery = " + selectQuery);
		Statement stmt = null;
		ResultSet rs = null;
		try
		{
			...
			while (rs.next())
			{
				forbiddenValues.add(new Integer(rs.getInt(1)).toString()); // on met ces valeurs dans le vecteur
			}
		}
		finally
		{
