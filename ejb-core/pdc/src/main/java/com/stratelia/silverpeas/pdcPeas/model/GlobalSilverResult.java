...
public class GlobalSilverResult extends GlobalSilverContent implements java.io.Serializable 
{
	...
	public GlobalSilverResult(GlobalSilverContent gsc){
		super(gsc.getName(), gsc.getDescription(), gsc.getId(), gsc.getSpaceId(), gsc.getInstanceId(), gsc.getDate(), gsc.getUserId());
	...
	public GlobalSilverResult(MatchingIndexEntry mie){
		//super(mie.getTitle(), mie.getPreView(), mie.getObjectId(), null, mie.getComponent(), mie.getCreationDate(), mie.getCreationUser());
		super(mie);
		indexEntry = mie;
		...
		{
			super.setThumbnailURL(FileServerUtils.getUrl(null, mie.getComponent(), mie.getThumbnail(), mie.getThumbnailMimeType(), mie.getThumbnailDirectory()));
			
			String[] directory = new String[1];
			directory[0] = mie.getThumbnailDirectory();
			
			File image = new File(FileRepositoryManager.getAbsolutePath(mie.getComponent(), directory)+mie.getThumbnail());
