...
public class PdcSubscriptionBmEJB implements SessionBean {
    ...
    public PDCSubscription getPDCSubsriptionById(int id) throws RemoteException {
        ...
        try {
            ...
            throw new PdcSubscriptionRuntimeException("PdcSubscriptionBmEJB.getPDCSubsriptionById",
                    PdcSubscriptionRuntimeException.ERROR,
                    "PdcSubscription.CANNOT_FIND_SUBSCRIPTION_BYUSID", String.valueOf(id), re);
        } finally {
            closeConnection(conn);
        }
    ...
    public void updatePDCSubscription(PDCSubscription subscription) throws RemoteException {
                SilverTrace.info("PdcSubscription", "PdcSubscriptionBmEJB.updatePDCSubscription()", "root.MSG_GEN_ENTER_METHOD", "subscription = "+subscription.toString());
        Connection conn = null;
    ...
    public void checkAxisOnDelete(int axisId, String axisName) throws RemoteException {
        ...
        try {
            ...
            subscriptions = PdcSubscriptionDAO.getPDCSubscriptionByUsedAxis(conn, axisId);
            if (subscriptions == null || (subscriptions != null && subscriptions.size() <= 0)) {
                return;
            }
     ...
     */
    public void checkValueOnDelete(int axisId, String axisName, List oldPath, List newPath, List pathInfo) throws RemoteException {
        Connection conn = null;
        List subscriptions = null;
        ...
        try {
            ...
            for(int i = 0; i < subscriptions.size(); i++) {
              PDCSubscription subscription = (PDCSubscription)subscriptions.get(i);
                // for each subscription containing axis affected by value deletion check if any criteria value has been
                // deleted
    ...
    {
        SilverTrace.info("PdcSubscription", "PdcSubscriptionBmEJB.checkSubscriptions()", "root.MSG_GEN_ENTER_METHOD",
                "classifyValues = " + classifyValues + ", componentId = " + componentId + ", silverObjectid = " + silverObjectid);
        ...
        try {
            ...
            for (int i = 0; i < subscriptions.size(); i++) {
                PDCSubscription subscription = (PDCSubscription) subscriptions.get(i);
                // check if current subscription corresponds a list of classify values provided into the method
                if (isCorrespondingSubscription(subscription, classifyValues)) {
                    if (silverContent == null) {
                        ...
                        firstAdminId = getFirstAdministrator(organizationController,subscription.getOwnerId());
                    }
                    //The current subscription matches the new classification.
                    //Now, we have to test if subscription's owner is allowed to access the classified item.
                    ...
                    if (roles.length > 0) {
						//if user have got at least one role, sends a notification to the user specified in pdcSubscription
					  	sendSubscriptionNotif(subscription, spaceAndInstanceNames, componentId, silverContent, firstAdminId);
                    }
                }
            }
        ...
        */
        private SilverContentInterface getSilverContent(String componentId, int silverObjectId) {
            ...
            try {
                ...
                userRoles.add("admin");
                silverContents = contentInterface.getSilverContentById(silverobjectIds, componentId, "unknown", userRoles);
            } catch (Exception e) {
                throw new PdcSubscriptionRuntimeException("PdcSubscriptionBmEJB.sendSubscriptionNotif",
                        PdcSubscriptionRuntimeException.ERROR, "PdcSubscription.EX_CHECK_SUBSCR_FALIED", e);
            }
        ...
        */
        private ArrayList getSpaceAndInstanceNames(String componentId, OrganizationController organizationController) {
     ...
     */
    protected boolean checkSubscriptionRemove(PDCSubscription subscription, int axisId, List oldPath, List newPath) {
     ...
     */
    protected boolean checkValuesRemove(String originalPath, List oldPath, List newPath) {
        ...
        // check if extracted value presented in old path but not presented in newPath
     ...
     * <br> Ex1: path /2/3/4/5/  value: /5/ result:true
     * <br> Ex2: path /2/3/4/5/  value: /3/ result true
     * <br> Ex2: path /2/3/4/5/  value: /8/ result false
     */
     ...
     */
    protected boolean isCorrespondingSubscription(PDCSubscription subscription, List classifyValues) {
        SilverTrace.info("PdcSubscription", "PdcSubscriptionBmEJB.isCorrespondingSubscription()", "root.MSG_GEN_ENTER_METHOD", "subscription = "+subscription.toString()+", classifyValues = "+classifyValues.toString());
        ...
        /* The following algorithm implemented
               Loop every SearchCriteria and
                 for axis of SearchCriteria found ClassifyValue with such axis if true check getValue()
                 of ClassifyValue and getValue() of SearchCriteria. The start of the value String of ClassifyValue
                 should match the whole value String of SearchCriteria.
        */
    ...
    protected void sendDeleteNotif(PDCSubscription subscription, String axisName,
        ...
        SilverTrace.info("PdcSubscription", "PdcSubscriptionBmEJB.sendDeleteNotif()", "root.MSG_GEN_ENTER_METHOD");

        final String language = getDefaultUserLanguage(subscription.getOwnerId());
        final ResourceLocator resources = new ResourceLocator("com.silverpeas.pdcSubscription.multilang.pdcsubscription", language);
		...
		NotificationMetaData notifMetaData = new NotificationMetaData(NotificationParameters.NORMAL, 
												resources.getString(MESSAGE_DELETE_TITLE), 
												message.toString());
    ...
    protected void sendSubscriptionNotif(PDCSubscription subscription,
                                         ArrayList spaceAndInstanceNames,
                                         String componentId,
            ...
            throws NotificationManagerException
    {
        ...
        if (silverContent != null) {
            ...
            documentUrl = "";
			if (contentUrl != null) {
				documentUrl = "/RpdcSearch/jsp/GlobalContentForward?contentURL="+URLEncoder.encode(contentUrl)+"&componentId="+componentId;
			}
        ...
        sendNotification(userID, fromUserId, subscriptionName, instanceName, componentId, spaceName, classifiedDate, "standartMessage", documentUrl, documentName);
    }
    ...
    protected Connection openConnection() {
        try {
            ...
            throw new PdcSubscriptionRuntimeException("PdcSubscription.getConnection()", PdcSubscriptionRuntimeException.ERROR,
    ...
    protected void closeConnection(Connection con) {
        if (con != null) {
            try {
                ...
                SilverTrace.error("PdcSubscription", "PdcSubscriptionBmEJB.closeConnection()",
    ...
    protected void sendNotification(int userID, int fromUserID, String subscription, String component, String componentID,
                                    String workSpace, java.util.Date classifiedDate, String notificationMessageKey,
                                    String documentUrl, String documentName)
            throws NotificationManagerException
    {
        SilverTrace.info("PdcSubscription", "PdcSubscriptionBmEJB.sendNotification()", "root.MSG_GEN_ENTER_METHOD",
                "userID = " + userID + ", fromUserID = " + fromUserID + ", subscription = " + subscription +
                ", componentId = " + component + ", workSpaceId = " + workSpace + ", notificationMessageKey = " +
                notificationMessageKey + ", documentUrl = " + documentUrl);
		...
		NotificationMetaData notifMetaData = new NotificationMetaData(NotificationParameters.NORMAL, 
												resources.getString(notificationMessageKey), 
												message.toString());
    ...
    protected String getDefaultUserLanguage(int userID) {
        ...
        try {
            ...
            throw new PdcSubscriptionRuntimeException("PdcSubscriptionBmEJB.getDefaultUserLanguage()",
                    PdcSubscriptionRuntimeException.ERROR, "root.EX_CANT_GET_PREFERRED_USER_LANG", e);
        }
        return lang;
    }
