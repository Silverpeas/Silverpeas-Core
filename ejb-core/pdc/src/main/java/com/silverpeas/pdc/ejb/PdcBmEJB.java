...
public class PdcBmEJB implements javax.ejb.SessionBean
{
	...
	public ArrayList findGlobalSilverContents(ContainerPositionInterface containerPosition, List componentIds, boolean recursiveSearch, boolean visibilitySensitive)
	{
		ArrayList silverContentIds = new ArrayList();
		try
		{
			//get the silverContentids classified in the context
			silverContentIds.addAll(getPdcBm().findSilverContentIdByPosition(containerPosition, componentIds, recursiveSearch, visibilitySensitive));
		}
		catch (ContainerManagerException c)
		{
			throw new PdcBmRuntimeException("PdcBmEJB.findGlobalSilverContents", SilverpeasRuntimeException.ERROR,"root.EX_CANT_GET_REMOTE_OBJECT",c);
		}		

		return (ArrayList) getSilverContentsByIds(silverContentIds);
	}

	public ArrayList findGlobalSilverContents(ContainerPositionInterface containerPosition, List componentIds, String authorId, String afterDate, String beforeDate, boolean recursiveSearch, boolean visibilitySensitive)
	{
		ArrayList silverContentIds = new ArrayList();
		try
		{
			//get the silverContentids classified in the context
			silverContentIds.addAll(getPdcBm().findSilverContentIdByPosition(containerPosition, componentIds, authorId, afterDate, beforeDate, recursiveSearch, visibilitySensitive));
		}
		catch (ContainerManagerException c)
		{
			throw new PdcBmRuntimeException("PdcBmEJB.findGlobalSilverContents", SilverpeasRuntimeException.ERROR,"root.EX_CANT_GET_REMOTE_OBJECT",c);
	...
	{
		SilverTrace.info("Pdc", "PdcBmEJB.getValue", "root.MSG_GEN_PARAM_VALUE", "axisId = "+axisId+", valueId = "+valueId);
		Value value = null;
		try
		{
		...
		{
			throw new PdcBmRuntimeException("PdcBmEJB.getValue", SilverpeasRuntimeException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
		SilverTrace.info("Pdc", "PdcBmEJB.getValue", "root.MSG_GEN_PARAM_VALUE", "returned valueId = "+value.getValuePK().getId());
		return value;
	}
	...
	public int getSilverContentId(String objectId, String componentId)
	{
		...
		try
		{
			silverContentId = getContentManager().getSilverContentId(objectId, componentId);	
		}
		catch (Exception e)
		{
			throw new PdcBmRuntimeException("PdcBmEJB.getSilverContentId", SilverpeasRuntimeException.ERROR,"root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
	...
	public ArrayList getPositions(int silverContentId, String componentId)
	{
		...
		try
		{
			positions = (ArrayList) getPdcBm().getPositions(silverContentId, componentId);	
		}
		catch (PdcException e)
		{
			throw new PdcBmRuntimeException("PdcBmEJB.getPositions", SilverpeasRuntimeException.ERROR,"root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
		return positions;
	}
	...
	public List getSilverContentIds(List docFeatures)
	{
		...
		try
		{
			silverContentIds = (ArrayList) getContentManager().getSilverContentId(docFeatures);
		}
		catch (Exception e)
		{
			throw new PdcBmRuntimeException("PdcBmEJB.getSilverContentIds", SilverpeasRuntimeException.ERROR,"root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
	...
	public String getInternalContentId(int silverContentId)
	{
		...
		{
			throw new PdcBmRuntimeException("PdcBmEJB.getInternalContentId", SilverpeasRuntimeException.ERROR,"root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
		return objectId;
	}
	...
	public AxisHeader getAxisHeader(String axisId)
	{
		...
		{
			throw new PdcBmRuntimeException("PdcBmEJB.getAxisHeader", SilverpeasRuntimeException.ERROR,"root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
		return axis;
	}
	...
	{
        SilverTrace.info("Pdc", "PdcBmEJB.getSilverContentsByIds", "root.MSG_GEN_PARAM_VALUE", "silverContentIds = " + silverContentIds);
		...
		try
		{
		    ...
		    SilverTrace.info("Pdc", "PdcBmEJB.getSilverContentsByIds", "root.MSG_GEN_PARAM_VALUE", "alInstanceIds = " + alInstanceIds);	
		}
		catch (ContentManagerException c)
		{
			throw new PdcBmRuntimeException("PdcBmEJB.getSilverContentsByIds", SilverpeasRuntimeException.ERROR,"root.EX_CANT_GET_REMOTE_OBJECT",c);
		}
		...
		for (int j = 0; j < alInstanceIds.size(); j++) {
			...
			try
			{
			    ...
			    SilverTrace.info("Pdc", "PdcBmEJB.getSilverContentsByIds", "root.MSG_GEN_PARAM_VALUE", "allSilverContentIds = " + allSilverContentIds + " in instance " + instanceId);
			}
			catch (ContentManagerException c)
			{
				throw new PdcBmRuntimeException("PdcBmEJB.getSilverContentsByIds", SilverpeasRuntimeException.ERROR,"root.EX_CANT_GET_REMOTE_OBJECT",c);
            ...
            if (contentP != null) {
				try
				{
					...
					silverContentTempo = contentInterface.getSilverContentById(allSilverContentIds, instanceId, null, new ArrayList());	
				}
				catch (ContentManagerException c)
				{
					throw new PdcBmRuntimeException("PdcBmEJB.getSilverContentsByIds", SilverpeasRuntimeException.ERROR,"root.EX_CANT_GET_REMOTE_OBJECT",c);
				...
				{
					throw new PdcBmRuntimeException("PdcBmEJB.getSilverContentsByIds", SilverpeasRuntimeException.ERROR,"root.EX_CANT_GET_REMOTE_OBJECT",e);
				}
        ...
        SilverTrace.info("Pdc", "PdcBmEJB.getSilverContentsByIds", "root.MSG_GEN_PARAM_VALUE", "silverContent size= " + alSilverContents.size());
		...
		SilverTrace.info("Pdc", "PdcBmEJB.getSilverContentsByIds", "root.MSG_GEN_PARAM_VALUE", "silverContentIds = " + silverContentIds.toString());
	 ...
	 */
    private List transformSilverContentsToGlobalSilverContents(List silverContentTempo) {
