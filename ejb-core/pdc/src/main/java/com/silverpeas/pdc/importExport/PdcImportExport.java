...
public class PdcImportExport {
	...
	//Méthodes
	/**
	 * Méthodes créant les liens entre les silverObjectId et les positions définies dans un xml 
	 * mappé dans la classe PdcPositionsType.
	 ...
	 * @return - liste des values, fils du value d id fatherValueId, null si le père est une feuille de l'arbre
	 * @throws PdcException
	 */
	private ArrayList getValueTree(int axisId, String fatherValueId) throws PdcException {
		...
		if (listValueId != null) {//L'exception oject non trouvé n'est pas gérée dans la méthode DAO!!!
