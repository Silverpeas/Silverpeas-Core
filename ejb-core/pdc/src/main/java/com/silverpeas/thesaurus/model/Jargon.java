...
/**
* This class contains a full information about a Jargon
* a Jargon is linked to a Vocabulary and a User (UserDetail or Group)
*/
...
public class Jargon extends SilverpeasBean {
	...
	public String readUserName() {
		...
		if (type == 0) //user
		{
			UserDetail userDetail = organizationController.getUserDetail(new Long(getIdUser()).toString());
			if (userDetail != null)		
				userName = userDetail.getLastName() + " " + userDetail.getFirstName();		
		}
		else //group
		{
			Group group = organizationController.getGroup(new Long(getIdUser()).toString());
			if (group != null)		
	...
	public String readVocaName() throws ThesaurusException {
		...
		{
			throw new ThesaurusException("Jargon.readVocaName",SilverpeasException.ERROR,"Thesaurus.EX_CANT_GET_VOCABULARY_NAME","",e);
		}
   ...
   {
		return (getIdUser().equals( ( (Jargon) theOther ).getIdUser()) && 
				getType() == ( (Jargon) theOther).getType());
   }
