...
public class VersioningBmEJB implements SessionBean {
	...
	private static ResourceLocator resources = new ResourceLocator("com.stratelia.webactiv.util.versioning.Versioning", "");
    ...
    public Document getDocument(DocumentPK pk) {
        ...
        try {
            ...
            throw new VersioningRuntimeException("VersioningBmEJB.getDocument",
                    SilverpeasRuntimeException.ERROR, "versioning.GETTING_DOCUMENT_FAILED", pk, re);
        } finally {
            closeConnection(con);
        }

        if (result == null) {
            throw new VersioningRuntimeException("VersioningBmEJB.getDocument", SilverpeasRuntimeException.ERROR,
    ...
    public ArrayList getDocuments(ForeignPK foreignPK) {
        ...
        try {
            ...
            throw new VersioningRuntimeException("VersioningBmEJB.getDocuments",
                    SilverpeasRuntimeException.ERROR, "versioning.GETTING_DOCUMENT_FAILED", foreignPK, re);
        } finally {
            closeConnection(con);
        }
    ...
    public DocumentPK createDocument(Document docToCreate, DocumentVersion initialVersion) {
        Connection con = openConnection();
        DocumentPK documentPK = null;
        if (docToCreate == null) {
            throw new VersioningRuntimeException("VersioningBmEJB.createDocument",
                    SilverpeasRuntimeException.ERROR, "root.EX_NULL_VALUE_OBJECT_OR_PK", docToCreate);
        }
        docToCreate.setStatus(Document.STATUS_CHECKINED);

        try {
            documentPK = VersioningDAO.createDocument(con, docToCreate, initialVersion);
        } catch (Exception re) {
            throw new VersioningRuntimeException("VersioningBmEJB.createDocument",
                    SilverpeasRuntimeException.ERROR, "versioning.CREATING_NEW_DOCUMENT_FAILED", docToCreate, re);
        } finally {
            closeConnection(con);
        }
        ...
        if (documentPK == null) {
            throw new VersioningRuntimeException("VersioningBmEJB.createDocument",
                    SilverpeasRuntimeException.ERROR, "versioning.CREATING_NEW_DOCUMENT_FAILED", docToCreate);
        }
    ...
    {
		SilverTrace.debug("versioning", "addDays", "root.MSG_GEN_PARAM_VALUE", "nbDay = " + nbDay);
    ...
    private void updateDates(Document doc)
    {
		...
		String day = orga.getComponentParameterValue(doc.getInstanceId(), "nbDayForReservation");
		if (StringUtil.isDefined(day))
		{
			int nbDay = Integer.parseInt(day);
			SilverTrace.debug("versioning", "updateDates", "root.MSG_GEN_PARAM_VALUE", "nbDay = " + nbDay);
			// 2. calcul la date d'expiration en fonction de la date d'aujourd'hui et de la durée de réservation
			Calendar calendar = Calendar.getInstance(Locale.FRENCH);
			//calendar.add(Calendar.DATE, nbDay);
			SilverTrace.debug("versioning", "updateDates", "root.MSG_GEN_PARAM_VALUE", "calendar.getTime() AVANT = " + calendar.getTime());
			addDays(calendar, nbDay);
			SilverTrace.debug("versioning", "updateDates", "root.MSG_GEN_PARAM_VALUE", "calendar.getTime() APRES = " + calendar.getTime());
		...
		if (delayReservedFile >= 0 && delayReservedFile <= 100)
		{
			...
			if (StringUtil.isDefined(day))
			{
				...
				SilverTrace.debug("versioning", "updateDates", "root.MSG_GEN_PARAM_VALUE", "delayReservedFile = " + delayReservedFile);
				SilverTrace.debug("versioning", "updateDates", "root.MSG_GEN_PARAM_VALUE", "result = " + result);
				if (result > 2)
				{
					Calendar calendar = Calendar.getInstance(Locale.FRENCH);
    ...
    public void checkDocumentOut(DocumentPK documentPK, int ownerId, java.util.Date checkOutDate) {
        Connection con = openConnection();
		SilverTrace.debug("versioning", "checkDocumentOut", "root.MSG_GEN_PARAM_VALUE", "instanceId = " + documentPK.getId());

        try {
        	Document doc = getDocument(documentPK);
        	updateDates(doc);
        	
    		SilverTrace.debug("versioning", "checkDocumentOut", "root.MSG_GEN_PARAM_VALUE", "expiryDate = " + doc.getExpiryDate());

            VersioningDAO.checkDocumentOut(con, documentPK, ownerId, checkOutDate, doc.getAlertDate(), doc.getExpiryDate());
           ...
           if(lastVersion.isOpenOfficeCompatibleDocument()){
            ...
            throw new VersioningRuntimeException("VersioningBmEJB.checkDocumentOut",
                    SilverpeasRuntimeException.ERROR, "versioning.CHECK_DOCUMENT_OUT", documentPK, re);
        } finally {
            closeConnection(con);
        }
    }
    ...
    public void checkDocumentIn(DocumentPK documentPK, int userId) {
        Connection con = openConnection();
		SilverTrace.debug("versioning", "checkDocumentIn", "root.MSG_GEN_PARAM_VALUE", "instanceId = " + documentPK.getId());
		 
		try {
            Document doc = VersioningDAO.getDocument(con, documentPK);

            SilverTrace.debug("versioning", "checkDocumentIn", "root.MSG_GEN_PARAM_VALUE", "doc.getTypeWorkList() = " + doc.getTypeWorkList() + " / doc.getTypeWorkList() = " + doc.getTypeWorkList() );
            ...
            while(iter.hasNext()) {
              RepositoryHelper.getJcrDocumentService().deleteDocument((DocumentVersion) iter.next());
            }
        } catch (Exception re) {
            throw new VersioningRuntimeException("VersioningBmEJB.checkDocumentIn",
                    SilverpeasRuntimeException.ERROR, "versioning.CHECK_DOCUMENT_IN", documentPK, re);
        } finally {
            closeConnection(con);
        }
    }
    ...
    protected void checkDocumentInOrdered(Connection con, Document doc, int userId) throws SQLException  {
        VersioningDAO.checkDocumentIn(con, doc.getPk());
        ArrayList versions = VersioningDAO.getDocumentVersions(con, doc.getPk());
        DocumentVersion lastVersion = (DocumentVersion) versions.get(versions.size() - 1);
        if (lastVersion.getStatus() == DocumentVersion.STATUS_VALIDATION_REQUIRED) {
    ...
    public void updateDocument(Document documentToUpdate) {
        ...
        try {
            ...
            throw new VersioningRuntimeException("VersioningBmEJB.updateDocument",
                    SilverpeasRuntimeException.ERROR, "versioning.UPDATING_DOCUMENT_FAILED", documentToUpdate, re);
        } finally {
            closeConnection(con);
        }
    }
    ...
    public void updateDocumentVersion(DocumentVersion documentVersion) {
        ...
        try {
            ...
            throw new VersioningRuntimeException("VersioningBmEJB.updateDocumentVersion",
                    SilverpeasRuntimeException.ERROR, "versioning.UPDATING_DOCUMENT_VERSION_FAILED", documentVersion, re);
        } finally {
            closeConnection(con);
        }
    }

	public void updateDocumentForeignKey(DocumentPK documentPK, ForeignPK foreignKey) {
		Connection con = openConnection();
		try {
			VersioningDAO.updateDocumentForeignKey(con, documentPK, foreignKey);
		} catch (Exception re) {
			throw new VersioningRuntimeException("VersioningBmEJB.updateDocument", SilverpeasRuntimeException.ERROR, "versioning.UPDATING_DOCUMENT_FAILED", documentPK, re);
		} finally {
			closeConnection(con);
		}
	}
    ...
    public void updateWorkList(Document document) {
        ...
        try {
            ...
            throw new VersioningRuntimeException("VersioningBmEJB.updateWorkList",
                    SilverpeasRuntimeException.ERROR, "versioning.UPDATING_WORKER_LIST_FAILED", document, re);
        } finally {
            closeConnection(con);
        }
    }
    ...
    public void deleteWorkList(Document document, boolean keepSaved) {
        ...
        try {
            ...
            throw new VersioningRuntimeException("VersioningBmEJB.deleteWorkList",
                    SilverpeasRuntimeException.ERROR, "versioning.DELETING_WORKER_LIST_FAILED", document, re);
        } finally {
            closeConnection(con);
        }
    }
    ...
    public ArrayList getDocumentVersions(DocumentPK documentPK) {
        ...
        try {
            ...
            throw new VersioningRuntimeException("VersioningBmEJB.getDocumentVersions",
                    SilverpeasRuntimeException.ERROR, "versioning.GETTING_ALL_DOCUMENT_VERSIONS_FAILED", documentPK, re);
        } finally {
            closeConnection(con);
        }

        if (result == null) {
            throw new VersioningRuntimeException("VersioningBmEJB.getDocumentVersions", SilverpeasRuntimeException.ERROR,
                    "versioning.GETTING_ALL_DOCUMENT_VERSIONS_FAILED", "No versions found" + documentPK);
        }
	...
	public List getAllPublicDocumentVersions(DocumentPK documentPK) {
		...
		try {
			publicVersions = VersioningDAO.getAllPublicDocumentVersions(con, documentPK);
		} catch (Exception re) {
			throw new VersioningRuntimeException("VersioningBmEJB.getAllPublicDocumentVersions",
					SilverpeasRuntimeException.ERROR, "versioning.GETTING_ALL_PUBLIC_DOCUMENT_VERSIONS_FAILED",
					documentPK, re);
		} finally {
			closeConnection(con);
		}
    ...
    /**
     * Adding new <code>DocumentVersion</code> in a case of Non-ordered and Non-validated document type
     */
    ...
    public DocumentVersion addNonOrderedValidatedVersion(Document doc, DocumentVersion newVersion) {
        ...
        {
            con = openConnection();
            if (worker.getId() == newVersion.getAuthorId() && newVersion.getType() == DocumentVersion.TYPE_PUBLIC_VERSION) {
                con = openConnection();
                newVersion.setDocumentPK(doc.getPk());
                ...
                try {
                    ...
                    if (pk == null) {
                        throw new VersioningRuntimeException("VersioninBmEJB.addNonOrderedValidatedVersion",
                                SilverpeasRuntimeException.ERROR, "versioning.CANNOT_CREATE_NEWVERSION", newVersion);
                    }
                } catch (SQLException e) {
                    throw new VersioningRuntimeException("VersioninBmEJB.addNonOrderedNonValidatedVersion",
            ...
            if (worker == null) {
                throw new VersioningRuntimeException("VersioninBmEJB.addNonOrderedValidatedVersion",
                        SilverpeasRuntimeException.ERROR, "versioning.EX_NOVALIDATOR_IN_VALIDATED_LIST", doc);
            }
            String docUrl = getDocumentUrl(doc);

            try {
                sendNotification(worker.getId(), "notification.pleaseValidate", docUrl, currUser, doc);
            } catch (NotificationManagerException e) {
                SilverTrace.error("versioning", "VersioningBmEJB.addNonOrderedValidatedVersion",
    ...
    public DocumentVersion addOrderedValidatedVersion(Document doc, DocumentVersion newVersion) {
        ...
        try {
            Worker currWorker = (Worker) workList.get(currUserNum);
            nextWorker = getNextWorker(workList, currUserNum, true);

            if (nextWorker != null) {
                nextUserID = nextWorker.getId();
                if (currWorker.getOrder() == nextWorker.getOrder() ||
                        !nextWorker.isWriter()) {
                    newVersion.setStatus(DocumentVersion.STATUS_VALIDATION_REQUIRED);
                } else {
                    newVersion.setStatus(DocumentVersion.STATUS_VALIDATION_NOT_REQ);
            ...
            DocumentVersionPK pk = addVersion(newVersion);
            if (pk == null) {
                throw new VersioningRuntimeException("VersioninBmEJB.addOrderedValidatedVersion",
                        SilverpeasRuntimeException.ERROR, "versioning.CANNOT_CREATE_NEWVERSION", newVersion);
            }
            newVersion.setPk(pk);
            ...
            if (nextUserID != -1 && nextUserID != doc.getOwnerId()) {
                ...
                String docUrl = getDocumentUrl(doc);
                try {
                    String message = (newVersion.getStatus() == DocumentVersion.STATUS_VALIDATION_REQUIRED)? "notification.pleaseValidate": "notification.processWork";
                    sendNotification(nextUserID, message, docUrl, currWorker.getId(), doc);
                } catch (NotificationManagerException e) {
                    SilverTrace.error("versioning", "VersioningBmEJB.sendNotification", "root.EX_NOTIFY_USERS_FAILED",
                            Integer.toString(nextUserID), e);
                }
            ...
            throw new VersioningRuntimeException("VersioninBmEJB.addOrderedValidatedVersion",
                    SilverpeasRuntimeException.ERROR, "versioning.EX_CURRENT_WORKLIST_ORDER_OUTOFBOUNDS", newVersion, e);
     ...
     */
    protected Worker getNextWorker(ArrayList workList, int currentUserNum, boolean skipThisValidator)
     ...
     * @throws ArrayIndexOutOfBoundsException in a case of incorrect currentUserNum index provided
     */
    protected Worker getPrevWorker(ArrayList workList, int currentUserNum, boolean skipThisWorker)
            throws ArrayIndexOutOfBoundsException
    {
        Worker currWorker = (Worker) workList.get(currentUserNum);
    ...
    public DocumentVersionPK addVersion(DocumentVersion newVersion) {
        ...
        try {
			...
			throw new VersioningRuntimeException("VersioningBmEJB.addVersion", SilverpeasRuntimeException.ERROR, "root.EX_SQL_QUERY_FAILED", "", e);
		} finally {
            closeConnection(con);
        }
    ...
    public void deleteDocument(DocumentPK document_pk){
        ...
        try {
            ...
            throw new VersioningRuntimeException("VersioningBmEJB.deleteDocument", SilverpeasRuntimeException.ERROR, "root.EX_SQL_QUERY_FAILED", "", e);
        }
        finally {
            closeConnection(con);
        }
	...
	public void deleteDocumentsByForeignPK(ForeignPK foreignPK){
		try {
			...
			throw new VersioningRuntimeException("VersioningBmEJB.deleteDocumentsByForeignPK",
						   SilverpeasRuntimeException.ERROR, "root.EX_SQL_QUERY_FAILED", "", e);
		}
	}

	public void deleteDocumentsByInstanceId(String instanceId){
		SilverTrace.info("versioning", "VersioningBmEJB.deleteDocumentsByInstanceId", "root.MSG_GEN_ENTER_METHOD", "instanceId = "+instanceId);
		Connection con = openConnection();
		try {
			VersioningDAO.deleteDocumentsByInstanceId(con, instanceId);
		}
		catch (Exception e) {
			throw new VersioningRuntimeException("VersioningBmEJB.deleteDocumentsByInstanceId", SilverpeasRuntimeException.ERROR, "root.EX_SQL_QUERY_FAILED", "instanceId = "+instanceId, e);
		}
		finally {
			closeConnection(con);
		}
                                 ...
                                 java.util.Date validationDate)
    {
        ...
        try {
            ...
            if (currDoc.getTypeWorkList() == 1) {
                ...
                throw new VersioningRuntimeException("VersioninBmEJB.validateDocument",
                        SilverpeasRuntimeException.ERROR, "versioning.UNKNOWN_DOC_WORKLIST_TYPE", currDoc);
            }
     ...
     */
    protected void validateApprovalOrdered(Document doc, int validatorID,
                                           String comment,
                                           java.util.Date validationDate,
                                           Connection conn)
    {
        ...
        if (nextUserID == -1) {
            ...
            try {
                ...
                throw new VersioningRuntimeException("VersioningBmEJB.validateApprovalOrdered",
                        SilverpeasRuntimeException.ERROR, "versioning.VALIDATE_DOCUMENT__FAILED", doc, e);
            }
            ...
            try {
                ...
                throw new VersioningRuntimeException("VersioningBmEJB.validateApprovalOrdered",
                        SilverpeasRuntimeException.ERROR, "versioning.VALIDATE_DOCUMENT_FAILED", doc, e);
            }
            ...
            String docUrl = getDocumentUrl(doc);
            try {
                sendNotification(nextUserID, "notification.processWork", docUrl, currWorker, doc);  // notification

            } catch (NotificationManagerException e) {
                SilverTrace.error("versioning", "VersioningBmEJB.validateApprovalOrdered()", "root.EX_NOTIFY_USERS_FAILED",
                        Integer.toString(nextUserID), e);
            }
        }
    }
     ...
     */
    protected DocumentVersion validateDocumentVersion(DocumentVersion docVersion, String comment,
                                                      java.util.Date validationDate,
                                                      int validatorId,
                                                      boolean isPublic,
                                              ...
                                              Connection conn)
    {
        ...
        if (!isNonOrderedValidator(doc, currVersion, validatorID)) {
            throw new VersioningRuntimeException("VersioninBmEJB.validateApprovalNonOrdered",
                    SilverpeasRuntimeException.ERROR, "versioning.VALIDATION_NOT_REQUIED", doc);
        }

        validateDocumentVersion(currVersion, comment, validationDate, validatorID, true, false, conn);

        try {
            VersioningDAO.checkDocumentIn(conn, doc.getPk());
        } catch (Exception e) {
            throw new VersioningRuntimeException("VersioningBmEJB.validateApprovalNonOrdered",
                    SilverpeasRuntimeException.ERROR, "versioning.VALIDATION_NOT_REQUIED", doc, e);
        }
        ...
        try {
            sendNotification(versionCreatorID, "notification.workValidated", documentUrl, currWorker, doc);
        } catch (NotificationManagerException e) {
            SilverTrace.error("versioning", "VersioningBmEJB.sendNotification", "root.EX_NOTIFY_USERS_FAILED",
                    Integer.toString(versionCreatorID), e);
        }
    }
    ...
    protected boolean isCurrentOrderedValidator(Document doc, int userID)
    {
        if (doc.getTypeWorkList() != 2) {
            throw new IllegalArgumentException("Type of work list in provided document isn't correct");
        }
     ...
     * @return true if userID provided can validate current version of document
     */
    boolean isNonOrderedValidator(Document doc, DocumentVersion currentVersion, int userID)
    {
        if (doc.getTypeWorkList() != 1) {
            throw new IllegalArgumentException("Type of work list in provided document isn't correct");
        }
        ...
        return false; // we cannot find provied user in work list with validation rights
    }
     ...
     * @param validationDate a date when validation process occured.
     */
    public void refuseDocument(Document currDoc, int validatorID,
                               String comment,
                               java.util.Date validationDate)
    {
        Connection con = openConnection();
        try {
            ...
            if (currDoc.getTypeWorkList() == 1) {
                refuseApprovalNonOrdered(currDoc, validatorID, comment, validationDate, con);
            } else if (currDoc.getTypeWorkList() == 2) {
                refuseApprovalOrdered(currDoc, validatorID, comment, validationDate, con);
            } else {
                throw new VersioningRuntimeException("VersioninBmEJB.refuseDocument",
                        SilverpeasRuntimeException.ERROR, "versioning.UNKNOWN_DOC_WORKLIST_TYPE", currDoc);
            }
            ...
            throw new VersioningRuntimeException("VersioningBmEJB.refuseDocument()", SilverpeasRuntimeException.ERROR,
                                            ...
                                            Connection conn)
    {
        ...
        if (!isNonOrderedValidator(doc, currVersion, validatorID)) {
            throw new VersioningRuntimeException("VersioninBmEJB.refuseApprovalNonOrdered",
                    SilverpeasRuntimeException.ERROR, "versioning.VALIDATION_NOT_REQUIED", doc);
        }

        int versionCreator = currVersion.getAuthorId();
        refuseDocumentVersion(currVersion, comment, validationDate, validatorID, conn);

        try {
            VersioningDAO.checkDocumentIn(conn, doc.getPk());
        } catch (Exception e) {
            throw new VersioningRuntimeException("VersioningBmEJB.refuseApprovalNonOrdered",
                    SilverpeasRuntimeException.ERROR, "versioning.VALIDATION_NOT_REQUIED", doc, e);
        }
        ...
        String documentUrl = getDocumentUrl(doc);
        try {
            sendNotification(versionCreator, "notification.refuseWork", documentUrl, currWorkerid, doc);
        } catch (NotificationManagerException e) {
            SilverTrace.error("versioning", "VersioningBmEJB.sendNotification", "root.EX_NOTIFY_USERS_FAILED",
                    Integer.toString(versionCreatorID), e);
        }
                                         ...
                                         Connection conn)
    {
        ...
        try {
            ...
            throw new VersioningRuntimeException("VersioningBmEJB.refuseApprovalOrdered",
                    SilverpeasRuntimeException.ERROR, "versioning.VALIDATION_NOT_REQUIED", doc, e);
        }

        if (nextUserID == -1) {
            //CurrentWorkListOrder should not be changed
        } else {
            ...
            String documentUrl = getDocumentUrl(doc);
            try {
                sendNotification(nextUserID, "notification.refuseWork", documentUrl, currWorkerid, doc);  // notification
            } catch (NotificationManagerException e) {
                SilverTrace.error("versioning", "VersioningBmEJB.sendNotification", "root.EX_NOTIFY_USERS_FAILED",
                        Integer.toString(nextUserID), e);
            }
     ...
     * @param docVersion version to be validated
     */
    protected DocumentVersion refuseDocumentVersion(DocumentVersion docVersion, String comment,
                                                    java.util.Date validationDate,
                                                    int validatorId,
                                                    Connection conn)
    {
    ...
    protected DocumentVersion getCurrentDocumentVersion(Document doc) {
        ...
        try {
            ...
            throw new VersioningRuntimeException("VersioninBmEJB.getCurrentDocumentVersion",
     ...
     */
    protected void sendNotification(int userID, String notificationMessageKey, String documentUrl, int fromUser, Document doc)
            throws NotificationManagerException
    {
        String				language	= getDefaultUserLanguage(userID);
        ResourceLocator		resources	= new ResourceLocator("com.stratelia.webactiv.util.versioning.multilang.versioning", language);
		...
		NotificationMetaData notifMetaData = new NotificationMetaData(NotificationParameters.NORMAL,
												resources.getString("notification.title"),
												message.toString());
    ...
    protected String getDocumentUrl(Document doc) {
        String url = URLManager.getApplicationURL() + "/RVersioningPeas/"+doc.getPk().getSpace()+"_"+doc.getPk().getComponentName()+"/versions.jsp?DocId="+doc.getPk().getId()+"&Id="+doc.getForeignKey().getId()+"&SpaceId="+doc.getPk().getSpace()+"&ComponentId="+doc.getPk().getComponentName()+"&Context=Images";
        return url;
    }
    ...
    protected String getDefaultUserLanguage(int userID) {
        String lang = "fr";
        try {
            PersonalizationBmHome personalizationBmHome = (PersonalizationBmHome)
                    EJBUtilitaire.getEJBObjectRef(JNDINames.PERSONALIZATIONBM_EJBHOME, PersonalizationBmHome.class);
            ...
            throw new VersioningRuntimeException("VersioningBmEJB.getDefaultUserLanguage()",
                    VersioningRuntimeException.ERROR, "root.EX_CANT_GET_PREFERRED_USER_LANG", e);
        }
        return lang;
    }
    ...
    protected Connection openConnection() {
        ...
        try {
            ...
            throw new VersioningRuntimeException("VersioningBmEJB.getConnection()", SilverpeasRuntimeException.ERROR,
    ...
    {
		SilverTrace.info("versioning", "VersioningBmEJB.deleteIndex()", "root.MSG_GEN_ENTER_METHOD", "documentPK = " + document.getPk().toString());
    ...
    private void deleteVersionFile(DocumentVersion version, String path)
    {
		...
		try
		{
			...
			SilverTrace.warn("versioning", "VersioningBmEJB.deleteVersionFile()", "root.EX_CANT_DELETE_FILE", "path = "+path);
		}
    }
    ...
    public List getAllFilesReserved(int ownerId)
    {
		...
		try {
			...
			throw new VersioningRuntimeException("VersioningBmEJB.getAllFilesReserved",
					SilverpeasRuntimeException.ERROR, "versioning.GETTING_ALL_PUBLIC_DOCUMENT_VERSIONS_FAILED", re);
		} finally {
			closeConnection(con);
		}
    ...
    public List getAllFilesReservedByDate(Date date, boolean alert)
    {
		...
		try {
			...
			throw new VersioningRuntimeException("VersioningBmEJB.getAllFilesReservedByDate",
					SilverpeasRuntimeException.ERROR, "versioning.GETTING_ALL_PUBLIC_DOCUMENT_VERSIONS_FAILED", re);
		} finally {
			closeConnection(con);
		}
    ...
    public Collection getAllDocumentsToLib(Date date) throws VersioningRuntimeException 
	{
		Connection con = openConnection();
		try
		{
		...
		{
			throw new VersioningRuntimeException ("VersioningBmEJB.getAllAttachmentToLib()", SilverpeasException.ERROR, "versioning.EX_RECORD_NOT_LOAD", se);
		}
		finally
		{
			closeConnection(con);
    ...
    public void notifyUser(NotificationMetaData notifMetaData, String senderId, String componentId) throws VersioningRuntimeException 
	{
		Connection con = openConnection();
		SilverTrace.info("versioning", "VersioningBmEJB.notifyUser()", "root.MSG_GEN_EXIT_METHOD");
		try 
		{
		...
		{
			throw new VersioningRuntimeException("VersioningBmEJB.notifyUser()", SilverpeasRuntimeException.ERROR, "versioning.MSG_DOCUMENT_NOT_EXIST", e);
		} 
		finally 
		{
			closeConnection(con);
     ...
     */
    public void saveReadersAccessList(String componentId, ArrayList groupIds, ArrayList userIds) throws VersioningRuntimeException 
	{
		Connection con = openConnection();
		try
		{
		...
		{
			throw new VersioningRuntimeException ("VersioningBmEJB.saveAccessList()", SilverpeasException.ERROR, "versioning.EX_SAVE_ACCESSLIST_FAILED", se);
		}
		finally
		{
			closeConnection(con);
     ...
     * @throws VersioningRuntimeException
     */
    public void saveWorkersAccessList(String componentId, String documentId, int listType) throws VersioningRuntimeException 
    {
		Connection con = openConnection();
		try
		{
		...
		{
			throw new VersioningRuntimeException ("VersioningBmEJB.saveWorkersAccessList()", SilverpeasException.ERROR, "versioning.EX_SAVE_ACCESSLIST_FAILED", se);
		}
		finally
		{
			closeConnection(con);
     ...
     * @throws VersioningRuntimeException
     */
    public void removeReadersAccessList(String componentId) throws VersioningRuntimeException 
	{
		Connection con = openConnection();
		try
		{
		...
		{
			throw new VersioningRuntimeException ("VersioningBmEJB.removeAccessList()", SilverpeasException.ERROR, "versioning.EX_REMOVE_ACCESSLIST_FAILED", se);
		}
		finally
		{
			closeConnection(con);
    ...
    public void removeWorkersAccessList(String componentId) throws VersioningRuntimeException 
	{
		Connection con = openConnection();
		try
		{
     ...
     * @throws VersioningRuntimeException
     */
    public ArrayList getReadersAccessListUsers(String componentId) throws VersioningRuntimeException
    {
		...
		{
			throw new VersioningRuntimeException ("VersioningBmEJB.getReadersAccessListUsers()", SilverpeasException.ERROR, "versioning.EX_GET_ACCESSLIST_USERS", se);
		}
		finally
		{
			closeConnection(con);
     ...
     * @throws VersioningRuntimeException
     */
    public ArrayList getWorkersAccessListUsers(String componentId) throws VersioningRuntimeException
    {
    	ArrayList usersIds = new ArrayList();
		Connection con = openConnection();
		try
		...
		{
			throw new VersioningRuntimeException ("VersioningBmEJB.getWorkersAccessListUsers()", SilverpeasException.ERROR, "versioning.EX_GET_ACCESSLIST_USERS", se);
		}
		finally
		{
			closeConnection(con);
     ...
     * @throws VersioningRuntimeException
     */
    public ArrayList getReadersAccessListGroups(String componentId) throws VersioningRuntimeException
    {
		...
		{
			throw new VersioningRuntimeException ("VersioningBmEJB.getReadersAccessListGroups()", SilverpeasException.ERROR, "versioning.EX_GET_ACCESSLIST_GROUPS", se);
		}
		finally
		{
			closeConnection(con);
     ...
     * @throws VersioningRuntimeException
     */
    public ArrayList getWorkersAccessListGroups(String componentId) throws VersioningRuntimeException
    {
    	ArrayList groupsIds = new ArrayList();
		Connection con = openConnection();
		try
		...
		{
			throw new VersioningRuntimeException ("VersioningBmEJB.getWorkersAccessListGroups()", SilverpeasException.ERROR, "versioning.EX_GET_ACCESSLIST_USERS", se);
		}
		finally
		{
			closeConnection(con);
    ...
    public int getSavedListType(String componentId)
    {
		...
		{
			throw new VersioningRuntimeException ("VersioningBmEJB.getSavedListType()", SilverpeasException.ERROR, "versioning.EX_GET_SAVE_LIST_TYPE", se);
		}
		finally
		{
			closeConnection(con);
