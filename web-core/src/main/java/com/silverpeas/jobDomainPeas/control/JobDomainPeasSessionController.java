...
public class JobDomainPeasSessionController extends AbstractComponentSessionController
{
    ...
    {
        super(mainSessionCtrl, componentContext, "com.silverpeas.jobDomainPeas.multilang.jobDomainPeasBundle", "com.silverpeas.jobDomainPeas.settings.jobDomainPeasIcons");
    ...
    public boolean isAccessGranted()
    {
    	...
    	if (getUserManageableGroupIds().size() > 0)
    		return true;
    	if (getUserDetail().isAccessAdmin() || getUserDetail().isAccessDomainManager())
    		return true;
    ...
    public UserDetail getTargetUserDetail() throws JobDomainPeasException
    {
        ...
        if ((m_TargetUserId != null) && (m_TargetUserId.length() > 0))
        {
            ...
            if (valret == null)
            {
                throw new JobDomainPeasException("JobDomainPeasSessionController.getTargetUserDetail()",SilverpeasException.ERROR,"jobDomainPeas.EX_USER_NOT_AVAILABLE","UserId="+m_TargetUserId);
            }
        }
        return valret;
    }
    ...
    public UserFull getTargetUserFull() throws JobDomainPeasException
    {
        ...
        if ((m_TargetUserId != null) && (m_TargetUserId.length() > 0))
        {
            ...
            if (valret == null)
            {
                throw new JobDomainPeasException("JobDomainPeasSessionController.getTargetUserFull()",SilverpeasException.ERROR,"jobDomainPeas.EX_USER_NOT_AVAILABLE","UserId="+m_TargetUserId);
            }
        }
        return valret;
    }

    public void createUser(String userLogin,String userLastName,String userFirstName,String userEMail,String userAccessLevel,boolean userPasswordValid, String userPassword, HashMap properties) throws JobDomainPeasException, JobDomainPeasTrappedException
    {
        UserDetail theNewUser = new UserDetail();
        String     idRet = null;
        String   existingUser;

        SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.createUser()", "root.MSG_GEN_ENTER_METHOD", "userLogin="+userLogin+" userLastName="+userLastName+" userFirstName="+userFirstName+" userEMail="+userEMail+" userAccessLevel="+userAccessLevel);

        existingUser = m_AdminCtrl.getUserIdByLoginAndDomain(userLogin, m_TargetDomainId);
        if ((existingUser != null) && (existingUser.length() > 0))
        {
            JobDomainPeasTrappedException te = new JobDomainPeasTrappedException("JobDomainPeasSessionController.createUser()",SilverpeasException.ERROR,"admin.EX_ERR_LOGIN_ALREADY_USED");
            te.setGoBackPage("displayUserCreate");
            throw te;
        }

        theNewUser.setId("-1");
        if ((m_TargetDomainId != null) && (m_TargetDomainId.equals("-1") == false) && (m_TargetDomainId.length() > 0))
        {
        ...
        {
            throw new JobDomainPeasException("JobDomainPeasSessionController.createUser()",SilverpeasException.ERROR,"admin.EX_ERR_ADD_USER");
        }
        refresh();
        setTargetUser(idRet);
        ...
        if (uf != null)
        {
            ...
            if ((idRet == null) || (idRet.length() <= 0))
            {
                throw new JobDomainPeasException("JobDomainPeasSessionController.createUser()",SilverpeasException.ERROR,"admin.EX_ERR_UPDATE_USER");
            }
        }
	...
	*
	*/	
    private void regroupInGroup(HashMap properties, String lastGroupId) throws JobDomainPeasException {
    	
		//Traitement du domaine SQL
		if(	! "-1".equals(getTargetDomain().getId()) &&
			! "0".equals(getTargetDomain().getId()) &&
			getTargetDomain().getDriverClassName().equals("com.stratelia.silverpeas.domains.sqldriver.SQLDriver") ) 
		{
			
			ResourceLocator specificRs = new ResourceLocator(getTargetDomain().getPropFileName(), "");
			int numPropertyRegroup = SilverpeasSettings.readInt(specificRs, "property.Grouping", -1);
			...
			if(numPropertyRegroup > -1) {
				String nomPropertyRegroupement = SilverpeasSettings.readString(specificRs, "property_"+numPropertyRegroup+".Name", null);
				...
				if(nomPropertyRegroupement != null) {
					...
					if(lastGroupId != null) {
						...
						newUserIds = (String []) lNewUserIds.toArray(new String[lNewUserIds.size()]);
	    	...
	    	if(nomRegroup != null && nomRegroup.length()>0) {
	    		...
	    		Group group = m_AdminCtrl.getGroupByNameInDomain(nomRegroup, m_TargetDomainId);
	...
	* @param InputStream
	*/	
    public void importCsvUsers(FileItem filePart) throws UtilTrappedException, JobDomainPeasTrappedException, JobDomainPeasException
    {
    	InputStream is;
		try {
			is = filePart.getInputStream();
		} catch (IOException e) {
			JobDomainPeasTrappedException jdpe = new JobDomainPeasTrappedException("JobDomainPeasSessionController.importCsvUsers", SilverpeasException.ERROR, "jobDomainPeas.EX_CSV_FILE", e);
			jdpe.setGoBackPage("displayUsersCsvImport");
			throw jdpe;	
		}
    	CSVReader csvReader = new CSVReader(getLanguage());
    	csvReader.initCSVFormat("com.silverpeas.jobDomainPeas.settings.usersCSVFormat", "User", ";", getTargetDomain().getPropFileName(), "property_");
        ...
        for (int i=0; i<csvValues.length; i++)
        {
        	...
        	if(nom.length() == 0) {//champ obligatoire
        		listErrors.append(getString("JDP.ligne")+ " = "+Integer.toString(i+1)+", ");
        		...
        		listErrors.append(getString("JDP.colonne")+ " = 1, ");
        		listErrors.append(getString("JDP.valeur")+ " = "+nom+", ");
            	listErrors.append(getString("JDP.nbCarMax")+" 100 "+getString("JDP.caracteres")+"<BR>");
        	}
        	...
        	if(login.length() == 0) {//champ obligatoire
            	...
            	listErrors.append(getString("JDP.obligatoire")+"<BR>");
        	} else if (login.length() < JobDomainSettings.m_MinLengthLogin) {//verifier jobDomainPeasSettings.getString("minLengthLogin") char min
        		listErrors.append(getString("JDP.ligne")+ " = "+Integer.toString(i+1)+", ");
        		listErrors.append(getString("JDP.colonne")+ " = 3, ");
        		listErrors.append(getString("JDP.valeur")+ " = "+login+", ");
            	listErrors.append(getString("JDP.nbCarMin")+" "+JobDomainSettings.m_MinLengthLogin+" "+getString("JDP.caracteres")+"<BR>");
        	} else if (login.length() > 20) {//verifier 20 char max
        		listErrors.append(getString("JDP.ligne")+ " = "+Integer.toString(i+1)+", ");
        		listErrors.append(getString("JDP.colonne")+ " = 3, ");
        		listErrors.append(getString("JDP.valeur")+ " = "+login+", ");
            	listErrors.append(getString("JDP.nbCarMax")+" 20 "+getString("JDP.caracteres")+"<BR>");
        	} else {//verif login unique
        		existingLogin = m_AdminCtrl.getUserIdByLoginAndDomain(login, m_TargetDomainId);
        	...
        	if (! JobDomainSettings.m_BlanksAllowedInPwd && motDePasse.indexOf(" ") != -1) {
        		listErrors.append(getString("JDP.ligne")+ " = "+Integer.toString(i+1)+", ");
        	...
        	} else if (motDePasse.length() < JobDomainSettings.m_MinLengthPwd) {//verifier jobDomainPeasSettings.getString("minLengthPwd") char min
        		listErrors.append(getString("JDP.ligne")+ " = "+Integer.toString(i+1)+", ");
        		listErrors.append(getString("JDP.colonne")+ " = 6, ");
        		listErrors.append(getString("JDP.valeur")+ " = "+motDePasse+", ");
            	listErrors.append(getString("JDP.nbCarMin")+" "+JobDomainSettings.m_MinLengthPwd+" "+getString("JDP.caracteres")+"<BR>");
        	} else if (motDePasse.length() > 32) {//verifier 32 char max
        		listErrors.append(getString("JDP.ligne")+ " = "+Integer.toString(i+1)+", ");
        		listErrors.append(getString("JDP.colonne")+ " = 6, ");
        		listErrors.append(getString("JDP.valeur")+ " = "+motDePasse+", ");
            	listErrors.append(getString("JDP.nbCarMax")+" 32 "+getString("JDP.caracteres")+"<BR>");
        	}
        	...
        	if(csvReader.getM_specificNbCols() > 0) {
        		if("-1".equals(getTargetDomain().getId()) ||
                	...
                	if (position.length() > 100) {
                		...
                		listErrors.append(getString("JDP.valeur")+ " = "+position+", ");
                    	listErrors.append(getString("JDP.nbCarMax")+" 100 "+getString("JDP.caracteres")+"<BR>");
                	}
                	...
                	//cellularPhone
                	cellularPhone = csvValues[i][13].getValueString(); //verifier 20 char max
                	...
                	if (address.length() > 500) {
                    	...
                    	listErrors.append(getString("JDP.nbCarMax")+" 500 "+getString("JDP.caracteres")+"<BR>");
                	}
        		} else {//domaine SQL
        			...
        			for(int j=0; j<csvReader.getM_specificNbCols(); j++) {
        				if(Variant.TYPE_STRING.equals(csvReader.getM_specificColType(j))) {
        					...
        					if (informationSpecifiqueString.length() > 50) {
                        		...
                        		listErrors.append(getString("JDP.valeur")+ " = "+informationSpecifiqueString+", ");
                            	listErrors.append(getString("JDP.nbCarMax")+" 50 "+getString("JDP.caracteres")+"<BR>");
                        	}
        				} 
        			}
        		}
        ...
        if(listErrors.length()>0) {
    		JobDomainPeasTrappedException jdpe = new JobDomainPeasTrappedException("JobDomainPeasSessionController.importCsvUsers", SilverpeasException.ERROR, "jobDomainPeas.EX_CSV_FILE", listErrors.toString());
			jdpe.setGoBackPage("displayUsersCsvImport");
			throw jdpe;	
    	}
        ...
        for (int i=0; i<csvValues.length; i++)
        {
        	...
        	createUser(login, nom, prenom, email, userAccessLevel, true, motDePasse, properties); //l'id User créé est dans m_TargetUserId
    ...
    private String getLastGroupId(UserFull theUser)
    {
    		...
    		getTargetDomain().getDriverClassName().equals("com.stratelia.silverpeas.domains.sqldriver.SQLDriver") ) 
    	{
    		...
    		if(nomLastGroup != null && nomLastGroup.length()>0) {
	    		//Recherche le groupe dans le domaine
	    		Group group = m_AdminCtrl.getGroupByNameInDomain(nomLastGroup, m_TargetDomainId);
	    		if(group != null) {
	    			return group.getId();
	    		}
    		}
    ...
    public void modifyUser(String idUser,String userLastName,String userFirstName,String userEMail,String userAccessLevel,boolean userPasswordValid, String userPassword, HashMap properties) throws JobDomainPeasException
    {
        UserFull theModifiedUser = null;
        String idRet = null;

        SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.modifyUser()", "root.MSG_GEN_ENTER_METHOD", "UserId="+idUser+" userLastName="+userLastName+" userFirstName="+userFirstName+" userEMail="+userEMail+" userAccessLevel="+userAccessLevel);

        theModifiedUser = m_AdminCtrl.getUserFull(idUser);
        if (theModifiedUser == null)
        {
            throw new JobDomainPeasException("JobDomainPeasSessionController.modifyUser()",SilverpeasException.ERROR,"admin.EX_ERR_UNKNOWN_USER");
        }
        ...
        if ((idRet == null) || (idRet.length() <= 0))
        {
            throw new JobDomainPeasException("JobDomainPeasSessionController.modifyUser()",SilverpeasException.ERROR,"admin.EX_ERR_UPDATE_USER", "UserId="+idUser);
        }
        refresh();
        setTargetUser(idRet);
    ...
    public void modifySynchronizedUser(String idUser,String userAccessLevel)throws JobDomainPeasException
    {
        UserDetail theModifiedUser = null;
        String idRet = null;

        SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.modifySynchronizedUser()", "root.MSG_GEN_ENTER_METHOD", "UserId="+idUser);

        theModifiedUser = m_AdminCtrl.getUserDetail(idUser);
        if (theModifiedUser == null)
        {
            throw new JobDomainPeasException("JobDomainPeasSessionController.modifySynchronizedUser()",SilverpeasException.ERROR,"admin.EX_ERR_UNKNOWN_USER");
        }
        theModifiedUser.setAccessLevel(userAccessLevel);
        idRet = m_AdminCtrl.updateSynchronizedUser(theModifiedUser);
        if ((idRet == null) || (idRet.length() <= 0))
        {
            throw new JobDomainPeasException("JobDomainPeasSessionController.modifySynchronizedUser()",SilverpeasException.ERROR,"admin.EX_ERR_UPDATE_USER", "UserId="+idUser);
        }
        refresh();
        setTargetUser(idRet);
    }
    ...
    public void deleteUser(String idUser) throws JobDomainPeasException
    {
        ...
        SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.deleteUser()", "root.MSG_GEN_ENTER_METHOD", "UserId="+idUser);
        ...
        if ((idRet == null) || (idRet.length() <= 0))
        {
            throw new JobDomainPeasException("JobDomainPeasSessionController.deleteUser()",SilverpeasException.ERROR,"admin.EX_ERR_DELETE_USER", "UserId="+idUser);
        }
    ...
    {
    	return m_AdminCtrl.getSpecificPropertiesToImportUsers(m_TargetDomainId, getLanguage()).iterator();
    }
    ...
    public void importUser(String userLogin) throws JobDomainPeasException
    {
        ...
        SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.importUser()", "root.MSG_GEN_ENTER_METHOD", "userLogin="+userLogin);

        idRet = m_AdminCtrl.synchronizeImportUser(m_TargetDomainId,userLogin);
        if ((idRet == null) || (idRet.length() <= 0))
        {
            throw new JobDomainPeasException("JobDomainPeasSessionController.importUser()",SilverpeasException.ERROR,"admin.MSG_ERR_SYNCHRONIZE_USER", "userLogin="+userLogin);
        }
        refresh();
        setTargetUser(idRet);
    }
    ...
    public void importUsers(String[] specificIds) throws JobDomainPeasException
    {
    	...
    	{
    		SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.importUsers()", "root.MSG_GEN_ENTER_METHOD", "specificId="+specificIds[i]);

            String idRet = m_AdminCtrl.synchronizeImportUser(m_TargetDomainId, specificIds[i]);
            if (!StringUtil.isDefined(idRet))
            {
                //throw new JobDomainPeasException("JobDomainPeasSessionController.importUser()",SilverpeasException.ERROR,"admin.MSG_ERR_SYNCHRONIZE_USER", "specificId="+specificIds[i]);
            }
    	}
    	refresh();
    }
    ...
    {
        SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.searchUsers()", "root.MSG_GEN_ENTER_METHOD", "query="+query.toString());

        //UserDetail[] existingUsers = m_AdminCtrl.getUsersOfDomain(m_TargetDomainId);
    ...
    public void synchroUser(String idUser) throws JobDomainPeasException
    {
        ...
        SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.synchroUser()", "root.MSG_GEN_ENTER_METHOD", "UserId="+idUser);

        idRet = m_AdminCtrl.synchronizeUser(idUser);
        if ((idRet == null) || (idRet.length() <= 0))
        {
            throw new JobDomainPeasException("JobDomainPeasSessionController.synchroUser()",SilverpeasException.ERROR,"admin.MSG_ERR_SYNCHRONIZE_USER");
        }
        refresh();
        setTargetUser(idRet);
    }
    ...
    public void unsynchroUser(String idUser) throws JobDomainPeasException
    {
        ...
        SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.unsynchroUser()", "root.MSG_GEN_ENTER_METHOD", "UserId="+idUser);

        idRet = m_AdminCtrl.synchronizeRemoveUser(idUser);
        if ((idRet == null) || (idRet.length() <= 0))
        {
            throw new JobDomainPeasException("JobDomainPeasSessionController.unsynchroUser()",SilverpeasException.ERROR,"admin.EX_ERR_DELETE_USER");
        }
        if (m_TargetUserId.equals(idUser))
        {
            m_TargetUserId = null;
    ...
    public void returnIntoGroup(String groupId) throws JobDomainPeasException
    {
        ...
        else
        {
            ...
            while ((i >= 0) && (((GroupNavigationStock)m_GroupsPath.get(i)).isThisGroup(groupId) == false))
            {
    ...
    public void removeGroupFromPath(String groupId) throws JobDomainPeasException
    {
        ...
        {
            i = 0;
            while ((i < m_GroupsPath.size()) && (((GroupNavigationStock)m_GroupsPath.get(i)).isThisGroup(groupId) == false))
            {
                i++;
            }
    ...
    public void goIntoGroup(String groupId) throws JobDomainPeasException
    {
        if (StringUtil.isDefined(groupId))
        {
        	if (getTargetGroup() == null || (getTargetGroup() != null  && !getTargetGroup().getId().equals(groupId)))
        	{
	            ...
	            if (GroupNavigationStock.isGroupValid(targetGroup))
	            {
	                ...
	                GroupNavigationStock newSubGroup = new GroupNavigationStock(groupId, m_AdminCtrl, manageableGroupIds);
        ...
        {
            throw new JobDomainPeasException("JobDomainPeasSessionController.setTargetGroup()",SilverpeasException.ERROR,"jobDomainPeas.EX_GROUP_NOT_AVAILABLE","GroupId="+groupId);
        }
        setTargetUser(null);
    }
	...
	public void initUserPanelForGroupManagers(String compoURL) throws SelectionException, JobDomainPeasException
	{
		...
		PairObject[] hostPath = {new PairObject(getMultilang().getString("JDP.roleManager") + " > "+generalMessage.getString("GML.selection"),  null)};
		...
		sel.setSelectedElements((String[]) profile.getAllUsers().toArray(new String[0]));
		sel.setSelectedSets((String[]) profile.getAllGroups().toArray(new String[0]));
	}
    ...
    public boolean isGroupRoot(String groupId) throws JobDomainPeasException
    {
        ...
        {
            if ((gr.getSuperGroupId() == null) || (gr.getSuperGroupId().length() <= 0) || (gr.getSuperGroupId().equals("-1")))
            {
                return true;
            }
            else
    ...
    public Group[] getSubGroups(boolean isParentGroup) throws JobDomainPeasException
    {
        ...
        if (isParentGroup)
        {
            if (m_GroupsPath.size() <= 0)
            {
                throw new JobDomainPeasException("JobDomainPeasSessionController.getTargetGroup()",SilverpeasException.ERROR,"jobDomainPeas.EX_GROUP_NOT_AVAILABLE");
            }
            groups = ((GroupNavigationStock)m_GroupsPath.lastElement()).getGroupPage();
        }
        else
        { // Domain case
        ...
        for (int i = 0; i < groups.length; i++)
        {
            if (groups[i] != null)
            {
            	groups[i].setNbUsers(getOrganizationController().getAllSubUsersNumber(groups[i].getId()));
            }
        }
        return groups;
    }

    public String[][] getSubUsers(boolean isParentGroup) throws JobDomainPeasException
    {
        ...
        if (isParentGroup)
        {
            ...
            usDetails = ((GroupNavigationStock)m_GroupsPath.lastElement()).getUserPage();
        }
        else
        { // Domain case
        ...
        for (i = 0; i < usDetails.length; i++)
        {
            if (usDetails[i] != null)
            {
                ...
                valret[i][2] = EncodeHelper.javaStringToHtmlString(getSureString(usDetails[i].getFirstName()));
    ...
    public boolean[] getPageNavigation(boolean isParentGroup)
    {
        ...
        if (isParentGroup)
        {
            if (m_GroupsPath.size() > 0)
            {   
                GroupNavigationStock theGroupStock = ((GroupNavigationStock)m_GroupsPath.lastElement());
    ...
    public void previousGroupPage(boolean isParentGroup) throws JobDomainPeasException
    {
        if (isParentGroup)
        {
            if (m_GroupsPath.size() <= 0)
            {
                throw new JobDomainPeasException("JobDomainPeasSessionController.previousGroupPage()",SilverpeasException.ERROR,"jobDomainPeas.EX_GROUP_NOT_AVAILABLE");
            }
        ...
        else
        {
            if (m_TargetDomain == null)
            {
                throw new JobDomainPeasException("JobDomainPeasSessionController.previousGroupPage()",SilverpeasException.ERROR,"jobDomainPeas.EX_DOMAIN_NOT_AVAILABLE");
            }
            m_TargetDomain.previousGroupPage();
        }
    }

    public void nextGroupPage(boolean isParentGroup) throws JobDomainPeasException
    {
        if (isParentGroup)
        {
            if (m_GroupsPath.size() <= 0)
            {
                throw new JobDomainPeasException("JobDomainPeasSessionController.nextGroupPage()",SilverpeasException.ERROR,"jobDomainPeas.EX_GROUP_NOT_AVAILABLE");
            }
        ...
        else
        {
            if (m_TargetDomain == null)
            {
                throw new JobDomainPeasException("JobDomainPeasSessionController.nextGroupPage()",SilverpeasException.ERROR,"jobDomainPeas.EX_DOMAIN_NOT_AVAILABLE");
            }
            m_TargetDomain.nextGroupPage();
        }
    }

    public void previousUserPage(boolean isParentGroup) throws JobDomainPeasException
    {
        if (isParentGroup)
        {
            if (m_GroupsPath.size() <= 0)
            {
                throw new JobDomainPeasException("JobDomainPeasSessionController.previousUserPage()",SilverpeasException.ERROR,"jobDomainPeas.EX_GROUP_NOT_AVAILABLE");
            }
        ...
        else
        {
            if (m_TargetDomain == null)
            {
                throw new JobDomainPeasException("JobDomainPeasSessionController.previousUserPage()",SilverpeasException.ERROR,"jobDomainPeas.EX_DOMAIN_NOT_AVAILABLE");
            }
    ...
    public void nextUserPage(boolean isParentGroup) throws JobDomainPeasException
    {
        if (isParentGroup)
        {
            if (m_GroupsPath.size() <= 0)
            {
                throw new JobDomainPeasException("JobDomainPeasSessionController.nextUserPage()",SilverpeasException.ERROR,"jobDomainPeas.EX_GROUP_NOT_AVAILABLE");
            }
        ...
        else
        {
            if (m_TargetDomain == null)
            {
                throw new JobDomainPeasException("JobDomainPeasSessionController.nextUserPage()",SilverpeasException.ERROR,"jobDomainPeas.EX_DOMAIN_NOT_AVAILABLE");
            }
    ...
    public String getPath(String baseURL, String toAppendAtEnd) throws JobDomainPeasException
    {
        ...
        for(i =0; i<m_GroupsPath.size(); i++)
        {
            ...
            if (((i+1) < m_GroupsPath.size()) || (m_TargetUserId != null) || (toAppendAtEnd != null))
            {
                strPath.append("<a href=\"" + baseURL + "groupReturn?Idgroup=" + theGroup.getId() + "\">" + EncodeHelper.javaStringToHtmlString(theGroup.getName()) + "</a>");
            }
            else
            {
        ...
        if (m_TargetUserId != null)
        {
            ...
            {
                strPath.append("<a href=\"" + baseURL + "userContent?Iduser=" + m_TargetUserId + "\">" + EncodeHelper.javaStringToHtmlString(getTargetUserDetail().getDisplayedName()) + "</a>");
            }
            else
            {
    ...
    public boolean createGroup(String idParent, String groupName,String groupDescription, String groupRule) throws JobDomainPeasException
    {
        ...
        SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.createGroup()", "root.MSG_GEN_ENTER_METHOD", "ParentId="+idParent+" Name="+groupName+" Desc="+groupDescription);
        theNewGroup.setId("-1");
        if (StringUtil.isDefined(m_TargetDomainId) && !m_TargetDomainId.equals("-1"))
        {
        ...
        if (!StringUtil.isDefined(idRet))
        {
            throw new JobDomainPeasException("JobDomainPeasSessionController.createGroup()",SilverpeasException.ERROR,"admin.EX_ERR_ADD_GROUP");
        }
        refresh();
    ...
    {
        SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.modifyGroup()", "root.MSG_GEN_ENTER_METHOD", "GroupId="+idGroup+" Desc="+groupDescription);

        Group theModifiedGroup = m_AdminCtrl.getGroupById(idGroup);
        if (theModifiedGroup == null)
        {
            throw new JobDomainPeasException("JobDomainPeasSessionController.modifyGroup()",SilverpeasException.ERROR,"admin.EX_ERR_UNKNOWN_GROUP");
        }
        ...
        if (!StringUtil.isDefined(idRet))
        {
            throw new JobDomainPeasException("JobDomainPeasSessionController.modifyGroup()",SilverpeasException.ERROR,"admin.EX_ERR_UPDATE_GROUP");
        }
        refresh();
        return isGroupRoot(idRet);
    }
    ...
    {
        SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.updateGroupSubUsers()", "root.MSG_GEN_ENTER_METHOD", "GroupId="+idGroup);

        Group theModifiedGroup = m_AdminCtrl.getGroupById(idGroup);
        if (theModifiedGroup == null)
        {
            throw new JobDomainPeasException("JobDomainPeasSessionController.updateGroupSubUsers()",SilverpeasException.ERROR,"admin.EX_ERR_UNKNOWN_GROUP");
        }
        theModifiedGroup.setUserIds(userIds);
        String idRet = m_AdminCtrl.updateGroup(theModifiedGroup);
        if ((idRet == null) || (idRet.length() <= 0))
        {
            throw new JobDomainPeasException("JobDomainPeasSessionController.updateGroupSubUsers()",SilverpeasException.ERROR,"admin.EX_ERR_UPDATE_GROUP");
        }
        refresh();
        return false;
    }
    ...
    public boolean deleteGroup(String idGroup) throws JobDomainPeasException
    {
        ...
        SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.deleteGroup()", "root.MSG_GEN_ENTER_METHOD", "GroupId="+idGroup);

        String idRet = m_AdminCtrl.deleteGroupById(idGroup);
        if (!StringUtil.isDefined(idRet))
        {
            throw new JobDomainPeasException("JobDomainPeasSessionController.deleteGroup()",SilverpeasException.ERROR,"admin.EX_ERR_DELETE_GROUP");
        }
    ...
    {
        SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.synchroGroup()", "root.MSG_GEN_ENTER_METHOD", "GroupId="+idGroup);

        String idRet = m_AdminCtrl.synchronizeGroup(idGroup);
        if (!StringUtil.isDefined(idRet))
        {
            throw new JobDomainPeasException("JobDomainPeasSessionController.synchroGroup()",SilverpeasException.ERROR,"admin.MSG_ERR_SYNCHRONIZE_GROUP");
        }
        refresh();
        return isGroupRoot(idRet);
    }
    ...
    public boolean unsynchroGroup(String idGroup) throws JobDomainPeasException
    {
        ...
        SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.unsynchroGroup()", "root.MSG_GEN_ENTER_METHOD", "GroupId="+idGroup);

        String idRet = m_AdminCtrl.synchronizeRemoveGroup(idGroup);
        if (!StringUtil.isDefined(idRet))
        {
            throw new JobDomainPeasException("JobDomainPeasSessionController.unsynchroGroup()",SilverpeasException.ERROR,"admin.EX_ERR_DELETE_GROUP");
        }
        removeGroupFromPath(idGroup);
        refresh();
        return haveToRefreshDomain;
    ...
    {
        SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.importGroup()", "root.MSG_GEN_ENTER_METHOD", "groupName="+groupName);

        String idRet = m_AdminCtrl.synchronizeImportGroup(m_TargetDomainId,groupName);
        if (!StringUtil.isDefined(idRet))
        {
            throw new JobDomainPeasException("JobDomainPeasSessionController.importGroup()",SilverpeasException.ERROR,"admin.MSG_ERR_SYNCHRONIZE_GROUP");
        }
        refresh();
        return isGroupRoot(idRet);
    }
    ...
    {
    	return isGroupManager() && !getUserDetail().isAccessAdmin() && !getUserDetail().isDomainAdminRestricted();
    }
    ...
    private Group[] filterGroupsToGroupManager(Group[] groups)
    {
    	...
    	for(int g=0; g<groups.length; g++)
    	{
    		...
    		else
    		{
        		...
        		List subGroupIds = Arrays.asList(m_AdminCtrl.getAllSubGroupIdsRecursively(group.getId()));
    ...
    public String createDomain(String domainName, String domainDescription, String domainDriver, String domainProperties, String domainAuthentication, String silverpeasServerURL, String domainTimeStamp) throws JobDomainPeasException, JobDomainPeasTrappedException
    {
        ...
        SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.createDomain()", "root.MSG_GEN_ENTER_METHOD", "domainName="+domainName);
        ...
        if (!StringUtil.isDefined(idRet))
        {
            throw new JobDomainPeasException("JobDomainPeasSessionController.createDomain()",SilverpeasException.ERROR,"admin.MSG_ERR_ADD_DOMAIN");
        }
        refresh();
        return idRet;
    }
    
    private void verifCreateDomain(String domainName, boolean domainSql) throws JobDomainPeasTrappedException
    {
    	JobDomainPeasTrappedException trappedException = new JobDomainPeasTrappedException("JobDomainPeasSessionController", SilverpeasException.WARNING, "jobDomainPeas.WARN_DOMAIN_SQL_NAME");
    	if(domainSql)
    	...
    	if (domainSql)
    	{
	    	...
	    	//pour les properties com.stratelia.silverpeas.domains.domain<domainName>.properties 
	    	//et com.stratelia.silverpeas.authentication.autDomain<domainName>.properties
	    	ResourceLocator propInitialize = new ResourceLocator("com.stratelia.silverpeas._silverpeasinitialize.settings._silverpeasinitializeSettings", "");
	    	String pathInitialize = propInitialize.getString("pathInitialize");
	    	int indexOfInitialize = pathInitialize.indexOf("initialize");
	    	String cheminFichierDomain = pathInitialize.substring(0, indexOfInitialize) + "properties"+File.separator+"com"+File.separator+"stratelia"+File.separator+"silverpeas"+File.separator+"domains";
	    	...
	    	String cheminFichierAutDomain = pathInitialize.substring(0, indexOfInitialize) + "properties"+File.separator+"com"+File.separator+"stratelia"+File.separator+"silverpeas"+File.separator+"authentication";
    ...
    public String createSQLDomain(String domainName, String domainDescription, String silverpeasServerURL) throws JobDomainPeasException, JobDomainPeasTrappedException
    {
    	
    	//0- Vérif domainName
    	verifCreateDomain(domainName, true);
    	
    	//1-Création sur le fileSystem du properties com.stratelia.silverpeas.domains.domain<domainName>
    	SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.createSQLDomain()", "root.MSG_GEN_ENTER_METHOD", "Création sur le fileSystem du properties com.stratelia.silverpeas.domains.domain<domainName>");
        ...
        try {
          sortie = new PrintWriter(new FileWriter(fileDomain));
        	...
        	throw new JobDomainPeasException("JobDomainPeasSessionController.createSQLDomain()",SilverpeasException.ERROR,"admin.MSG_ERR_ADD_DOMAIN", e);
        }
    	...
    	String cryptMethod = SilverpeasSettings.readString(templateDomainSql, "database.SQLPasswordEncryption", Authentication.ENC_TYPE_MD5);
    	boolean allowPasswordChange = SilverpeasSettings.readBoolean(templateDomainSql, "allowPasswordChange", false);
    	
        BufferedReader readerTemplateDomainSQL = null;
        try {
        	String cheminFichierTemplateDomainSQL = pathInitialize.substring(0, indexOfInitialize) + "properties"+File.separator+"com"+File.separator+"stratelia"+File.separator+"silverpeas"+File.separator+"domains"+File.separator+"templateDomainSQL.properties";
        	File fileTemplateDomainSQL = new File(cheminFichierTemplateDomainSQL);
        	readerTemplateDomainSQL = new BufferedReader(new FileReader(fileTemplateDomainSQL));
    	...
    	sortie.println("database.SQLClassName					= "+propAdmin.getString("AdminDBDriver"));
    	sortie.println("database.SQLJDBCUrl						= "+propAdmin.getString("WaProductionDb"));
    	sortie.println("database.SQLAccessLogin					= "+propAdmin.getString("WaProductionUser"));
    	sortie.println("database.SQLAccessPasswd				= "+propAdmin.getString("WaProductionPswd"));
    	sortie.println("");
    	sortie.println("database.SQLUserTableName				= Domain"+domainName+"_User");
    	sortie.println("database.SQLGroupTableName				= Domain"+domainName+"_Group");
    	sortie.println("database.SQLUserGroupTableName			= Domain"+domainName+"_Group_User_Rel");
    	sortie.println("");
    	...
    	//2-Création sur le fileSystem du properties com.stratelia.silverpeas.authentication.autDomain<domainName>
    	SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.createSQLDomain()", "root.MSG_GEN_ENTER_METHOD", "Création sur le fileSystem du properties com.stratelia.silverpeas.authentication.autDomain<domainName>");
    	String cheminFichierAutDomain = pathInitialize.substring(0, indexOfInitialize) + "properties"+File.separator+"com"+File.separator+"stratelia"+File.separator+"silverpeas"+File.separator+"authentication";
    	String nomFichierAutDomain = "autDomain"+domainName+".properties";
        ...
        sortie.println("# ----------------------------------------");
        sortie.println("");
        sortie.println("# Fallback type : could be one of the following values : none, ifNotRejected, always");
        ...
        sortie.println("# Available types are : com.stratelia.silverpeas.authentication.AuthenticationNT, com.stratelia.silverpeas.authentication.AuthenticationSQL and com.stratelia.silverpeas.authentication.AuthenticationLDAP");
        sortie.println("");
        sortie.println("autServersCount							= 1");
        sortie.println("");
        sortie.println("autServer0.type							= com.stratelia.silverpeas.authentication.AuthenticationSQL");
        sortie.println("autServer0.enabled						= true");
        sortie.println("autServer0.SQLJDBCUrl 					= "+propAdmin.getString("WaProductionDb"));
        sortie.println("autServer0.SQLAccessLogin  				= "+propAdmin.getString("WaProductionUser"));
        sortie.println("autServer0.SQLAccessPasswd 				= "+propAdmin.getString("WaProductionPswd"));
        sortie.println("autServer0.SQLDriverClass  				= "+propAdmin.getString("AdminDBDriver"));
        sortie.println("autServer0.SQLUserTableName 			= Domain"+domainName+"_User");
    	...
    	//3-Création en base de données des 3 nouvelles tables du domaine : Domain<domainName>_Group, Domain<domainName>_Group_User_Rel et Domain<domainName>_User
        SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.createSQLDomain()", "root.MSG_GEN_ENTER_METHOD", "Création en base de données des 3 tablse du Domain");
        try {
        	...
        	try {
				...
				throw new JobDomainPeasException("JobDomainPeasSessionController.createSQLDomain()",SilverpeasException.ERROR,"admin.MSG_ERR_ADD_DOMAIN", e1);
			}
		...
		SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.createSQLDomain()", "root.MSG_GEN_ENTER_METHOD", "Insertion d'une entrée dans la table ST_Domain");
        Domain theNewDomain = new Domain();
        String idRet = null;
        theNewDomain.setId("-1");
        theNewDomain.setName(domainName);
        ...
        theNewDomain.setPropFileName("com.stratelia.silverpeas.domains.domain"+domainName);
        ...
        if ((idRet == null) || (idRet.length() <= 0))
        {
        	...
        	throw new JobDomainPeasException("JobDomainPeasSessionController.createSQLDomain()",SilverpeasException.ERROR,"admin.MSG_ERR_ADD_DOMAIN");        	
        }
        return idRet;
    }
    ...
    public String modifyDomain(String domainName, String domainDescription, String domainDriver, String domainProperties, String domainAuthentication, String silverpeasServerURL, String domainTimeStamp) throws JobDomainPeasException, JobDomainPeasTrappedException
    {
    	...
    	for(int j=0; j<tabDomain.length; j++) {
    		domain = tabDomain[j];
    		if(! domain.getId().equals(theNewDomain.getId()) && domain.getName().toLowerCase().equals(domainName.toLowerCase()))
    			throw trappedException;
    	}
        ...
        if (!StringUtil.isDefined(m_TargetDomainId) || m_TargetDomainId.equals("-1"))
        {
            throw new JobDomainPeasException("JobDomainPeasSessionController.modifyDomain()",SilverpeasException.ERROR,"admin.EX_ERR_UPDATE_DOMAIN");
        }
        SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.modifyDomain()", "root.MSG_GEN_ENTER_METHOD", "domainName="+domainName);
        theNewDomain.setName(domainName);
        theNewDomain.setDescription(domainDescription);
        theNewDomain.setDriverClassName(domainDriver);
        theNewDomain.setPropFileName(domainProperties);
    ...
    public String modifySQLDomain(String domainName, String domainDescription, String silverpeasServerURL) throws JobDomainPeasException, JobDomainPeasTrappedException
    {
        Domain theNewDomain = getTargetDomain();
        ...
        if ((m_TargetDomainId == null) || (m_TargetDomainId.equals("-1")) || (m_TargetDomainId.equals("0")) || (m_TargetDomainId.length() <= 0))
        {
            throw new JobDomainPeasException("JobDomainPeasSessionController.modifySQLDomain()",SilverpeasException.ERROR,"admin.EX_ERR_UPDATE_DOMAIN");
        }
        SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.modifySQLDomain()", "root.MSG_GEN_ENTER_METHOD", "domainName="+domainName);
        theNewDomain.setName(domainName);
        theNewDomain.setDescription(domainDescription);
        theNewDomain.setSilverpeasServerURL(silverpeasServerURL);
        idRet = m_AdminCtrl.updateDomain(theNewDomain);
    ...
    public void deleteDomain() throws JobDomainPeasException
    {
        ...
        if ((m_TargetDomainId == null) || (m_TargetDomainId.equals("-1")) || (m_TargetDomainId.equals("0")) || (m_TargetDomainId.length() <= 0))
        {
            throw new JobDomainPeasException("JobDomainPeasSessionController.deleteDomain()",SilverpeasException.ERROR,"admin.MSG_ERR_DELETE_DOMAIN");
        }
    ...
    public void deleteSQLDomain() throws JobDomainPeasException
    {
        ...
        if ((m_TargetDomainId == null) || (m_TargetDomainId.equals("-1")) || (m_TargetDomainId.equals("0")) || (m_TargetDomainId.length() <= 0))
        {
            throw new JobDomainPeasException("JobDomainPeasSessionController.deleteSQLDomain()",SilverpeasException.ERROR,"admin.MSG_ERR_DELETE_DOMAIN");
        }
    	...
    	try {
			...
			throw new JobDomainPeasException("JobDomainPeasSessionController.deleteSQLDomain()",SilverpeasException.ERROR,"admin.MSG_ERR_DELETE_DOMAIN", e1);
		}
	...
	public String initSelectionPeasForGroups(String compoURL) throws JobDomainPeasException
	{
		...
		PairObject hostComponentName = new PairObject(getTargetGroup().getName(),  compoURL + "groupContent");
        ...
        if ((m_TargetDomainId != null) && (!m_TargetDomainId.equals("-1")) && (m_TargetDomainId.length() > 0))
        {
            // Add extra params
        ...
        sel.setSelectedElements(SelectionUsersGroups.getUserIds(((GroupNavigationStock)m_GroupsPath.lastElement()).getAllUserPage()));
	...
	*/
	...
	public String initSelectionPeasForOneGroupOrUser(String compoURL) throws JobDomainPeasException
	{
		...
		PairObject hostComponentName = new PairObject(getTargetDomain().getName(),  compoURL + "domainContent");
		PairObject[] hostPath = new PairObject[0];
    ...
    {
    	SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.synchroSQLDomain()", "root.MSG_GEN_PARAM_VALUE", "------------SYNCHRO SQL DOMAIN APPELE----------- domainId="+m_TargetDomainId);
        if (m_theThread == null)
        {
            ...
            SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.synchroSQLDomain()", "root.MSG_GEN_PARAM_VALUE", "------------THREAD SYNCHRO SQL DOMAIN LANCE-----------");
        }
        else
        {
            SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.synchroSQLDomain()", "root.MSG_GEN_PARAM_VALUE", "------------!!!! SYNCHRO DOMAIN SQL : DEUXIEME APPEL !!!!!-----------");
        }
    }
    ...
    protected String synchronizeSilverpeasViaWebService() {
    	...
    	try {
	        ...
	        SynchroReport.warn("jobDomainPeas.synchronizeSilverpeasViaWebService","Domaine : " + theDomain.getName()+ " (id : " + theDomain.getId()+")",null);
			...
			try {
				...
				throw new JobDomainPeasException("JobDomainPeasSessionController.synchroSQLDomain()",SilverpeasException.ERROR,"admin.MSG_ERR_SYNCHRONIZE_DOMAIN", e1);
			}	
			...
			try {
				...
				throw new JobDomainPeasException("JobDomainPeasSessionController.synchroSQLDomain()",SilverpeasException.ERROR,"admin.MSG_ERR_SYNCHRONIZE_DOMAIN", e);
			}
			
			synchroUserWebService.startConnection();
			
			//Insertion / Update de la société
    		sReport += synchroUserWebService.insertUpdateDomainWebService(theDomain.getId(), theDomain.getName());
	    	...
	    	if(listGroupToInsertUpdate != null && listGroupToInsertUpdate.size()>0) { 
	    		...
	    		sReport += synchroUserWebService.insertUpdateListGroupWebService(theDomain.getId(), theDomain.getName(), listGroupToInsertUpdate);
	    		
	    		//Suppression des groupes
	    		/*Collection listGroupSilverpeas = Arrays.asList(m_AdminCtrl.getAllGroupIds()); 
	    		sReport += synchroUserWebService.deleteListGroupWebService(theDomain.getId(), listGroupSilverpeas);*/
	    	...
	    	if( (listUserToInsertUpdate != null && listUserToInsertUpdate.size()>0) || 
	    		...
	    		if(listUserToInsertUpdate != null && listUserToInsertUpdate.size()>0) {
	    			sReport += synchroUserWebService.insertUpdateListUserWebService(theDomain.getId(), listUserToInsertUpdate, listGroupToInsertUpdate);
	    		}
	    		
	    		//Suppression des users
	    		if(listUserToDelete != null && listUserToDelete.size()>0) {
	    			sReport += synchroUserWebService.deleteListUserWebService(theDomain.getId(), listUserToDelete); 
	    		}*/
	    		//Suppression des users
	    		if(listUserToDelete != null && listUserToDelete.size()>0) {
	    			sReport += synchroUserWebService.deleteListUserWebService(theDomain.getId(), listUserToDelete); 
    		...
    		SilverTrace.error("JobDomainPeasSessionController", "JobDomainPeasSessionController.synchronizeSilverpeasViaWebService", "admin.MSG_ERR_SYNCHRONIZE_DOMAIN", e);
    		SynchroReport.error("JobDomainPeasSessionController.synchronizeSilverpeasViaWebService", "Problème lors de la synchronisation : " + e.getMessage(),null );
    ...
    {
        SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.synchroDomain()", "root.MSG_GEN_PARAM_VALUE", "------------SYNCHRO DOMAIN APPELE----------- domainId="+m_TargetDomainId);
        if (m_theThread == null)
        {
            ...
            m_SynchroReport = "";
            m_theThread.startTheThread();
            SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.synchroDomain()", "root.MSG_GEN_PARAM_VALUE", "------------THREAD SYNCHRO DOMAIN LANCE-----------");
        }
        else
        {
            SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.synchroDomain()", "root.MSG_GEN_PARAM_VALUE", "------------!!!! SYNCHRO DOMAIN : DEUXIEME APPEL !!!!!-----------");
        }
    }
    ...
    public void getP12(String userId) throws JobDomainPeasException
    {
    	...
    	try {
			X509Factory.buildP12(user.getId(), user.getLogin(), user.getLastName(), user.getFirstName(), user.getDomainId());
		} catch (UtilException e) {
			throw new JobDomainPeasException("JobDomainPeasSessionController.getP12()", SilverpeasException.ERROR, "admin.MSG_ERR_CANT_GET_P12", e);
		}
    }
    ...
    private void revocateCertificate(UserDetail user) throws JobDomainPeasException
    {
	    try {
			...
			throw new JobDomainPeasException("JobDomainPeasSessionController.revocateCertificate()", SilverpeasException.ERROR, "admin.MSG_ERR_CANT_REVOCATE_CERTIFICATE", e);
		}
    }
	...
	{
		SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.getListUsersSelected()", "", "listSelectedUsers (taille) = (" + listSelectedUsers.size() + ") "+ listSelectedUsers.toString());
	...
	public void setListSelectedUsers(ArrayList list)
	{
		...
		SilverTrace.info("jobDomainPeas", "JobDomainPeasSessionController.setListSelectedUsers()", "", "listSelectedUsers (taille) = (" + listSelectedUsers.size() + ") "+ listSelectedUsers.toString());
	}
