...
//	Variables
   ...
   */
  private PublicationBm getPublicationBm() throws ImportExportException {
  ...
  private FormTemplateBm getFormTemplateBm() throws ImportExportException {
   ...
   * @throws ImportExportException
   */
  private NodeBm getNodeBm() throws ImportExportException {
  ...
  /**
   * Méthode de création ou mise à jour d'une publication utilisée par le manager d'importation de repository du
   * * moteur d'importExport.
   * Cas particulier: si une publication de même nom existe déjà dans le composant, alors une nouvelle publication ne sera créée
   * que si le premier node à lier ne contient pas la publication de même nom.
   * @param pubDetailToCreate - publication à créer ou à mettre à jour.
   * @return l'objet PublicationDetail contenant les informations de la publication créée ou mise à jour.
   * @throws ImportExportException
   */
  private PublicationDetail processPublicationDetail(UnitReport unitReport, UserDetail userDetail, PublicationDetail pubDetailToCreate, List listNode_Type) {
    ...
    if (existingTopics.size() == 0 && !isKmax()) {
      ...
      if (!pubIdExists) {
        try {
          ...
          if (itListNode_Type.hasNext()) {
            ...
            pubDet_temp = getPublicationBm().getDetailByNameAndNodeId(pubDetailToCreate.getPK(), pubDetailToCreate.getName(), node_Type.getId());
          }
        } catch (Exception pre) {
          pubAlreadyExist = false;
        }
      ...
      if (isKmax()) {
        try {
          pubDet_temp = getPublicationBm().getDetailByName(pubDetailToCreate.getPK(), pubDetailToCreate.getName());
          if (pubDet_temp != null) {
        ...
        if (isKmelia()) {
          while (itListNode_Type.hasNext()) {
            ...
            try {
              ...
              if (pubAlreadyExist) {
                //Vérification dans le cas d une publication déjà existante de sa présence dans le thème
                try {
                  getPublicationBm().getDetailByNameAndNodeId(pubDet_temp.getPK(), pubDet_temp.getName(), node_Type.getId());
                } catch (Exception ex) {
   ...
   * @param pubContent - object de mapping castor contenant les informations d importation du contenu
   * @throws ImportExportException
   */
  public void createPublicationContent(UnitReport unitReport, int pubId, PublicationContentType pubContent, String userId) throws ImportExportException {
  ...
  private void createXMLModelContent(UnitReport unitReport, XMLModelContentType xmlModel, String pubId, String userId) throws Exception {
    ...
    if (infoId == null || "0".equals(infoId)) {
      ...
      PublicationTemplateManager.addDynamicPublicationTemplate(getCurrentComponentId() + ":" + xmlFormShortName, xmlFormShortName + ".xml");
    }

    PublicationTemplate pub = PublicationTemplateManager.getPublicationTemplate(getCurrentComponentId() + ":" + xmlModel.getName());
    ...
    for (int f = 0; f < xmlFields.size(); f++) {
      ...
      if (field != null) {
        ...
        if (fieldTemplate != null) {
          FieldDisplayer fieldDisplayer = TypeManager.getDisplayer(field.getTypeName(), fieldTemplate.getDisplayerName());
          if (field.getTypeName().equals(FileField.TYPE)) {
            ...
            String imageName = imagePath.substring(imagePath.lastIndexOf(File.separator) + 1, imagePath.length());
            ...
            if (size > 0) {
              AttachmentDetail ad = createAttachmentDetail(pubId, getCurrentComponentId(), physicalName, imageName, imageMimeType, size, context, userId);
  ...
  private void createDBModelContent(UnitReport unitReport, DBModelContentType dbModelType, String pubId) throws ImportExportException, RemoteException {
    ...
    ModelPK modelPK = new ModelPK(Integer.toString(dbModelType.getId()), "useless", getCurrentComponentId());
    ...
    if (infoDetailFromPub != null && !infoDetailFromPub.getPK().getId().equals("0")) {
      ...
      if (Integer.parseInt(modelDetail.getId()) == dbModelType.getId()) {
        ...
        unitReport.setError(UnitReport.ERROR_CANT_UPDATE_CONTENT);//le model existant n'est pas le meme que l'importé
      }
      ...
      unitReport.setError(UnitReport.ERROR_CANT_UPDATE_CONTENT);//la pub existante contient un wysiwyg
    }
  }

  /**
   * Méthode préparant les dataModels pour la création en base d un contenu DBModel, cette méthode se charge également de
   * la copie des fichiers du contenu sur le serveur
   * @param dbModelType - objet de mapping castor contenant les informations de contenu de type DBModel
   * @return
   */
...
//		if infoDetailModel exists...
    ...
    if (listTextParts != null) {
      ...
      while (itListTextParts.hasNext()) {
        ...
        listInfoText.add(new InfoTextDetail(null, new Integer(textOrder++).toString(), null, textPart));
      }
    }
    ...
    if (listImagesParts != null) {
      ...
      while (itListImagesParts.hasNext()) {
        ...
        listInfoImage.add(new InfoImageDetail(null, new Integer(imageOrder++).toString(), null, imagePath, imagePath.substring(imagePath.lastIndexOf(File.separator) + 1), "", mimeType, size));
      }
    ...
    InfoDetail infoDetail = new InfoDetail(new InfoPK("unknown", "useless", getCurrentComponentId()), listInfoText, listInfoImage, null, null);
   ...
   * @param wysiwygType - objet de mapping castor contenant les informations de contenu de type Wysiwyg
   */
  private void createWysiwygContent(UnitReport unitReport, int pubId, WysiwygContentType wysiwygType, String userId) throws UtilException, WysiwygException, ImportExportException {
    ...
    try {
      ...
      throw new ImportExportException("GEDImportExport.createPublicationContent()", "importExport.EX_CANT_CREATE_CONTENT", "file = " + f.getPath(), ex);
    }
    if (wysiwygText == null) {
      unitReport.setError(UnitReport.ERROR_NOT_EXISTS_OR_INACCESSIBLE_FILE_FOR_CONTENT);
      throw new ImportExportException("GEDImportExport.createPublicationContent()", "importExport.EX_CANT_CREATE_CONTENT", "file = " + f.getPath());
    }
    ...
    if (WysiwygController.haveGotWysiwyg("useless", getCurrentComponentId(), Integer.toString(pubId))) {
      ...
      try {
        WysiwygController.deleteWysiwygAttachmentsOnly("useless", getCurrentComponentId(), Integer.toString(pubId));
    ...
    String newWysiwygText = replaceWysiwygImagesPathForImport(unitReport, pubId, f.getParent(), wysiwygText, imagesContext);
    newWysiwygText = removeWysiwygStringsForImport(newWysiwygText);
    newWysiwygText = replaceWysiwygStringsForImport(newWysiwygText);
    WysiwygController.createFileAndAttachment(newWysiwygText, wysiwygFileName, "useless", getCurrentComponentId(), "wysiwyg", Integer.toString(pubId), userId);
  }

  /**
   * Méthode chargée de copier les fichiers images référencés par le contenu wysiwyg sur le serveur et de mettre à jour
   * le contenu wysiwyg avec ces nouveaux liens
   ...
   */
  private String replaceWysiwygImagesPathForImport(UnitReport unitReport, int pubId, String wysiwygImportedPath, String wysiwygText, String imageContext) throws ImportExportException {
    ...
    if (wysiwygText.indexOf("img src=\"", finPath) == -1) {
      ...
      while ((debutPath = wysiwygText.indexOf("img src=\"", finPath)) != -1) {
        ...
        if (imageSrc.indexOf("http://") != 0) {
          ...
          if (!f.isAbsolute()) {
            //si le wysiwyg est issu d une export, les liens image ne comporte que leur nom(donc pour l import, l utilisateur doit placer
            //les images wysiwyg dans le meme dossier que le wysiwyg OU eremplacer leur chemin par l absolu
          ...
          try {
            attDetail = attachmentIE.importWysiwygAttachment(Integer.toString(pubId), getCurrentComponentId(), attDetail, imageContext);
            ...
            SilverTrace.error("importExport", "GEDImportExport.replaceWysiwygImagesPathForImport()", "importExport.CANNOT_FIND_FILE", e);
   ...
   * @return - le contenu wysiwyg mis à jour
   */
  private String replaceWysiwygStringForImport(String oldString, String newString, String wysiwygText) {
   ...
   * @param componentId - id du composant de la publication dans laquelle on va creer le contenu DBModel
   * @param listInfoImageDetail - liste des InfoImageDetails correspondant aux images du contenu DBModel
   * @return - la liste des InfoImageDetails mise à jour avec les nouveaux fichiers créés sur le serveur
   */
  private Collection copyDBmodelImagePartsForImport(UnitReport unitReport, String componentId, Collection listInfoImageDetail) {
    ...
    while (itListInfoImageDetail.hasNext()) {
      ...
      String type = infoImage.getPhysicalName().substring(infoImage.getPhysicalName().indexOf(".") + 1, infoImage.getPhysicalName().length());
      String newName = new Long(new java.util.Date().getTime()).toString() + "." + type;
      infoImage.setPhysicalName(newName);
      String to = FileRepositoryManager.getAbsolutePath(componentId) + File.separator + "images" + File.separator + newName;
  ...
  /**
   * Méthode de copie des images DBModel d'un contenu dans le dossier d'exportation d'une publication
   ...
   */
  public ArrayList copyDBmodelImagePartsForExport(String exportPublicationPath, ArrayList listImageParts, String exportPublicationRelativePath) {
    ...
    if ((listImageParts != null) && (listImageParts.size() != 0)) {
      for (int i = 0; i < listImageParts.size(); i++) {
        ...
        File f = new File(imagePath);
        try {//TODO: a revoir
          FileRepositoryManager.copyFile(imagePath, exportPublicationPath + File.separator + f.getName());
  ...
  /**
   * Méthode copiant les images contenues dans le dossier d'exportation de la publication. Cette méthode met à jour le fichier
   * wysiwyg avec les nouveaux chemins d'images avant de le copier dans l'exportation
  ...
  public void copyWysiwygImageForExport(String pubId, String componentId, String exportPublicationPath) {
    ...
    Vector vectAttachment = AttachmentController.searchAttachmentByPKAndContext(foreignKey, imagesContext);
    ...
    while (itListAttachment.hasNext()) {
      ...
      try {
        FileRepositoryManager.copyFile(path + File.separator + attDetail.getPhysicalName(), exportPublicationPath + File.separator + attDetail.getLogicalName());
      } catch (IOException ex) {
        //TODO: gerer l exception!!
      }
    }
   ...
   * @throws ImportExportException
   */
  protected abstract NodePK addSubTopicToTopic(NodeDetail nodeDetail, int topicId, MassiveReport massiveReport) throws Exception;
   ...
   */
  public PublicationDetail createPublicationForUnitImport(UnitReport unitReport, UserDetail userDetail, PublicationDetail pubDetail, List listNode_Type) {
   ...
   * @return le publicationDetail de la publication créée
   */
  public PublicationDetail createPublicationForMassiveImport(UnitReport unitReport, UserDetail userDetail, PublicationDetail pubDetail, int nodeId) throws ImportExportException {
  ...
  public PublicationType getPublicationCompleteById(String pubId, String componentId) throws ImportExportException {
    ...
    try {
      CompletePublication pubComplete = getCompletePublication(new PublicationPK(pubId, getCurrentComponentId()));
      ...
      if (infoDetail != null && (infoDetail.getInfoImageList().size() != 0 || infoDetail.getInfoTextList().size() != 0)) {
        ...
        if ((listInfoImage != null) && (listInfoImage.size() != 0)) {
          ...
          while (itListInfoImage.hasNext()) {
            ...
            String path = FileRepositoryManager.getAbsolutePath(componentId) + File.separator + "images";
        ...
        pubContent = new PublicationContentType();
        List xmlFields = getFormTemplateBm().getXMLFieldsForExport(publicationDetail.getPK().getInstanceId() + ":" + publicationDetail.getInfoId(), pubId);
        SilverTrace.info("importExport", "GEDImportExport.getPublicationCompleteById()", "root.MSG_GEN_PARAM_VALUE", "# of xmlField = " + xmlFields.size());
   ...
   * @throws ImportExportException
   */
  public List getAllTopicsOfPublication(String pubId, String componentId) throws ImportExportException {
   ...
   * @return pubDetail
   */
  protected abstract PublicationDetail createPublication(PublicationDetail pubDetail) throws Exception;

  public Collection getPublicationCoordinates(String pubId, String componentId) throws ImportExportException {
    try {
  ...
  private void processThumbnail(String filePath, PublicationDetail pubDetail) {
    ...
    if (type.equalsIgnoreCase("gif") || type.equalsIgnoreCase("jpg") || type.equalsIgnoreCase("jpeg") || type.equalsIgnoreCase("png")) {
      String dest = FileRepositoryManager.getAbsolutePath(pubDetail.getPK().getInstanceId()) + "images" + File.separator + physicalName;

      try {
        FileRepositoryManager.copyFile(filePath, dest);
      } catch (Exception e) {
        SilverTrace.error("importExport", "GEDImportExport.processThumbnail()", "root.MSG_GEN_PARAM_VALUE", "filePath = " + filePath, e);
      }
  ...
  protected abstract void updatePublication(PublicationDetail pubDet_temp, PublicationDetail pubDetailToCreate, UserDetail userDetail)
      throws Exception;

  protected abstract String createPublicationIntoTopic(PublicationDetail pubDet_temp, NodePK topicPK, UserDetail userDetail) throws Exception;

  protected abstract void addPublicationToTopic(PublicationPK pubPK, NodePK topicPK) throws Exception;
