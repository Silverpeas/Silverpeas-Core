...
public class ExportIcalManager {
	 ...
	 */
	public String exportIcalAgenda(String startDate, String endDate) throws
	Exception, AgendaException, FileNotFoundException, ValidationException, IOException, ParseException
	{
		String returnCode = AgendaSessionController.EXPORT_SUCCEEDED;
		String calendarIcsFileName = AgendaSessionController.AGENDA_FILENAME_PREFIX + getUserId() + ".ics";
		...
		{ 
			FileOutputStream fileOutput = new FileOutputStream(FileRepositoryManager.getTemporaryPath() + calendarIcsFileName);
			try
			{
			...
			{
				fileOutput.close();
				FileFolderManager.deleteFile(FileRepositoryManager.getTemporaryPath() + calendarIcsFileName);
				throw new AgendaException("ExportIcalManager.exportIcalAgenda()",SilverpeasException.ERROR,"agenda.EXPORT_ICAL_FAILED",e);
			}
		}
		return returnCode;
	}
	 ...
	 * @throws ParseException
	 */
	public String exportIcalAgenda() throws
		Exception, AgendaException, FileNotFoundException, ValidationException, IOException, ParseException
	{
	 ...
	 * @throws ParseException
	 */
	public String exportIcalAgendaForSynchro() throws
	Exception, AgendaException, FileNotFoundException, ValidationException, IOException, ParseException
	{
		String calendarIcsFileName = AgendaSessionController.AGENDA_FILENAME_PREFIX + getUserId() + ".ics";
	 ...
	 */
	private Calendar getIcsCalendarContent(Calendar calendarIcs, String startDate, String endDate)
		throws ParseException, RemoteException, SocketException, AgendaException, URISyntaxException
	{
		SilverTrace.info("agenda","ExportIcalManager.getIcsCalendarContent()","root.MSG_GEN_ENTER_METHOD");
		SilverTrace.debug("agenda","ExportIcalManager.getIcsCalendarContent()","root.MSG_GEN_PARAM_VALUE","startDate="+startDate+" endDate="+endDate);
		...
		if (exportAll)
		{
			...
			SilverTrace.debug("agenda","ExportIcalManager.getIcsCalendarContent()","root.MSG_GEN_PARAM_VALUE","exportAll");
			SilverTrace.debug("agenda","ExportIcalManager.getIcsCalendarContent()","root.MSG_GEN_PARAM_VALUE","startDate="+startDate+" endDate="+endDate);
		}
		...
		while (itSchedules.hasNext())
		{
			...
			if (obj instanceof Schedulable)
			{
				...
				SilverTrace.debug("agenda","ExportIcalManager.getIcsCalendarContent()","root.MSG_GEN_PARAM_VALUE","eventAgenda.getStartDay()="+eventAgenda.getStartDay()+" eventAgenda.getStartDate()="+eventAgenda.getStartDate());
				...
				//Add Classification
				event.getProperties().add(new Clazz(eventAgenda.getClassification().getString()));
				//Add Priority
				event.getProperties().add(new Priority(eventAgenda.getPriority().getValue()));
			    	...
			    	while (attendeesIt.hasNext())
			    	{
			    		...
			    		if (user != null)
			    		{
			    			...
			    			if (StringUtil.isDefined(email))
			    				event.getProperties().add(new net.fortuna.ical4j.model.property.Attendee(email));
			    		}
			    	}
				}
		...
		SilverTrace.info("agenda","ExportIcalManager.getIcsCalendarContent()","root.MSG_GEN_EXIT_METHOD");
		return calendarIcs;
	}
	 ...
	 * @throws ParseException
	 */
	private Collection getSchedulableCalendar(String startDate, String endDate) throws RemoteException, ParseException
	{
		return calendarBm.getPeriodSchedulablesForUser(DateUtil.date2SQLDate(startDate, getLanguage()),
				DateUtil.date2SQLDate(endDate, getLanguage()),
				getUserId(),
			null, new ParticipationStatus(ParticipationStatus.ACCEPTED).getString());
	}
		...
		if (calendarBm == null)
		{
			...
			catch (Exception e)
			{
				throw new AgendaRuntimeException(
					"ExportIcalManager.setCalendarBm()",
