 ...
 */
/**
 * A REST Web resource representing a given subscription. It is a web service that provides an
 * access to a subscription referenced by its URL.
 */
@Service
@Scope("request")
...
public class SubscriptionResource extends RESTWebService {
  ...
  protected SubscriptionEntity[] asWebEntities(Collection<? extends Subscription> subscriptions) {
    ...
    for (Subscription subscription : subscriptions) {
      URI subscriptionURI =
          getUriInfo().getRequestUriBuilder().path(subscription.getTopic().getId()).
          build();
