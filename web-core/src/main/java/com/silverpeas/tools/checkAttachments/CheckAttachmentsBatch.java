...
public class CheckAttachmentsBatch
{
  ...
  public CheckAttachmentsBatch()
	...
	public List<CheckAttachmentDetail> check(String attachmentType, String reqLanguage) throws Exception
	{
	...
	public List<OrphanAttachment> getOrphansFiles(String context) throws RemoteException
	{
	  ...
	  if (spaces != null)
	  {
	    ...
	    {
  			SilverTrace.info("admin", "CheckAttachmentBatch.getOrphansFiles(context)", "root.MSG_GEN_PARAM_VALUE", "spaceid = " + spaces[i]);
  		  ...
  		  for (int j = 0; componentsId!=null && j < componentsId.length; j++)
  		  {
    			...
    			SilverTrace.info("admin", "CheckAttachmentBatch.getOrphansFiles(context)", "root.MSG_GEN_PARAM_VALUE", "pathFolder = " + pathFolder);
    			...
    			if (listFiles != null){
    			    for ( int k = 0; k < listFiles.length; k++) {
      					SilverTrace.info("admin", "CheckAttachmentBatch.getOrphansFiles(context)", "root.MSG_GEN_PARAM_VALUE", "file = " + listFiles[k].getName());
	                ...
	                SpaceInstLight spaceInst = oc.getSpaceInstLightById(componentInst.getDomainFatherId());
        					orphanAttachment.setSpaceLabel(spaceInst.getName(language));
        					orphanAttachment.setComponentLabel(componentInst.getLabel(language) + " ("+componentsId[j]+")");
            ...
            SilverTrace.info("admin", "CheckAttachmentBatch.getOrphansFiles(context)", "root.MSG_GEN_PARAM_VALUE", "path vide = " + path);
    			}
  		  }
	    }
	  }
	...
	public List<AttachmentDetail> getAttachmentsByPhysicalName(String physicalName)
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				if (attDetail != null)
				{
          SilverTrace.info("admin", "CheckAttachmentBatch.getAttachmentsByPhysicalName(physicalName)", "root.MSG_GEN_PARAM_VALUE", "attDetail = " + attDetail.getPK().getId());
   ...
   */
	public List<CheckAttachmentDetail> getAttachments(String attachmentType) throws SQLException, RemoteException
	{
		Connection con = openConnection();
		...
		try
		{
			...
			while (rs.next())
			{
			  boolean display = false;
				...
				if (attDetail != null)
				{
          if (attDetail.getContext().equals(attachmentType)) //Images, wysiwyg, XMLFormImages
            display = true;
          ...
          if (display)
          {
            ...
            cad.setComponentLabel(componentInst.getLabel(language) + " ("+componentInst.getId()+")");
            cad.setContext(attDetail.getContext());
            
            boolean isPublication = false;
            if (attDetail.getInstanceId().startsWith("kmelia") || attDetail.getInstanceId().startsWith("toolbox"))
            {
            ...
            if (isPublication)
            {
              PublicationPK pubPK = new PublicationPK(attDetail.getForeignKey().getId(), attDetail.getInstanceId());
              PublicationDetail pubDetail = getPublicationDetail(pubPK);
                            
              if (pubDetail != null)
              {
                String publicationPath = spaceInst.getName(language) + " > " + componentInst.getLabel(language) + " > "
                    + displayPath(pubDetail, false, 3) + " > " + pubDetail.getName(language);
                cad.setPublicationPath(publicationPath);
    
                String actionsDate = orgaController.getUserDetail(pubDetail.getCreatorId()).getDisplayedName() + " le " + DateUtil.getOutputDate(pubDetail.getCreationDate(), language);
            ...
            if (att.exists()) {
              if (att.length() != attDetail.getSize()) {
                cad.setStatus("Taille en BD: " + attDetail.getSize() + " octets, Lue: " + att.length() + " octets");
              } else
                cad.setStatus("OK");
            } else {
  ...
  public String displayPath(PublicationDetail pubDetail, boolean linked, int beforeAfter) throws RemoteException {
    List<NodePK> nodesPK = (List<NodePK>) pubDetail.getPublicationBm().getAllFatherPK(pubDetail.getPK());
      ...
      if (path != null)
      {
        ...
        while (iterator.hasNext()) {
          ...
          if (!nodeInPath.getNodePK().getId().equals("0")) {
            linkedPathString.append("<a href=\"javascript:onClick=topicGoTo('").append(nodeInPath.getNodePK().getId()).append("')\">").append(Encode.javaStringToHtmlString(nodeInPath.getName())).append("</a>");
