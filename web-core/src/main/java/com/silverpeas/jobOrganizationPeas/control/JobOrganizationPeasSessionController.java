...
public class JobOrganizationPeasSessionController extends AbstractComponentSessionController
{
	...
	public String[][] getCurrentUserGroups() {
		if (currentGroups==null) {
			...
			String[] groupIds = getAdminController().getDirectGroupsIdsOfUser(getCurrentUserId());
		...
		if (currentGroups == null)
			SilverTrace.info("jobOrganizationPeas","JobOrganizationPeasSessionController.getCurrentUserInfo","root.MSG_GEN_PARAM_VALUE","Groups NULL !");
		else
			SilverTrace.info("jobOrganizationPeas","JobOrganizationPeasSessionController.getCurrentUserInfo","root.MSG_GEN_PARAM_VALUE","Groups="+currentGroups);
		return currentGroups;
	}

	/**
	 * @return an array of (name, value). name can be either a string or a resource key starting by 'GML.' or 'JOP.'
	 */
	public String[][] getCurrentUserInfos() {
		if (currentInfos==null) {
			...
			for (int iSL=nbStdInfos ; iSL<currentInfos.length ; iSL++) {
				...
				if (!currentKey.equals("password")) {
					// Label
					currentInfos[iSL][0]=userInfos.getSpecificLabel(getLanguage(),currentKey);
				    // Valeur
		...
		if (currentInfos == null)
			SilverTrace.info("jobOrganizationPeas","JobOrganizationPeasSessionController.getCurrentUserInfo","root.MSG_GEN_PARAM_VALUE","User NULL !");
		else
			SilverTrace.info("jobOrganizationPeas","JobOrganizationPeasSessionController.getCurrentUserInfo","root.MSG_GEN_PARAM_VALUE","User="+currentInfos);
		return currentInfos;
	}
	...
	public String[][] getCurrentGroupInfos() {
		if (currentInfos==null) {
			...
			if (getCurrentGroupId()==null) return null;
			...
			if (parentId==null || parentId.equals(""))
				...
				currentInfos[3][1]=getAdminController().getGroupName(groupInfos.getSuperGroupId());
		}
		if (currentInfos == null)
			SilverTrace.info("jobOrganizationPeas","JobOrganizationPeasSessionController.getCurrentGroupInfo","root.MSG_GEN_PARAM_VALUE","Infos NULL !");
		else
			SilverTrace.info("jobOrganizationPeas","JobOrganizationPeasSessionController.getCurrentGroupInfo","root.MSG_GEN_PARAM_VALUE","infos="+currentInfos);
		return currentInfos;
	}
	...
	public String[][] getCurrentInfos() {
		if (getCurrentGroupId()!=null) {
			SilverTrace.debug("jobOrganizationPeas","JobOrganizationPeasSessionController.getCurrentInfos","root.EX_NO_MESSAGE","Returns groupInfos");
			return getCurrentGroupInfos();
		}
		if (getCurrentUserId()!=null) {
			SilverTrace.debug("jobOrganizationPeas","JobOrganizationPeasSessionController.getCurrentInfos","root.EX_NO_MESSAGE","Returns userInfos");
			return getCurrentUserInfos();
		}
		SilverTrace.debug("jobOrganizationPeas","JobOrganizationPeasSessionController.getCurrentInfos","root.EX_NO_MESSAGE","Returns no infos");
		return null;
	}
	...
	public String[] getCurrentSpaces() {
		if (currentSpaces==null) {
			...
			if (getCurrentGroupId()!=null)
				spaceIds = getAdminController().getGroupManageableSpaceIds(getCurrentGroupId());
			if (getCurrentUserId()!=null)
				spaceIds = getAdminController().getUserManageableSpaceIds(getCurrentUserId());
			if (spaceIds == null) return null;
	...
	/**
	 * @return a map of the official (business) names of the installed components indexed by the internal names
	 */
	...
	private String getComponentOfficialName(String internalName) {
        try
        {
    		return ((WAComponent)getComponentOfficialNames().get(internalName)).getLabel();
        }
        catch (Exception e)
        {
			SilverTrace.info("jobOrganizationPeas","JobOrganizationPeasSessionController.getComponentOfficialName","root.MSG_GEN_PARAM_VALUE","!!!!! ERROR getting official name="+internalName,e);
    		return internalName;
        }
	}
	...
	public List getCurrentProfiles() {
		if (currentProfiles==null) {
			...
			if (getCurrentGroupId()!=null)
				profileIds = getAdminController().getProfileIdsOfGroup(getCurrentGroupId());
			if (getCurrentUserId()!=null)
			...
			for (int iProfile=0 ; iProfile<profileIds.length ; iProfile++) {
				...
				currentComponent 	= getAdminController().getComponentInst(currentProfile.getComponentFatherId());
				if (currentComponent.getStatus() == null)
				{
					...
					if (!distinctProfiles.contains(dProfile))
					{
						...
						if (!StringUtil.isDefined(profile2Display[3]))
							profile2Display[3] = getAdminController().getProfileLabelfromName(currentComponent.getName(), currentProfile.getName());
			...
			String[] spaceNames=getAdminController().getSpaceNames((String[])spaceIds.toArray(new String[spaceIds.size()]));
	...
	*/
	public String initSelectionPeas()
    {
        ...
        String cancelUrl = m_context + Selection.getSelectionURL(Selection.TYPE_USERS_GROUPS);
        PairObject hostComponentName = new PairObject(getString("JOP.pseudoPeas"),cancelUrl);
