...
/**
 * One implementation of WorkflowEngine
...
public class WorkflowEngineImpl implements WorkflowEngine
{
	...
	public void process(TaskDoneEvent event) throws WorkflowException
	{
		...
		if (action!= null && action.getKind().equals("create"))
		{
			...
			instance = (UpdatableProcessInstance) instanceManager.createProcessInstance(model.getModelId());
		...
		try
		{
			...
			{
				db.rollback();
				throw new WorkflowException("WorkflowEngineImpl.process", "workflowEngine.EX_ERR_PROCESS_EVENT", we);
			}
		...
		{
			throw new WorkflowException("WorkflowEngineImpl.process", "workflowEngine.EX_ERR_PROCESS_EVENT", pe);
		}
		finally
		{
	 ...
	 * Remove users as working users and unassign corresponding tasks
	 * Add users as working users and assign corresponding tasks
	 */
	public void reAssignActors(UpdatableProcessInstance instance, Actor[] unAssignedActors, Actor[] assignedActors, User user)
		throws WorkflowException
	{
		...
		try
		{
			...
			{
				db.rollback();
				throw new WorkflowException("WorkflowEngineImpl.assignActors", "EX_ERR_REASSIGN_ACTORS", we);
			}
			...
			try
			{
				...
				{
					instance.removeWorkingUser(unAssignedActors[i].getUser(), unAssignedActors[i].getState(), unAssignedActors[i].getUserRoleName());
				}
				...
				{
					instance.addWorkingUser(assignedActors[i].getUser(), assignedActors[i].getState(), assignedActors[i].getUserRoleName());
				}
			...
			{
				db.rollback();
				throw new WorkflowException("WorkflowEngineImpl.reAssignActors", "EX_ERR_REASSIGN_ACTORS", we);
			}
		...
		{
			throw new WorkflowException("WorkflowEngine.reAssignActors", "workflowEngine.EX_ERR_REASSIGN_ACTORS", pe);
		}
		finally
		{
			WorkflowJDOManager.closeDatabase(db);
	 ...
	 * Over-locks the process instance by admin
	 */
	private void manageLocks(GenericEvent event, UpdatableProcessInstance instance) throws WorkflowException
	{
		if (instance != null)
		{
			instance.lock();
