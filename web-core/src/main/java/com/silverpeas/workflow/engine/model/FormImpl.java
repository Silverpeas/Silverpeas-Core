 ...
 */
public class FormImpl extends AbstractReferrableObject implements Form, AbstractDescriptor, Serializable 
{
    private String                 name;
    private String                 role;
    ...
    /**
	 * Get the name of HTML file to show this form
	 * if no HTML file is defined, XMLForm will be used to create the form
	 * @return form's name
	 */
    ...
    /**
	 * Set the name of HTML file to show this form
	 * if no HTML file is defined, XMLForm will be used to display the form
	 * @return form's name
	 */
    ...
    /*
     * (non-Javadoc)
     * @see com.silverpeas.workflow.api.model.Form#addTitle(com.silverpeas.workflow.api.model.ContextualDesignation)
     */
		...
		throws WorkflowException
	{
		...
		try
		{
			...
			for (int i=0; i<inputList.size(); i++)
			{
				...
				if (readOnly) ft.setReadOnly(true);
				else          ft.setReadOnly(input.isReadonly());
				ft.setMandatory(input.isMandatory());
				if (input.getDisplayerName() != null && input.getDisplayerName().length()>0)
