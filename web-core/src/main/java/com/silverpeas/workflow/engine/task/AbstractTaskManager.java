...
/**
 * The AbstractTaskManager implements all the silverpeas internal
 * TaskManager methods.
 *
...
abstract public class AbstractTaskManager implements TaskManager 
{
    ...
    */
   public Task createTask(Actor actor,
                          ProcessInstance processInstance)
      throws WorkflowException
   {
      return new TaskImpl(actor.getUser(), actor.getUserRoleName(), processInstance, actor.getState());
   }

   /**
    * Builds new tasks (assigned or assignable).
    */
   public Task[] createTasks(Actor[] actors,
                             ProcessInstance processInstance)
      throws WorkflowException
   {
    ...
    */
   public Task[] getTasks(User user, String roleName, ProcessInstance processInstance)
      throws WorkflowException
   {
       ...
       for (int i=0; i<stateNames.length ; i++)
       {
			...
			tasks[i] = new TaskImpl(user, roleName, 
									processInstance, state, 
									steps, sentQuestions, 
									relevantQuestions, pendingQuestions);
       }
   ...
   /**
    * Returns the creation task of a processModel
    * or null if the user is not allowed to create a new instance.
    */
   public Task getCreationTask(User user, String roleName, ProcessModel processModel)
      throws WorkflowException
   {
      return new CreationTaskImpl(user,
                                  roleName,
                                  processModel);
   }
}
