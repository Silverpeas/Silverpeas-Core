	...
	*/
		...
		throws WorkflowException
	{
		...
		else try
		{
			...
			query = db.getOQLQuery( "SELECT settings FROM com.silverpeas.workflow.engine.user.UserSettingsImpl settings"
	...
	public void update(DataRecord data, RecordTemplate template)
	{
		...
		for (int i=0; fieldNames!=null && i<fieldNames.length; i++)
		{
			try
			{
				...
				if (field == null)
				{
					SilverTrace.warn("workflowEngine", "UserSettingsImpl.update", "workflowEngine.EX_ERR_GET_FIELD", fieldNames[i]);
					continue;
				}
		...
		throws WorkflowException
	{
		...
		{
			throw new WorkflowException("UserSettingsImpl.create", "workflowEngine.EX_ERR_CASTOR_CREATE_USER_SETTINGS", pe);
		}
		finally
		{
			WorkflowJDOManager.closeDatabase(db);
