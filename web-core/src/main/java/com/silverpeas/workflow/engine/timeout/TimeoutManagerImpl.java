...
public class TimeoutManagerImpl implements TimeoutManager, SchedulerEventHandler
{
	...
	public void initialize()
	{
		try
		{
	        ResourceLocator settings = new ResourceLocator("com.silverpeas.workflow.engine.schedulerSettings", "");
			...
			SimpleScheduler.getJob(this, TIMEOUT_MANAGER_JOB_NAME, cronString, this, "doTimeoutManagement");
		}
		catch (Exception e)
		{
			SilverTrace.error("workflowEngine", "TimeoutManagerImpl.initialize", "workflowEngine.EX_ERR_INITIALIZE", e);
		}
    ...
    public void handleSchedulerEvent(SchedulerEvent aEvent)
    {
        switch (aEvent.getType())
        {
			case SchedulerEvent.EXECUTION_NOT_SUCCESSFULL:
				SilverTrace.error("workflowEngine", "TimeoutManagerImpl.handleSchedulerEvent", "The job '" + aEvent.getJob().getJobName() + "' was not successfull");
	            break;
	
			case SchedulerEvent.EXECUTION_SUCCESSFULL:
	            SilverTrace.debug("workflowEngine", "TimeoutManagerImpl.handleSchedulerEvent", "The job '" + aEvent.getJob().getJobName() + "' was successfull");
	            break;

			default:
	            SilverTrace.error("workflowEngine", "TimeoutManagerImpl.handleSchedulerEvent", "Illegal event type");
	            break;
        }
    }
	...
	public void doTimeoutManagement(Date date)
	{
		...
		try
		{
			...
			SilverTrace.debug("workflowEngine", "TimeoutManagerImpl.doTimeoutManagement", "", "peas Id found : '" + peasIds.length);
			for (int i=0; i<peasIds.length; i++)
			{
				...
				SilverTrace.debug("workflowEngine", "TimeoutManagerImpl.doTimeoutManagement", "", states.length+" states found in model" + model.getName());
				for (int j=0; j<states.length; j++)
				{
					...
					try
					{
						if (timeoutAction != null && timeoutInterval != -1)
						{
							...
							ProcessInstance[] instances = Workflow.getProcessInstanceManager().getProcessInstancesInState(peasIds[i], states[j]);
							SilverTrace.debug("workflowEngine", "TimeoutManagerImpl.doTimeoutManagement", "", "instances found : '" + instances.length);
							for (int k=0; k<instances.length; k++)
							{
								...
								if (step != null)
								{
									...
									if (interval>timeout)
									{
										TimeoutEvent event = new TimeoutEventImpl(instances[k], states[j], timeoutAction);
										...
										SilverTrace.warn("workflowEngine",
														 ...
														 "model Id : '" + peasIds[i]
														 + "' instance Id : '" + instances[k].getInstanceId()
														 + "' state : '" + states[j].getName()
		...
		{
            SilverTrace.error("workflowEngine", "TimeoutManagerImpl.doTimeoutManagement", "workflowEngine.EX_ERR_TIMEOUT_MANAGEMENT", e);
		}
		finally
		{
			Date endDate = new Date();
			long delay = (endDate.getTime() - beginDate.getTime()) / 1000;
			SilverTrace.debug("workflowEngine", "TimeoutManagerImpl.doTimeoutManagement", "Durée de traitement : " + delay + " seconds.");			
		}
	}
}