...
import com.silverpeas.workflow.api.event.*;
import com.silverpeas.workflow.api.error.*;
...
public class WorkflowErrorImpl implements WorkflowError 
{
	 ...
	 */
	public WorkflowErrorImpl(ProcessInstance instance, GenericEvent event, HistoryStep step, Exception exception)
	{
		...
		if (user != null) this.userId		= event.getUser().getUserId();

		this.state							= event.getResolvedState();
		if (state != null) this.stateName	= event.getResolvedState().getName();
	...
	public ProcessInstance getProcessInstance() throws WorkflowException
	{
		if (processInstance == null)
		{
			if (instanceId != null)
			{
				processInstance = Workflow.getProcessInstanceManager().getProcessInstance(instanceId);
			}
		}
