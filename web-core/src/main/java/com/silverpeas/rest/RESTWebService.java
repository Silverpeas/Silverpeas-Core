...
public abstract class RESTWebService {
   ...
   * This parameter isn't mandatory as the session key can be found from an active HTTP session.
   * If neither HTTP session nor session key is available for the incoming request, user credentials
   ...
   * refered resource. This parameter must be used when requests aren't sent through an opened
   * HTTP session. It should be the prefered way for a REST client to access resources in
  ...
  ;
  private PersonalizationBm persoBm = null;
  ...
  /**
   * Sets a specific session management service other than the default one in Silverpeas.
   * At Silverpeas bootstrapping, a service for session management is created and injected as
   * dependency in each new REST web service instance (for doing, the web service must be managed
   * by an IoC container).
   * This method is mainly for testing purpose.
   ...
   * respectively a HTTP status code UNAUTHORIZED (401) or FORBIDEN (403).
   ...
   * identification or the authorization fails, then a WebApplicationException is thrown with
   * respectively a HTTP status code UNAUTHORIZED (401) or FORBIDEN (403).
   * If the preferences can be retrieved, then null is returned.
   ...
   * underlying referenced resource that belongs to a Silverpeas component instance.
   * This method should be called at each incoming request processing to ensure a strong security.
   * User information is retreived from the context of the incoming HTTP request. If no user
   * information can be retrieved, then a WebApplicationException is thrown with a status code
   * UNAUTHORIZED (401).
   * When the check fails, a WebApplicationException is thrown with the HTTP status code set
   ...
   * the specified Silverpeas component instance.
   * If the user isn't authorized to access the specified component, then a WebApplicationException
   * exception is thrown with a HTTP status code FORBIDDEN (403).
   ...
   * The session key is first retrieved from the HTTP header parameter X-Silverpeas-Session. If no
   * such parameter is set, it is then retrieved from the current HTTP session if any. If the
   * incoming request isn't sent within an active HTTP session, then an empty string is returned as
   * no HTTP session was defined for the current request.
  ...
  /**
   * Validates the current user session with the specified session key.
   * If the incoming request is within an opened HTTP session (not so stateless, should be avoided
  ...
  /**
   * Authenticates the user by using the specified credentials.
   * Once the user well authenticated, return details about him. If the authentication fails, then a
   * WebApplicationException exception is thrown with an HTTP status code UNAUTHORIZED (401).
   * The implementation of this method is for taking into account the Silverpeas security doesn't
