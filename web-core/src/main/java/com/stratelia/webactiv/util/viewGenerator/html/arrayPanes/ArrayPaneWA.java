...
public class ArrayPaneWA implements ArrayPane
{
    ...
    /**
     * In some cases, it may be preferable to specify the routing address (via {@link #setRoutingAddress(String address)})
	...
	/**
     * Default constructor
     * as this class may be instanciated by method newInstance(),
     * constructor contains no parameter.
     * init methods must be used to initialize properly the instance.
     *
     * @see init
     */
    public ArrayPaneWA()
    ...
    /**
     * Generic class to display a typical WA array table pane.
     * A unique name identifier is to be used in html pages for this array
     * specific actions (exemple : sort on a specific column)
     ...
     * @see
     */
    public void init(String name, javax.servlet.ServletRequest request, HttpSession session)
    {
     ...
     * @see
     */
    public void init(String name, String url, javax.servlet.ServletRequest request, HttpSession session)
    {
        columns		= new Vector();
        lines		= new Vector();
        this.name	= name;
        ...
        if (target != null)
        {
            if (target.equals(name))
            {
                ...
                SilverTrace.info("viewgenerator", "ArrayPaneWA.ArrayPaneWA()", "root.MSG_GEN_PARAM_VALUE", " ACTION_PARAMETER_NAME = '" + action + "'");
                if (action != null)
                {
                    if (action.equals("Sort"))
                    {
                        String newState = (String) request.getParameter(COLUMN_PARAMETER_NAME);
    ...
    /**
     * This method sets the routing address. This is actually the URL of the page to which requests will be routed
     * when the user clicks on a column header link.
     */
     ...
     * @return The new column header.
     * You can use this object to modify the
    ...
    {
        SilverTrace.info("viewgenerator", "ArrayPaneWA.setColumnToSort()", "root.MSG_GEN_PARAM_VALUE", " columNumber = '" + columnNumber + "'");
    ...
    /**
     * This methods sets the sort mode for all columns. The columns cells may or may not take this value into
     * account.
     */
    ...
    public String print()
    {
        ...
        PaginationSP pagination = new PaginationSP();
   		pagination.init(lines.size(), state.getMaximumVisibleLine(), state.getFirstVisibleLine());
        ...
        if (baseUrl.indexOf("?") < 0)
        	...
        	baseUrl += "&";
        baseUrl += ACTION_PARAMETER_NAME+"=ChangePage&"+TARGET_PARAMETER_NAME+"="+getName()+"&"+INDEX_PARAMETER_NAME+"=";
        ...
        {
        	SilverTrace.info("viewgenerator", "ArrayPaneWA.print()", "root.MSG_GEN_PARAM_VALUE", "Tri des lignes");
        ...
        result.append("<table bgcolor=\"ffffff\" width=\"100%\" cellspacing=\"").append(m_CellsSpacing).append("\" cellpadding=\"").append(m_CellsPadding).append("\" border=\"").append(m_CellsBorderWidth).append("\">");
		if (m_CellsSpacing == 0)
		{
			result.append("<tr>");
			result.append("<td colspan=\"").append(columnsCount).append("\">");
			result.append("<img src=\"").append(getIconsPath()).append("/1px.gif\" width=\"1\" height=\"1\">");
			result.append("</td>");
			result.append("</tr>\n");
		}
        if (getTitle() != null)
        ...
        else
        {
            ...
            for (int i = first; (i < lines.size()) && (i < first + max); i++)
            {
                if (m_CellsSpacing == 0)
                {
                    result.append(((ArrayLine) lines.elementAt(i)).printWithPseudoColumns());
                }
                else
                {
    ...
    public String getUrl()
    {
        ...
        // routing address computation. By default, route to the short name for the calling page
        if (m_RoutingAddress == null)
        {
            String address = ((javax.servlet.http.HttpServletRequest) getRequest()).getRequestURI();
    ...
    public String getIconsPath()
    {
        ...
        return generalSettings.getString("ApplicationURL") + GraphicElementFactory.getSettings().getString("IconsPath");
    }
