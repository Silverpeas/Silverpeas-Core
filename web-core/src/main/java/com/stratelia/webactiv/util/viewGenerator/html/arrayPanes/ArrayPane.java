...
/**
 * The ArrayPane interface gives us the skeleton for all
 * funtionnalities we need to display typical WA array
 * table pane. Exemple : <BR>
 * <CODE>
...
public interface ArrayPane extends SimpleGraphicElement
{
    ...
    /**
     * Add a line to the table. Be carefull : each line form the array has
     * to contain the same cell number. If not, the array will contain
     * some empty cells, and won't be sortable.
     * @return an ArrayLine, to be used to add cells and
     * to modify default display options.
     */
     ...
     * If the number of line is greater than this maximum, only the first lines will be visible, and
     * some buttons to view next and previous lines will be added.
    ...
    /**
     * Modify the column behaviour. Useful if you have a passive column which does not need
     * to trigger an hyperlink when its header is cliked
     * @deprecated
     ...
     * cases when you may not want it to be derived from the calling page.
     * This method is called by the constructor if you precise an url to the GraphicElementFactory.
     */
