...
public class Event extends Object
{
    ...
    {
    	init(event.getId(), event.getName(), event.getStartDate(), event.getEndDate(), event.getUrl(), event.getPriority());
    ...
    public boolean isInDay(Day day)
    {
        ...
        SilverTrace.info("viewgenerator", "Event.isInDay()", "root.MSG_GEN_PARAM_VALUE", "dateDay = "+dateDay.toString()+", startDate = "+startDate.toString());
    ...
    /*
     * to control if the date of event is in the week
     * if the start date of event is before the satrat day of week, the start day of event equals the start day of week
     * if the end date of event is over the end day of week, the end day of event equals end day of week
     * @param Date: start date and end date of week
     * @return boolean
     * catch java.text.ParseException, write this exception int the log file
    ...
    /*
     * to initialize the parametar spanDay. This parameter represent le nombre de jour couvert par l'événement
     * it's use by the print method of the class of extend  AbstractMonthCalendar
     * default value for spanDay is 0
     * @return int
    ...
    private int initSpanDay(Date date)
    {
        ...
        if (span == 1)
        {
            SilverTrace.info("viewgenerator", "Event.initSpanDay()", "root.MSG_GEN_PARAM_VALUE", "return 2");
            return 2;
        }
        else
        {
        	SilverTrace.info("viewgenerator", "Event.initSpanDay()", "root.MSG_GEN_PARAM_VALUE", "return "+span+1);
            return span + 1;
        }
    }
    ...
    /**
     * to compare the event with an other event
     * this method return true if the event parameter span the same day, false else if
     * @return boolean
     */
