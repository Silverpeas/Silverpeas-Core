...
public class AgendaHtmlView
{
    ...
    private static final java.text.SimpleDateFormat completeFormat = new java.text.SimpleDateFormat("yyyy/MM/dd HH:mm");
    private static final java.text.SimpleDateFormat hourFormat = new java.text.SimpleDateFormat("HH:mm");
    ...
    public void add(Schedulable schedule)
    {
        ...
        try
        {
            ...
            {
                if (schedule.getStartHour().compareTo(Schedulable.hourMinuteToString(BEGINHOUR, 0)) < 0)
                {
            ...
            {
                if (schedule.getEndHour().compareTo(Schedulable.hourMinuteToString(ENDHOUR, 0)) > 0)
                {
            ...
            {
                // if the schedule start and end day are different, let's cut in smaller schedules
                Calendar start = Calendar.getInstance();

                start.setTime(schedule.getStartDate());
                while (DateUtil.date2SQLDate(start.getTime()).compareTo(schedule.getEndDay()) <= 0)
                {
                    Schedulable copy = schedule.getCopy();

                    copy.setStartDay(DateUtil.date2SQLDate(start.getTime()));
                    if (DateUtil.date2SQLDate(start.getTime()).compareTo(schedule.getStartDay()) != 0)
                    {
                        if (schedule.getStartHour() != null)
                        {
                    ...
                    copy.setEndDay(null);
                    if (DateUtil.date2SQLDate(start.getTime()).compareTo(schedule.getEndDay()) != 0)
                    {
                        if (schedule.getEndHour() != null)
                        {
        ...
        {
            SilverTrace.warn("agenda", "AgendaHtmView.add(Schedulable schedule)", "agenda.MSG_ADD_SCHEDULE_FAILED", "id=" + schedule.getId() + ", name=" + schedule.getName(), e);
        }
    }
    ...
    {
        SilverTrace.debug("agenda", "AgendaHtmView.getHtmlViewByMonth(String startDate)", "schedules"+schedules.size());
        ...
        catch (java.text.ParseException e)
        {
            ...
            SilverTrace.warn("agenda", "AgendaHtmView.getHtmlViewByMonth(String startDate)", "agenda.MSG_CANT_GET_VIEW_MONTH", "return= null", e);
            return "";
        }
    }
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            for (int j = 0; j < 4; j++)
            {
                ...
                try
                {
                    ...
                    result += calendarHtmlView.getHtmlView(DateUtil.parse(year + "/" + m + "/01"), agendaSessionController);
                    month++;
                }
                catch (java.text.ParseException e)
                {
                    SilverTrace.warn("agenda", "AgendaHtmView.getHtmlViewByMonth(String startDate)", "agenda.MSG_CANT_GET_VIEW_YEAR", "return= null", e);
                    return "";
                }
                result += "</TD>\n";
            }
    ...
    public String getHtmlViewByDay(String today) throws AgendaException
    {
        ...
        {
            SilverTrace.warn("agenda", "AgendaHtmView.getHtmlViewByDay(String today)", "agenda.MSG_CANT_GET_VIEW_DAY", "return= null", e);
            return "";
        }
        ...
        if (all.size() > 0)
        {
            ...
            for (int i = 0; i < all.size(); i++)
            {
                ...
                if (i == 0)
                {
                    result += "    <TD width=\"50\" rowspan=\"" + all.size() + "\" align=\"right\" bgcolor=\"#FFFFFF\" nowrap valign=\"top\">";
                    result += "&nbsp;</TD>";
                }
                ...
                {
	                result += "      <A HREF=\"" + "javascript:onClick=viewJournal('" + schedule.getId() + "')\"";
	                result += getInfoBulle(schedule);
                    result += Encode.javaStringToHtmlString(schedule.getName()) + "</A>";
                }

                result += " &nbsp;(" + agendaSessionController.getString("allDay") + ")    </TD>";
                result += "  </span></TR>\n";
            }
        }
        else
        ...
        while (i < ENDHOUR)
        {
            ...
            {
                throw new AgendaRuntimeException("AgendaHtmView.getHtmlViewByDay(String today)", SilverpeasException.ERROR, "agenda.EX_CANT_GET_VIEW_DAY", e);
            }
            ...
            {
                if (((goOn.size() != 0) && (lastGoOn.size() == 0)) || ((goOn.size() == 0) && (lastGoOn.size() != 0)))
                {
                    result += "      </TABLE>\n";
                    result += "    </TD>";
                    result += "  </TR>\n";
            ...
            else
            {
                for (int goOnIterator = 0; goOnIterator < goOn.size(); goOnIterator++)
                {
                    ...
                    if (start)
                    {
                        ...
                        do
                        {
                            ...
                            String tmpNextHour = Schedulable.quaterCountToHourString((i + length + 1) * 4);
                        ...
                        result += "<TD width=\"" + ((int) (600 / maxColumns)) + "\" class=\"" + color + "\" rowspan=\"" + length + "\">";
                        if (isOtherAgenda)
                        {
                        	if (schedule.getClassification().isPrivate())
                        	{
		                        ...
		                        {
		                            result += schedule.getStartHour() + " - " + schedule.getEndHour() + "<BR>";
		                        }
		                        else
                ...
                {
                    result += "        <TD class=\"intfdcolor4\" width=\"" + ((int) (600 / maxColumns)) + "\">&nbsp;</TD>";
                }
            }
        ...
        if (calendarVisible)
        {
            ...
            result += "         <tr>\n";
            result += "            <td align=center class=\"txtbigdate\">" + day.get(Calendar.DAY_OF_MONTH);
            result += "            </td></tr>";
            result += "            <tr><td align=center class=\"txtnav3\">" + agendaSessionController.getString("mois" + day.get(Calendar.MONTH));
            result += "            </td></tr>";
            result += "            <tr><td align=center class=\"txtnav3\">" + DateUtil.getInputDate(day.getTime(), agendaSessionController.getLanguage());
            result += "            </TD></TR>\n";
            result += "            <TR><TD>\n";

            try
            {
                result += (new CalendarHtmlView()).getHtmlView(DateUtil.parse(today), agendaSessionController);
            }
            catch (Exception e) {}
    ...
    public String getHtmlViewByWeek(String firstDay) throws AgendaException
    {
        ...
        result.append("<TABLE border=\"0\" align=\"center\" width=\"98%\" cellspacing=\"0\" cellpadding=\"2\" class=\"grille\">\n");
        result.append("<TR>");
        result.append("<TD>");
        result.append("<TABLE border=\"0\" align=\"center\" width=\"100%\" cellspacing=\"1\" cellpadding=\"1\">\n");
        result.append("  <TR bgcolor=\"#ffffff\" nowrap>\n");
        result.append("    <td rowspan=\"2\" align=\"right\" valign=\"bottom\"> <img src=\"icons/1px.gif\"><br>");
        result.append("</TD>\n");
        Calendar day = Calendar.getInstance();
        ...
        {
            SilverTrace.warn("agenda", "AgendaHtmView.getHtmlViewByWeek(String firstDay)", "agenda.MSG_CANT_GET_VIEW_WEKK", "return= null", e);
            return "";
        }
        ...
        for (int i = 0; i < WEEKDAYNUMBER; i++)
        {
        	if (agendaSessionController.isHolidayDate(day.getTime()))
        	{
			    ...
			    result.append("<span ");
			    result.append(weekDayOffStyle+ ">" + agendaSessionController.getString("jour" + day.get(Calendar.DAY_OF_WEEK)).substring(0, 3));
			    result.append(" " + day.get(Calendar.DAY_OF_MONTH) + "</span>");
        	}
        	else
        	{
    		    result.append("    <TD class=\"intfdcolor2\" valign=\"bottom\" width=\"14%\" align=\"center\">");
			    result.append("<A HREF=\"javascript:onClick=selectDay('" + DateUtil.getInputDate(day.getTime(), agendaSessionController.getLanguage()) + "')\" class=\"txtnav\">");
			    result.append(agendaSessionController.getString("jour" + day.get(Calendar.DAY_OF_WEEK)).substring(0, 3));
			    result.append(" " + day.get(Calendar.DAY_OF_MONTH));
			    result.append("</A>");
			    dayList[i] = new SchedulableList(DateUtil.date2SQLDate(day.getTime()), schedules);
        	}
		    result.append("</TD>\n");
        ...
        for (int j = 0; j < WEEKDAYNUMBER; j++)
        {
            ...
            if (all.size() > 0)
            {
                ...
                for (int i = 0; i < all.size(); i++)
                {
                    ...
                    {
                    	result.append("    <TD class=privateEvent>" + agendaSessionController.getString("privateEvent"));
                    }
                    else
                    {
                    	if (isOtherAgenda)
                        ...
                        result.append("      <A HREF=\"javascript:onClick=viewJournal('" + schedule.getId() + "')\"");
        ...
        result.append("<tr> ");
        result.append("<td colspan=\"8\" class=\"intfdcolor3\"><img src=\"icons/1px.gif\" height=\"2\" width=\"1\"></td>");
        result.append("</tr>\n");
	                ...
	                for (int m = 0; m < starting.size(); m++)
	                {
	                    ...
	                    if (startObj instanceof Schedulable)
	                    {
	                        ...
	                        else
	                        {
	                        	if (isOtherAgenda)
	                        		color = "publicEvent";
	                        	tmpResult.append( "<A HREF=\"javascript:onClick=viewJournal('" + schedule.getId() + "')\"");
	                            tmpResult.append( getInfoBulle(schedule));
	                            if (starting.size() == 1)
	                                tmpResult.append( schedule.getStartHour() + "<BR>");
	                            tmpResult.append( Encode.javaStringToHtmlString(schedule.getName()) + "</A>");
	                        }
        ...
        {
            SilverTrace.warn("agenda", "AgendaHtmView.getHtmlViewByWeek(String firstDay)", "agenda.MSG_CANT_GET_VIEW_WEEK", "return= null", e);
        ...
        for (int i = 0; i < WEEKDAYNUMBER; i++)
        {
        	...
        	{
			    result.append("<span ");
			    result.append(dayOffStyle +  ">" + agendaSessionController.getString("jour" + day.get(Calendar.DAY_OF_WEEK)).substring(0, 3));
			    result.append(" " + day.get(Calendar.DAY_OF_MONTH));
			    result.append("</span>");
        	}
        	else
        	{
	            result.append("<A HREF=\"javascript:onClick=selectDay('" + DateUtil.getInputDate(day.getTime(), agendaSessionController.getLanguage()) + "')\">");
	            result.append(agendaSessionController.getString("jour" + day.get(Calendar.DAY_OF_WEEK)).substring(0, 3));
	            result.append(" " + day.get(Calendar.DAY_OF_MONTH));
	            result.append("</A>");
	            dayList[i] = new SchedulableList(DateUtil.date2SQLDate(day.getTime()), schedules);
    ...
    private int getDuration(Schedulable schedule)
    {
        try
        {
            ...
            java.util.Date startDate = completeFormat.parse(schedule.getStartDay() + " " + sHour);
            ...
            java.util.Date endDate = completeFormat.parse(schedule.getEndDay() + " " + eHour);
        ...
        {
            SilverTrace.warn("agenda", "AgendaHtmView.getDuration(Schedulable schedule)", "agenda.MSG_CANT_DURATION", "id=" + schedule.getId() + " return=0", e);
            return 0;
        }
    }
    ...
    private int getDuration(SchedulableGroup group)
    {
        ...
        {
            SilverTrace.warn("agenda", "AgendaHtmView.getDuration(SchedulableGroup group)", "agenda.MSG_CANT_DURATION", "return=0", e);
            return 0;
        }
    }
    ...
    private String getInfoBulle(Schedulable schedule) throws AgendaException
    {
    	...
    	if (!StringUtil.isDefined(schedule.getDescription()) && categories.isEmpty())
    		return ">";
		...
		result.append("',CAPTION,'" + Encode.javaStringToJsString(categs) + "');\" onmouseout=\"return nd();\">");
        return result.toString();
    }
}