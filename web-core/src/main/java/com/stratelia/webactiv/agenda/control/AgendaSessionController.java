...
public class AgendaSessionController extends AbstractComponentSessionController
{
	...
	{
		super(mainSessionCtrl, context, "com.stratelia.webactiv.agenda.multilang.agenda");
	...
	{
		if (isUseRss())
			return "/rssAgenda/"+getAgendaUserId()+"?userId="+getUserId()+"&login="+URLEncoder.encode(getUserDetail().getLogin())+"&password="+URLEncoder.encode(getOrganizationController().getUserFull(getUserId()).getPassword());
		return null;
	}
	...
	protected void notifyAttendees(String id, String title, String text, String url)
	{
		try
		{
		...
		{
			SilverTrace.warn("agenda", "AgendaSessionController.notifyAttendees()", "agenda.MSG_CANT_SEND_MAILS", "for title=" + title, e);
		}
	}
		...
		throws RemoteException, CalendarException
	{
		...
		if (changed)
		{
			...
			String url = URLManager.getURL(URLManager.CMP_AGENDA)+ "journal.jsp?JournalId="	+ id + "&Action=Update";
			notifyAttendees(
				id,
				getString("titleUpdate"),
				getString("subject")
					+ "\n '" + journal.getName()
					+ "' " + getString("with") + " "
	...
	private boolean eventHasChanged(JournalHeader journal, Date startDay, String startHour, Date endDay, String endHour)
	{
		...
		if (DateUtil.datesAreEqual(journal.getStartDate(), startDay) && DateUtil.datesAreEqual(journal.getEndDate(), endDay))
		{
		...
		SilverTrace.debug(
			"agenda",
			"AgendaSessionController.eventHasChanged",
	...
	public Collection getJournalAttendees(String journalId) throws AgendaException
	{
		try
		{
	...
	public void setJournalAttendees(String journalId, String[] userIds) throws AgendaException
	{
		try
		{
			...
			UserDetail you = getUserDetail();
			String url = URLManager.getURL(URLManager.CMP_AGENDA)+ "journal.jsp?JournalId="	+ journalId + "&Action=Update";
			notifyAttendees(
				journalId,
				getString("NewRDV"),
				you.getDisplayedName()
					+ " "
					+ getString("NewRDVcalled")
					+ " '"
					+ journal.getName()
					+ "' \n"
					+ " "
	...
	public void setJournalParticipationStatus(String journalId, String userId, String status) throws AgendaException
	{
		...
		catch (Exception e)
		{
			throw new AgendaException(
				...
				SilverpeasException.ERROR,
				"agenda.EX_CANT_SET_STATUS",
				"journalId=" + journalId + " ,userId=" + getUserId() + ",status=" + status,
				e);
		}
	}
	 ...
	 * @see
	 */
	public Collection getJournalCategories(String journalId) throws AgendaException
	{
		try
		{
		...
		catch (Exception e)
		{
			throw new AgendaException(
				...
				SilverpeasException.ERROR,
				"agenda.EX_CANT_GET_JOURNAL_CATEGORY",
				"journalId=" + journalId,
				e);
		}
	}
	...
	public void setJournalCategories(String journalId, String[] categoryIds) throws AgendaException
	{
		...
		catch (Exception e)
		{
			throw new AgendaException(
				...
				SilverpeasException.ERROR,
				"agenda.EX_CANT_SET_JOURNAL_CATEGORY",
				"journalId=" + journalId,
				e);
		}
	...
	public AgendaHtmlView getCurrentHtmlView() throws RemoteException
	{
		...
		if (currentDisplayType == AgendaHtmlView.BYDAY)
		{
			...
			{
				agendaView = new AgendaHtmlView(currentDisplayType, getWeekFirstDay(getCurrentDay()), this, getSettings());
				schedules = getWeekSchedulables();
			}
			else
				if (currentDisplayType == AgendaHtmlView.BYMONTH)
				{
					agendaView = new AgendaHtmlView(currentDisplayType, getMonthFirstDay(getCurrentDay()), this, getSettings());
					schedules = countMonthSchedulables();
				}
				else
					if (currentDisplayType == AgendaHtmlView.BYYEAR)
					{
						agendaView = new AgendaHtmlView(currentDisplayType, getYearFirstDay(getCurrentDay()), this, getSettings());
					}
					else
					{
						return null;
	...
	public Collection getDaySchedulables() throws RemoteException
	{
		...
		return calendarBm.getDaySchedulablesForUser(
			DateUtil.date2SQLDate(getCurrentDay()),
			agendaUserId,
			categoryId,
	...
	public Collection countMonthSchedulables() throws RemoteException
	{
		...
		return calendarBm.countMonthSchedulablesForUser(
			month,
			agendaUserId,
			categoryId,
			getParticipationStatus().getString());
	 ...
	 * @see
	 */
	public Collection getBusyTime(String userId, java.util.Date day) throws RemoteException
	{
		Collection result = calendarBm.getDaySchedulablesForUser(DateUtil.date2SQLDate(day), userId, null, ParticipationStatus.ACCEPTED);
		...
		if (!userId.equals(getUserId()))
		{
			...
			while (i.hasNext())
			{
				...
				if (!schedule.getClassification().isConfidential())
				{
					...
					if (schedule.getDelegatorId().equals(getUserId()))
					{
						toView = true;
					}
					else
					{
						Collection attendees = calendarBm.getJournalAttendees(schedule.getId());
	...
	public Collection getTentativeSchedulables() throws AgendaException
	{
		...
		catch (Exception e)
		{
			throw new AgendaException(
				...
				SilverpeasException.ERROR,
				"agenda.EX_CANT_GET_TENTATIVE_SCHEDULABLES",
				"userId=" + getUserId(),
				e);
		}
	}
	...
	public void selectDay(String day)
	{
		...
		{
			SilverTrace.warn("agenda", "AgendaSessionController.selectDay(String day)", "agenda.MSG_CANT_CHANGE_DAY", "", e);
		}
	}
	...
	*/
	public String initSelectionPeas()
	{
		...
		PairObject hostComponentName = new PairObject(getString("agenda"), m_context + "/Ragenda/jsp/Main");
		PairObject[] hostPath = new PairObject[1];
		hostPath[0] = new PairObject(getString("editionListeDiffusion"), m_context + "/Ragenda/jsp/Main");
	...
	*/
	...
	public void close()
	{
		try
		{
			if (calendarBm != null) calendarBm.remove();
		}
	...
	*/
	public String initUserPanelOtherAgenda()
	{
		...
		PairObject hostComponentName = new PairObject(getString("agenda"), m_context + "/Ragenda/jsp/Main");
		PairObject[] hostPath = new PairObject[1];
		hostPath[0] = new PairObject(getString("viewOtherAgenda"), m_context + URLManager.getURL(URLManager.CMP_AGENDA) + "Main");
		String hostUrl = m_context + URLManager.getURL(URLManager.CMP_AGENDA) + "ViewOtherAgenda";
		String cancelUrl = m_context + URLManager.getURL(URLManager.CMP_AGENDA) + "Main";
	...
	*/
	...
	{
		SilverTrace.debug("agenda","AgendaSessionController.exportIcalAgenda()","root.MSG_GEN_ENTER_METHOD");
		String returnCode = new ExportIcalManager(this).exportIcalAgenda(startDate, endDate); 
		SilverTrace.debug("agenda","AgendaSessionController.exportIcalAgenda()","root.MSG_GEN_EXIT_METHOD");
		return returnCode;
	}
	...
	{
		SilverTrace.debug("agenda","AgendaSessionController.importIcalAgenda()","root.MSG_GEN_ENTER_METHOD");
		String returnCode = new ImportIcalManager(this).importIcalAgenda(fileCalendar); 
		SilverTrace.debug("agenda","AgendaSessionController.importIcalAgenda()","root.MSG_GEN_EXIT_METHOD");
		return returnCode;
	}
	...
	{
		SilverTrace.debug("agenda","AgendaSessionController.synchroIcalAgenda()","root.MSG_GEN_ENTER_METHOD");
		URL iCalendarServerUrl 	= new URL(urlICalendar);
		String returnCodeSynchro = new SynchroIcalManager(this).synchroIcalAgenda(iCalendarServerUrl, getIcalendarFile(), loginIcalendar, pwdIcalendar);
		SilverTrace.debug("agenda","AgendaSessionController.synchroIcalAgenda()","root.MSG_GEN_EXIT_METHOD");
	...
	{
		return new File(FileRepositoryManager.getTemporaryPath() + AgendaSessionController.AGENDA_FILENAME_PREFIX + getUserId() + ".ics");
	}
	 ...
	 */
	public void changeDayOfWeekStatus(String year, String month, String day) throws RemoteException, ParseException
	{
		SilverTrace.info("agenda", "AgendaSessionController.changeDayOfWeekStatus()", "root.MSG_GEN_ENTER_METHOD", "year="+year+", month="+month+", day="+day);
		...
		while (currentCalendar.get(Calendar.DAY_OF_WEEK)!=new Integer(day).intValue())
		{
			currentCalendar.add(Calendar.DATE, 1);
		}
		...
		currentCalendar.set(Calendar.MONTH, iMonth);
		currentCalendar.set(Calendar.DATE, 1);
		SilverTrace.info("agenda", "AgendaSessionController.changeDayOfWeekStatus()", "root.MSG_GEN_EXIT_METHOD", "year="+year+", month="+month+", day="+day);
	}
	...
	public boolean isHolidayDate(Date date)
	{
		...
		{
            throw new AgendaRuntimeException("AgendaSessionController.isHolidayDate()", SilverpeasException.ERROR, "agenda.MSG_IS_HOLIDAY_DATE_FAILED",e);
		}
	...
	public boolean isSameDaysAreHolidays(Calendar cal, int currentMonth)
	{
		...
		{
			if (!isHolidayDate(localCalendar.getTime()) && localCalendar.get(Calendar.MONTH) == currentMonth)
	...
	public String getMyAgendaUrl()
	{
		...
		String url = "/SubscribeAgenda/"+AGENDA_FILENAME_PREFIX+"?userId="+getUserId()+"&login="+getUserDetail().getLogin()+"&password="+oc.getUserFull(getUserId()).getPassword(); 
		return url;
	}
