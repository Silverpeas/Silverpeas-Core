...
public class TestAdmin extends Object
{
    ...
    static public CompoSpace addComponent(String sComponentName) throws Exception
    {
        ...
        if (m_Admin == null)
        {
			 ...
			 String sKey = lpAuth.authenticate(m_sAdministratorLogin, m_sAdministratorPassword, "0", null);
    ...
    static public MainSessionController getMainSessionController(CompoSpace cs) throws Exception
    {
		 LoginPasswordAuthentication lpAuth = new LoginPasswordAuthentication();
		 lpAuth.authenticate(m_sAdministratorLogin, m_sAdministratorPassword, "0", null);
         MainSessionController msc = new com.stratelia.silverpeas.peasCore.MainSessionController("User1 for a test", "");
    ...
    static public MainSessionController getMainSessionController() throws Exception
    {
		 LoginPasswordAuthentication lpAuth = new LoginPasswordAuthentication();
		 lpAuth.authenticate(m_sAdministratorLogin, m_sAdministratorPassword, "0", null);
         MainSessionController msc = new com.stratelia.silverpeas.peasCore.MainSessionController("User1 for a test", "");
    ...
    static public Connection openProdConnection(boolean bAutoCommit) throws AdminException
    {
        try
        {
            Connection connection = openConnection(m_sWaProductionDb, m_sWaProductionUser, m_sWaProductionPswd);
        ...
        catch(Exception e)
        {
            if(!(e instanceof AdminException && ((AdminException) e).isAlreadyPrinted()))
                SilverTrace.error("peasCore", "TestAdmin.openProdConnection", "peasCore.EX_CANT_CREATE_PROD_CONNECTION", e);
            throw new AdminException(true);
        }
    }

    // Open an Admin connection
    static public Connection openAdminConnection(boolean bAutoCommit) throws AdminException  //!!! private
    {
        try
        {
            Connection connection = openConnection(m_sWaAdminDb, m_sWaAdminUser, m_sWaAdminPswd);
            connection.setAutoCommit(bAutoCommit);
            return connection;
        }
        catch(Exception e)
        {
            SilverTrace.error("peasCore", "TestAdmin.openAdminConnection", "peasCore.peasCore.EX_CANT_CREATE_ADMIN_CONNECTION", e);
            throw new AdminException(true);
        }
    }

    // Open a connection
    static private Connection openConnection(String sDbUrl, String sUser, String sPswd) throws AdminException
    {
        try
        {
    ...
    static public void closeConnection(Connection connection)
    {
        ...
        {
            //Debug.error(200,"TestAdmin closeConnection", "Cannot close connection: " + connection + "   with code: " + e.getErrorCode(), e, null);
            SilverTrace.error("peasCore", "TestAdmin.closeConnection", "root.EX_CONNECTION_CLOSE_FAILED", connection.toString() ,e);
