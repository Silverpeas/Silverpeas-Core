 ...
 * Class declaration
 *
 * This object is a singleton used by
 * LoginServlet : when the user log in,
...
{
	// Global constants
	public static final SimpleDateFormat NOTIFY_DATE_FORMAT = new SimpleDateFormat(" HH:mm (dd/MM/yyyy) ");
	...
	private void initSessionManager()
	{
		...
		try
		{
			...
			resources = java.util.ResourceBundle.getBundle("com.stratelia.silverpeas.peasCore.SessionManager", new Locale("", ""));
    		...
    		scheduledSessionManagementTimeStamp = Long.parseLong(resources.getString("scheduledSessionManagementTimeStamp")) * (long)60 * (long)1000; // Translate from mn tu ms
            userSessionTimeout = Long.parseLong(resources.getString("userSessionTimeout")) * (long)60 * (long)1000; // Translate from mn tu ms
            if (scheduledSessionManagementTimeStamp > userSessionTimeout)
            {
                scheduledSessionManagementTimeStamp = userSessionTimeout;
            }
            adminSessionTimeout = Long.parseLong(resources.getString("adminSessionTimeout")) * (long)60 * (long)1000; // Translate from mn tu ms
			...
			// Writing journal
			SilverLog.logConnexion("SessionManager starting","TimeStamp=" + Long.toString(scheduledSessionManagementTimeStamp / 60000),"UserSessionTimeout=" + Long.toString(userSessionTimeout / 60000) + " adminSessionTimeout=" + Long.toString(adminSessionTimeout / 60000));
	...
	/**
	 * Set the server last acessed time by the user.
	 * Used to verify if the session duration has expired (because of timeout).
	 *
	 *
	 * @param session
	 *
	...
	public void setLastAccess(HttpSession session)
	{
	    ...
	    {
	        SilverTrace.debug("peasCore","SessionManager.setLastAccess","L'objet de session n'a pas ete retrouve dans la variable userDataSessions !!! - sessionId = " + session.getId());
	    }
	 ...
	 * That job fires a SchedulerEvent of the type 'EXECUTION_NOT_SUCCESSFULL',
	 * or 'EXECUTION_SUCCESSFULL'.
	 *
	 * The timestamp (every time the job will execute) settings is given by minutes
	 * and logically must be less than the session timeout.
	 *
	...
	public void initSchedulerTimeStamp() throws SchedulerException
	{
		...
		SilverTrace.debug("peasCore","SessionManager.initSchedulerTimeStamp","scheduledSessionManagementTimeStamp in minutes=" + minute);
		...
		SimpleScheduler.getJob(myInstance,SESSION_MANAGER_JOB_NAME,startMinutes,null,null,null,null,myInstance,"doSessionManagement");
	}
	...
	public void handleSchedulerEvent(SchedulerEvent aEvent)
	{
		...
		switch (aEvent.getType())
		{
			case SchedulerEvent.EXECUTION_NOT_SUCCESSFULL :
				SilverTrace.error("peasCore","SessionManager.handleSchedulerEvent","The job '" + aEvent.getJob().getJobName() + "' was not successfull");
                break;
			case SchedulerEvent.EXECUTION_SUCCESSFULL :
				SilverTrace.debug("peasCore","SessionManager.handleSchedulerEvent","The job '" + aEvent.getJob().getJobName() + "' was successfull");
				break;
			default :
				SilverTrace.error("peasCore", "SessionManager.handleSchedulerEvent", "Illegal event type");
				break;
		}
	}

	/**
	 * This method stores the users's sessions, initialises time counters and
	 * log session's data.
	 * The stored session may become invalid (if the user close the browser,
	 * this class is not notified).
	 *
	 ...
	 * @see removeSession
	 */
	public synchronized void addSession(HttpSession session, HttpServletRequest request, MainSessionController controller)
	{   
        String      anIP = request.getRemoteHost();
		try
		{
			SilverTrace.debug("peasCore", "SessionManager.addSession", "sessionId=" + session.getId() + " - userId : " + controller.getUserId());
    ...
    public void removeSession(String sessionId)
	{
        ...
        {
            SilverTrace.debug("peasCore","SessionManager.removeSession","L'objet de session n'a pas ete retrouve dans la variable userDataSessions !!! (sessionId = " + sessionId + ")");
        }
	}
	...
	protected void removeSession(SessionInfo si)
	{
		try
		{
            SilverTrace.debug("peasCore", "SessionManager.removeSession", "START on session=" + si.m_SessionId + " - " + si.getLog());
            // Writing journal
            ...
            SilverTrace.debug("peasCore", "SessionManager.removeSession", "DONE on session=" + si.m_SessionId + " - " + si.getLog());
	...
	private void removeInQueueMessages(String userId, String sessionId)
	{
		try
		{
			...
			{
				dao = SilverpeasBeanDAOFactory.getDAO("com.stratelia.silverpeas.notificationserver.channel.server.ServerMessageBean");
				whereClause = " USERID=" + userId + " AND SESSIONID='" + sessionId + "'";
				dao.removeWhere(pk, whereClause);
			}
			
			//Remove "end of session" messages
			dao = SilverpeasBeanDAOFactory.getDAO("com.stratelia.silverpeas.notificationserver.channel.popup.POPUPMessageBean");
		...
		{
			SilverTrace.error("peasCore","SessionManager.removeInQueueMessages()","root.EX_NO_MESSAGE","USERID=" + userId + " AND SESSIONID = " + sessionId,e);
		}
	}
	...
	public void setIsAlived(HttpSession session)
	{
        ...
        {
            SilverTrace.debug("peasCore","SessionManager.setIsAlived","L'objet de session n'a pas ete retrouve dans la variable userDataSessions !!! - sessionId = " + session.getId());
        }
	}
	...
	public Collection getDistinctConnectedUsersList()
	{
		...
		while (sessionsInfos.hasNext())
		{
			...
			if (!distinctConnectedUsersList.containsKey(userLogin) && !si.m_User.isAccessGuest())
	...
	/**
	 * This method is called every scheduledSessionManagementTimeStamp minute by the scheduler,
	 * it notify the user when timeout has expired and then invalidates the
	 * session if the user has not accessed the server.
	 * The maximum minutes duration of session before invalidation is
	...
	public void doSessionManagement(Date currentDate)
	{
		try
		{
			...
			SessionInfo[] allSI = (SessionInfo[])userDataSessions.values().toArray(new SessionInfo[0]);
			for (int i = 0; i < allSI.length; i++)
			{
                SessionInfo si = allSI[i];
                long userSessionTimeoutMillis = (si.m_User.isAccessAdmin()) ? adminSessionTimeout : userSessionTimeout;
                ...
                if (currentTime - si.m_DateLastAccess >= userSessionTimeoutMillis)
                {
                    ...
                    if ((duration < maxRefreshInterval) && !userNotificationSessions.contains(si.m_SessionId))
                    {
                        try
                        {
                            notifyEndOfSession(si.m_User.getId(),currentTime + scheduledSessionManagementTimeStamp,si.m_SessionId);
                        }
                        catch (NotificationManagerException ex)
                        {
                            SilverTrace.error("peasCore","SessionManager.doSessionManagement","notificationManager.EX_CANT_SEND_USER_NOTIFICATION", "sessionId="+si.m_SessionId+" - user="+si.getLog(), ex);
                        }
                        finally
                        {
	...
	{
		SilverTrace.debug("peasCore", "SessionManager.notifyEndOfSession", "userId=" + userId + " sessionId=" + sessionId);
		...
		NotificationMetaData notifMetaData =
			new NotificationMetaData(NotificationParameters.NORMAL, msgTitle, m_Multilang.getString("EndOfSessionNotificationMsgText"));
	...
	* The unique instance of the SessionManager will be destroyed.
	*/
	public void shutdown()
	{
		...
		{
			SilverTrace.error("peasCore", "SessionManager.shutdown", "root.EX_NO_MESSAGE", ex);
		}
	...
	public void closeHttpSession(HttpSession session)
	{
        ...
        {
            SilverTrace.debug("peasCore","SessionManager.closeHttpSession","L'objet de session n'a pas ete retrouve dans la variable userDataSessions !!!");
        }
	}
}
