...
public class BrowseGroupPanel extends BrowsePanelProvider
{
    ...
    public BrowseGroupPanel(String language, ResourceLocator rs, CacheManager cm, SelectionUsersGroups sug)
    {
    ...
    public void refresh(String[] filters)
    {
        ...
        if ((((PanelMiniFilterEdit)m_MiniFilters[0]).m_Text != null) && (((PanelMiniFilterEdit)m_MiniFilters[0]).m_Text.length() > 0))
        {
        ...
        {
        	m_Ids = m_oc.searchGroupsIds(false, m_SelectionExtraParams.getComponentId(), m_SelectionExtraParams.getProfileIds(), modelGroup);
        }
