...
public class CacheManagerJdbcConnector extends CacheManager {
	...
	public CacheManagerJdbcConnector(String language, ResourceLocator local, ResourceLocator icon, Selection selection)
	{
	...
	public PanelMiniFilterToken[] getPanelMiniFilters(int what)
	{
        if (what == CM_SET)
        {
            ...
            theArray[0] = new PanelMiniFilterEdit(0, Integer.toString(what), "",
            	m_Context + m_Icon.getString("selectionPeas.filter"), m_Local.getString("selectionPeas.filter"),
            	m_Local.getString("selectionPeas.filter"));
            return theArray;
        }
	...
	public PanelMiniFilterSelect getSelectMiniFilter(int what)
	{
        if (what == CM_SET)
        {
            return new PanelMiniFilterSelect(999, Integer.toString(what), "set",
            	m_Context + m_Icon.getString("selectionPeas.selectAll"),
            	m_Context + m_Icon.getString("selectionPeas.unSelectAll"),
            	m_Local.getString("selectionPeas.selectAll"),
            	m_Local.getString("selectionPeas.unSelectAll"),
            	m_Local.getString("selectionPeas.selectAll"),
            	m_Local.getString("selectionPeas.unSelectAll"));
        }
        else if (what == CM_ELEMENT)
