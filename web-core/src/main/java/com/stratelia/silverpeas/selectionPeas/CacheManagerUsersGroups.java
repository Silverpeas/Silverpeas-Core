...
public class CacheManagerUsersGroups extends CacheManager
{
    ...
    protected PanelLine getLineFromId(int what, String id)
    {
        ...
        if (what == CM_SET)
        {
            ...
            if (SelectionPeasSettings.m_DisplayNbUsersByGroup)
            {
                if (SelectionPeasSettings.m_DisplayDomains)
                {
                    ...
                    theValues[COL_GROUP_NAME] = Encode.javaStringToHtmlString(theGroup.getName());
                    theValues[COL_GROUP_DESCRIPTION] = Encode.javaStringToHtmlString(theGroup.getDescription());
                    theValues[COL_GROUP_NBUSERS] = Integer.toString(m_oc.getAllSubUsersNumber(theGroup.getId()));
                    theValues[COL_GROUP_NBUSERS+1] = Encode.javaStringToHtmlString(m_ac.getDomain(theGroup.getDomainId()).getName());
                }
                else
                {
            ...
            else
            {
                if (SelectionPeasSettings.m_DisplayDomains)
                {
                    ...
                    theValues[COL_GROUP_NAME] = Encode.javaStringToHtmlString(theGroup.getName());
                    theValues[COL_GROUP_DESCRIPTION] = Encode.javaStringToHtmlString(theGroup.getDescription());
                    theValues[COL_GROUP_DESCRIPTION+1] = Encode.javaStringToHtmlString(m_ac.getDomain(theGroup.getDomainId()).getName());
                }
                else
                {
        ...
        else if (what == CM_ELEMENT)
        {
            ...
            {
                theValues = new String[4];
                theValues[COL_USER_LASTNAME] = Encode.javaStringToHtmlString(theUser.getLastName());
                theValues[COL_USER_FIRSTNAME] = Encode.javaStringToHtmlString(theUser.getFirstName());
                theValues[COL_USER_EMAIL] = Encode.javaStringToHtmlString(theUser.geteMail());
                theValues[COL_USER_DOMAIN] = Encode.javaStringToHtmlString(m_ac.getDomain(theUser.getDomainId()).getName());
            }
            else
            {
                theValues = new String[3];
