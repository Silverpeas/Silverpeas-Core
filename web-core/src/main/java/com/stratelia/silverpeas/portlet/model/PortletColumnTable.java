...
public class PortletColumnTable extends AbstractTable {
   ...
   */
  static final protected String PORTLETCOLUMN_COLUMNS
    = "id,spaceId,columnWidth,nbCol";
  ...
  static final private String SELECT_PORTLETCOLUMN_BY_ID
    = "Select " + PORTLETCOLUMN_COLUMNS + " from ST_PortletColumn Where id = ?";
  ...
  public PortletColumnRow[] getAllBySpaceId(int aSpaceId, String orderField)
    throws UtilException
  {
    String req = "select " + PORTLETCOLUMN_COLUMNS +
                 " from ST_PortletColumn" +
  ...
  public PortletColumnRow[] getAllBySpaceId(int spaceId)
    throws UtilException
  {
    return (PortletColumnRow[]) getRows(SELECT_ALL_PORTLETCOLUMN_WITH_GIVEN_SPACEID,spaceId)
           .toArray(new PortletColumnRow[0]);
  }

  static final private String SELECT_ALL_PORTLETCOLUMN_WITH_GIVEN_SPACEID = "select " + PORTLETCOLUMN_COLUMNS + " from ST_PortletColumn where spaceId=?";
  ...
  {
    return (PortletColumnRow[]) getRows(SELECT_ALL_PORTLETCOLUMN).toArray(new PortletColumnRow[0]);
  }

  static final private String SELECT_ALL_PORTLETCOLUMN
    = "select " + PORTLETCOLUMN_COLUMNS + " from ST_PortletColumn";
  ...
  {
    return (PortletColumnRow[]) getRows(query)
           .toArray(new PortletColumnRow[0]);
  }
  ...
  static final private String INSERT_PORTLETCOLUMN
    = "insert into"
  ...
  static final private String UPDATE_PORTLETCOLUMN
    = "Update ST_PortletColumn set"
    + " spaceId = ?,"
    + " columnWidth = ?,"
    + " nbCol = ?"
    + " Where id = ?";
     ...
     throws UtilException
  {
    ...
    static final private String DELETE_PORTLETCOLUMN
       = "delete from ST_PortletColumn where id=?";
  ...
  protected Object fetchRow(ResultSet rs)
     throws SQLException
  {
    return new PortletColumnRow(
      rs.getInt("id"),
      rs.getInt("spaceId"),
      rs.getString("columnWidth"),
      rs.getInt("nbCol")) ;
  }
