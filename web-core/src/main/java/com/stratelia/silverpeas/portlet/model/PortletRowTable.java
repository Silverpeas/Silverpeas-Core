...
public class PortletRowTable extends AbstractTable {
  ...
  static final private String SELECT_PORTLETROW_BY_ID
    = "Select " + PORTLETROW_COLUMNS + " from ST_PortletRow Where id = ?";


  /**
   * Returns all the PortletRow rows having a given portletColumnId
   */
  public PortletRowRow[] getAllByPortletColumnId(int aPortletColumnId, String orderField)
    throws UtilException
  {
    String req = "select " + PORTLETROW_COLUMNS +
                 " from ST_PortletRow" +
  ...
  public PortletRowRow[] getAllByInstanceId(int InstanceId)
    throws UtilException
  {
    return (PortletRowRow[]) getRows(SELECT_ALL_PORTLETROW_WITH_GIVEN_INSTANCEID,InstanceId)
           .toArray(new PortletRowRow[0]);
  }

  static final private String SELECT_ALL_PORTLETROW_WITH_GIVEN_INSTANCEID = "select " + PORTLETROW_COLUMNS + " from ST_PortletRow where InstanceId=?";
  ...
  public PortletRowRow[] getAllByPortletColumnId(int portletColumnId)
    throws UtilException
  {
    return (PortletRowRow[]) getRows(SELECT_ALL_PORTLETROW_WITH_GIVEN_PORTLETCOLUMNID,portletColumnId)
           .toArray(new PortletRowRow[0]);
  }

  static final private String SELECT_ALL_PORTLETROW_WITH_GIVEN_PORTLETCOLUMNID = "select " + PORTLETROW_COLUMNS + " from ST_PortletRow where portletColumnId=?";
  ...
  {
    return (PortletRowRow[]) getRows(SELECT_ALL_PORTLETROW).toArray(new PortletRowRow[0]);
  }

  static final private String SELECT_ALL_PORTLETROW
    = "select " + PORTLETROW_COLUMNS + " from ST_PortletRow";
  ...
  {
    return (PortletRowRow[]) getRows(query)
           .toArray(new PortletRowRow[0]);
  }
  ...
  static final private String INSERT_PORTLETROW
    = "insert into"
  ...
  static final private String UPDATE_PORTLETROW
    = "Update ST_PortletRow set"
    + " InstanceId = ?,"
    + " portletColumnId = ?,"
    + " rowHeight = ?,"
    + " nbRow = ?"
     ...
     throws UtilException
  {
    ...
    static final private String DELETE_PORTLETROW
       = "delete from ST_PortletRow where id=?";
  ...
  {
    PortletRowRow[] portletRowToBeDeleted
       = getAllByPortletColumnId(portletColumnId);
    for (int i=0; i<portletRowToBeDeleted.length; i++)
    {
       delete(portletRowToBeDeleted[i].getId());
    }
  ...
  protected Object fetchRow(ResultSet rs)
     throws SQLException
  {
    return new PortletRowRow(
      rs.getInt("id"),
      rs.getInt("InstanceId"),
      rs.getInt("portletColumnId"),
      rs.getInt("rowHeight"),
      rs.getInt("nbRow")) ;
  }
