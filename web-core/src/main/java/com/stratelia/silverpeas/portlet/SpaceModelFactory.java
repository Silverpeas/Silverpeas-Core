...
/**
 * The SpaceModelFactory is a class used for its statics methods.
 * its construct and maintain the SpaceModel hierarchie in memory.
 * It implement the persistance for the SpaceModel class, ie : it provide read and save methods from/to a database.
 * It can read/save the user specific state for the portlets (min, max or normal) either.
 */
...
static
{
     ...
     * 
     * // Static initializer : i.e. executed only one time for the class
     * static {
     * try {
     * // Get the initialContext
     * Context ctx = new InitialContext();
     * // Look up myDataSource
     * dataSource = (DataSource) ctx.lookup ("jdbc/Silverpeas");
    ...
    /**
     * As the SpaceModelFactory is a class used for its statics methods,
     * It doesn't need any contructor
     */
    ...
    public static SpaceModel getSpaceModel(PortletSchema os, String aSpaceId) throws PortletException
    {
        ...
        if (aSpaceId == null)
        {
            throw new PortletException("SpaceModelFactory.getSpaceModel(PortletSchema os, String aSpaceId)", SilverpeasException.ERROR, "portlet.EX_SPACE_NULL");
        }
        if (aSpaceId.equalsIgnoreCase(""))
        {
            throw new PortletException("SpaceModelFactory.getSpaceModel(PortletSchema os, String aSpaceId)", SilverpeasException.ERROR, "portlet.EX_SPACE_EMPTY");
        }
        ...
        {
            throw new PortletException("SpaceModelFactory.getSpaceModel(PortletSchema os, String aSpaceId)", SilverpeasException.ERROR, "portlet.EX_CANT_GET_SPACE_MODEL", e);
        }
     ...
     * @param isAdmin true if the user is editing the spaceModel.
     * in this case, the unallowed portlets are not remove from the spaceModel.
    ...
    public static SpaceModel getSpaceModel(MainSessionController msc, String aSpaceId, boolean isAdmin) throws PortletException
    {
        ...
        {
            throw new PortletException("SpaceModelFactory.getSpaceModel(MainSessionController msc, String aSpaceId, boolean isAdmin)", SilverpeasException.ERROR, "portlet.EX_CANT_GET_SPACE_MODEL", e);
        }
        finally
        {
            schema.close();
    ...
    /*
     * public static void portletSaveState(String aSpaceId, String aUserId, Portlet aPortlet) throws PortletException {
     * PortletSchema os = null ;
     * // Test the arguments
     * if (aSpaceId == null) {throw new PortletException("spaceId is null");}
     * if (aSpaceId.equalsIgnoreCase("")) {throw new PortletException("spaceId is empty");}
     * 
     * if (aUserId == null) {throw new PortletException("userId is null");}
     * if (aUserId.equalsIgnoreCase("")) {throw new PortletException("UserId is empty");}
     * 
     * int spaceNum = Integer.parseInt(aSpaceId) ;
     * 
     * try {
     * // Create a schema for accessing tables
     * //      os = new PortletSchema(dataSource.getConnection()) ;
     * os = new PortletSchema() ;
     * PortletStateTable tPortletState = os.portletState ;
     * 
     * // To be atomised in the future
     * UserTable tUser = os.user ;
     * UserRow ur = tUser.getByLdapId(aLDAPUserId) ;
     * int aUserId = ur.getId() ;
     * 
     * String req = "Select * from ST_PortletState" +
     * "  Where portletRowId = " + aPortlet.getRowId() +
     * "  and userId = " + aUserId ;
     * PortletStateRow psr = tPortletState.getPortletState(req) ;
     * // If there is not yet a State for this portlet and this user
     * if (psr == null) {
     * //We have to create one
     * psr = new PortletStateRow(-1, aPortlet.getState(), aUserId, aPortlet.getRowId()) ;
     * } else {
     ...
     * 
     * } catch (UtilException e) {
     * throw new PortletException(e, "Error Saving the portlet State") ;
     * } finally {
     * try {
     * if (os != null) {
     * os.close();
    ...
    public static void portletSaveState(SpaceModel space, Portlet aPortlet) throws PortletException
    {
        ...
        {
            throw new PortletException("SpaceModelFactory.portletSaveState(SpaceModel space, Portlet aPortlet)", SilverpeasException.ERROR, "portlet.EX_CANT_SAVED_STATE", e);
        }
        finally
        {
            if (os != null)
    ...
    private static Portlet getPortlet(int instanceId, int rowId) throws PortletException
    {
        ...
        try
        {
            ...
            portlet = new Portlet(instanceId, rowId, URLManager.getURL(cir.getName(), sSpaceId, cir.getName()+sComponentId),
            cir.getName(), sComponentId, cir.getLabel(), cir.getDescription(), 
            null,  // TitlebarUrl
            null,  // IconUrl
            null,  // ContentUrl
            null,  // MaxContentUrl
            "headerUrl", "footerUrl", "helpUrl", maximizable, minimizable, Portlet.NORMAL);
        }
        catch (Exception e)
        {
            throw new PortletException("SpaceModelFactory.portletSaveState(SpaceModel space, Portlet aPortlet)", SilverpeasException.ERROR, "portlet.EX_CANT_GET_PORTLET", e);
        }
        return portlet;
    }
     ...
     * All instances that are not allready added the this space minus instance that are not "portlettizable" yet.
    ...
    /**
     * saveSpaceModel Save all the spaceModel structure in database.
     * the save in done by deleting and recreating the structure.
    ...
    public static void saveSpaceModel(SpaceModel space) throws PortletException
    {
        ...
        {
            throw new PortletException("SpaceModelFactory.saveSpaceModel(SpaceModel space)", SilverpeasException.ERROR, "portlet.EX_CANT_SAVE_SPACE", e);
        }
        finally
        {
            if (os != null)
    ...
    public static boolean portletAvailable(MainSessionController mainSessionCtrl, String spaceId)
    {
        ...
        try
        {
			...
			SilverTrace.info("portlet", "SpaceModelFactory.portletAvailable()", "root.MSG_GEN_PARAM_VALUE" , "NbRows = " + ciRows.length);
            // Get all the available components for this userIs and this spaceId
        ...
        {
			SilverTrace.error("portlet", "SpaceModelFactory.portletAvailable()", "root.MSG_ERR_UNKNOWN" , "PB !!!", e);
            ret = false;
        }
        finally
        {
