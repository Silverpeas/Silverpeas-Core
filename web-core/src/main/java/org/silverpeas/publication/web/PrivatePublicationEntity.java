 ...
 * and accessed on private mode.
 */
public class PrivatePublicationEntity extends PublicationEntity {

  private static final long serialVersionUID = -4832735157028485696L;

  public PrivatePublicationEntity() {
    super();
  }
  
  protected PrivatePublicationEntity(PublicationDetail publication, URI uri) {
   ...
   */
  public static PrivatePublicationEntity fromPublicationDetail(
      final PublicationDetail publication, URI uri) {
    return new PrivatePublicationEntity(publication, uri);
  }
  ...
  protected URI getAttachmentURI(SimpleDocument attachment, String baseURI, String token) {
    URI privateUri;
    try {
      privateUri = new URI(baseURI + "private/attachments/" + attachment.getInstanceId() + "/"
          + attachment.getId() + "/"
